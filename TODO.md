...
- [ ] **`worker/regenerate.ts` 的背景 job 錯誤會讓測試 process 永遠不退出**（本案測試時發現，與 tts_voice 無關）：`persistRegenerateJob`（`backend/src/worker/regenerate.ts:187`）遇到 `FOREIGN KEY constraint failed` 時該錯誤沒有被正確吞掉／結束，導致含這條路徑的測試檔案（`provider-availability.test.ts`、`upload-pipeline-control-permission.test.ts`）即使所有斷言都通過，process 仍會掛著不退出（實測掛了 5+ 小時），CI 或本機重跑全套測試時可能因此卡住整個 pipeline。已在未改動的 master 上重現。
