...
- **⚠ 仍未實機發聲**：這台機器**現在有一個 CPU 版的 `audiocpp_cli`**（`.audiocpp/build/linux-cpu-release/bin/`，約 400 MB，連同原始碼共 3.1 GB，是上述測試意外建出來的；已 gitignore，要省空間可整個刪掉 `.audiocpp/`），**但沒有下載任何模型**，所以還沒有真的發過聲。旗標已如上驗證，剩下要確認的是：(1) 下載模型後真的能合成、音檔可播；(2) 選 GPU 時 log 沒有出現「retrying this segment on the CPU backend」（這台沒有 GPU，該路徑只有假的 CLI 測過）；(3) 若用會唸出指示的家族，`AUDIOCPP_TTS_PROMPT_STEERING` 維持關閉。設定與安裝流程見 [`docs/audiocpp-local-tts.md`](docs/audiocpp-local-tts.md)。
