...
[ ] Manim `animate.shake(m, progress, opts)` 搖晃效果：應新增 `animate.shake(m, progress, opts)` 函式，模擬元素水平搖晃的效果（適合用於強調、警示、出錯場景）；以 `progress` 控制搖晃幅度（progress 0 = 無偏移，0.5 = 最大幅度，1 = 回到原位），透過 `Math.sin(progress * Math.PI * cycles)` 計算水平位移（translateX），opts 支援 `amplitude`（搖晃幅度 px，預設 8）和 `cycles`（搖晃週期數，預設 4）；對非 transform 支援元素退回為 opacity；新增至少 2 個 vm 測試（一個驗證中間 progress 有非零偏移，一個驗證 progress=0 和 progress=1 都為零偏移）。
