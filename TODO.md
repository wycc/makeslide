...
- [ ] **階段 5：GSAP 共用同一 harness**（加一個 `seek(t)` 動作驅動時間軸），對應 `docs/react-slide-design.md` §16 第 5 點。
- [ ] **待決事項**（設計文件 §11）：iframe reparenting 能否保住互動狀態；**ZIP 匯入是否重新編譯驗證**（若直接採用 sidecar 內現成的 `compiled`，匯入一份 ZIP 就等於執行任意 JS——需先查證現況）；宣告式互動（tier 1，不執行任意 JS、可被既有覆寫機制理解）能覆蓋多少實際需求，覆蓋率高的話 `react-app` 的優先度可以往後調。
