...
- [ ] `auth.ts` 的 session/oauth-state cookie 補上 `Secure` 屬性（僅生產環境）：`setCookie()`/`clearCookie()` 應在 `process.env.NODE_ENV === 'production'` 時於 `Set-Cookie` 字串額外附加 `; Secure`，本機開發（`NODE_ENV` 非 `production`，常跑在無 TLS 的 `http://localhost`）維持不變以避免登入失效；需新增測試驗證生產環境下 `Set-Cookie` 標頭包含 `Secure`、非生產環境不包含。
