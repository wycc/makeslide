...
## 重新產生語音時顯示進度（使用者要求，2026-09-19）★ 使用者要求，不計入計數

- [x] **後端**：所有合成語音的路徑都經過 `synthesizeOnePage`，在那裡登記到 `services/audioProgress.ts`：頁、步驟、字數、開始時間、分段數。`GET /api/pdfs/:id/audio-progress` 回傳這份簡報正在合成的項目，並附伺服器時間。
- [x] **預估方式**：TTS 呼叫進行中不會回報任何進度，所以單頁內的進度是「已過時間 ÷ 預估時間」。預估值是每個供應商的「每字秒數」移動平均，每次合成成功後更新，存在 `data/tts-throughput.json`。第一次沒有資料時用 0.1 秒／字。進度最高停在 95%，超過預估時進度條變琥珀色，文字改為「比預估久，仍在合成…」。有多段時，已完成的段數是進度的下限。
- [x] **前端**：共用元件 `AudioProgress`（忙碌時每秒輪詢）放在六個地方：逐字稿儲存（一般面板、全螢幕）、套用 AI 逐字稿建議的對話框、步驟旁白的單步配音和整頁改寫、整份重生的語音階段（Header 與重生對話框）。
- [x] **實測**：用真的 TTS（目前是 openrouter，不是 audio.cpp）在臨時簡報上合成 89 字，進度條每秒前進，超過預估後轉成提示，合成完自動消失，並學到 0.21 秒／字。臨時簡報已刪除。實測也抓到單元測試把假供應商寫進正式檔案的問題，已修正（測試環境不寫檔），並清掉那筆資料。
- 分支 `feat/audio-progress`、`fix/audio-progress-test-persist`（皆已 merge）。
- 未做：沒有逐一在瀏覽器實測步驟旁白和整份重生這兩處，只有接線測試。新簡報第一次產生語音時的處理畫面沒有加（那裡走的是既有的頁數進度）。

## 本頁問答無法改變逐字稿長度（使用者要求，2026-09-19）★ 使用者要求，不計入計數

- [x] **原因**：本頁問答的 `propose_script_edit` 只收 `instruction`，改寫時一律用簡報每頁字數設定，而且提示詞規定「必須控制在 X～Y 字、不可超過上限」，所以「寫長一點」「寫到 900 字」會被這條限制蓋掉。
- [x] **修法**：工具新增 `target_length`（逐字稿語言的單位：中文是字、英文是 words），換算成內部字數，並用和改寫 API 相同的 40–2000 範圍檢查；超出範圍時回錯誤給 AI，由它告訴使用者。對話上下文會告訴 AI 目前逐字稿字數和簡報預設字數，「長一點」這類相對要求才算得出來（長一點約 1.5 倍、加倍約 2 倍、短一點約 0.7 倍）。改寫的 token 上限隨目標字數放大。回歸測試在舊程式碼上會失敗。分支 `feat/chat-script-length`（已 merge）。
- 既有的其他做法：編輯區「逐字稿」分頁的 AI 改寫本來就有「這次字數」欄位；「重生」可以改整份簡報的每頁字數。
- 未做：沒有用真的模型實測它是否每次都會帶 `target_length`，目前靠工具說明和上下文提示引導。

## 插頁後，被往後推的頁面顯示成別頁的圖（使用者回報，2026-09-19）★ 使用者回報，不計入計數

- [x] **現象**：`rNo5g9JaYs` 第 28–30 頁從第 27 頁按下一頁進去時，頁碼正確，畫面卻是第 31 頁（React 頁）。資料庫、檔案、API 回的圖片都正確。無頭 Chrome 每次都是新快取，所以重現不了。
- [x] **根因**：頁面圖片、縮圖、音訊網址是 `/pages/<頁碼>/…?t=<updated_at>`。插入空白頁、分頁、AI 新增多頁都會把後面的頁面往後推，但沒有更新 `updated_at`（刪除和排序有更新）。這份簡報在 9/18 分五次插入第 23–27 頁，最後一頁 `zwVDlTThH2` 依序當過第 28、29、30 頁，`updated_at` 始終是 `16:05:48.185Z`。所以瀏覽器裡這三個網址存的都是它的圖。單頁應用在同一份文件中重用同網址的圖片，不看快取標頭，預載器又會事先抓進來，所以分頁沒重新整理就一直錯。
- [x] **修法**：三條改頁碼的路徑（`POST /pages` 插入、`/pages/:n/split`、`addPagesFromPrompt`）在移動頁碼時一併寫入新的 `updated_at`。回歸測試 `page-renumber-updated-at.test.ts` 在舊程式碼上會失敗。已直接把此簡報第 28–31 頁的 `updated_at` 更新。分支 `fix/renumber-bumps-updated-at`（已 merge）。
- [ ] **未處理**：新插入的空白頁（第 23–27 頁）`audio_path` 有值但檔案不存在，播放時 `/pages/27/audio` 會 404 並重試數次，之後被當成沒有語音、立刻自動前進。另外 `page_watch_progress`、`page_artifact_*`、`page_generation_prompts` 依設計不隨頁碼移動，插頁後會對應到錯的頁。

## Markdown 支援圍欄程式碼區塊（使用者要求＋截圖，2026-09-19）★ 使用者要求，不計入計數
...
## 文字逐行出現的動畫在匯入時不被支援（使用者回報，2026-09-20）★ 使用者回報，不計入計數

使用者指出 `VIUWSnfnsK` 第 8 頁的動畫 2–6 步都沒有變化，原本應該是文字一行一行出現，並推測這種動畫在轉換時不支援。查證後完全屬實。

- [x] **證據**：該頁 7 張影格中第 0–5 張**位元組完全相同**，只有第 6 張不同。讀原始 pptx 的 `p:timing` 後發現六次點擊全部指向同一個形狀 `spid=159`，差別只在 `<p:txEl><p:pRg st="N" end="N"/></p:txEl>`（第 0 到第 5 段文字）。解析器 `parseAnimationSteps` 只讀 `@spid`，段落資訊整個被丟掉。
- [x] **為什麼是「一路空到最後」**：`hiddenShapeIdsForStep` 的規則是「後面任何一次點擊會帶進來的，現在就先藏起來」。六次點擊都帶進 159，於是它被藏到最後一次點擊才出現。
- [x] **修正**：`PptxAnimationTarget` 新增段落層級目標，解析 `p:pRg`；新增 `hiddenParagraphsForStep` 與 `removeParagraphs`（把未揭露的 `a:p` 從形狀的 txBody 裁掉，形狀本身留著）；一行都還沒出現的形狀整個移除，避免留下空文字框；`canRenderFromOriginal` 改用 `isFullyBuiltStep`，否則段落動畫在形狀層級「什麼都沒藏」會被判定成可直接用原檔。
- [x] **順帶修掉通用形式**：可見度規則改寫成「依每個目標自己的事件清單，由這一步之前的最後一次點擊決定」。原本的規則對「同一個東西被兩次進入效果指到」也會藏到最後一次——那正是這次 bug 的一般情況，已補測試。
- [x] **附帶好處**：每一步揭露的文字現在是那一行的內容。先前段落動畫的每一步對旁白模型都只是「沒有文字的圖形」。
- [x] **驗證**：用真實的 pptx 重新推導該頁——文字確實一行一行出現（第 0→4 步每步多一行）。測試 11 條（舊版全數失敗），pptx／step 相關 101/101、tsc 通過、已 touch 進入點。
- [ ] **注意**：該頁最後兩次點擊指向的是**空白段落**（作者把空行也加了動畫），所以那兩步仍然沒有變化，這是投影片本身的內容。另外此修正只影響**之後的匯入**；`VIUWSnfnsK` 現有的影格要重新匯入才會更新。

## 分步頁逐字稿與動畫對不上：模型從來沒看過畫面（使用者回報，2026-09-20）★ 使用者回報，不計入計數

使用者指出 `rYFD1VwStl` 第 8 頁的動畫內容與逐字稿顯然不相當，並問是不是 LLM 根本沒看到正確的動畫。答案是：對，它一張畫面都沒看過。

- [x] **證據**：`stepNarration` 送出的訊息裡沒有任何影像，步驟只用「這一步新出現的**文字**」描述。把第 8 頁當時的輸入重建出來，7 個展開步驟得到的是同一行「畫面新出現『沒有文字的圖形（例如箭頭、方框或連線）』」——因為這一頁每一步新出現的都是圖（機率向量、one-hot、3×3 矩陣、紅框、箭頭），公式在 PPTX 裡也是圖片。模型只好拿整頁文字＋簡報大綱寫成連貫講解再平均切成 8 段。實際動畫第 4 步是「把前兩步加上的向量收回去」，那一段逐字稿卻在講取對數。
- [x] **修正**（依使用者裁示採「首幀＋每步變化區域」）：新服務 [stepFrames.ts](backend/src/services/pptx/stepFrames.ts)——`frameChange` 逐像素比對相鄰影格，算出變化區域的外框，並以「由紙變墨／由墨變紙」的像素數分類成新增／收起／替換（用整塊區域的平均墨水量會被沒變的公式稀釋，實測分不出來，已改成逐像素）；`stepFramePictures` 送出第一張完整畫面加上每一步的裁切圖，數量設上限。匯入時影格早就寫好了（`writePageSteps` 在前、旁白在後），所以不必重算圖。
- [x] **降級**：沒有影格、影格解不開、或該步畫面沒變化的頁面，維持原本的純文字描述；壞掉的單一影格只損失它自己那一張圖。
- [x] **驗證**：在拋棄式副本上重新產生第 8 頁逐字稿——第 6 步的新逐字稿寫出「labels such as zero, one, and one」，`[0 1 1]` 這個值只存在於畫面上，文字管道從來沒有它；第 5、7 步也都對上了矩陣與框選機率。測試 9 條（含接線 1 條先確認修改前會失敗），pptx 與 step 相關 90/90、tsc 通過、已 touch 進入點。
- [ ] **仍可再改善**：一個步驟若同時在相距很遠的位置變化（例如同時畫出箭頭與向量），外框會涵蓋大片區域，前幾步的旁白仍偏向「照著大綱講」而不是照著該步的變化講。之後可考慮改成分離的多個變化區域，或在完整畫面上框出變化處。

## 頁面有評論，全螢幕卻沒有圖示（使用者回報，2026-09-20）★ 使用者回報，不計入計數

- [x] **查證**：`rNo5g9JaYs` 第 31 頁確實有一則未解決評論（今天 03:02 從 AI 導師問答存入），API 的 `has_comment` 也回 true；重新載入後一般畫面與全螢幕四種版面（圖片／字幕／編輯／動畫）都會顯示 💬 徽章。所以徽章本身沒壞。
- [x] **真正的原因**：`has_comment` 是隨簡報資料一起來的旗標，而兩個寫入評論的路徑（AI 導師面板的「存成評論」、側邊欄的留言表單）都只更新自己的局部狀態，沒有回寫 detail。所以當場寫下的第一則評論，要等重新載入才看得到徽章。
- [x] **為什麼全螢幕特別像壞掉**：全螢幕用同一個旗標決定**要不要去抓評論**（`useFullscreenPageComments` 的 `enabled`），旗標是 false 就永遠不抓，那一頁看起來和完全沒有評論的頁面一模一樣。
- [x] **修正**：PlayPage 新增 `setPageHasComment(pageNumber, has)` 就地改 detail，兩個寫入路徑寫完就打開旗標，刪到剩零則就關掉。守門測試 4 條（3 條先確認修改前會失敗）。
- [x] **驗證**：在拋棄式副本上實際操作——送出留言後不重新整理，一般畫面立刻出現 💬，接著進全螢幕顯示「💬1」。前端 1147/1147、tsc 與 build 通過。

## AI 導師看不到學生所在頁（使用者回報＋截圖，2026-09-20）★ 使用者回報，不計入計數

使用者在 `rNo5g9JaYs` 第 31 頁問「這一頁還有哪些運算子或函式有同樣問題」，導師回答它沒有第 31 頁的文字或逐字稿，請使用者自己貼上。

- [x] **量出真正的規模**：`/pages/:n/ask` 的語料是把每一頁的頁面文字與逐字稿依序串起來，再從尾端截到 `ASK_DECK_CORPUS_MAX_CHARS`（14,000）。這份簡報的語料實測 **100,810 字元**——也就是提示詞裡只有第 1–5 頁，學生所在的第 31 頁連同其後全部被截掉。
- [x] **第二層原因：提示詞說謊**。系統提示詞寫「你會獲得整份簡報所有頁面的頁面文字與逐字稿」。模型其實握有 `get_page_text`／`get_page_script`／`get_page_image` 唯讀工具，但被告知「所有頁面都給你了」的模型不會去找，只會判定那一頁不存在、轉頭向學生要。
- [x] **新服務** [askCorpus.ts](backend/src/services/askCorpus.ts)：`buildAskCorpus` 先無條件保留學生所在頁（即使它自己就超過整個預算），再以它為中心向外擴散花掉剩餘預算，最後依頁碼排序輸出；`describeCorpusCoverage` 把沒附上的頁碼以區間（如「1–26、36」）列出，並明講可以用工具讀取、不可以向學生索取。系統提示詞同步改成「部分或全部頁面」。
- [x] **驗證**：對同一份簡報同一頁送出同一個問題，導師改為直接用第 31 頁的內容作答（in-place 操作覆寫 autograd 需要的資料），不再要求貼上頁面。守門測試 8 條（含端點接線 1 條先確認修改前會失敗）；ask／tutor 相關 76/76、tsc 通過，已 touch 進入點。
- [ ] **未處理**：14,000 的預算沒有調整——現在錢花在對的地方（所在頁與鄰頁約 9 頁），但長簡報的跨頁問題仍得靠模型自己呼叫工具補讀。若之後發現工具呼叫太頻繁，可再考慮提高上限。

## 有元素的頁面：換底圖＋AI 微調元素位置來美化（使用者要求，2026-09-20）★ 使用者要求功能，不計入計數

使用者問：頁面上已經有文字或其它元素時，能不能換一張底圖、再把元素微調到合理位置來美化，而且元素要維持可編輯。查證後「換底圖」那一半本來就有（`replace-image` 的 `mode=base` 會把新圖當底圖、元素重新合成上去），缺的是排版那一半；使用者裁示連 AI 生成背景一起做。

- [x] **新服務** [pageElementsBeautify.ts](backend/src/services/pageElementsBeautify.ts)：`buildBackgroundPrompt`（要求「絕對不能有文字」、中央與左上留白、依這一頁的文字決定主題）、`proposeElementLayout`（視覺模型看新底圖與元素清單，回傳每個 id 的 xPct/yPct/widthPct/heightPct 與文字的 fontSize／color，形狀照抄既有的 `cutoutPlacement.ts`）、`applyElementLayout`（純函式，只允許動幾何與文字的字級顏色；文字內容、元素型別、圖片素材一律沿用原值）、`ensureReadableText`（讀新底圖在每個文字框底下的亮度，對比不足就翻成黑或白）。
- [x] **數值規則**：位置可以夾進畫布（x=-5 代表「貼左緣」，夾住就是照辦），但尺寸與字級不合理就整項忽略、沿用原值——測試先抓到把 `fontSize: -20` 夾成最小值 12 的錯誤設計，改掉。
- [x] **新端點** `POST /pages/:n/elements/beautify`（background／relayout 可各自開關）與 `/beautify/undo`。動任何東西之前先存還原點（底圖＋元素成對），存不起來就整個中止；兩半各自降級（背景失敗仍用舊圖排版、排版失敗元素留在原位），失敗以 `warnings` 回報而不是整個請求失敗；復原一次用掉還原點。GET elements 多回 `has_beautify_undo`。
- [x] **前端**：元素分頁「更換底圖」旁新增「✨ AI 美化排版」展開面板（風格提示、兩個勾選、進行中提示）與「↩ 復原美化」；送出前先把草稿存檔（否則 AI 排的是舊版元素），結果也記進編輯器自己的 undo stack，換頁時清掉上一頁的美化狀態。
- [x] **驗證**：用拋棄式副本實測——生成背景 21 秒、兩個元素都被移動並改成在深藍底上看得見的亮色、文字內容完全沒被改寫；另一輪只做重新排版再復原，元素完全回到原狀、還原點用掉後再按回 409。測試 12 條（服務 8＋端點 6 的其中 6＋前端 4，關鍵案例先確認修改前會失敗）；後端 39/39、前端 1143/1143、tsc 與 build 皆通過，已 touch 進入點。
- [ ] **已知限制**：重新整理頁面後「復原美化」按鈕會消失（還原點檔案還在，只是 UI 狀態沒有持久化）；線段元素不參與重新排版（它是兩個端點而不是方框，照搬會把底線拉離文字）。
...
- [x] **實測重現並排除逐步播放的嫌疑**：以擁有者身分在無頭 Chrome 播該頁，把每一步快轉到接近結尾。四段動畫依序播完（`steps/0..3/audio`），最後一步播到底後 `paused`，停在第 4 頁。但同一份簡報的第 2 頁（純圖片頁）也一樣停住，所以和動畫、分步頁都無關。
- [x] **真正原因是徽章說謊**：面板上那顆徽章只看 `classroomMode`——上課模式關著就一律顯示「連續播放」，完全不管真正決定換頁的 `autoAdvance`。而 `getStoredAutoAdvance()` 在沒設定過時回傳 `false`，也就是全新瀏覽器預設**不會**自動換頁。實測確認：全新設定檔的徽章寫「連續播放」，播放設定對話框裡的「自動播放下一頁」卻是 OFF。使用者在動畫頁上最容易發現，因為分步會一段接一段跑完、然後整個停掉，看起來就像那一頁壞了。
- [x] **依使用者裁示修正**：`getStoredAutoAdvance()` 預設改成開啟（只有明確存成 `0`／`false` 才停，已手動關掉的人不受影響）；徽章改成三種狀態——上課模式／連續播放／播完停在本頁，兩個語系都加了字串。
- [x] **驗證**：全新瀏覽器的設定顯示 ON，第 4 頁四段動畫播完後一路往下播到第 14 頁沒停；把設定關掉時徽章顯示「播完停在本頁」。守門測試 2 條先確認修改前會失敗；前端廣泛套件 1139/1139、tsc 與 build 皆通過。

## 從圖片頁進入 React 頁仍會閃（使用者回報＋截圖，2026-09-16）★ 使用者要求，不計入計數

- [x] **根因一：海報是新建的 `<img>`，要重新載入**。使用者截圖是深藍底加左上角一個破圖示。實測證實 Chrome 對「載入中」的 `<img>`（即使 `alt=""`）會畫細灰框和角落圖示。舊版海報在 ReactSlideFrame 裡新建一個 `<img>`：本機快取是熱的所以量不到，但圖片端點 `max-age=300`，一頁講幾分鐘後快取就過期，遠端連線下新元素得重新下載，下載期間就露出這個樣子。改成把海報放在 SlideRenderer React 分支外框的**第一個子元素**，也就是圖片分支放 `<img>` 的位置，React 因此沿用前一頁那個已載入、正在顯示的節點。萬一不是沿用的節點（例如前一頁是 GSAP 動畫頁），未載入完成前隱藏。
- [x] **根因二：框的尺寸在第一幀之後才量**。`useEffect` 在瀏覽器繪製後才執行，所以新掛上的框第一幀以 1920×1080 排版，海報被推到下半截、被裁掉。改用 `useLayoutEffect`。
...
- [x] **原因**：分步頁一次只播「目前這一步」的音檔（`playableStepAudioUrl`），但字幕仍拿**頁層級逐字稿**切句——而分步頁的頁層級逐字稿是各步接起來的全文——再平均攤到這一步的音檔長度上。8 步的句子全部壓進第 1 步的 27 秒，第 3 秒（約 11%）自然就落在第 2 步的文字。先前把「這頁有沒有語音」從頁層級改成看步驟時，字幕這一半沒有一起改。
- [x] **修正**：新增 `spokenScriptFor(page, step, pageScript)`（`playableStepAudioUrl` 的對應函式：正在播的那段語音實際唸的字），`PlayPage` 的切句改用它；沒有旁白的步驟就沒有字幕（理由和它沒有語音一樣，不退回整頁）。分步頁一律不採用 Whisper 時間軸——那是對頁層級音檔對齊的。`currentScript` 不動，編輯與版本紀錄仍是頁層級那一份。
- 測試：`pageAudio.test.ts` 新增 3 條（含回報情境重現：同一段 27.9 秒，用步驟旁白時每個時間點的字幕都屬於第 1 步；用整頁逐字稿時第 20 秒已在唸第 2 步）＋接線守門 1 條；播放頁相關 214/214、前端 `tsc` 與 `vite build` 通過。
- **未能在原簡報上驗證**：`rYFD1VwStl` 不在這台機器（demo16 與本機主 repo 的資料庫、storage 都沒有），是從程式邏輯與截圖上的數字（00:03／00:27、字幕內容為第 2 步）確認的。那一台安裝需要 pull master 並重建前端才會生效。

## 語音合成進度的剩餘時間估計不合理（使用者回報，2026-09-20）★ 使用者回報，不計入計數

使用者截圖：第 4 頁語音合成中、第 5/7 段、進度條約 57%，卻顯示「已 62 秒，約剩 654 秒」（524 個英文字、估計朗讀 3:45）。分支 `fix/audio-progress-estimate` → master／worktree/demo16。

- [x] **原因一：工作量用「字元數」計**（預設 0.1 秒／字元）。這只對中文成立——英文一個字五、六個字元，朗讀時間卻只相當於一個半中文字，所以英文頁會被高估好幾倍。和先前逐字稿編輯區「2499 字 → 10:25，實際兩分半」是**同一個錯**，那次只修了前端的 `speakingTimeEstimate.ts`，後來新增的進度估計又犯了一次。而且學到的速率是各語言混在一起的移動平均，同一台機器中英文都配音時，這個數字對兩邊都不準。
- [x] **原因二：已完成的段落沒有用來修正估計**。4/7 段只拿去墊高進度條，剩餘時間仍照一開始的估計倒數，所以畫面上兩個數字互相矛盾。
- [x] **修正**：工作量改為「語音秒數」（`speechSeconds()`：CJK 逐字 4 字／秒、其餘逐詞 140 字／分，與前端逐字稿估計同一組數字）；速率改為「每秒語音要花幾秒合成」，預設 0.4（＝舊預設對中文的意義）。每段完成時回報已合成的文字，估計改跟這一頁的實測速度——依已完成比例加權，做完五分之一後完全採信（開頭一個很短的段落大半是啟動成本，不能照單全收）；重試時從重試那一刻重新量，不把失敗那次的時間算成速度。
- [x] **速率檔換檔名** `tts-synthesis-rate.json`：舊的 `tts-throughput.json` 存的是秒／字元，單位不同，讀進來每個估計都會短四倍。舊檔留著不讀（demo16 上本來就還沒產生）。
- 測試：`audio-progress.test.ts` 7/7（新增 3 條：中英文同一句話的語音秒數相近而字元數差三倍以上；**回報情境重現**——初始估計落在 60–130 秒而不是 716 秒，62 秒做完 4/7 後修正為約 108 秒；實測覆蓋錯誤速率、短開頭只微調、重試重新量），語音合成相關 71/71、前端 `audioProgress` 4/4、後端 `tsc` 通過。API 形狀不變（`chars` 保留），前端邏輯不必改。
- 未做：只有一段的頁面（沒有語氣標記、單一講者）沒有段落可量，仍然只靠學到的速率；audio.cpp 這類單一行程的引擎頁內也拿不到更細的進度。
...
- [x] **已驗證（2026-09-20）**：把 master 同步到 demo16 時 `server.ts` 沒有變動，只有 route／service／worker 檔案變了，後端子行程仍在 fast-forward 後數秒內自行重載（08:36:54），新路由 `GET /api/pdfs/:id/audio-progress` 隨即可用。也就是排除 `node_modules` 之後 `tsx watch` 對非進入點檔案恢復正常，**不必再每次 `touch backend/src/server.ts`**——但同步後仍應比對子行程啟動時間確認。
...
- [x] **單位錯置**（上一輪我自己寫進去的）：regenerate 把 `script_max_chars_per_page` 當成 `charsPerStep` 傳給逐步旁白。已改成傳 `pageTargetChars`。
- [x] **但單位只是比較小的那一半問題**：以「每步固定字數」為模型本來就不對。步與步的份量不同，強迫等長正是使用者說的「不順暢」。
- [x] **改成整頁預算**（`pageNarrationBudget`）：`budget = f(每頁設定值, 步數)`，**錨點定在三步**——使用者裁示「500 應該是一般動畫頁（約三步）的量」，錨在一步會讓那個數字變成「靜態頁的量」、每個動畫頁都因此偏長。成長溫和（每多一步加基準的 15%）並以 4 倍封頂：24 步是更長的說明，但不是三步頁的八倍。以 500 為例：3 步→500、5 步→615、8 步→788、24 步→1712、上限 2000。
- [x] **分配規則寫進提示詞**（這是重點）：交出整頁總量，並明講**不要平均分配**——只改一個數字或多一條箭頭的步驟一兩句話帶過；帶進新概念或完整公式的步驟講透、可以佔掉很大一部分；任何一步都有下限（不准用「接下來看這裡」充數）與上限（不得吃掉整頁四成）。判斷依據是 pptx 本來就給我們的「這一步新出現了什麼文字」。
- [x] **三種長度意圖**，因為「講長一點」和「寫好一點」是不同的請求：整頁字數（正常控制）／明確每步字數（真的想等長時）／維持現有長度（只改內容不動節奏）。三者都是一次性，不會改掉簡報設定——常態偏好仍然放在簡報設定裡。API／MCP／面板三處都提供。
- [x] **`script_chars_per_step` 語意收斂**成「明確每步覆寫」（進階），不再吃到每頁字數的退值；每頁字數就是每頁的，靜態頁照舊。
- 測試：預算的錨點、成長與上限（含「五步頁不可以接近五頁份」這條回歸）、提示詞必須交出整頁總量且禁止平均分配、三種意圖的優先順序與「沒有東西可維持時要退回一般預算」。相關套件 88/88、前端 33/33、build 與兩邊 tsc 通過。分支 `feat/page-narration-budget` → master，**並已 touch 進入點讓後端載入**。
- 未做：預算公式的係數（錨點三步、每步 15%、上限 4 倍）是照使用者裁示定的，沒有做過大量實測校準；真的覺得偏長或偏短時，最快的調法是改每頁字數，其次才是動這三個常數。

## 實測：`tsx watch` 只對進入點有反應（2026-09-13）★ 開發環境事實，不計入計數

使用者回報「還是失敗」，截圖是已經被移除的錯誤訊息。查出來不是程式碼問題——**後端跑的是 17:00 啟動的行程，而修正是 17:13 寫進檔案的**。

- [x] **先前「後端是 `tsx watch`，改了會自動載入」的說法是錯的**（這個對話裡我講過好幾次）。實測：`touch backend/src/worker/regenerate.ts`（靜態被 import 的檔案）等 8 秒，服務行程 pid **完全不變**；`touch backend/src/server.ts` 則立刻重啟。也就是 `tsx watch src/server.ts` 在這台機器上只監看進入點本身。
- [x] **不是 inotify 名額問題**：上限 65536，實際使用量個位數。
- [x] **判斷「線上到底跑哪一版」的正確方法**（先前那次也用錯過）：`ss -lntp | grep :3000` 找出**真正在聽 port 的那個行程**（ps 裡同名的行程可能有好幾個、而且都不是它），再比對它的啟動時間與 `find backend/src -name '*.ts' -newermt '<今天>'` 的最新檔案時間。行程啟動時間 = 最後一次載入程式碼的時間。
- 影響：**每一次後端改動之後都要 `touch backend/src/server.ts`**（或重啟 `start.sh`）才會生效，不能假設 watch 會做。這個對話後續每一輪都照此處理。
- 未做：沒有去追 tsx 4.21 為什麼只監看進入點（可能是設定、可能是該版本行為）。真正要解決的話，`tsx watch --include 'src/**/*.ts'` 或改用 nodemon 都是方向，但那會改變使用者既有的啟動方式，留給使用者決定。

## 一次性提示／字數，以及逐字稿被切在句中（使用者要求＋回報，2026-09-13）★ 使用者要求與回報，不計入計數

### 一次性的提示與字數（`feat/one-off-rewrite-hint` → master）

使用者指出：AI 改寫除了目標字數，還需要一個「額外提示」欄位，而且它與「提示詞」的差別在於**下次重生不會續用**。

- [x] **兩條改寫路徑都加了只活一次的覆寫**：`POST …/rewrite-script` 的 `target_chars`，`POST …/pptx-narration`（與 MCP `narrate_pptx_steps`）的 `chars_per_step` 與 `instruction`。兩者都不寫回簡報。
- [x] **一次性提示特別不能進 narration plan**：plan 會存成 `narration-plan.json` 並被之後每一次重生沿用，一句「這次多舉一個例子」掉進去就會變成永久的寫作綱要。
- [x] **提示在規則的最後、輸出格式之前**：擺最後才蓋得過通則，但不能蓋過「請以 JSON 回覆」那一行，否則回應會不再是可解析的 JSON。
- [x] **前端兩處都有欄位並寫明性質**：逐字稿分頁的 AI 改寫列（這次字數＋這次提示），以及分步面板的重寫（這次提示）。兩者都在換頁時清空——為某一頁打的提示不該被順手套到下一頁——並指向「提示詞」分頁說明哪裡才是每次都生效的地方。
- 測試：[one-off-rewrite-hint.test.ts](backend/test/one-off-rewrite-hint.test.ts) 7 條——帶著兩種覆寫呼叫之後簡報設定與提示詞必須原封不動、超出範圍要擋下而不是默默夾住、提示在 system prompt 裡但排在 JSON 那行之前、以及產生 plan 的呼叫拿不到這個參數。

### 逐字稿被切在句中（使用者回報）（`fix/step-script-truncation` → master）

使用者回報：重生後第 10 頁每一步的逐字稿都被切斷。實地檢查：**每一步都剛好 300 字元且斷在詞中間**。

- [x] **根因一：截斷的單位與提示詞的單位不一致**。提示詞用 `scriptLengthFor` 以「英文字數」要求長度（150 字元目標 → 約 78 words），而截斷數的是**字元**——78 words 約 450 字元，於是每一步都被切在 300。前一輪把固定 300 改成「目標的兩倍」時沒發現這個單位錯配，中文剛好兩種算法一致所以看起來沒問題，英文一路壞著。
- [x] **根因二：輸出預算固定 2000 tokens**。24 步的頁根本塞不下，JSON 會被截斷，只有先回來的那幾步會被寫入。
- [x] **改法**：截斷不再負責控制長度（那是提示詞的事），只留下防爆底線，上限用 manifest 自己的 `MAX_STEP_SCRIPT_CHARS`，而且**切在句子邊界**——多說一點沒關係，但旁白是要唸出來的，斷在半句就是壞的；整段沒有句號可切時仍然照切（不然一個超長句會讓整步只剩一點點）。輸出預算改成隨「步數 × 目標字數」成長（下限維持 2000、上限 16000）。寫入 manifest 處的 `slice` 也換成同一個函式，那是第二個會切在句中的地方。
- 測試：三條新測試（預設目標下的英文旁白必須原封不動＝這次的回歸、失控的長答案必須結束在句號、預算必須隨工作量成長），**都先確認在舊的 cap 與固定預算下會失敗**（只退回那兩個函式、保留其餘程式碼）。相關套件 32/32。
- 未做：**已經被切斷的資料沒有自動修復**——第 10 頁那五步（以及任何在這個缺陷期間重生過的頁）要再重生一次才會拿到完整的句子。

## 前端補齊：PPTX 匯入入口、逐步旁白編輯與重生（使用者要求，2026-09-13）★ 使用者要求功能，不計入計數

使用者要求「全面加上前端 UI」「順便把匯入 PPT 加進來」，以及「讓使用者可以直接修改動畫的逐字稿和重新產生語音」。分兩個分支完成。

### PPTX 匯入入口（`feat/pptx-import-ui` → master）

- [x] **後端早就有 `POST /api/pdfs/from-pptx`，但前端一行都沒呼叫**——只能透過 MCP 或 curl 匯入 PowerPoint。現在是上傳選單的第二項。
- [x] **一個 file input 服務兩種來源，所以它必須知道自己是為誰打開的**：`accept` 要跟著選擇（否則 .pptx 在選檔器裡根本選不到），端點也要（否則 PowerPoint 被送到 PDF 路由，回來變成「這不是 PDF」）。
- [x] **PPTX 不開提示詞對話框**：它建立的簡報已經有頁面了，問「要怎麼產生」等於在問一份已經存在的簡報。改成告訴使用者接下來真正會發生的事——每個動畫步驟都會渲染成一張畫面、需要數分鐘、而且**完成後還沒有旁白**。
- [x] **匯入階段寫進 `pdfs.progress_step`**（parsing／rendering／building）：首頁本來就在處理中時每 5 秒輪詢，而卡片連續幾分鐘只寫「處理中」，跟卡住看起來一模一樣。
- [x] 這條路**不受 LLM 停用影響**（與 PDF 流程不同）：頁面是 LibreOffice 從檔案本身畫出來的，模型只在之後的旁白才需要。
- 測試：[pptxUploadWiring.test.ts](frontend/src/components/pptxUploadWiring.test.ts) 5 條接線守門＋i18n；前端 build 與兩邊 tsc 通過；後端 pptx 匯入套件 18/18。

### 逐步旁白的編輯與重生（`feat/step-narration-editing` → master）

- [x] **新增後端端點 `PUT /api/pdfs/:id/pages/:n/steps/:k/script`**：寫入某一步的旁白並**重新合成該步語音**。重生是預設而不是選項——字與聲音不可以各說各話，而「這一步的稿子已經不是那段音檔在唸的東西」正是這個端點要防的狀態。`voice: false` 保留給「先改好幾步再一起配音」，而且會明講語音還是舊的。清空某一步會把它的音檔一起拿掉（不然那一步會繼續講話），合成失敗則保住文字、丟掉過期音檔；頁層級 `script.txt` 跟著更新，因為匯出、搜尋與 AI 導師讀的都是它。
- [x] **前端**：分步頁的「逐字稿」分頁整個換成 [StepNarrationPanel](frontend/src/pages/play/StepNarrationPanel.tsx)——**取代**原本的編輯框而不是並排，因為那個編輯框在這種頁面上什麼也改不到。每一步顯示自己的文字、有沒有語音與長度，可以「儲存並重生語音」或「只存文字」。
- [x] **面板也能用 AI 重寫整頁**：指定每步字數，只重做這一頁（其他頁的旁白與語音不動），並把字數存回簡報設定（否則下一次重寫又回到預設），因為工作要跑數分鐘所以輪詢進度。
- 測試：[step-script-edit.test.ts](backend/test/step-script-edit.test.ts) 8 條（逐步隔離、`script.txt` 跟上、清空即移除音檔、越界時要講出實際步數、權限）＋[stepNarrationPanelWiring.test.ts](frontend/src/pages/play/stepNarrationPanelWiring.test.ts) 5 條接線守門；相關後端套件 117/117，前端 build 與兩邊 tsc 通過。
- 未做：沒有實機視覺驗證（部署擋在 Google OAuth 後）。匯入進度只到「階段」而沒有逐幀百分比（`/pptx-import/status` 有細節，卡片沒顯示）。整份重寫旁白仍然只能從 API／MCP 觸發——面板上的按鈕只重做當下這一頁。

## 分步旁白的長度可調，並能只重做指定頁（使用者要求，2026-09-13）★ 使用者要求功能，不計入計數

使用者反映分步頁的逐字稿太短、希望更詳細，並問「一般 AI 重生時字數可指定，為什麼分步頁不行」。方案經使用者確認後實作（他選了「新增每步字數設定」＋「階段一：字數可調＋可指定頁重生」）。

- [x] **先釐清一個誤解**：不是「一句對一步」，而是「一段對一步」——模型被要求回傳剛好 `stepCount` 個元素，每段配一個 `step-NN.m4a`，因為畫面前進靠該步音檔播完。每段幾句不限，短是因為提示詞寫死。
- [x] **兩個寫死的數字**（缺一不可）：提示詞的「請為每一步寫一到三句口語旁白」，以及回來之後 `slice(0, 300)` 的硬截斷。只改前者，模型寫長了也會被砍掉。
- [x] **長度改成設定，而且是「每一步」而不是「每一頁」**：`pdfs.script_chars_per_step`，退到每頁字數、再退到系統預設。per-step 是關鍵——一頁分 24 步是一段解說分成 24 拍，總長要隨步數成長，只有把數字定義在「步」上才會如此。靜態頁維持用每頁字數（它本來就是一般頁）。指示用與一般逐字稿**同一組** `scriptCharBounds`／`scriptLengthFor` 產生，所以英文自動以 words 計、「這段該多長」在產品裡只有一個答案。截斷上限改成跟著目標走（2 倍，下限 300、上限 `MAX_STEP_SCRIPT_CHARS`）。
- [x] **只重做指定頁**：`POST /pptx-narration` 與 MCP `narrate_pptx_steps` 加 `pages` 與 `chars_per_step`。原本為了修一頁要重跑 26 頁的 LLM＋TTS。大綱仍然涵蓋整份（一頁該講什麼取決於它在整場裡的位置），只有被指定的頁會被重寫，其餘頁的旁白與語音完全不動；進度改成以「實際要做的頁數」計。
- [x] **`script_chars_per_step` 只在欄位有送出時才寫入**，舊版前端不會把它悄悄清成 null。
- 測試：[pptx-narration-length.test.ts](backend/test/pptx-narration-length.test.ts) 11 條——退值鏈、每步下限（60）可以低於每頁下限（80，因為一拍本來就比一整頁短）、送出與省略的差別、請求驗證（頁碼 0／小數、字數 39／2001）、截斷跟著目標、以及提示詞不得再出現句數。相關套件：pptx 全部 72/72、逐字稿生成與 MCP 24/24。分支 `feat/step-narration-length` → master。
- 未做：**前端沒有 UI**——每步字數只能透過 `PATCH /api/pdfs/:id/script-settings` 或 MCP 設定，重生分步旁白也仍然只有 API／MCP 入口（播放頁沒有按鈕）。播放頁的「逐字稿編輯框」在分步頁依然是陷阱：它改的是頁層級 `script.txt`，分步頁播放時唸的是 `steps[].script`，所以在那裡改了不會被唸出來——這是使用者確認方案時列為階段二／三、本輪刻意不做的部分。

## PPTX 匯入：有退場動畫的頁面，最後一步是錯的（使用者回報，2026-09-13）★ 使用者回報缺陷，不計入計數

使用者回報 `rNo5g9JaYs` 第 10 頁：「第五步把之前的元素都顯示了出來，而且少了最後一步」。查證後兩個現象是**同一個缺陷**。

- [x] **根因在 [renderFrames.ts](backend/src/services/pptx/renderFrames.ts) 的省成本捷徑**：原本寫 `if (request.stepIndex >= stepCount) fromOriginal.push(request)`，註解說「每張投影片的最後一格就是原始檔裡的樣子」。這個假設**只在沒有退場（exit）效果時成立**。第 10 頁的點擊序列是 `entr 434` →（`exit 434` ＋ `entr 439`）→（`exit 439` ＋ `entr 443`）→ `entr 446`：全部點完的畫面比原檔**少**東西，因為原檔仍含已退場的 434 與 439。於是最後一格拿了原檔那張圖，把兩組早該退場的說明文字畫回來疊在最後那條長算式上（第一個現象），而真正的最後一格（443＋446）**從來沒有被渲染**（第二個現象）。實測那兩張圖的墨量完全相同（0.01630738 對 0.01630738），因為它們本來就是同一張。
...
- [x] **`PUT /script` 補上提交**（[detail.ts](backend/src/routes/pdfs/detail.ts)）：原本只寫檔案、更新 DB 與 metadata，所以這樣寫進去的逐字稿**沒有版本可還原**，`/script/history` 也看不到它。commit 訊息記下來源：bearer token 來的寫 `via MCP`，其餘寫 `via API`。**這個區分不是裝飾**——在這之前「這頁是 makeslide 產生的還是 agent 寫的」之所以答得出來，正是因為 agent 的寫入完全不留 commit；現在它會留了，訊息就必須承擔原本由「沒有記錄」承擔的資訊。
- [x] **判斷來源不需要新機制**：[server.ts](backend/src/server.ts) 早就把 MCP／OAuth 的 bearer token 換成一份合成 session，其 email 固定是 `<account>@mcp.local`，所以 `isMcpTokenRequest()` 看的就是這個；順手把該網域從 hook 裡的字面值提成共用常數（[auth.ts](backend/src/routes/auth.ts)）。
- [x] **內容沒變就不會多一個版本**：`commitPresentationFile` 對未變更的檔案是 no-op，所以打開逐字稿再原封存回去不會堆出一串彼此完全相同的版本。
- 測試：[script-put-version-control.test.ts](backend/test/script-put-version-control.test.ts) 6 條（4 條行為測試**先確認在修正前會失敗**）——兩種 commit 訊息、`/script/history` 兩個版本且新的在前、舊版本讀得回來（讀不回來的歷史等於不能還原）、未變更不新增版本。相關既有套件：presentation-git 20/20、mcp-token-auth／detail-permission／versioning-permission／scripts-txt／narration-transcript 合計 131/131。分支 `feat/script-put-version-control` → master。

接著使用者問「react page 的語音和一般語音有何不同，為什麼 react page 明明有語音卻顯示無語音」。查出來與 React 無關：

- [x] **根因是分步語音**（[pptx-animated-import-design.md](docs/pptx-animated-import-design.md) §4）：動畫 PPTX 匯入的頁面語音是一步一段（`<uid>.step-00.m4a`…），`pages.audio_path` 留空，所以 API 回的頁層級 `audio_url` 是 null、真正的網址在 `steps[].audio_url`。任何用頁層級欄位問「這頁有沒有語音」的地方，都會把一頁錄了五段旁白的簡報判成無聲——播放鍵因此 disabled 且標成「此頁無語音」。實測這台機器上 **17 個分步頁面全部如此，而且全部是 React 頁**（`importPptx` 透過 `writeReactSlideForPage` 建立），所以看起來像 React 頁的問題；另外 27 個非分步的 React 頁 `audio_path` 都有值、播放正常。
- [x] **改成一律問同一個函式**：`playableStepAudioUrl` 本來就答得對（一般頁自動退回頁層級），把 seek、seek-to-time、retry、pause-playback 效果、`<audio>` 的兩條錯誤路徑，以及面板的播放鍵與「已暫停」圓標全部改用它；context 多帶一個解析後的 `currentStepAudioUrl`，讓面板判斷的值與播放器實際載入的是同一個。預取下一頁改問第 0 步——進頁就是從那裡開始。
- 測試：[stepAudioWiring.test.ts](frontend/src/pages/play/stepAudioWiring.test.ts) 3 條原始碼守門（**先確認在修正前會失敗**），釘住「不得再用 `currentPage.audio_url`／`playablePageAudioUrl(currentPage)` 判斷」。純函式部分沿用既有的 `pageAudio` 與 `stepPlayback` 測試，合計 30/30。分支 `fix/step-page-audio-detection` → master。

最後使用者指出播放器那列的 play/pause 圖示不見了。查證屬實：

- [x] **是 `f25c7400`（四大版面抽離重構）搬走的**，原位置在頁碼與「下一頁」之間，重構後只留下一行「已移到投影片舞台右上角」的註解——剩下的那列有音量、有進度條、有上下頁，卻沒有任何地方可以開始播放。按鈕已放回原位。
- [x] **兩處共用同一個 `renderPlaybackButton`**：它其實是三顆按鈕（載入失敗→重試／沒有旁白→disabled 並說明／真正的切換），手寫兩份必然漂移。notebook 頁乾脆不畫按鈕而不是畫一顆按不動的；「無旁白」的判斷用的是上面那條分步語音的結果。舞台角落那顆保留（它疊在投影片上，看著投影片就能按），要不要只留一顆等使用者決定。
- 測試：[playbackButtonWiring.test.ts](frontend/src/pages/play/playbackButtonWiring.test.ts) 3 條（**先確認在修正前會失敗**）。抽出共用函式後回頭更新了分步語音那條守門，並**還原舊程式碼重跑確認它仍然會失敗**（沒有退化成恆真）。前端 `tsc`＋`vite build` 通過。分支 `fix/player-row-play-button` → master。
- 未做：沒有實機視覺驗證（部署擋在 Google OAuth 後，自動化過不了登入）；`PlayPageFullscreen` 的播放鍵沒有 disabled 狀態（本來就沒有），這輪沒動。

## MCP 也能改頁面型別與 React 頁的程式碼（使用者要求，2026-09-09）★ 使用者要求功能，不計入計數

使用者問「目前是否有功能可以改變頁面的類別，並在類別是 react 時直接改變其中的 script」。查證結果：UI 有（[PageTypeDialog.tsx](frontend/src/pages/play/PageTypeDialog.tsx) 三選一、[ReactSlideTab.tsx](frontend/src/pages/play/ReactSlideTab.tsx) 有程式碼編輯區）、REST API 有（[react-slide.ts](backend/src/routes/pdfs/react-slide.ts) 20 個端點），但 **MCP 一個都沒有**——53 個工具裡 notebook 方向齊全（`set_page_notebook`／`convert_page_to_slide`），React 方向整個缺席，`mcp-server.ts` 唯一出現 "react" 的地方是一個顯示用的中文標籤。AI 導師的工具（[aiTools.ts](backend/src/services/aiTools.ts)）也沒有。

- [x] **四個工具，形狀照抄 notebook 那組**：`get_page_react_slide`（讀 JSX＋這份簡報的主題）／`set_page_react_slide`（寫 JSX 並轉成 React 頁）／`generate_page_react_slide`（AI 依一句描述生成整頁）／`convert_react_page_to_slide`（轉回圖片投影片）。全部建立在 React 分頁本來就在用的 REST 端點上，沒有新增後端路由——那條路上有權限檢查與烘焙守門，繞過去等於把它們關掉。ChatGPT 那條 HTTP 傳輸（[mcpHttp.ts](backend/src/routes/mcpHttp.ts)）沿用同一份 `TOOLS`／`callTool`，不必另外接線就一起有了。
- [x] **兩件不講清楚 agent 就會做錯的事**：(1) 從沒當過 React 頁的頁面**也讀得到程式碼**（後端回預設骨架，與 `get_page_notebook` 同樣的設計），所以回應開頭一定要講出這一頁真正的型別，否則「我拿到程式碼了」會被讀成「這一頁是 React 頁」，下一次寫入就把一張圖片頁換掉了。(2) 轉回圖片時的烘焙**就是**把 React 頁上加的東西變成圖片像素的那一步（[page-overlay-and-fusion.md](docs/page-overlay-and-fusion.md) §2.2），失敗不能吞掉——`force` 有提供，但描述直接寫明它的後果是退回轉成 React 之前的舊圖。
- [x] **程式碼契約在送出前先擋一次**：`window.SlideComponent` 沒指派、空字串、超過 60000 字都在工具裡就回絕。後端當然也會擋，但它的訊息長在編譯錯誤裡，而「少寫一行指派」從編譯錯誤看不出來，先講一次不花任何往返。
- [x] **長度上限是複製而不是 import**：`mcp-server.ts` 只依賴 Node 內建模組才能被 curl 下來單獨執行（檔頭的安裝說明就是這樣寫的）。複製的代價是會與後端漂移，所以加了一條守門測試直接比對 `services/reactSlide.ts` 的實際值。
- [x] **文件**：[mcp-guide.md](docs/mcp-guide.md) 新增「React 投影片頁面」一節（雙語，含程式碼契約、主題 CSS 變數、背景採用、烘焙前提與 `force` 的後果），並把過時的「全部 47 個工具」更正為 53。
- 測試：新增 [mcp-react-slide.test.ts](backend/test/mcp-react-slide.test.ts) 14 條（12 條走真的 stdio MCP＋工具清單守門＋常數守門），**13 條先確認在改動前會失敗**。進入 React 模式需要瀏覽器而測試環境刻意停用烘焙，所以那條測試依環境斷言正確的那一種行為（能烘焙→型別真的翻成 `react`；不能→必須被 `BAKE_UNAVAILABLE` 擋下且型別不動），**兩個分支都實跑過**（有無 `MAKESLIDE_TEST_ALLOW_BROWSER` 各一次，均 14/14）；「改一個已經是 React 頁的程式碼」則繞過受限的 PUT 直接進入 React 模式來測，兩種環境都涵蓋得到。驗證：後端 tsc 全綠、MCP 相關 133/133、**後端完整套件 2171 項 2142 通過 26 失敗，失敗清單與 master 基線逐字相同**（切回 master 重跑比對）。
- 註：完整套件第一次跑時多了一條失敗（`MCP output-language parameter over stdio`），單獨跑該檔通過、重跑完整套件也不再出現——是既有的平行執行互相干擾（本檔多處記錄過的同一現象），但本輪多加了一個會 spawn 子行程的測試檔，確實讓它更容易被踩到。
- 未做：`generate_page_react_slide` 沒有實測（需要模型金鑰，與 `generate_page_notebook` 同樣的限制）；AI 導師的工具集仍然沒有 React 相關工具，那是另一個授權模型（唯讀＋提案），沒有一併加。**線上要等下一次重啟 `start.sh` 才會有這四個工具**。

## 提供一個可以給 ChatGPT 用的 MCP 伺服器（使用者要求，2026-09-09）★ 使用者要求功能，不計入計數

- [x] **釐清為什麼既有的 MCP 接不上 ChatGPT**（查證 OpenAI 文件與 ChatGPT connector 行為）：`mcp-server.ts` 只講 stdio，而 ChatGPT 只對公開 HTTPS 網址發 HTTP 請求；更關鍵的是它新增自訂 connector 時**認證只有「OAuth」與「不認證」兩個選項**，沒有可以填 API key 或自訂 header 的欄位，所以設定頁產生的 MCP auth token 完全沒地方放。把 token 塞進網址（`/mcp/<token>`）也走不通——ChatGPT 對網址內含密鑰的伺服器會判定不安全而拒絕。結論是要讓 ChatGPT 認得，就得真的實作 OAuth（使用者於本輪裁示走這條，並要求暴露全部工具）。
- [x] **`POST /mcp`：無狀態 Streamable HTTP 傳輸**（[mcpHttp.ts](backend/src/routes/mcpHttp.ts)）。工具定義與實作原封不動沿用 `mcp-server.ts` 的 `TOOLS`／`callTool`，一份程式碼兩種傳輸，不會漂移成「stdio 有、HTTP 沒有」。工具呼叫仍走後端 REST API 而不是直接呼叫內部函式——REST 那條路上有權限檢查、帳號情境與狀態機守門，繞過去等於把它們全部關掉；只把「用誰的身分」換成這次請求的 OAuth token。不發 `Mcp-Session-Id`，後端重啟或多開 process 都不會弄斷 ChatGPT 那端。
- [x] **最小 OAuth 2.1 授權伺服器**（[mcpOAuth.ts 服務](backend/src/services/mcpOAuth.ts)＋[路由](backend/src/routes/mcpOAuth.ts)）：探索 metadata（`code_challenge_methods_supported` 必須列 S256，否則 ChatGPT 直接判不合規）、動態註冊、授權碼＋PKCE S256、refresh token 輪替。使用者不必註冊任何外部服務、也沒有 client id／secret 要設定——ChatGPT 自己註冊，使用者只在同意頁按一次「允許連線」。授權綁定到按下核可的那個帳號，能做的事等同該帳號在瀏覽器裡能做的，授權模型沒有放寬。token 以 SHA-256 雜湊入庫。
- [x] **補上 ChatGPT 硬性要求的 `search`／`fetch`**：名稱必須逐字相符且唯讀——**沒開 Developer Mode 時，少了這兩個 ChatGPT 會拒絕整個 connector**，深度研究模式也只用這兩個。`search` 直接借用既有的 `/api/search`（語意搜尋失敗自動退回關鍵字），因此連可讀性過濾都是沿用的，搜得到的永遠只有該帳號本來就讀得到的簡報。Claude Code 也拿得到這兩個工具，只是多兩個搜尋入口。
- [x] **`mcp-server.ts` 的改動限縮在「可被 import」**：base URL 與 token 從模組常數改成 per-call 情境（一個 process 現在要同時服務多個帳號），stdio 傳輸改成只在「這個檔案是行程進入點」時才啟動。零依賴、可 curl 下來直接跑的性質不變，既有 stdio 測試 112/112 照樣通過。
- [x] **踩到並修掉的實作陷阱**：`WWW-Authenticate` header 原本帶中文說明，Node 會以 `ERR_INVALID_CHAR` 拒發、整個回應變成 500——client 就再也看不到那個指路用的 `resource_metadata`，ChatGPT 只會看到連不上而沒有下一步。header 改純英文，中文訊息留在 body。另外 Fastify 內建只解析 JSON，OAuth 的 form-urlencoded 需要自備 parser（寫在 plugin 內，不影響其餘 API）。
- 測試：新增 [mcp-remote-oauth.test.ts](backend/test/mcp-remote-oauth.test.ts) 26 條，全走真的 HTTP（`tools/call` 會讓後端從內部再打一次迴圈請求回自己，沒有真的在監聽就跑不起來，而那正是「OAuth token 有沒有被認回正確帳號」的唯一實測點）。安全性質逐條釘住：PKCE 驗證失敗、授權碼只能用一次、未註冊的 `redirect_uri` 絕不轉址（放行等於任何人都能把授權碼導走）、token 端點的 `redirect_uri` 必須相符、refresh 輪替後舊值立即失效、未認證時 `WWW-Authenticate` 必須帶 `resource_metadata`。端到端驗證工具呼叫建立出來的簡報 `owner_sub` 確實是當初授權的帳號。驗證：後端 tsc 全綠、MCP 相關 112/112、**後端完整套件 2150 項 2121 通過 26 失敗，對照 master 的 2124／2095／26，該 26 條逐字相同**（切回 master 重跑比對）。
- 文件：新增 [chatgpt-mcp-guide.md](docs/chatgpt-mcp-guide.md)（雙語，含前置需求、設定步驟、安全性與疑難排解），[mcp-guide.md](docs/mcp-guide.md) 開頭加指路段落，`.env.example` 加 `MAKESLIDE_PUBLIC_URL` 與 `MAKESLIDE_MCP_LOOPBACK_URL` 兩項說明。
- 未做：**沒有對真正的 ChatGPT 實測**——需要一個公開 HTTPS 網址與 ChatGPT Developer Mode 帳號，本機環境兩者都沒有。協議與 OAuth 行為是照 OpenAI 文件與 RFC 實作並以測試驗證的，但「ChatGPT 這個 client 實際連得上」尚待使用者用自己的部署確認。另外授權後沒有畫面可以列出或撤銷已授權的 connector（目前要從資料庫 `mcp_oauth_tokens` 刪列），視需要可再補一個設定頁區塊。
...
- [x] **根因在 [server.ts](backend/src/server.ts) 的 `resolveAccountIdForRequest`**：只要請求路徑帶簡報 `:id`，整條請求鏈就跑在該簡報 `owner_sub` 的帳號情境裡。這對「幫這份簡報做事」的工作（pipeline／regenerate／add-pages）是對的，[accountContext.ts](backend/src/services/accountContext.ts) 開頭也寫明用意是「結果不受觸發者的登入身分影響」。但 AI 導師的路徑 `/api/pdfs/:id/pages/:n/ask` 也帶 `:id`，而它是**觀看者的即時互動**，不是簡報的工作——結果是任何讀得到簡報的人（含只拿到分享連結的登入者）問導師，花的都是擁有者的 key、每週額度與計費，擁有者既看不到也擋不掉。
- [x] **改成發問者自己的帳號**（使用者裁示：一律用自己的 key，沒設就擋）：守門與 `streamChatText` 都包進 `runWithAccountId(askerAccountId, …)`，key、額度與 `recordDefaultSourceLlmUsage` 的歸屬一起換人。沒 key 就回 400 `API_KEY_MISSING`——前端 [common.ts](frontend/src/lib/api/common.ts) 的 `parseErrorBody` 早就會把這個 code 轉成「請先設定 API key」對話框，所以前端不必改。
- [x] **`toolContext.accountId` 刻意留給擁有者**：唯讀工具是拿它比對簡報 `owner_sub` 來授權讀取的（[aiTools.ts](backend/src/services/aiTools.ts) 第 113／144 行），換成發問者會讓工具讀不到這份簡報，導師就查不到跨頁資料。這是這次唯一會靜默壞掉的地方，所以在改動處寫了註解說明為什麼兩個 accountId 不能合成一個。
- [x] **順帶釐清另外兩條會造成同樣錯覺的路徑**（這次沒改，但已確認行為）：(1) 部署環境若有 `OPENAI_API_KEY`，每個沒填 key 的帳號都會繼承那把「營運者共用預設 key」，受 `DEFAULT_SOURCE_WEEKLY_QUOTA_USD`（預設 1 USD／週，每週四 UTC 重設）限制；(2) `/api/system/openai-key-status` 的 `has_key` 只看主要 provider（onboarding 提示看它），`llm_enabled` 才把次要 provider 的備援算進去——所以「提示跳出來」不等於「功能不能用」。
- 測試：新增 [page-ask-account.test.ts](backend/test/page-ask-account.test.ts) 3 條——讀者沒 key 被擋（400 API_KEY_MISSING）、讀者有 key 時模型收到的是**讀者的** model id 而非擁有者的、擁有者問自己的簡報照樣可用。前兩條**先確認在修正前會失敗**。沒 key 那條刻意不注入 stub client（`hasTestOpenAIClient()` 會讓守門一律放行），並把擁有者的 base URL 指向 `http://127.0.0.1:1/v1`，萬一守門錯誤放行也只會立刻 ECONNREFUSED、不會打到外部服務。驗證：後端 tsc 全綠、導師相關既有測試 36/36、測驗與權限 146/146、**後端完整套件 2122 項 2093 通過 26 失敗，該 26 條與 master 基線逐字相同**（切回 master 重跑比對），與本次改動無關。
- 未做：`tutor-quiz`（AI 出題練習）也是學生端互動、同樣走擁有者的 key，這輪沒動——它牽涉的是「學生練習」而非單次問答，關掉的影響面不同，留給使用者裁示。
...
- [x] **根因** [speakingTimeEstimate.ts](frontend/src/lib/speakingTimeEstimate.ts)：整列建立在「一個字元 ≈ 一個音節」上（`Math.round(chars / 4)`）。中文成立，英文完全不成立——英文一個字平均五、六個字元，同一段話的字元數約是中文的三倍，再除以同一個 4，時間就高估三、四倍。而且被算進去的 `2499` 還包含 `[[ 語氣 ]]` 標記與 `Speaker N:` 標籤，那些送進 TTS 前就被剝掉、根本不會唸。
- [x] **改為照文字自己的語言計數**：CJK 逐字（維持每秒 4 字）、拉丁逐詞（每分鐘 140 字，與後端 [contentLanguage.ts](backend/src/services/contentLanguage.ts) 換算逐字稿長度目標時用的同一組數字，「要寫多長」與「估計多久」不再各說各話），兩邊各算各的再相加。判斷依據是**文字本身而不是簡報的語言設定**——這一列講的是編輯框裡這一段有多長，單頁被改寫成另一個語言時也要對；中英夾雜自然就是兩種單位各算一份。
- [x] **不會被唸的不算**：`[[ 語氣 ]]`、舊版 `{{ … }}`、Gemini 的單括號英文標籤、行首 `Speaker N:` 標籤（與後端 `stripSpokenToneTags`／`splitSpeakerPrefix` 對齊）。副作用：既有中文簡報顯示的字數會比以前略少——少掉的正是不會發聲的部分。
- [x] **標籤跟著單位走**：英文 `{n} words`／「{n} 個英文字」、中文維持 `{n} chars`／「{n} 字」，兩者都佔可觀時間時用合併形式 `{n} chars + {w} words`。次要語言佔比不到 10% 的秒數就不換單位（中文稿裡一個產品名不該讓那列變成「N 字 + 1 個英文字」），但它的時間照算。UI 語言與內容語言是兩回事，這裡由內容決定單位、由 UI 語言決定用哪個 locale 字串。
- [x] **順手移除** `frontend/src/lib/speechDuration.ts`（與其測試）：同一套 `chars ÷ 4` 的第二份實作，沒有任何地方引用，留著就是一份與新版互相矛盾的估算。
- 測試 [speakingTimeEstimate.test.ts](frontend/src/lib/speakingTimeEstimate.test.ts) 11 項（原 4）：中文維持原速率、英文 140 字＝一分鐘、數字不論幾位都算一個字、標記與講者標籤不計入、**回歸守門**（約 2500 字元的雙人英文頁 → 約 1:57，舊版 10:25）、混合計數、次要語言不換單位、空白輸入、標籤 key 選擇、`m:ss` 格式。驗證：前端 `tsc`＋`vite build` 通過、全套 1130/1130；以截圖那三段實地驗算：舊版 `311 字 · 1:18` → 新版 `30 個英文字 · 0:13`。分支 `fix/english-script-length-estimate`，已以 `--no-ff` merge 回 master。**未做實機視覺驗證**。
- 已知限制：每分鐘 140 字是通用值，不隨 `tts_speed` 或人設調整；縮寫（"API"）與長數字（"541 million"）都各算一個字，實際唸起來比一般字久一些，所以英文的估計偏保守。
...
- [x] **設計文件** [docs/page-elements.md](docs/page-elements.md)：與既有「React 頁＋烘焙」（[page-overlay-and-fusion.md](docs/page-overlay-and-fusion.md)）平行的另一條路——元素是結構化 JSON 而非 JSX、合成用 `node-canvas`（已是專案相依）而非 headless Chrome、不用轉換頁面型別、GSAP 動畫照常在合成圖上播。真相來源原則不變：**`<uid>.jpg` 從「底圖」變成「合成結果」，底圖搬到 `<uid>.base.jpg`**，全 codebase 十幾個讀 `image_path` 的 AI 路徑與八個匯出路徑一個都不用改——這就是「先合成單張圖再用原來的」。
- [x] **資料模型**：`pages/<uid>.elements.json`（`version`＋元素陣列，順序＝疊放順序）、`<uid>.base.jpg`、素材 `<uid>.el-<nanoid8>.<ext>`；`pages.elements_path` 欄位（NULL＝沒有元素）。座標存 0..1 比例（與手寫畫布同），字級／線寬／內距存「參考像素」（底圖高 1080 為基準），瀏覽器與伺服器各乘自己的比例。顏色只收 `#rrggbb`／`#rrggbbaa`／`rgba()`；字型鍵 `sans`／`serif`／`mono`／`kai` 兩端各對一組字型堆疊。
- [x] **後端** [pageElements.ts](backend/src/services/pageElements.ts)＋[pageElementsRender.ts](backend/src/services/pageElementsRender.ts)＋[page-elements.ts](backend/src/routes/pdfs/page-elements.ts)：zod 全面驗證（≤100 個、文字 ≤2000 字、顏色白名單、素材檔名須以該頁 uid 為前綴）；`PUT …/elements` 首次存檔把當下的圖片採用為底圖、合成（CJK 逐字／拉丁逐詞換行、對齊、底色圓角、底線、旋轉、透明度、圖片 contain/cover/fill、七種圖案）、重做縮圖與封面、提交進簡報 git；清空元素則底圖復位並刪掉所有元素檔。素材上傳 `sharp` 實際解碼、≤8 MB、長邊 >2048 縮圖、不收 SVG。`GET …/base-image` 給前端疊圖用。
- [x] **兩種「寫入新圖」的語意**：`replace-image` 多收 `mode`——`base`（預設；更換底圖，元素保留並重新合成）／`fuse`（AI 重繪的結果已含元素像素，元素層整個拿掉）。前端套用 AI 預覽時依頁面有無元素自動帶 `fuse`，預覽對話框先講明「元素會成為圖片的一部分」；背景批次重繪（`worker/regenerate.ts`）寫回後同樣 fuse。刪頁一併清元素檔；匯出／匯入加 `page-elements.json` sidecar。
- [x] **前端顯示**：detail 內嵌 `elements`＋`base_image_url`；有元素的頁面 `<img>` 改顯示底圖（顯示合成圖再疊一層會重影），[PageElementsLayer](frontend/src/components/slide/PageElementsLayer.tsx) 作為 `SlideRenderer` 的 children 掛在一般面板與全螢幕兩個版面（排在手寫畫布之前，筆跡蓋在元素上），動畫頁跟著 GSAP 一起動；同步 follower／分享唯讀都看得到。
- [x] **前端編輯**：新增「🧩 元素」分頁（[PageElementsTab](frontend/src/pages/play/PageElementsTab.tsx)）——加入文字／上傳圖片（可多選）／七種圖案／更換底圖；屬性面板依選取顯示字型、字級、粗斜底線、水平／垂直對齊、行高、文字色與底色（原生色盤＋hex 輸入＋透明度滑桿）、內距、圓角、填滿方式、填色／描邊／線寬、疊放順序、位置尺寸旋轉不透明度。投影片上直接點選、拖曳、八個縮放把手（Shift 鎖長寬比）、旋轉把手（Shift 吸附 15°）、雙擊文字就地編輯、Delete／方向鍵微調／Ctrl+D／Ctrl+Z／Ctrl+Shift+Z。[usePageElements](frontend/src/pages/play/usePageElements.ts) 管草稿：停手 800ms 自動儲存、換頁或離開分頁時立即送出、儲存狀態與重試。
- [x] **貼上與上傳**：圖片頁貼上／拖放圖片一律變成圖片元素並切到元素分頁（React 頁維持換底圖、notebook 忽略），連貼三張就是三個元素；元素分頁開著時貼純文字變成文字元素。原本「貼上就換底圖」改為明確的「更換底圖」按鈕。
- [x] 測試：後端 [page-elements.test.ts](backend/test/page-elements.test.ts) 11 項（schema／換行／合成取樣像素含旋轉、透明度、文字、圖片／PUT 流程含底圖採用與復位／權限與頁面型別／素材上傳與孤兒清除／`replace-image` 兩種 mode／刪頁清檔／share token）；前端 [pageElements.test.ts](frontend/src/lib/pageElements.test.ts) 9 項純函式＋[pageElementsWiring.test.ts](frontend/src/pages/play/pageElementsWiring.test.ts) 6 項原始碼守門（三個顯示點都掛層、底圖規則、貼上分流、fuse、分頁接線、前後端字型／圖案／顏色白名單一致）。
- [x] **文字改為 Markdown＋數學、線條改成兩端點元素、合成改以 Chrome 優先**（使用者接著要求，2026-09-05）：(1) 文字元素內容是 Markdown，走筆記／留言共用的 `MarkdownMath`（標題、粗斜體、程式碼、安全連結、條列、表格、KaTeX 行內／區塊數學），`.ms-el-md` 樣式以 `em` 相對元素字級；播放時連結可點、編輯時點連結是選取。(2) 直線／箭頭不再是圖案：`LineElement` 存兩個端點 `x1,y1,x2,y2`，投影片上各自拖曳（Shift 吸附 0／45／90°）、拖線身整條平移，箭頭是兩端各自的旗標，面板可直接輸入端點座標。(3) 合成優先用 headless Chrome（沿用 React 頁烘焙的 `renderSlideToJpeg`）——[pageElementsDocument.ts](backend/src/services/pageElementsDocument.ts) 把底圖＋元素排成與畫面相同 CSS 的 HTML，文字由 [markdownMathHtml.ts](backend/src/services/markdownMathHtml.ts)（`MarkdownMath` 的伺服器端雙胞胎，KaTeX 字型內嵌成 data URL、只在有數學時才帶）渲染；沒有 Chrome 或執行時失敗則退回 node-canvas 畫純文字投影。**已用真實 Chrome 合成一頁實測**（標題、行內與區塊公式、連結底線、表格、箭頭、旋轉星形皆正確）。前後端各一份 Markdown 文法，守門測試比對 token 正規表達式與 `.ms-el-md` 樣式區塊。`katex` 加進 backend 相依（lockfile 只多一行）。分支 `feat/page-elements-markdown-lines`，已 merge 回 master 並同步 `worktree/demo16`。
- 驗證：後端 `tsc` 全綠、`page-elements` 15/15（新增 4 項）、`reactSlideBake` 9/9、`export-import-notebook` 2/2、`page-operations-permission` 31/31；前端 `tsc`＋`vite build` 通過、前端全套 1116/1116（守門測試新增 2 項）。（第一版數字：新測試 11/11、前端 1114/1114。）分支 `feat/page-elements`（三個 commit：設計文件／後端／前端），已以 `--no-ff` merge 回 master 並同步到 `worktree/demo16`（2026-09-05）；**未做實機視覺驗證**。
...
- [x] **出現時機與位置交給 AI**（使用者接著要求，2026-09-05；文件 §9.5）：[cutoutPlacement.ts](backend/src/services/cutoutPlacement.ts) 把**抹除後的頁面**、每塊剪下的圖片（附原位）與該頁逐字稿句子（`splitScriptIntoSentences`，最多 20 句）交給有視覺能力的 LLM，逐塊回答「講到哪一句時出現」與「顯示在哪裡」。前者寫成 `startTrigger: { type: 'transcript-line', line, anchor: 'start' }`（數字 `start` 仍是依序值作為沒有旁白時間的備援），後者寫進 `params`，高度一律由圖片長寬比推出、超出頁面往內推。提示詞明說原位通常就是對的、只有明顯更好才搬且不得蓋到文字。模型沒回或超出範圍退回原位；模型失敗或沒有 LLM（`llmAvailability().enabled`）整組退回「原位＋依序出現」。`placer` 與 `eraser` 一樣注入，測試用假 placer 驗證觸發與位置真的進了規格、也驗證 placer 拋錯時退回原位。回應與面板逐塊顯示「第 N 句開始時出現：『…』」與位置。後端 `page-cutouts` 6/6（新增 2 項）、前端全套 1119/1119。分支 `feat/cutout-placement`，已 merge 回 master 並同步 `worktree/demo16`。**未用真實 LLM 實測**挑句與定位品質。
- [x] **自動偵測要剪的區域**（使用者接著要求「是否能自動決定切割的範圍」，2026-09-05；文件 §9.6）：[cutoutDetect.ts](backend/src/services/cutoutDetect.ts)＋`POST …/cutouts/detect`。第一段是決定性的影像分析——縮到 320px、邊框最常見顏色當背景、色距超過門檻為內容、膨脹讓字形連成塊、4-連通元件外框、相近框合併、去掉雜點與整頁框；第二段（有 LLM 時）把畫了紅色編號框的圖與講稿交給視覺模型，只請它回答「哪些編號屬於同一單位、哪些是裝飾不保留、每個單位是什麼」——**模型不產生座標**（避開 VLM 報座標不準的老問題），座標一律來自像素分析。模型失敗或沒有 LLM 就回第一段結果。偵測結果不直接剪：填進區域清單並打開框選模式讓使用者確認增刪；模型的說明成為剪下素材的 caption（`regions[].label`）。測試 [cutout-detect.test.ts](backend/test/cutout-detect.test.ts) 5 項（合成投影片三個區塊含「字形連成一塊」、空白頁與整頁大塊回空、遮罩／元件／合併純函式、refinement 映射、路由含 refiner 注入與失敗退回）。後端 5/5＋`page-cutouts` 6/6、前端全套 1120/1120。分支 `feat/cutout-detect`，已 merge 回 master 並同步 `worktree/demo16`。**未用真實 LLM 實測分組品質**；像素分析對背景是漸層或照片的頁面效果會較差（門檻以單一背景色為準）。
- [x] **重生對話框加入批次剪下**（使用者要求「在重生中加入自動切割圖片並產生動畫的選項，一次處理大量頁面」，2026-09-05；文件 §9.7）：新增重生步驤 `cutout`（body `cutouts: { prompt?, animate? }`），對選中的每個圖片頁依序做偵測（§9.6，有 LLM 就分組標註）→ 裁下、抹除（§9.2）→ 定時定位（§9.5）。**排在最後**——要處理重生後的圖片，且 AI 焦點動畫會整份改寫規格。快照把圖片與動畫規格都備份（`snapshotTargetsFor()`），「回復」能一起還原；偵測不到的頁略過、抹除失敗的頁記錄後繼續。三個模型協作者集中在 [cutoutDeps.ts](backend/src/services/cutoutDeps.ts)，互動路由與批次步驤用同一組（測試 hook 也搬過去）。worker 對剪下服務改用動態 import——靜態 import 會經 `page-operations → worker/regenerate → pageCutouts → reactSlideTextExtract → reactSlide` 形成循環，測試載入時撞 TDZ。前端重生對話框多一個「✂️ 自動剪下區域並產生動畫」選項與說明，進度列多一個步驤標籤。測試 [regenerate-cutouts.test.ts](backend/test/regenerate-cutouts.test.ts) 2 項（四頁：兩頁有區塊、一頁空白、一頁 React——只處理三頁、兩頁產生素材與 `startTrigger` 效果、回復後圖片位元組與 `render_type` 復原；某頁抹除失敗不中斷）。後端 `regenerate-*` 既有測試 17/17；前端 `tsc`＋`vite build` 通過、全套 1120/1120。分支 `feat/regenerate-cutouts`，已 merge 回 master 並同步 `worktree/demo16`。
- [x] **開場不留白、圖片先於語句**（使用者要求，2026-09-06；文件 §9.5）：最上方且上緣落在頁面上方 25%（標題位置）的那一塊改為 `start: 0`、不掛 `startTrigger`，一開始就顯示；其餘塊由「講到第 N 句時出現」改為「第 N-1 句開始時淡入」（`startTrigger.line = N-1, anchor: 'start'`），保證圖片一定在講到它之前出現；配到第一句的直接立即顯示。回應與面板多回報 `reveal`（`immediate`／`before-sentence`／`timeline`）。測試：`page-cutouts` 新增 1 項（標題區、配到第一句、非標題三種情況）並更新既有期望，`regenerate-cutouts` 同步更新；後端 7/7＋2/2，前端 38/38、`vite build` 通過。分支 `feat/cutout-reveal-timing`，已 merge 回 master 並同步 `worktree/demo16`。
- [x] **編輯時看得到已剪下的區域**（使用者回報「之前切割的區域似乎沒有顯示出來」，2026-09-06；文件 §9.8）：剪下後底圖那一塊已抹成背景、疊回來的圖片只在播到它的動畫時才出現，回到元素分頁看起來像少了東西。元素分頁多一節「這一頁已剪下 N 個區域」——列出 `source: 'cutout'` 的插圖（縮圖、說明、位置），勾選「編輯時顯示在原位」（預設開）就以虛線框把每塊畫回動畫效果指定的位置（沒有效果的用原 bbox；[CutoutFiguresPreview.tsx](frontend/src/components/slide/CutoutFiguresPreview.tsx)，純顯示不吃指標），並可一鍵到動畫分頁調整。資料來自 `fetchPageFigures`＋`fetchPageAnimation`，換頁與頁面更新時重抓。守門測試 1 項；前端 `tsc`＋`vite build` 通過、全套 1128/1128。分支 `feat/cutout-existing-preview`，已 merge 回 master 並同步 `worktree/demo16`。
- [x] **剪下歷史＋草稿式編輯：還原／重新框選／隱藏**（使用者要求通盤檢討後採方案 A 並確認細節，2026-09-06；文件 §9.9）：(1) 底圖不再被改寫——第一次剪下時存無損 PNG「剪下原圖」，每剪一塊存 AI 重繪框內像素的「補丁」，`pages/<uid>.cutouts.json` 記錄框／素材／補丁／效果；底圖永遠＝原圖＋仍生效的補丁一次合成、只壓一次 JPEG（[cutoutHistory.ts](backend/src/services/cutoutHistory.ts)）。(2) 還原＝從清單拿掉並重新合成，位元組級精確、不需模型，素材與效果一併刪除（新增 `removePageFigure`）；重新框選＝還原＋以新框重剪（裁圖取自原圖，抹除時模型看到的是目前畫面）；隱藏／顯示只把效果移出／放回規格（移出的效果存在清單裡以便原樣放回）。(3) 草稿式：還原、重新框選、新框全部先記在前端草稿，畫面即時近似——待還原的塊以裁圖疊回原框（綠框）、新框用斜線網底標示將被抹除；按「套用變更」才送 `POST …/cutouts/apply { restore, cut }`，伺服器套用還原、從歷史合成、只對新框跑 AI、底圖寫一次；失敗的框留在草稿可重試；隱藏／顯示立即生效（`PATCH …/cutouts/:figureId`）。(4) 邊界：AI 重繪／更換底圖／融合會清掉歷史（素材與效果保留，只剩隱藏與貼回式還原）；改版前剪的區域以「舊資料」標示，還原時把裁圖貼回原框。批次重生走同一條 `applyCutoutChanges()`，因此也會建立歷史；worker 對剪下服務仍是動態 import。測試 [cutout-history.test.ts](backend/test/cutout-history.test.ts) 4 項（原圖與補丁、精確還原且不呼叫模型、隱藏顯示、舊資料貼回、更換底圖使歷史失效）；`page-cutouts` 7/7、`regenerate-cutouts` 2/2、`cutout-detect` 7/7；前端守門 14/14、全套 1128/1128、`vite build` 通過。分支 `feat/cutout-history-draft`，已 merge 回 master 並同步 `worktree/demo16`。**未做實機視覺驗證**。
- [x] **預覽用未剪下的原圖**（使用者回報「preview 在切割完後幾乎都變成空白」，2026-09-06；文件 §9.10）：有剪下歷史的頁面，縮圖與第一頁封面改由「剪下原圖」產生（`cutoutPreviewSourcePath()`，在 `finishPageImage` 與縮圖路由兩處），縮圖比 manifest 舊時在請求時重建一次（改版前已剪的頁面因此自動更新）。detail 多回 `has_cutouts`，一般面板對這種頁面改用完整圖片而非縮圖（`slideImageUrlForPage`）——縮圖現在是原圖，播放時動畫需要抹除後的底圖才有東西可揭露；全螢幕不變。歷史清掉時回到一般規則。測試：`cutout-history` 新增 1 項（縮圖保留紅色方塊、`/image` 仍是抹除後的底圖、`has_cutouts` 隨還原變 false）；前端 lib 測試補 1 斷言；後端 14/14、前端 1128/1128。分支 `fix/cutout-preview-source`，已 merge 回 master 並同步 `worktree/demo16`。
...
- [x] 修正（使用者要求「把備註改成內容，只顯示備註或是逐字稿內容，不要有內容以外的編輯 UI；要修改還是回原畫面，這個只做為頁面的總結」＋「內容 tab 應該是頁面的預設畫面」「tab 的位置應該不受下一頁上一頁影響」）：分頁改名為 **內容**（`EditTab` 的 `'note'` → `'content'`），內容規則不變但**整個唯讀**——有備註顯示備註（Markdown 渲染）、沒有就顯示逐字稿（`whitespace-pre-wrap` 純文字，逐字稿是含 `[[ ]]` 語氣標記的講稿原文，照原樣顯示比套 Markdown 忠實），兩者皆空給一句提示；標題列、編輯按鈕、textarea、儲存控制全部拿掉。編輯入口回到側邊欄的備註區與全螢幕的備註面板，逐字稿仍在隔壁分頁。另外把預設分頁改成內容（`useScriptEditor` 的初始值），並**移除上一輪的換頁自動選分頁**——它會在每次上一頁／下一頁時把分頁換掉。守門測試改為：內容分頁排第一且是預設、分頁區塊內不得出現任何編輯 UI、每個 `setEditTab` 都必須來自 `onClick`（自動切分頁的程式碼一回來就會紅）。前端 `tsc`＋`vite build` 通過、全套 1080/1080。分支 `feat/page-content-tab`。
...
- [x] 修正（使用者回報「似乎沒有看到預覽」，附側邊欄編輯中的截圖）：第一版只給全螢幕面板預覽——需求字面上說的是全螢幕——但**備註實際上是在側邊欄寫的**，編輯 Markdown 卻看不到渲染結果等於盲打。`PageNoteEditorFields` 的 `preview` 改成 `'split' | 'stack' | false`：全螢幕維持 `split`（左原始碼、右預覽），側邊欄用 `stack`（預覽接在編輯區下方、`max-h-64` 可捲動）。不沿用 `md:grid-cols-2` 是因為 Tailwind 斷點看的是 viewport 而非容器——寬螢幕但側欄窄時會被擠成兩條細長欄。守門測試補上「側邊欄也帶預覽」一條。前端 `tsc`＋`vite build` 通過、全套 1077/1077。分支 `fix/page-note-sidebar-preview`。
- [x] 修正（使用者要求「預覽請使用左右分割」）：側邊欄改回與全螢幕相同的左右分割（`preview` 回到布林），左邊 Markdown 原始碼、右邊即時預覽。分欄改用 `grid-cols-[repeat(auto-fit,minmax(14rem,1fr))]` 而不是 `md:` 斷點——這個編輯器會被放進寬度差很多的容器（全螢幕面板 vs. 可收合的側邊欄），Tailwind 斷點看的是 viewport，照斷點分欄會在寬螢幕的窄側欄裡擠出兩條細長欄；`auto-fit` 看容器自己有多寬，放得下就左右分割、真的太窄（< 28rem）才讓預覽掉到下一列。已確認這個任意值有進到 build 後的 CSS（不是只寫在原始碼裡沒生效）。分支 `fix/page-note-preview-split`。
- 驗證：前端 `tsc --noEmit` 通過、`vite build` 通過、前端全套 1077/1077 通過（新增 9 項）。分支 `feat/page-note-markdown-editor`，已 merge 回 master 並同步到 `worktree/demo16`。**未做實機視覺驗證**（需要有備註的真實簡報與登入環境）。
...
現況盤點：產生語言原本只有帳號層級一份（`settings.env` 的 `CONTENT_LANGUAGE`），要做一份英文簡報就得先去設定頁改、生成完再改回來，而且之後任何一次重生都會默默跟著當下的設定跑。查證時發現 2026-08-09 有一支未 merge 的舊分支 `feat/per-deck-content-language` 已經做完「upload 時可選」那一半（DB 欄位＋AsyncLocalStorage 語言情境＋四個建立入口的選擇器），但沒有「事後在簡報設定裡改」的路徑；該分支 base 停在 8/9 的 master。

- [x] **把舊分支 rebase 到現在的 master**：`feat/per-deck-content-language` 從 8/9 的 base 重定基底到 master，無衝突。核心機制沿用它的設計——`pdfs.content_language`（NULL = 沿用帳號設定，舊資料維持原行為）＋ [contentLanguageContext.ts](backend/src/services/contentLanguageContext.ts) 的 AsyncLocalStorage 覆蓋層（比照 `accountContext`），`getRuntimeAiSettings()` 讀 `contentLanguage` 時先看這一層，因此散在整條管線裡的四十幾個讀取點一個都不用改簽章；pipeline／regenerate／add-pages 在起點、帶 `:id` 的 HTTP 請求在 `onRequest` hook 進入該簡報的語言情境。
- [x] **簡報設定裡可以改**（本輪新增）：[detail.ts](backend/src/routes/pdfs/detail.ts) 新增 `PATCH /api/pdfs/:id/content-language`（`canEditPdf` 把關、同步 `metadata.json`），前端在「生成設定」對話框（[TtsDialog.tsx](frontend/src/pages/play/TtsDialog.tsx)）放上與上傳畫面同一個 `ContentLanguagePicker`，隨語音／主持模式一起存。**只有真的改過才送出**——否則按一次「儲存設定」就會把還在「沿用帳號設定」（`content_language` 為 NULL）的舊簡報寫死成當下的設定值。舊簡報顯示的是 `account_content_language`（沿用會解析出的那一種），使用者按下儲存前不會被寫死。
- [x] **改語言只影響之後產生的內容**：已經產生好的逐字稿／圖片／語音不會自動翻譯，對話框的提示文案直接講明這件事（`play.ttsDialog.contentLanguageHint`，zh-TW／en），免得使用者以為改完就會整份換語言。
- [x] **補上 rebase 後才出現的兩個建立路徑**：`POST /api/pdfs/from-slides`（MCP `upload_slide` 用的端點，8/22 才進 master）改為接受選填 `content_language`，沒帶就比照其他入口記下當下的帳號設定；匯入 `export.zip` 時把封存檔裡的 `content_language` 帶過來（舊 zip 沒有這個欄位就維持 NULL＝沿用本機設定）。
- 驗證：後端 `tsc` 全綠（僅剩 `opencc-js`／`ppu-paddle-ocr` 兩個既有的環境缺套件錯誤）、[deck-content-language.test.ts](backend/test/deck-content-language.test.ts) 11/11（本輪新增 4 組：設定 PATCH 的寫入／讀回／產生情境、無效值被拒、非擁有者 403，以及**驗證 `onRequest` hook 真的會進入簡報語言情境**——請求內同步重生所依賴的機制，先前沒有任何測試端到端跑過它），相關既有測試 `upload-slide`／`export-import-zip-sources`／`export-import-notebook`／`detail-permission` 共 92 項全過；前端 `tsc`＋`vite build` 通過、前端全套 1069/1069（含新增的 2 個 i18n 鍵斷言）。分支 `feat/per-deck-content-language`。
- [x] **MCP 也開放這個設定**（使用者接著要求，同日）：四個建立工具（`upload_pdf`／`upload_txt`／`upload_slide`／`create_blank_deck`）都加上選填 `content_language`，`define_prompt` 也收（開始生成前的最後一次機會），另新增 `set_content_language` 供事後修改。五處共用同一份 `CONTENT_LANGUAGE_PROPERTY` 說明——agent 真正需要知道的是「省略＝使用系統的輸出語言，且**當下就記在這份簡報上**，之後系統設定改了也不會影響它」，這句話各寫一份遲早會有一個漏掉。`set_content_language` 的回應明講既有內容不會被翻譯，免得 agent 回報一個它其實沒做到的語言切換。新增 [mcp-content-language.test.ts](backend/test/mcp-content-language.test.ts)（6 組，走真實 stdio transport）：分別驗證 **schema 真的有公開這個參數**（`tools/list`——工具少了參數，對 agent 而言等於這個功能不存在）與**值真的送達**（含省略時落在帳號語言而非留 NULL）；`upload_pdf`（需要真的 PDF 給 poppler 數頁）與 `define_prompt`（會啟動 pipeline 呼叫模型）只做 schema 斷言。`docs/mcp-guide.md` 同步：補上工具列與說明，並順手補回文件漏列的四個既有工具（`set_page_prompt`／`get_page_preview_url`／`update_animation_effect`／`delete_animation_effect`）、修正早已過時的工具總數（寫 42、實際 46，現為 47），表格與數字現在都與 `mcp-server.ts` 註冊的工具一致。驗證：後端 `tsc` 全綠、新測試 6/6、既有 MCP 測試逐檔全過（`mcp-upload-slide` 7、`mcp-page-crud` 12、`mcp-token-auth` 8、`mcp-page-assets` 12、`mcp-figures` 9、`mcp-notebook` 12、`mcp-animation` 19，共 79 項無回歸）。
- [x] **合併回 master**（使用者再次要求「（設定生成風格對話框）加上語言選擇」，同日）：使用者截圖的「設定生成風格」對話框就是 [PromptModal.tsx](frontend/src/components/PromptModal.tsx)，語言選擇（`promptModal.contentLanguage`／`contentLanguageHint`，排在「主持模式」與「語音人物」之間）在前兩輪就做好了，但整條線一直只存在於 `feat/per-deck-content-language` 分支上——沒 merge，跑 master 的畫面自然看不到，於是看起來像沒做。本輪把分支重定基底到最新 master（只落後兩個 TODO 文件 commit，無衝突）後 `--no-ff` merge 回 master，功能正式生效。驗證：前後端 `tsc` 皆全綠、`deck-content-language.test.ts` 11/11、前端 `i18n.test.ts` 25/25（含語言鍵完整性斷言）。

## 全螢幕頂端徽章與投票鈕／工具列互相重疊（使用者回報，2026-09-03）★ 使用者回報 bug，不計入計數

使用者回報：全螢幕時，投票和頁面評論的圖示會重疊。

根因：全螢幕頂端同一條水平線上其實釘著**三組互不知情的 absolute 疊層**——中央的指示徽章列（🗳／📝／💬，`left-1/2 top-4`）、右上的 🗳 投票鈕與版面切換／離開工具列（都是 `right-4 top-4`）、以及 follower 的「提問」按鈕（`left-4 top-4`）。它們各自釘座標，沒有任何機制讓彼此讓開：視窗一窄、或工具列因語系／版面模式變長，右側那組就會往中央長過來壓住 💬／🗳 徽章。是否重疊只取決於當下視窗寬度，所以在寬螢幕上看起來一直沒事。

- [x] **改由 layout 保證不重疊**：[PlayPageFullscreen.tsx](frontend/src/pages/play/PlayPageFullscreen.tsx) 把這三組收進同一個 `absolute inset-x-0 top-0 z-40` 的三欄格線 `grid-cols-[1fr_auto_1fr]`（左：follower 提問；中：指示徽章列；右：投票鈕＋版面工具列＋離開）。中欄 `auto` 先取得所需寬度、左右欄各分剩餘空間，欄與欄之間只會互相推擠，任何寬度都不會疊在一起；徽章仍置中、工具列仍靠右，視覺與原本一致。
- [x] **點擊行為不變**：格線容器橫跨整個畫面寬度，故維持 `pointer-events-none`（否則會擋住底下全高的上一頁／下一頁點擊區與投影片的播放／暫停切換），可互動元素各自 `pointer-events-auto`；工具列與投票鈕原有的 `stopPropagation` 全部保留。投票鈕從 `z-40` 併入容器層級，展開的投票面板（`right-4 top-20`）與留言面板（`top-16`）仍在頂端列下方。
- [x] **守門測試**：新增 [fullscreenTopBar.test.ts](frontend/src/pages/play/fullscreenTopBar.test.ts)——斷言原始碼中不再出現任何自己釘在 `top-4` 的疊層（`left-1/2`／`right-4`／`left-4`），且徽章、投票鈕、版面工具列、離開按鈕都落在同一個頂端格線容器內，容器本身不吃點擊而子元素有開 `pointer-events-auto`。比照 [floatingLayerOrder.test.ts](frontend/src/pages/play/floatingLayerOrder.test.ts) 走原始碼層級斷言：此專案沒有渲染測試環境，且「幾何是否重疊」隨視窗寬度而變，釘住修法本身比釘住某個剛好會顯現的視窗尺寸可靠。
- 驗證：前端 `tsc --noEmit` 通過、`vite build` 通過、前端全套 1068/1068 通過（含新增 3 項）。分支 `fix/fullscreen-top-overlay-collision`，已 merge 回 master 並同步到 `worktree/demo16`。


## 全螢幕評論標記可點擊查看（使用者要求，2026-09-03）★ 使用者要求，不計入計數

使用者要求：簡報頁面有頁面評論時，請在全螢幕模式特別標記，讓使用者可以點擊查看評論。

現況盤點：全螢幕頂端本來就有 💬 標記（`feat/page-note-comment-indicators` 那輪加的，資料來自 deck detail 的 `has_comment`），但它是靜態 `<span>`、且外層容器為 `pointer-events-none`——只能看到「這頁有留言」，要讀內容必須離開全螢幕、回側邊欄的留言區，授課中等於做不到。

- [x] **徽章改為可點按鈕**：[PlayPageFullscreen.tsx](frontend/src/pages/play/PlayPageFullscreen.tsx) 的 `has_comment` 標記改成 `pointer-events-auto` 的 `<button>`（`stopPropagation` 以免冒泡到根容器的播放／暫停切換，比照旁邊的 🗳 開始投票按鈕），點擊開關本頁留言面板；徽章同時顯示留言則數，有未解決留言時加脈動外環（`animate-pulse` + ring）強化標記。
- [x] **唯讀留言面板**：新增 [FullscreenCommentsPanel.tsx](frontend/src/pages/play/FullscreenCommentsPanel.tsx)——`useFullscreenPageComments(pdfId, pageNumber, enabled, shareToken)` 以既有 `listPageComments` 載入本頁留言（帶 share token，故分享連結的觀看者也讀得到），**徽章的則數與面板共用同一份狀態、不重抓**；面板以 `sortCommentsUnresolvedFirst` 未解決在前列出作者、相對時間與 `MarkdownMath` 內容，已解決者淡化加刪除線，另有重新整理／關閉。全螢幕以觀看／授課為主，刻意只做唯讀（新增／編輯／解決仍在側邊欄），避免在全螢幕塞進一整套編輯 UI。
- [x] **換頁行為**：換到沒有留言的頁面時自動收掉面板（比照既有提問面板 `fullscreenQuestionsOpen` 的自動收合），避免空面板卡在畫面上；換到另一有留言的頁面則自動改載該頁留言。
- [x] i18n：新增 `play.fullscreen.commentsBadge`（帶 `{count}`）／`commentsTitle`（帶 `{page}`）／`commentsLoading`／`commentsFailed`／`commentsEmpty`／`commentsRefresh`／`commentsClose`／`commentsResolvedTag`（zh-TW／en），並加入 `i18n.test.ts` 既有的「PlayPageFullscreen locale keys are complete」清單。
- 驗證：前端 `tsc --noEmit` 通過、`vite build` 通過、前端全套 1065/1065 通過（含新增的 8 個 i18n 鍵斷言）。分支 `feat/fullscreen-comment-badge`，已 merge 回 master。

## audio.cpp 語音人物選單只有一個空字串選項，開始生成時報 zod 驗證錯誤（使用者回報＋截圖，2026-08-23）★ 使用者回報 bug，不計入計數

使用者在「設定生成風格」對話框按「開始生成」時看到 `String must contain at least 1 character(s)`。追查發現：`TTS_VOICES_BY_PROVIDER.audiocpp` 一直是空陣列，理由（原註解）是「audio.cpp 的音色屬於目前安裝的模型家族，前端無從列舉」。但使用者追問「audiocpp 其實有 speaker 列表，為什麼沒有被使用」——查證後發現這個理由已經過時：`AUDIOCPP_QWEN3_VOICES`（本專案 audio.cpp 實際使用的 Qwen3-TTS CustomVoice 家族內建的 9 個 speaker）早就存在，但只接給了設定頁的 `AudioCppVoiceField`，从未回頭接進每份簡報可個別選音色的 `TTS_VOICES_BY_PROVIDER`，導致選單只剩一個代表「使用設定中的聲音」的空字串選項，而這個空字串又撞上後端兩處 `z.string().min(1)` 驗證（`/start` 與 `PATCH /tts-settings`），因此被拒。

- [x] **把清單接上、修正連帶的驗證邏輯**（分支 `fix/audiocpp-voice-list`，已 merge 回 master）：(1) `TTS_VOICES_BY_PROVIDER.audiocpp` 改為 `AUDIOCPP_QWEN3_VOICES` 的 9 個 id，`DEFAULT_TTS_VOICE_BY_PROVIDER.audiocpp` 維持空字串（仍是合法值，代表「沿用 family 預設」，對應 `audioCppEffectiveVoice` 的 fallback 邏輯）；`PromptModal.tsx`／`TtsDialog.tsx` 的「使用設定中的聲音」選項改成固定顯示（不再只在清單為空時才出現），讓使用者能在「明確挑一個 speaker」與「繼承預設」之間選。(2) 放寬 `StartBodySchema.tts_voice` 與 `PATCH /tts-settings` 的 `tts_voice` schema，移除 `min(1)`——空字串本來就是合法值，下游本來就會把它視為 `null`／繼承預設。(3) **意外抓到一個更嚴重的獨立 bug**：`isSupportedVoiceByProvider` 對非 openai 的 provider 一律拿 Gemini 音色清單去比對，所以就算清單修好了，選了真正的 speaker（如 `vivian`）送出 `/start` 一樣會被 400 打回「不支援的 tts_voice」——已加上 audiocpp 的短路（音色是否合法取決於伺服器裝的是哪個模型家族，無法用靜態清單擋）。(4) 同步修正一句過時文案（原本寫「本機引擎無固定音色清單」，現在已經有了）。**測試過程中的插曲**：跑後端 `provider-availability.test.ts`／`upload-pipeline-control-permission.test.ts` 時第一次卡住 5+ 小時，一路查到 `worker/regenerate.ts` 裡一個背景 regenerate job 在 `persistRegenerateJob` 丟出 `SqliteError: FOREIGN KEY constraint failed` 後沒有正確結束，讓整個測試檔案的 process 永遠不退出——已在**完全未改動的原始 master 程式碼**上重現同樣的卡住，確認與本次修改無關；用 `--test-timeout` 限時重跑後確認兩個檔案全部 23/23 個子測試（含 `/start`、tts_voice 相關）都通過，只有檔案本身因為這個既有 bug 被標記逾時，記錄於下方待辦。驗證：前後端 `tsc` 全綠、前端 `vite build` 成功、前端測試套件 1059/1059、`provider-availability.test.ts`＋`upload-pipeline-control-permission.test.ts` 子測試 23/23（檔案層級因既有無關 bug 逾時）。
- [x] **測試 process 永遠不退出**（本案測試時發現，與 tts_voice 無關；分支 `fix/export-timers-block-process-exit`，已 merge 回 master）：**上一輪的歸因是錯的，本輪查出真因**。原本記為「`worker/regenerate.ts` 的 `persistRegenerateJob` 遇到 `FOREIGN KEY constraint failed` 沒有正確結束」——那個錯誤其實**已經被 catch 並 log**（`regenerate job runner rejected`），只是剛好是最後一行輸出，看起來像卡在那裡；它本身是測試在 `finally` 刪掉 pdfs 列、背景 job 隨後才寫 `regenerate_jobs` 造成的競態，會 log 但不會擋住退出。真正把 process 撐住的是[export-job.ts](backend/src/routes/pdfs/export-job.ts) 與 [batch-export.ts](backend/src/routes/pdfs/batch-export.ts) 兩個 module-level 的 5 分鐘清理 `setInterval` **沒有 `.unref()`**——它們清的是純記憶體中的 job Map，卻讓 event loop 永遠有 handle。**影響範圍也比原本記的大得多**：路由由 `buildApp()` 內的 `await import()` 載入，所以**全部 136 個會 `buildApp()` 的測試檔都會掛住**，不只原本點名的那兩個（實測 `auth-cookie-secure`／`blank-deck`／`error-handler-info-leak` 同樣掛住）。修法就是補上 `.unref()`，與專案既有慣例一致（`server.ts` 的 rescan timer、`page-operations.ts` 的 SSE keep-alive 都已 `unref`，後者註解明寫「so a forgotten timer can never hold the process open」）。查法：patch 全域 `setTimeout`／`setInterval` 記錄建立堆疊，在 `app.close()` 後印出仍存活的 timer，直接指出這兩行。新增 [process-exits-after-import.test.ts](backend/test/process-exits-after-import.test.ts)：spawn 一個 child 建立並關閉 app、斷言它自己退出（`spawnSync` 的 `timeout` 逾時會回 signal），比 grep 原始碼可靠，任何未來忘記 `unref` 的 timer 都會被抓到。**探針一定要 `buildApp()` 而不只是 `import server.ts`**——第一版只 import，因為路由是動態載入的，即使拿掉 `.unref()` 也照樣通過，等於白寫；已改正並實測「拿掉 `unref` 會失敗、補回來會通過」。驗證：後端 `tsc` 全綠；原本掛住的 `provider-availability`＋`upload-pipeline-control-permission` 現在 23/23 通過**且正常退出**；export 相關 6 檔 29/29；**後端全套現在能跑完了——2071 項、2043 通過、25 失敗、37 秒**（以前是無限掛住），25 個失敗逐一核對均為既有、與本次無關（Gemini API 逾時 7、git commit／merge 3、react-slide 6、regenerate-rollback 3、TTS preview 1、MCP／share／chat 等），並實際切回未修的 master 抽樣重跑不會掛住的那批，得到完全相同的 11 個失敗。

## MCP 補上頁面圖表素材工具（使用者提問，2026-08-22）★ 使用者要求，不計入計數

使用者問「MCP 中有上傳和檢查投影片素材的功能嗎？例如將參考 PDF 或圖片上傳」，接著澄清「素材指的是這一頁中的檔案」——即播放頁編輯區「圖表素材」分頁顯示的內容。盤點確認：這些圖表由 `extractPdfFigures`（`backend/src/worker/steps/extractPdfFigures.ts`）在 PDF 上傳後自動從原始 PDF 抽取（內嵌點陣圖與向量繪製區域皆有），存成 `storage/<pdfId>/figures.json` manifest；預設面積最大的兩張會被 `regenerate_page_image` 拿去當參考圖，使用者只能透過 `PUT /api/pdfs/:id/pages/:n/figures/selection` 勾選排除、**完全無法上傳新增**。REST API 與網頁前端（`FigureAssetsTab.tsx`）都已支援列出與排除，但 `mcp-server.ts` 完全沒有對應工具——外部 agent 碰不到這個功能。

- [x] **新增三個 MCP 工具**（分支 `feat/mcp-page-figure-tools`，已 merge 回 master）：`get_page_figures`（列出某頁偵測到的圖表，含 id／caption／context／來源／是否排除）、`set_page_figure_selection`（整批覆寫排除清單，空陣列即清空）、`save_page_figure_image`（把指定圖表存到本機檔案供 agent 檢視，因為 agent 無法直接看圖）。三者皆純包裝既有 REST 端點（`GET/PUT /api/pdfs/:id/pages/:n/figures[/selection]`、`GET /api/pdfs/:id/figures/:figureId/image`），後端路由一行未動；工具說明中明講圖表「無法手動上傳新增」，避免 agent 誤以為能新增素材。新增 `backend/test/mcp-figures.test.ts`（8 組，走真實 stdio transport，仿 `mcp-page-assets.test.ts` 的驅動方式，並沿用 `figure-assets.test.ts` 的 `figures.json` manifest 播種手法），涵蓋列表、排除、清空排除、存檔、查無此圖表、查無此頁、空白簡報無圖表等情境。`docs/mcp-guide.md` 同步補上三列工具說明、工具總數 38→41。驗證：後端 `tsc` 全綠、新測試 8/8、`test/mcp-*.test.ts` 全套 71/71、`test/*figure*.test.ts` 全套無新增失敗、後端全套 2009 項（1985 通過、23 失敗——逐一核對均為既有、與本次改動檔案無關的環境相依失敗：Gemini API 逾時／git commit／react-slide／regenerate-rollback／TTS preview 等）。

## MCP 補上 upload_slide：結構化大綱＋逐頁參考圖（使用者提問後決定新增，2026-08-22）★ 使用者要求，不計入計數

延續上一輪的討論，使用者接著問「使用 MCP upload_txt 時，是否能先不要產生簡報，我 agent 有機會先將所有素材傳上去」。查證發現 `upload_txt` 本來就不會馬上生成（要等 `define_prompt`），但這段空窗期完全無法掛素材：簡報連 `pages` 資料列都不存在（要等 `define_prompt` 觸發 pipeline、AI 依大綱分頁後才動態建立），且 `define_prompt`／`/start` 的輸入全是文字設定、沒有任何檔案／圖片欄位。追問使用者「先傳素材」想達成的效果，確認是「讓 AI 生成每頁圖片時參考這些素材」。深入盤點後發現關鍵限制：**`upload_txt` 大綱裡的 `Slide N:` 編號不保證等於最終頁碼**——大綱夠長或編號跳號時，`splitTextWithLlm.ts` 會整份丟給 LLM 重新規劃、合併、拆分、重排順序，使用者寫的數字可能完全作廢，因此無法用「這張圖給第 N 頁」這種方式精準綁定。使用者裁示：新增 `upload_slide` 工具，大綱直接用結構化 JSON 傳（不用自由文字），陣列索引即最終頁碼，天生不會被 AI 重新排序。

- [x] **實作 `upload_slide`**（分支 `feat/mcp-upload-slide`，已 merge 回 master）：新增 REST `POST /api/pdfs/from-slides`（`backend/src/routes/pdfs/slides-upload.ts`），依結構化 `{title?, slides:[{title,bullets,summary?}]}` JSON（multipart 文字欄位）＋每頁最多 2 張參考圖（`slide_<i>_ref_<n>` 檔案欄位）建立簡報：依陣列順序（`pageNumber = index+1`，保證可靠）直接 `INSERT INTO pages`（`status='pending'`）、把參考圖存進 `storage/<pdfId>/figures/` 並寫入 `figures.json`（**以真正的 slide page_number 為 key**，而非原始 PDF 頁碼）與 `split-figure-map.json`（對有素材的頁面做 identity mapping `{n:[n]}`）、寫 `source.txt`（串接所有頁面的渲染文字）。**關鍵發現**：`pipeline.ts` 既有的「重用既有分頁」分支（約 570-655 行，`isTextImport && existingPages.length > 0` 時觸發，跳過 `splitTextWithLlm`）會自動讀到這些檔案並把參考圖傳給 AI 生圖 API（`renderTextPagesWithLlm.ts` 既有的 `sourcePdfPages → figures.json` 查找邏輯），**因此 `pipeline.ts` 與 `renderTextPagesWithLlm.ts` 一行都不用改**；也因為沿用同一套 `figures.json`／`figure-selection.json` 格式，上一輪新增的 `get_page_figures`／`set_page_figure_selection` MCP 工具對這些使用者上傳的參考圖直接生效，不需要再開新的查詢/排除工具。把 `pdfFigures.ts` 私有的 `MAX_FIGURE_REFERENCES_PER_PAGE` 常數 export 出來，供新路由的每頁參考圖上限驗證重用（避免兩處數字各自維護）。MCP 端在 `mcp-server.ts` 新增 `apiUploadMultipart()`（JSON 欄位＋動態數量檔案欄位的通用 multipart 上傳，`apiUploadPdf`/`apiUploadImage`/`apiUploadText` 都只支援單一固定 `file` 欄位、不適用）與 `upload_slide` 工具本身。**驗證方式含一段額外的資料組裝驗證**（因為要證明「pipeline 若現在真的跑，會查到正確的參考圖」不需要真的觸發 AI 生成）：實際呼叫新端點建立一份帶參考圖的簡報後，直接呼叫 pipeline 內部用的 `loadSplitPageFigureMap`／`getFigureReferencesForPages`，確認能查到正確頁碼與正確的圖表 id。新增 `backend/test/upload-slide.test.ts`（4 組，REST 層級，涵蓋建立成功／頁碼順序／figures.json 與 split-figure-map.json 內容／既有 figures REST 路由能讀到剛上傳的參考圖／單頁超過 2 張圖被拒／空陣列與缺欄位被拒／超過 60 頁上限被拒）與 `backend/test/mcp-upload-slide.test.ts`（7 組，走真實 stdio transport，仿 `mcp-page-assets.test.ts`，涵蓋建立、頁碼順序、`get_page_figures` 讀到剛上傳的參考圖、無素材頁面回報無圖表、本機檔案不存在／空陣列的錯誤處理）。`docs/mcp-guide.md` 同步補上 `upload_slide` 一列說明、工具總數 41→42。驗證：後端 `tsc` 全綠、新測試 11/11、相關既有測試（`mcp-figures`／`figure-assets`／`mcp-page-assets`）25/25 無回歸、後端全套 2020 項（1996 通過、23 失敗——逐一核對與上一輪確認過的既有無關環境相依失敗完全同一批：Gemini API 逾時／git commit／react-slide／regenerate-rollback／TTS preview）。

## 切到 CGU Air 後 AI 動畫看起來還在用 OpenAI（使用者回報＋截圖，2026-08-19）★ 使用者回報 bug，不計入計數

- [x] **查證＋修正**（分支 `fix/animation-ai-error-transparency`，已 merge 回 master）：查證結果是**設定其實已生效、打的就是 CGU Air**——真正的問題是 CGU Air gateway 額度用完，每次呼叫都 429 `credit_balance_exhausted`，而 gateway 原樣轉發上游 OpenAI 的錯誤文案（連 platform.openai.com 的儲值網址都帶著），後端又把真實錯誤吞掉只回泛用的「Failed to generate custom-script animation code」，使用者自然斷定切換沒生效。修正見工作記錄。
- [ ] **多個後端 process 之間帳號設定不同步**（查證本案時發現）：`perAccountCache` 是 process 內的 Map，讀過一次就永遠用快取；在 A process 的設定頁改 provider 只更新 A 的快取與 `settings.env`，同時在跑的 B process 要重啟才會看到。本機同時開著 port 3000 與 8888 兩個 dev server 時，就會出現「設定改了但（另一個入口的）行為沒變」。修法方向：`loadPerAccountSettings` 以 `settings.env` 的 mtime 做快取失效（每次讀 stat 一次，成本可忽略）。
- [ ] **LLM log 檔路徑假設 cwd 是 repo root**（查證本案時發現）：`LLM_REQUEST_LOG_FILE = path.join(process.cwd(), 'backend', 'data', …)`，從 `backend/` 目錄起跑的 process 會把 log 寫到 `backend/backend/data/`——同一台機器兩個 process 各寫一份，查問題時很容易看錯檔案。應改以模組位置（`import.meta.url`）或設定值定位。


## 互動 React 頁（`react-app`）＋ 外部 coding agent 透過 MCP 送程式進來試跑（使用者要求，2026-08-16）★ 使用者要求，不計入計數

使用者要求：希望 React 頁裡可以有一個簡單的程式呈現可互動的畫面（輸入資料、按下按鍵做計算），極端情況是「一個完整的應用程式在一頁裡執行」；同一個底層也要能用在 GSAP 動畫上做比較複雜的效果。架構上**不在 MakeSlide 內建 coding agent**，而是讓外部的 coding agent（如 Claude Code）寫出符合 MakeSlide 要求的程式，透過 MCP 送進來、在 MakeSlide 環境中試跑、依結果修正到能運行為止。需考慮雲端多人環境的安全要求。先寫設計文件再依文件實作。

設計文件：[`docs/react-app-design.md`](docs/react-app-design.md)。分支：`docs/react-app-design`（文件）。

- [x] **設計文件**（分支 `docs/react-app-design`，已 merge 回 master）：定調「執行在 client、驗證在 server」——互動頁跑在觀眾自己的瀏覽器裡，觀眾人數對伺服器零成本，因此 headless 瀏覽器的角色是**存檔前的准入檢查**而不是執行環境；這也是無窮迴圈唯一擋得住的地方（CSP 管得住外連，管不住 `while(true)`，而一個寫壞的互動頁會凍住觀眾的整個分頁）。隔離維持既有的 opaque-origin iframe（評估過獨立網域／Web Worker／宣告式 DSL 三個替代方案，前兩者成本不成比例），補上目前**全站完全沒有**的 CSP：`connect-src 'none'` 讓互動頁無法把觀眾輸入的資料送到外部——這是「觀眾輸入資料按按鈕計算」這個情境下真實存在的外洩路徑，而 sandbox 屬性擋不住。既有的 regex deny list 降級為存檔時的 UX 提示（`window['fet'+'ch']` 就能繞過 regex，繞不過 CSP）。
- [x]（2026-09-09 完成大半，分支 `feat/mcp-react-slide-tools` → master；**`get_react_slide_contract` 仍未做**，見下方工作記錄） **階段 1：MCP 加 React 頁工具**（`get_page_react_slide`／`set_page_react_slide`／`get_react_slide_contract`）。實作了四個工具：`get_page_react_slide`（讀 JSX＋主題）、`set_page_react_slide`（寫 JSX 並轉成 React 頁）、`generate_page_react_slide`、`convert_react_page_to_slide`。契約目前寫在工具描述裡而**不是**做成可查詢的 `get_react_slide_contract()`——描述每次對話都要付 context，而 CSP 清單與畫布尺寸還會隨部署變，這條留著。目前 `mcp-server.ts` 有 40+ 個工具但 react-slide 一個都沒有，外部 agent 根本碰不到這種頁面。`get_react_slide_contract()` 回傳 MakeSlide 當下的規格（元件契約、CSP 允許/禁止清單、主題 token、畫布尺寸），讓 agent 不必猜、也不必把規格寫死在自己的提示詞裡——這是「用外部 agent 取代內建 coding agent」能成立的前提。純包裝既有端點，不觸碰執行路徑。
- [ ] **階段 2：沙箱與烘焙文件加 CSP**（靜態 `react` 頁一併收緊，純安全強化）。注意必須保留 `'unsafe-eval'`（runtime 用 `new Function(code)()` 執行編譯結果），但這不影響 `connect-src 'none'` —— CSP 各 directive 互相獨立，要擋的是資料外洩不是程式碼執行。meta CSP 不支援 `report-to`，違規回報改由 harness 用 CDP 收集。
- [ ] **階段 3：`run_react_slide` harness ＋ 配額／併發（三者同批，不可拆）**。重用既有 `playwright-core` 基礎，產出執行報告（console/pageerror、被 CSP 擋下的請求、截圖）而非圖片；`actions` 用宣告式 JSON 而非讓 agent 送 JS 進來 `evaluate`（後者等於在伺服器上開一個任意程式碼執行面）。截圖回傳給 agent 看是關鍵——Claude Code 看得懂圖，可以直接判斷「按下按鈕後數字有沒有出現」。**配額不可延後**：烘焙目前沒有任何 semaphore／queue，一個 agent 迴圈（哪怕只是寫壞停不下來）就能吃光整台機器的 CPU 拖垮所有其他使用者，沒有配額的 harness 本身就是 DoS 面。
- [ ] **階段 4：`react-app` 頁面型別**。關閉元素覆寫與 `MutationObserver` 重套（覆寫會把互動改動的內容還原回去，且 `!important` 覆寫在 cascade 中會殺死 CSS 動畫）。三個非安全障礙需各自先做技術驗證：(1) 播放時 iframe 收不到指標事件（現在寫死 `pointerEvents: inspect ? 'auto' : 'none'`，播放時的點擊屬於 player）；(2) 進出全螢幕會 remount 導致觀眾輸入到一半的狀態消失——參照 `NotebookPanelSingleton` 的單例＋DOM reparenting，但 iframe reparenting 多半會 reload，需實測；(3) 烘焙拍到的是初始狀態，契約要求初始畫面必須自我說明而非一片空白。
- [ ] **階段 5：多檔目錄模式**（使用者要求：程式都塞在一個大檔案裡對比較大的程式不方便）。`pages/<uid>.slide/` 取代單一 `.slide.jsx`（兩者互斥，`react_slide_path` 指向哪個就是哪個模式，不需要新欄位）；匯出/匯入/版控天然支援（整個 `pdfDir()` 打包、`fs.cp` recursive、`git add` 接受目錄）。**關鍵性質是「bundle 只是編譯期的事、執行期完全不變」**——沙箱、烘焙、harness 拿到的仍是單一 IIFE 字串，一行都不用改。編譯從 esbuild `transform` 換成 `build({bundle:true})`，但**不讓 esbuild 看見真實檔案系統**：整個目錄先讀進記憶體成 map，plugin 攔截所有 resolve/load 只在 map 裡查，於是 `import '../../../etc/passwd'` 不是「被規則擋下」而是根本不存在，裸模組名同理失敗（順便杜絕 npm 套件被 bundle 進來）。deny list 只放寬一格：允許相對路徑 import 與 export，仍拒絕裸模組名／絕對路徑／URL／動態 `import()`；deny list 與 `window.SlideComponent` 契約檢查改成對 **bundle 後的結果**跑（那才是真正執行的東西）。MCP 需要 list/read/write/delete 個別檔案的工具——**多檔真正的好處不是「程式可以很大」而是 agent 改一小處不必重送整份**。只開放給 `react-app`：`ensureElementIds` 與元素編輯都是對單一字串操作、靠 id 定位，而 `react-app` 本來就關閉覆寫，這個問題在它身上不存在。
- [ ] **階段 6：GSAP 共用同一 harness**（加一個 `seek(t)` 動作驅動時間軸），對應 `docs/react-slide-design.md` §16 第 5 點。
- [x] **ZIP 匯入挾帶程式碼的漏洞**（分支 `fix/react-slide-import-recompile`，已 merge 回 master；查證設計文件待決事項時發現，與互動頁無關、是當時就存在的問題，故先行修掉）：匯出把 `.slide.jsx` 與 `.slide.js` 都打包、匯入整個目錄複製，而讀取路徑**只在 compiled 缺失時**才重新編譯——所以一份手工構造的 ZIP 可以讓 `.slide.jsx` 看起來人畜無害、`.slide.js` 是完全不同的東西，而沙箱執行的正是後者；存檔時的 deny list 完全碰不到這條路徑，在 CSP 補上之前那段程式碼還能自由外連。改為匯入時一律從各頁自己的原始碼重新編譯（走與正常存檔相同的 `validateAndCompileReactSlide`，長度上限／deny list／契約檢查一次到位），原始碼編不過的頁面把過期的 `.slide.js` **刪除**而非留著——留著就是這個漏洞本身；該頁保留原始碼（可修）與 JPG（畫面仍有東西），讀取路徑之後會按需重編。另修掉 `react-slides.json` 沒被列入 `SIDECAR_FILES` 的疏漏（其餘 sidecar 都有，導致它被原樣複製進新簡報的儲存目錄）。新增 3 個測試涵蓋「竄改的 .slide.js 被原始碼取代」「原始碼過不了 deny list 時不留下編譯檔」「編不過的頁被拒且不影響其他頁」。驗證：後端 tsc、新測試 3/3、匯出匯入與 React 相關回歸 12/12 ＋ 純函式 44/44，0 失敗。
- [ ] **待決事項**（設計文件 §12）：iframe reparenting 能否保住互動狀態（決定階段 4 的狀態保存走哪條路，需實測）；宣告式互動（tier 1，不執行任意 JS、可被既有覆寫機制理解）能覆蓋多少實際需求，覆蓋率高的話 `react-app` 的優先度可以往後調；多檔模式是否需要放行 `.css`／`.json` 檔。
...
- [x] **「轉成 Notebook」按鈕改成「更改頁面類別」對話框**（使用者要求）：頁面型別變成三種之後，單向的「轉成 X」按鈕就不成立了——使用者要問的是「這一頁該是什麼」，而答案必須包含換回來。對話框在 圖片／React／Notebook 之間切換，三個方向各自呼叫該型別自己的端點，**過程中不刪任何東西**（JPG、`.ipynb`、React 程式碼都留著），所以可以放心試。切成 React 頁存的是「該頁已經有的程式碼」（沒有就用預設骨架），切換本身不呼叫 LLM。
- [x] **重生時 React 頁重寫程式碼而不是重畫圖**（使用者要求）：這種頁面的畫面就是程式碼，重畫 JPG 等於花一次圖片費用產生一張這頁不會顯示的檔案、而真正的投影片原封不動。重寫時餵的是**簡報大綱**（每頁一行、標記★這一頁）＋本頁完整逐字稿＋使用者的提示詞；大綱是必要的，否則每頁都會被當成整份簡報唯一的一頁來寫（封面重講議程、三頁各自從頭介紹同一個概念）。刻意不把舊程式碼餵回去——重生的語意是「依現在的逐字稿重做」，舊版面會把結果錨定在可能已不符的內容上。
- [x] **修好「React 頁畫面看不到」**（使用者回報）：資料、API、沙箱全部正常，問題在版面——圖片頁的外框是 `inline-block`（讓外框貼齊圖片寬度），而 iframe 容器要 `width:100%`，兩者互相等待的結果是寬度 0，投影片於是「在 DOM 裡、畫面上看不到」。React 分支改為明確 `display:block` ＋寬度，縮放同時看兩軸（只看寬度的話，播放器一限制高度就會溢出被裁掉）並置中。用真實瀏覽器（Playwright ＋系統 Chrome、實際簡報的第 13 頁）確認修正前後的畫面。
- [x] **點選模式打開時浮出「元素編輯」面板**（使用者要求）：置頂、可拖曳、可收合，點到的元素直接在面板裡改，不必捲回下方分頁——每次調整都要對著投影片判斷結果。刻意不是 modal（modal 會蓋住正在點的那張投影片）。面板依常用程度排序而不是把 31 個屬性全列出來：文字內容 ＋ 常用 8 個屬性（各給色票／數字＋單位／下拉／滑桿）＋ 其他已調整屬性 ＋ 任意屬性下拉。每個控制項顯示的是**元素的實際值**（有覆寫用覆寫、否則用 computed），所以面板讀起來是「這元素長什麼樣」而非「你覆寫了什麼」，且只有動過的屬性才寫回設定。色票要把 computed 的 `rgb(...)` 轉成 `#rrggbb`，否則 `<input type="color">` 會靜默顯示黑色；轉不出來的（`transparent`、漸層）不顯示色票，因為一個假的黑色方塊比沒有更糟。
- [x] **修好「點了元素但面板沒反應」**（使用者回報）：點選這件事有三個各自都會讓它**無聲失效**的環節，逐一處理——(1) 點到沒有路徑的地方（卡片留白、段落裡的 `<strong>`）原本直接忽略，改成 `closest('[data-ms-path]')` 往上找最近可選元素；(2)「開啟點選模式」是用 postMessage 推進沙箱的，但 iframe 若比父層的 listener 早載入完成，那則訊息就沒人接、之後點什麼都沒用——初始狀態改為同時烘進 srcdoc 的 `<body class="ms-inspect">`，且 iframe `onLoad` 也標記 ready；(3) 手寫標註畫布與選取框覆蓋整個舞台，點選模式開啟時改為 `pointer-events: none`。另外全螢幕的 SlideRenderer 原本根本沒接 `inspect`/`onSelect`（面板浮得出來、點了沒反應），面板 z-index 也提到全螢幕覆蓋層之上。診斷過程：用 Playwright 在沙箱與父視窗兩端各裝探針，確認 trusted 的真實點擊確實進得到沙箱、`ms-slide-select` 也確實送達——問題不在點擊本身而在上述三個條件。
- [x] **AI 背景圖產生了卻沒有顯示**（使用者回報）：檔案有、生成成功，但沙箱永遠拿不到它——背景圖端點需要 session cookie，而沙箱是 opaque origin：`about:srcdoc` 根本無法解析相對網址，就算給絕對網址，那也是跨站子資源請求、不會帶 cookie，於是 403、投影片默默沒有背景。改成**背景畫在沙箱外面**（父文件、iframe 之下），沙箱在有背景時保持透明疊在上面；`hasSlideBackground()` 集中判斷「到底有沒有背景可畫」，把「選了圖片但還沒產生」與「選了純色卻沒有顏色」都當成沒有背景，否則會用透明蓋掉主題自己的頁面底色。
- [x] **浮動面板與頁面類別對話框改用專案的語意色**（使用者回報「文字配色沒有跟系統主題、dark 模式文字有問題」）：原本混用硬編碼 `slate-*` 與語意 token，只有在其中一種主題下看起來正常——屬性標籤在另一種主題下會淡到幾乎看不見。改為只用 `surface`／`text`／`muted`／`border`／`primary`／`danger`，它們會跟著主題切換，且對比已由既有的 `contrastRatio.test.ts` 把關；屬性名改用 `text-text`，被覆寫的用 `text-primary` 標示。
- [x] **編輯區可以「分離」成浮動視窗**（使用者要求）：分頁列最右邊加 `⧉`，整個編輯區變成可拖曳、可調整大小的浮動視窗，視窗標題列與分頁列各有一個 `⤓` 靠回原位；原位置留一條可點擊的提示，免得收不回來。位置與大小記在 localStorage——合適的擺放位置取決於使用者的螢幕，每次進來都要重排就沒有意義了。
- [x] **點選模式不再被分頁切換偷偷關掉**（使用者回報「點選元素編輯又不行了」）：原本 `inspect` 綁在「React 分頁是否開著」上（三處：強制關閉的 effect、面板顯示條件、傳給 renderer 的 prop）。於是切去看一眼逐字稿、或任何會改到 editTab 的操作（在分離出去的編輯視窗裡尤其容易），點選模式就無聲關閉——使用者的體感就是「這功能隨時會莫名其妙失效」。使用者明確開的東西，就該開到他自己關掉為止：現在只有「這一頁不是 React 頁」才強制關閉（那時根本沒有沙箱可點），換頁則只清掉選取（元素路徑是跟著那一頁的結構走的）。
- [x] **修好「編輯區分離後就點不到投影片」**（使用者回報「還是不行」）：分離視窗預設開在 (80,120)、900×600——正好蓋住投影片，於是點選模式下每一次點擊都被視窗接走，`elementsFromPoint` 顯示最上層是那個 `<section>` 而不是 iframe。兩個修正：(1) 分離時**從原地浮起**——用編輯區當下的位置與寬度當初始值（並夾在畫面內、下方空間不足就往上移），視窗因此出現在投影片下方而不是壓在它上面；(2) 開啟點選模式時若視窗仍與投影片重疊，就自動把它移到投影片下緣——使用者剛按下「點選模式」，意圖毫無疑問，讓他去跟一個會吃掉所有點擊的視窗搏鬥沒有道理。
- [x] **元素編輯面板改為 portal 到 `<body>`、且只要點選模式開著就一定顯示**（使用者回報「滑過有藍框但面板完全沒出現」——這兩件事合起來說明沙箱端是好的，問題在面板沒被畫出來）：(1) `position: fixed` 是相對於「最近一個有 transform／filter／backdrop-filter 的祖先」，不是視窗——只要面板上方任何一層有這種屬性，視窗就會被定位到那個元素的座標系裡，可能整個跑到畫面外，而且畫面上沒有任何線索。改用 `createPortal(..., document.body)` 徹底避開。(2) 原本「這一頁不是 React 頁就不顯示面板」的條件會產生一模一樣的症狀——模式開著、卻沒有任何視窗證明它開著；改成一律顯示，並在面板裡用一行說明。(3) 面板現在會自報「第 N 頁 · 頁面型別：react」，下次再出問題可以直接從畫面上定位，不必猜。
- [x] **沙箱自報狀態（可點選元素數 / 最後點到什麼）＋ 渲染完成才指派路徑**：使用者連續回報「點了沒反應」，但 Playwright 在一般、分離、遮擋、甚至 CPU 降速 20 倍等情境下都選得到——測不出來就不該硬猜。改為兩手：(1) **穩健性**：路徑指派與覆寫套用改由 `MutationObserver` 在元件 DOM 變動時重跑（原本只在單一 `requestAnimationFrame` 跑一次，而 React 18 的 commit 是排程的、可能落在那一幀之後；真發生時會「hover 有藍框但點擊無效、且既有覆寫也套不上」），點擊當下若元素仍無路徑會再補一次。(2) **可觀測性**：面板顯示沙箱自報的「可點選元素：N · 最後點到：tag」——`N=0`（沒指派到路徑）與「點到 x 但找不到可選祖先」是兩種不同的故障，症狀卻一模一樣，現在從截圖就能分辨，不必再來回猜。
- [x] **烘焙：把 React 畫面寫回頁面的 JPG**（`feat/react-slide-bake`，設計文件 §9.1）——這原本是本功能最大的缺口：匯出 PDF/PPTX/影片/SCORM、縮圖與封面吃的都是 JPG，在烘焙之前它們拿到的是「這頁變成 React 頁之前」的舊圖，畫面上對、發出去的每個檔案都錯。改用無頭瀏覽器（`playwright-core` ＋ 系統 Chrome，不下載瀏覽器）渲染 1920×1080 JPEG 寫回 `image_path` 並重建縮圖，其餘功能完全不必改。烘焙文件是沙箱的靜態版（同畫布、同 token、同背景與覆寫，但沒有 inspector／postMessage／hover 外框），React UMD 與背景圖都**內嵌**（`setContent` 沒有 origin 可解析相對網址，且烘焙沒有 session 去打需驗證的背景圖端點），並等 `__msSlideReady`（React 18 commit 是排程的，第一幀截圖會拍到空白）。存程式碼／存設定／AI 生成／生成背景後在背景自動烘焙，另有「立即更新這一頁的圖片」按鈕；沒有瀏覽器的環境回 424 並說明，失敗只記 log 不影響存檔。
- [x] **兩個轉換方向都不再讓畫面消失**（使用者要求，`feat/react-slide-conversion-visuals`）：轉成 React 頁時把原本那張 JPG **複製**成背景（`mode:'image'` ＋ 0.35 遮罩讓新文字讀得清楚）——否則轉換在視覺上等於把投影片清空，檔案還在但畫面上原本的東西全沒了；刻意複製而非引用頁面 JPG，因為烘焙會把渲染結果寫回同一個 JPG，背景若指向它就會一次次把上一次的結果疊進去。使用者若已自己選過背景則完全不動。轉回一般投影片時則**先烘焙再還原**（順序不能反，烘焙需要該頁仍是 React 頁），讓那張圖是使用者剛剛看到的 React 畫面而不是變成 React 頁之前的舊圖；烘焙失敗不影響轉換本身。另外把「測試環境不啟動瀏覽器」明確化（`MAKESLIDE_TEST_ALLOW_BROWSER=1` 才實跑），自動烘焙也可用 `REACT_SLIDE_AUTO_BAKE=0` 關閉。
- [x] 其餘限制（設計文件 §9）：React 頁不與 GSAP 動畫、手寫標註以外的圖片局部編輯共存。
...
## 圖片模型設定：OpenAI／Gemini（Nano Banana 2）／Qwen-Image 2.1（使用者要求，2026-09-22）★ 使用者要求，不計入計數

使用者要求：新增一個 image model 的設定，可以在 openai／gemini／qwen image 2.1 之間做選擇；OpenAI 用 gpt-image-2.5，Gemini 用 Nano Banana 2。

- [x] **設定**（[aiSettings.ts](backend/src/services/aiSettings.ts)）：新增 `IMAGE_PROVIDER`（`''` = 跟著 LLM 供應商走，即原本 `getImageClient` 的規則：OpenAI／CGU Air 自己產圖、Gemini／OpenRouter 退回 OpenAI；`openai`／`gemini`／`qwen` 則固定用該服務）、每帳號的 `OPENAI_IMAGE_MODEL`（空 = 環境變數，預設 `gpt-image-2.5-flare`——2026-09-12 實測過裸的 `gpt-image-2.5` 不存在，只有 flare／sunburst 兩個變體，選單標示為 gpt-image-2.5）、`GEMINI_IMAGE_MODEL`（預設 `gemini-3.1-flash-image` = Nano Banana 2）、`QWEN_API_KEY`／`QWEN_BASE_URL`（預設 DashScope 國際端點 `https://dashscope-intl.aliyuncs.com/api/v1`）／`QWEN_IMAGE_MODEL`（預設 `qwen-image-2.1`）。`providerApiKeyOf`／`accountHasOwnProviderKey` 認得 `qwen`；設定 API 與 schema 補上六個欄位與 `has_qwen_key`。
- [x] **架構**：全 codebase 十個左右的產圖／修圖呼叫點都直接用 OpenAI SDK 的 `images.generate`／`images.edit`，不改它們。新增 [imageAdapters.ts](backend/src/services/imageAdapters.ts) 定義同形狀的 `ImageApiClient`：OpenAI 相容供應商包 SDK client；Gemini 走 `generateContent`（`responseModalities: ['TEXT','IMAGE']`，尺寸換算成最接近的 `imageConfig.aspectRatio`，舊模型拒絕 imageConfig 就重送一次不帶）；Qwen 走 DashScope `multimodal-generation`（尺寸換成 `w*h` 並夾在 512²–2048² 內，回傳的 24 小時 URL 立刻下載成 base64）。兩家都不支援遮罩：修圖時把遮罩當第三張輸入圖並在提示加註「只改透明區域」，屬盡力而為，精準 inpaint 仍以 OpenAI 為佳（設定頁提示有寫）。固定了圖片供應商後，`resolveImageProviderFailover` 回 null——LLM 的次要供應商跟圖片無關；`describeFailoverExhausted` 與 `providerLabel` 接受新的供應商名。
- [x] **設定頁**（[SettingsPage.tsx](frontend/src/pages/SettingsPage.tsx)）：供應商區塊多一個「圖片供應商」選單（跟著 LLM／OpenAI（gpt-image-2.5）／Gemini（Nano Banana 2）／Qwen-Image 2.1，缺金鑰一樣加「缺 key」後綴），OpenAI／Gemini 圖片模型欄位與 Qwen 的金鑰、端點、模型欄位依 [providerFieldVisibility.ts](frontend/src/pages/settings/providerFieldVisibility.ts) 新增的 `image(provider)` 規則顯示（固定供應商就顯示它的；跟著 LLM 走就顯示 LLM 角色那幾家的）。
- [x] 測試：後端 [image-adapters.test.ts](backend/test/image-adapters.test.ts) 7 項（尺寸換算、回應解析、`toFile` 轉 data URL、以假 fetch 驗證 Gemini／Qwen 的請求形狀、遮罩提示、imageConfig 重試、URL 下載、錯誤狀態）＋ `image-client-provider.test.ts` 新增 3 項（固定供應商與模型、缺金鑰即報 API_KEY_MISSING、固定後不失效轉移），17/17；前端可見性測試補 1 項、新增 [imageProviderWiring.test.ts](frontend/src/pages/settings/imageProviderWiring.test.ts) 守門 1 項，含 i18n 33/33；前後端 `tsc`、前端 `vite build` 通過。分支 `feat/image-provider-setting`，已 merge 回 master 並同步 `worktree/demo16`。
- **未做／待確認**：沒有對真的 Gemini 與 DashScope 打過請求（沒有金鑰）。DashScope 官方文件列的託管模型是 `qwen-image-3.0`／`3.0-pro`／`2.0-pro`／`plus`／`max`，`qwen-image-2.1` 是開源權重的名字，託管端不一定有這個代號——若 API 回「model not found」，到設定頁把 Qwen 圖片模型改成文件列的代號即可（設定頁提示已寫）。Gemini 的 `imageConfig` 欄位名是依既有的 generateContent 文件，若被拒會自動退回不帶尺寸。

## Qwen-Image-2.1 改為本機／遠端獨立服務（使用者要求，2026-09-22）★ 使用者要求，不計入計數

使用者要求：qwen-image-2.1 請使用本地模型（https://github.com/QwenLM/Qwen-Image-2.1）；把本地模型設計成分離的服務，本地不夠時可以部署在其它機器；提供一個在其它機器啟動的腳本，可以自動檢查並安裝需要的套件和環境。

- [x] **獨立服務** [scripts/qwen-image-server/server.py](scripts/qwen-image-server/server.py)：只用 Python 標準庫＋Pillow 起 HTTP 服務，載入官方 Diffusers 的 `QwenImage21Pipeline`（`Qwen/Qwen-Image-2.1`，bf16；`--cpu-offload`、`--dtype float16`、`--device cpu` 供低 VRAM／無 GPU）。API：`GET /health`、`POST /v1/images/generations`、`POST /v1/images/edits`（圖片與遮罩以 data URL 傳、最多 10 張參考圖、回 OpenAI 形狀的 `{data:[{b64_json}]}`）。遮罩沿用 OpenAI「透明＝可改」的慣例：模型不吃遮罩，服務把模型輸出貼回原圖、只覆蓋遮罩透明處，其餘像素原封不動——局部修圖邊界因此是精確的。可選 `--token` 要求 Bearer 驗證（對外開放埠時用）；一次只跑一張。`--stub` 不載模型、回寫著提示詞的佔位圖，給接線測試與無 GPU 的開發機。
- [x] **可攜的啟動腳本** [scripts/qwen-image-server/run.sh](scripts/qwen-image-server/run.sh)：整個目錄複製到任何機器即可用。檢查 Python ≥ 3.10 與 venv／ensurepip、git；讀 `nvidia-smi` 的 CUDA 驅動版本自動選 torch wheel index（cu118／124／126／128／130，沒有 GPU 退回 CPU 版並警告）；VRAM < 20 GB 提醒加 `--cpu-offload`；`HF_HOME` 所在磁碟不足 40 GB 警告；建 `.venv`、只裝缺的套件（torch、transformers、diffusers(git)、accelerate、pillow）並驗證 `QwenImage21Pipeline` 可匯入；`--check` 只檢查、`--download` 先抓 ~30 GB 權重、`--stub` 只需 pillow。文件 [docs/qwen-image-local.md](docs/qwen-image-local.md)（硬體需求表、部署到別台的步驟、MakeSlide 端設定、API 規格）。
- [x] **後端**：`QWEN_IMAGE_BACKEND`（`local` 預設｜`dashscope`）；local 走新的 `qwenLocalImageClient`（[imageAdapters.ts](backend/src/services/imageAdapters.ts)，JSON 協定、有 token 才帶 Bearer、遮罩原樣交給服務），`QWEN_BASE_URL` 留空 = `http://127.0.0.1:8765`，不需要金鑰；dashscope 維持上一輪的雲端轉接器，需金鑰。設定 API 與 schema 多 `qwen_image_backend`。
- [x] **設定頁**：Qwen 區塊多「Qwen 執行方式」選單（本機／遠端服務、DashScope 雲端），網址與金鑰欄位的 placeholder 與提示跟著切換；本機模式不加「缺 key」後綴。
- [x] 測試：後端 `image-adapters.test.ts` 新增本機轉接器（假 fetch）與**真的起 stub 服務的端到端測試**（生成 512x320、遮罩修圖只改透明半邊、無 token 回 401；用 python3 + pillow，沒有就 skip），9/9；`image-client-provider.test.ts` 本機模式免金鑰、DashScope 缺金鑰即報錯，11/11；前端守門補 Qwen 執行方式選單，33/33；前後端 `tsc`、前端 `vite build` 通過；`run.sh --stub` 實跑通過。分支 `feat/qwen-image-local-service`，已 merge 回 master 並同步 `worktree/demo16`。
- **這台機器的現實**：Quadro P4000 只有 8 GB VRAM 且是 Pascal（無 bf16），7B 生成模型＋Qwen3-VL 8B 文字編碼器約 30 GB 權重，在這裡只能 `--cpu-offload --dtype float16` 而且會非常慢；**沒有在這裡實際載入模型跑過**（權重也未下載）。建議依文件部署到有 24 GB 以上 VRAM 的機器，MakeSlide 端只要填網址與 token。

## 測驗防弊可關閉、老師端列出離開次數與時間、問答題只能用相機拍（使用者要求，2026-09-22）★ 使用者要求，不計入計數

使用者要求：線上測驗防弊還是很容易失誤，請讓它變成選項、可以完全關閉；關閉時只警告、還是可以回到頁面，但在 owner 列表中顯示離開的次數和時間。另外移除問答題的上傳檔案選項，只能用相機拍攝。

- [x] **防弊開關**：`quiz_sets.strict_proctor`（預設 1，舊測驗維持原本的嚴格行為），測驗編輯表單「防弊：離開作答畫面會自動交卷」勾選框，隨測驗儲存／複製／整份簡報複製。
- [x] **關閉時**（`QuizProctorGate` 的 `strict=false`）：離開就顯示警告與「返回全螢幕繼續作答」鈕，不倒數、不計違規、不自動交卷，重整後也不因「已開始過」而鎖定；規則頁改載 [quiz-rules-lenient.md](frontend/public/quiz-rules-lenient.md)（原規則寫著第二次違規自動交卷）。
- [x] **離開紀錄（兩種模式都記）**：離開當下記一筆 `{left_at, away_ms: null}`，返回時補上離開毫秒數，連同作答進度回報；後端以 `left_at` 合併（`mergeQuizLeaves`，每人最多 100 筆），學生重整後送的新清單不會洗掉先前的紀錄。老師端「測驗中的學員」每列顯示「離開 N 次」與每次的時刻（離開幾秒／尚未返回）。
- [x] **問答題只能用相機**：`EssayAnswerUploader` 拿掉「選擇檔案」（`<input type="file" capture>`），只保留 App 內相機拍照；移除 `quiz.essay.pickFile` 字串。
...
- [x] **全螢幕點選項列出是誰選的**（2026-09-23，使用者要求）：`QuizOptionStat` 新增 `pickers`（每個選項依作答順序記下代碼，無代碼用登入名稱，都沒有為 null）；全螢幕講評點任一選項展開「選了這個選項的 N 人」代碼標籤（錯誤選項即答錯的人，紅；正解綠；匿名顯示「匿名」），再點收起、換題自動收起、無作答紀錄時不可點。分支 `feat/quiz-review-option-pickers`，已 merge 回 master 並同步到 `worktree/demo16`。
- [x] **全螢幕每題分兩段**（2026-09-23，使用者要求）：進入每題先只顯示題目與選項（含正解與人數），解析區換成虛線提示；按「顯示解析」／→／PageDown／空白鍵才揭曉解析，再按一次才換下一題（新題再從隱藏開始）；← 先收起解析、再按回上一題（解析為揭曉狀態）。按鈕文字隨階段變成「顯示解析」／「下一題」。分支 `feat/quiz-review-two-stage`，已 merge 回 master 並同步到 `worktree/demo16`。
...
| 2026-08-22 | 新增 MCP `upload_slide`（使用者追問「upload_txt 能否先不產生簡報，讓 agent 先傳素材」後決定新增）：查證 `upload_txt` 本來就不會馬上生成，但空窗期完全無法掛素材——`pages` 資料列要等 `define_prompt` 觸發 pipeline 分頁後才建立，`define_prompt`／`/start` 也沒有任何檔案輸入管道。確認使用者要的效果是「AI 生成每頁圖片時參考素材」後，發現大綱裡的 `Slide N:` 編號不保證等於最終頁碼（`splitTextWithLlm` 可能整份重新規劃），無法做「這張圖給第 N 頁」的精準綁定。使用者裁示：`upload_slide` 大綱直接傳結構化 JSON（陣列索引＝最終頁碼，不會被 AI 重排）。新增 REST `POST /api/pdfs/from-slides`：依陣列順序直接建立 `pages` 資料列＋寫入 `figures.json`（以真正 page_number 為 key）與 `split-figure-map.json`（identity mapping）存放每頁參考圖。**關鍵發現**：`pipeline.ts` 既有的「重用既有分頁」分支本來就會讀這些檔案並把參考圖傳給 AI 生圖，`pipeline.ts`／`renderTextPagesWithLlm.ts` 零修改；也因沿用同一套 `figures.json` 格式，上一輪的 `get_page_figures`／`set_page_figure_selection` 直接對這些素材生效。新增 `upload-slide.test.ts`（4 組，REST）與 `mcp-upload-slide.test.ts`（7 組，真實 stdio），並額外驗證 pipeline 會用到的資料組裝邏輯（`loadSplitPageFigureMap`／`getFigureReferencesForPages`）確實查得到參考圖。`docs/mcp-guide.md` 同步（工具數 41→42）。驗證：後端 tsc、新測試 11/11、相關既有測試 25/25 無回歸、後端全套 2020 項（23 失敗與上一輪核對過的既有無關失敗同一批）。 | feat/mcp-upload-slide（已 merge 回 master） |
| 2026-08-22 | 新增 MCP 頁面圖表素材工具（使用者提問「MCP 有沒有上傳/檢查投影片素材的功能」後決定補齊）：確認播放頁編輯區「圖表素材」分頁（`extractPdfFigures` 自動從來源 PDF 抽取、供 `regenerate_page_image` 挑參考圖，使用者只能勾選排除、無法上傳新增）只有 REST API 與網頁前端支援，`mcp-server.ts` 完全沒有對應工具。新增 `get_page_figures`／`set_page_figure_selection`／`save_page_figure_image` 三個工具，純包裝既有 `GET/PUT /api/pdfs/:id/pages/:n/figures[/selection]`、`GET /api/pdfs/:id/figures/:figureId/image` 端點，後端路由未動；工具說明明講圖表無法手動上傳。新增 `mcp-figures.test.ts`（8 組，真實 stdio transport）。`docs/mcp-guide.md` 同步（工具表＋工具數 38→41）。驗證：後端 tsc、新測試 8/8、`mcp-*.test.ts` 71/71、`*figure*.test.ts` 無新增失敗、後端全套 2009 項（23 失敗皆為既有無關的環境相依失敗，已逐一核對） | feat/mcp-page-figure-tools（已 merge 回 master） |
| 2026-08-19 | 查修「切到 CGU Air 後 AI 動畫還是用 OpenAI」（使用者回報並附截圖）：**查證結論是切換其實已生效**——該帳號的動畫呼叫確實以 gpt-5.6 打 CGU Air，但 gateway 額度已用盡、每次都回 429 `credit_balance_exhausted`，且錯誤文案是上游 OpenAI 的原文（帶 platform.openai.com 儲值網址），而 custom-script SSE 端點把真實錯誤吞掉、只回泛用的「Failed to generate custom-script animation code」——看不到原因的失敗＋OpenAI 字樣的錯誤，自然被解讀成「還在用 OpenAI」。兩個修正：(1) 新增 `describeLlmFailure()`（仿既有 `describeImageEditFailure` 模式）把常見可行動的失敗（額度用盡／金鑰無效／金鑰未設定／模型不存在／rate limit／逾時／連不上）分類成固定的消毒中文字串——刻意不轉述 provider 原始訊息（OpenAI 的 401 會帶 API key 前綴、gateway 錯誤可能洩漏內部 URL），端點回錯時**指名 provider**（「CGU Air：LLM 服務額度已用盡，請儲值或改用其他 LLM 供應商」）——「額度用盡」不說是誰的額度，對剛切換 provider 的人反而是誤導；(2) LLM request/response log 補記 `provider` 欄位——OpenAI 與 CGU Air 的預設模型同名（gpt-4o-mini），光看 model 欄位無法回答「這通打到誰」，而那正是本案的關鍵問題。**驗證**：在該帳號情境下實跑 `streamChatText` 打 CGU Air 的真實 429 → 分類出正確訊息且 log 記到 `provider: cgu-air`；單元測試 llm-failure-message 9/9（含 gateway 真實回應 body）、llm-provider-failover 10/10、animationCustomScript 8/8、tsc 通過。`page-animation.test.ts` 在 master 上同樣卡死（既有環境問題：多個 dev server 佔著 DB），非本次回歸。查證中另發現兩個待辦（多 process 設定快取不同步、LLM log 路徑假設 cwd）已記於未完成項目。 | fix/animation-ai-error-transparency（已 merge 回 master） |
| 2026-08-19 | 修好「浮動視窗會被 header 擋住」（使用者回報並附截圖）：分離出去的編輯視窗是 `fixed z-[135]`，而播放頁 header 是 `z-[1000]`——把視窗拖到畫面上緣，整段標題列連同「⤓ 靠回原位」的按鈕就沉到 header 底下，看起來像視窗壞了，而且**收不回來**。使用者自己拖去哪的視窗就該待在哪：視窗改成 `z-[1010]`，浮在 header 之上。有兩層必須跟著移動才不會反過來出問題：(1) 元素編輯面板 `z-[140]`→`z-[1020]`，它標註的就是這個視窗底下的投影片，被蓋住就沒得看；(2) header 裡的快捷鍵對話框原本 `z-50`，被 header 自己的 `z-[1000]` stacking context 封住——寫多大的 z-index 都只在那個 context 內比較，於是一個帶全螢幕遮罩的對話框會被編輯視窗蓋掉一半，改成 portal 到 `<body>` 並用 `z-[1100]`（與 `AnimationEditorTab` 既有的做法一致）。**驗證**：Playwright 實跑——分離視窗、拖到 y=30，該點最上層的元素就是視窗自己的標題列；把 z 改回 135 同一個測試會命中 `<header>`（先重現失敗再修）。新增的 e2e 留在 `playback.spec.ts`，另加 4 組原始碼層級單元測試把「編輯視窗 > header、面板 > 編輯視窗、對話框 > 全部」這個相對順序釘住——問題是跨檔案的相對關係，任何一邊單獨改動都會再壞一次。前端 1059/1059（新增 4）、tsc＋vite build、播放頁 e2e 8/8。 | fix/detached-editor-above-header（已 merge 回 master） |
| 2026-08-18 | 新增「只有動畫」的進入點 `#/preview/:id?page=N`（使用者要求：可直接指定播放的頁面和動畫、不顯示其它部份）：播放頁做不到這件事——頁首、面板、控制列每一張截圖都會拍到，播放狀態也只能靠按鈕驅動、按鈕一換位置自動化就壞。新頁面只渲染投影片與它的動畫。`?effect=<id>` 只播其中一個效果，且**保留原本的 start 時間**（把它挪到 0 會把人正要看的時間行為藏起來）；指定不存在的效果會明講，因為空白畫面跟壞掉的動畫長得一模一樣。播放用單純的 rAF 時鐘而不是 `<audio>`：這個頁面不播語音，走音訊元素只會把自動播放政策與「音檔不存在」那條失敗路徑拖進唯一該單純顯示動畫的地方。另有 `?loop=1` 重播、`?hud=1` 角落顯示秒數，以及 `[data-preview-state="done"]` 讓自動化**等狀態而不是 sleep**。**截圖驗證抓到一個真的問題**：全域的 API key 對話框整個蓋住預覽頁——那正是這個進入點要避免的事（自動化拍到的會是對話框），已在 `/preview/` 路由下停用全域對話框。MCP `get_page_preview_url` 預設改指向這個路由（`bare: false` 才給完整播放頁），並把 effect/loop/hud 帶過去、對不存在的 effect id 先擋下來。**驗證**：用打包後的前端＋stub 後端在 Chromium 實跑——全頁預覽播到 done（2.50s、2 個效果）、單一效果只剩 1 個效果且時鐘確實在走（0.27→2.30）、找不到的效果顯示對應訊息、頁面上 header 與 button 皆為 0；MCP 端另測 bare 的各種選項組合與兩種錯誤。前端 1055/1055（新增 8 組預覽純函式）、後端 mcp-animation 18/18、前後端 tsc＋vite build。 | feat/bare-animation-preview（已 merge 回 master） |
| 2026-08-18 | 新增 MCP `get_page_preview_url`：回傳一個可直接在瀏覽器開啟、停在指定頁面的播放頁網址（使用者要求，供 Playwright／VSCode 內建瀏覽器除錯用）。先前要除錯一段動畫只能讀 spec 然後想像結果。**兩個讓這個連結對自動化瀏覽器真正可用的預設**：(1) 預設附上**唯讀分享 token**——沒有它 Playwright 只會停在登入頁；後端對同一個 access 會重用既有 token，所以重複呼叫不會一直長出新的分享連結，`share: false` 可退出。(2) 新增播放頁參數 **`?autoplay=1`** 並預設開啟——否則自動化那端得自己找播放鍵、按鈕換位置就壞掉。autoplay 的難點在**時機**：`playPause` 的無語音分支會檢查動畫總長，太早呼叫等於按了個空的，所以條件抽成 `shouldStartAutoplay()` 放在其他播放規則旁邊並測起來（要嘛語音可用、要嘛動畫長度已算出；只觸發一次；已在播就不再按，否則下一輪 effect 會把剛啟動的頁面按成暫停）。工具會**先讀該頁動畫**再組網址，頁碼錯誤會直接說，而不是回一個開起來空白的連結，回覆也會講明這一頁是不是動畫頁、有幾個效果。前端是 HashRouter，所以路徑放在 `#` 之後。**驗證**：用 stub 後端 spawn 真正的 MCP server——URL 可解析、走 hash 路由、帶對 page/autoplay，分享 token 含需跳脫的字元也能正確還原；`share: false` 時完全不會打 share API。前端 1047/1047（新增 2 組 autoplay 條件）、後端 mcp-animation 18/18（41 個工具）、前後端 tsc＋vite build。 | feat/mcp-page-preview-url（已 merge 回 master） |
| 2026-08-18 | 還原「MCP 啟動指令加 fallback」那次修改（使用者告知：先前抓不到 `raw.githubusercontent.com` 是 **GitHub 網站當時的問題**，不是指令的問題）：`curl … && exec …` 那段回到原始版本。**保留**另一項修正——本地 checkout 用的 `npx --prefix … tsx src/mcp-server.ts` 改為絕對路徑，那是使用者自己實測拿到 ERR_MODULE_NOT_FOUND、我也在無關的 cwd 下重現過的真缺陷，與 GitHub 無關。順帶把執行期寫在 repo 根目錄的暫存檔 `.tmp-<deck>-page-<n>.jpg` 從版控移除並加進 `.gitignore`（它被 `git add -A` 掃進了前一個 commit）。後端 mcp-animation 18/18、tsc、實跑 MCP server 確認仍正常回應。 | revert/mcp-launch-fallback（已 merge 回 master） |
| 2026-08-17 | 修正 MCP「本地 checkout」啟動指令（使用者照抄後拿到 ERR_MODULE_NOT_FOUND）：`mcp-server.ts` 註解裡寫的是 `npx --prefix /path/to/backend tsx src/mcp-server.ts`——**這個寫法不可能成立**：`--prefix` 只告訴 npm 去哪裡找套件，**不會改變工作目錄**，所以 `src/mcp-server.ts` 是相對 MCP client 的 cwd 解析，server 在印出任何有用資訊之前就以 ERR_MODULE_NOT_FOUND 死掉。（我上一輪把這段建議原樣抄給使用者，沒有先實測——這正是「文件裡的指令也要當成程式碼驗證」的例子。）改成**兩個路徑都用絕對路徑**（`node_modules/.bin/tsx` + 檔案全路徑），那其實正是專案自己的 `mcp-animation.test.ts` 一直在用、也就是唯一被驗證過的形式；另附「不想寫死 tsx 路徑」的替代寫法（`sh -c "cd <backend> && exec npx tsx src/mcp-server.ts"`），並把「為什麼那個看起來合理的版本會失敗」寫進註解。**驗證**：在不相關的 cwd（/tmp）下實跑——文件原本的指令重現了使用者回報的 ERR_MODULE_NOT_FOUND；修正後的指令正確回應 `initialize`，`tools/list` 列出全部 40 個工具（含 `update_animation_effect`／`delete_animation_effect`）；`cd` 替代寫法同樣可行。後端 mcp-animation 18/18、tsc。 | docs/mcp-local-launch（已 merge 回 master） |
| 2026-08-17 | 修正 MCP 啟動指令在下載失敗時「靜默消失」（使用者回報「raw.githubusercontent.com 好像拿不到，導致 MCP 不會動」）：先確認狀況——匿名 `git ls-remote` 成功，所以 repo 是 public、遠端可讀（本機沙箱對 GitHub 的 429/504 只是代理限制，測不出真實可及性）；而 `origin/master` 停在使用者自己 21:39 push 的版本，**新加的兩個 MCP 工具那個 commit 還沒 push**，所以即使抓得到檔案也不會有 `update_animation_effect`／`delete_animation_effect`。真正該修的是啟動指令本身：文件推薦的 `curl … && exec npx tsx …`，一旦下載失敗 `&&` 就把 `exec` 跳過——程序沒有輸出、直接結束，MCP client 只看到「這個 server 不存在」，完全沒有線索指向「下載失敗」，這正是它表現成「MCP 不會動」而不是「抓不到檔案」的原因。改為下載到 `.new` 再 `mv` 就位、失敗時**沿用上次抓到的版本**並在 stderr 說一聲，`exec` 不再掛在 `&&` 後面。**用 stub server 實測新舊指令**：舊指令遇到 404 時 exit 22 且從未啟動 server；新指令印出提示並成功跑起上次的版本。而且實測推翻了我自己原本寫進註解的一半說法——HTTP 錯誤**不會**弄壞快取檔案（`curl -f` 不動既有檔案），`.new` + `mv` 真正防的是**傳輸中途斷線**留下的半個檔案，註解已改成實測到的版本而不是我原先的假設。順帶記錄：本地 master 目前領先 origin 4 個 commit（含兩個 MCP 工具），要走 GitHub 那條路的話需要 push。後端 mcp-animation 18/18、tsc。 | docs/mcp-launch-fallback（已 merge 回 master） |
| 2026-08-17 | MCP 補上 `update_animation_effect` 與 `delete_animation_effect`（使用者問「動畫可以用 MCP 修改嗎」後決定補齊）：原本 MCP 有 5 個動畫工具，但**只能整份覆寫**——要改一個既有效果（調 duration、換掉某個 custom-script 的 code）只能用 `set_page_animation` 把整份 spec 重打一次，最多 20 個效果、其中一個可能帶著 20 KB 產生出來的 JavaScript，漏掉一個就等於把它刪掉。`add_animation_effect` 早就是為了避開這件事而存在，這次補上缺的另外兩半。`update_animation_effect` 只需送要改的欄位：`params` **逐欄合併**（「把方框往右移」只送 `xPct`，淺層合併會順手把方框大小重設掉）、`unset` 用來**移除**欄位（合併表達不了「沒有設定」——拿掉 `exitDuration` 正是讓效果不再自動淡出的方法）、`id`/`target` 不可改、找不到 id 時**列出實際存在的 id**（agent 下一步就是挑一個真的 id）、並比照 `add_animation_effect` 一併啟用該頁動畫（改了卻不會播是最難察覺的失敗）。`delete_animation_effect` 刪掉最後一個效果時會把 `enabled` 關掉——`renderTypeForSpec()` 只看那個旗標，不關的話這一頁仍被標成 `gsap-image` 卻沒有任何效果可播，播放頁會為它多走一趟動畫路徑；**這是照著工具說明寫的測試抓出來的**（說明宣稱會變回 static-image，實際不會）。兩個操作都**內聯**在 `mcp-server.ts` 而不是 import 後端 service：這個檔案刻意是「curl 一個 .ts 就能跑」的單檔，不能碰 monorepo 的其他部分。後端 mcp-animation 18/18（新增 4 組端到端 subtest：只改一個效果且其他不動、移除欄位、拒絕未知 id/受保護欄位/空 patch、刪到最後一個變回 static-image）、tsc。（順帶記錄：這支測試在本機「跑不完」其實是**測試結束後程序不退出**，實際只跑 1.2 秒——用 `> log` 導出再讀就看得到結果。） | feat/mcp-animation-effect-edit（已 merge 回 master） |
| 2026-08-17 | 修正「第二頁無法播放，只停在一開始」（使用者回報 deck `dyp31H5T0t` 第 2 頁）：去看那一頁的資料就發現了——DB 的 `audio_path` 指著 `F8PXH9kToZ.m4a`，但**那個檔案不在磁碟上**（TTS 失敗或檔案被刪）。**有網址不等於有語音**，而這剛好落進最糟的組合：播放器把它當成「有語音的頁面」，於是一直等一個永遠不會載入的音檔的 metadata；而「無語音動畫計時器」偏偏就是因為「這頁有語音」被跳過——結果沒有任何東西在推進 `currentTime`，動畫停在第一格，按播放也沒反應（`playPause` 走有語音分支，`audio.play()` 直接失敗）。改為：**一旦這一頁的語音載入失敗過，就把它當成沒有語音**（那也正是它實際的狀態）——動畫計時器接手、`playPause` 走無語音分支、句子時間軸退回估算值。三處判斷收斂成同一個 `isPageAudioUsable()`：先前正是因為「這頁到底有沒有語音」在不同地方答案不一致，才卡成一個誰都不動的狀態。載入成功（含手動/自動重試成功）會清掉標記，所以暫時性的失敗不會變成永久，既有的重試機制照常運作。順帶一提：DB 記錄與磁碟不一致這件事本身還在（前端現在只是能容錯），若常發生值得回頭查 TTS 失敗時為什麼仍寫了路徑。前端 1045/1045（新增 3 組：檔案缺失視為無語音、失敗標記只對該頁生效、沒有網址一律不可用）、tsc＋vite build。 | fix/missing-audio-file-playback（已 merge 回 master） |
| 2026-08-17 | 修正「沒有語音時動畫不能用」（使用者回報）：只要有任何效果錨定逐字稿（`startTrigger`——**AI 產生的動畫全都是**），沒有旁白語音的頁面就**完全看不到動畫**。根因是一個閉環：就緒判斷 `shouldResolvePageAnimationSpec` 要等音檔 metadata（`duration > 0`），而無語音頁的 `duration` 是**從動畫時間軸總長回推**的，那個總長要先解析 spec 才知道，而 spec 能不能解析正是這個判斷在決定——`ready` 於是永遠是 false。判斷現在知道「這頁到底有沒有語音」，沒有就直接解析；解析所依據的時間軸改用 `estimateNarrationSeconds()`（把 `buildSentenceTimeline` 內部那套「每句朗讀秒數＋句後停頓」的估時模型抽出來共用，本來只用於縮放字幕），讓錨定逐字稿的效果仍照句子順序播放——否則它們會全部退回字面 `start`（AI 產生的每一個都是 0）而在同一瞬間一起播完。**另一個坑**：無語音頁必須**固定**用估算值、不能改用那個回推的 `duration`——拿它回頭縮放時間軸會把每個效果的 start 推後 → 動畫總長變長 → duration 再變大，每次 render 拉長一點、永不收斂（實作時先寫成會震盪的版本，模擬 render loop 才看見）。**驗證**：以 AI 會產生的那種「三句、每句一個效果、字面 start 全為 0」的 spec 模擬 render loop——修正前 `ready` 每一輪都是 false、頁面長度恆為 0（動畫永遠不出現）；修正後第一輪就解析，效果落在 0 / 1.49 / 3.43 秒（依句序），連續六輪完全相同不震盪。前端 1042/1042（新增 5 組：無語音直接解析、有語音仍要等自己的 metadata、圖片仍是前提、估算長度、估算時間軸嚴格遞增）、tsc＋vite build。 | fix/animation-without-narration（已 merge 回 master） |
| 2026-08-17 | 修改既有動畫改走 **patch**，不再每次整份重新產生（使用者要求）：一份自訂腳本很快就到 20 KB，為了「把圓形改成藍色」重新輸出全部——慢、按輸出 token 計費，而且每次重生都是一次「把原本已經對的細節弄丟」的機會。有 `previousCode` 時（也就是最常見的「調整我現有的動畫」），第二階段改請 LLM 只輸出 search/replace 片段（`<<<<<<< SEARCH` … `=======` … `>>>>>>> REPLACE`）。**刻意不用 unified diff**：行號與 hunk 標頭正是 LLM 最會寫錯的東西，而逐字引用舊內容它最可靠。套用也刻意嚴格（`backend/src/services/codePatch.ts`）：每個 SEARCH 必須在現有程式碼中**恰好出現一次**，找不到或出現多次都算失敗——猜「他指的是第幾個」正是 patch 悄悄改到錯的地方的方式；任一區塊失敗就**整個不套用**，因為半套用的結果既不是使用者原本的版本也不是他要的版本（模型輸出被截斷造成的半個區塊同樣算失敗）。失敗自動**回退**成整份重新產生，並用 SSE 說一聲（不說的話使用者只會看到「明明只改一點卻整份重寫」）；套用後的程式碼仍走與全量產生相同的 unsafe／契約／長度檢查才送 `done`。patch 的串流走**獨立事件**（`patch-delta`／`patch-done`／`patch-fallback`）而不共用 `delta`：把 patch 片段灌進程式碼編輯器會看起來像程式碼被弄壞了，前端改顯示在對話區的進行中泡泡。輸出上限 6000 tokens（全量是 24000），省下的就是這一段。**驗證**：拿那個 deck **真實的 22.6 KB 動畫**實測——逐字引用檔案的 patch 套用成功且只改動它指名的 13 個字元、其餘完全不動，而過時的 SEARCH 會被拒絕而不是猜。後端 code-patch 13/13（新，含多區塊依序套用、後一個區塊改前一個的結果、prose/markdown 包裹、找不到、出現多次、半套用、空 SEARCH、刪除、半寫完的區塊）、前端 api SSE 2/2（新，斷言 patch 不會被當成 code delta，以及 fallback 必須在整份重寫之前先送出），前端 1037/1037、前後端 tsc＋vite build。 | feat/animation-input-capture（已 merge 回 master） |
| 2026-08-17 | AI 自訂動畫編輯器可附加參考圖片（使用者要求「用 paste 或直接從 local 載入檔案」）：描述版面遠比給一張圖難，而提示詞只有 300 字上限——現在對話框可附最多 3 張圖（貼上 ⌘/Ctrl+V、拖到輸入框、或按 🖼 從本機選檔），規劃與程式碼**兩個階段**都以 vision 輸入送給模型，提示詞並要求「依圖片重現版面/位置/形狀/配色」且**不要把圖片或任何外部網址載入程式碼**（sandbox 無網路，只能用 DOM/Canvas/SVG 自己畫）。圖片在瀏覽器端先縮圖再送（最長邊 1280、重新編碼為 JPEG）：現代螢幕的一張截圖動輒數 MB，而每一輪對話都會把附圖重送一次（也重複計費）。附圖只屬於這一次請求——對話紀錄留下「（附了 N 張參考圖片）」的痕跡但**不存圖片本身**，否則會撞上每則訊息 2000 字的上限並讓 spec 膨脹。**安全**：`usableReferenceImages()` 在後端重新檢查每個附件，只放行支援格式的 inline `data:` 圖片——這個欄位的內容會直接交給模型，若放進 `http:`／`file:` 網址，「附加圖片」就變成「叫伺服器去抓你指定的東西」（透明 PNG 轉 JPEG 會變黑，因此縮圖時先填白底）。**驗證**：Chromium 實跑——從本機選一張 3000x1800 PNG 與真正的 paste 事件，兩條路都得到 1280x768 的 JPEG data URL，且把瀏覽器實際產出的那串 data URL 餵給後端驗證函式會被接受（確認前後端兩邊的格式檢查一致，不是各寫一套 regex 各自過關）。前端 1035/1035（新增 4 組縮放/格式）、後端 animation-reference-images 5/5（新，含遠端網址/SVG/超大圖一律拒絕、數量上限）＋動畫與語言相關 59/59、前後端 tsc＋vite build。 | feat/animation-input-capture（已 merge 回 master） |
| 2026-08-17 | 浮動編輯視窗記住**大小**（使用者要求「大小位置應該記在 localStorage」）：位置本來就有記（拖曳走 React state，`makeslide.editorDetachedRect` 早就存在），**大小沒有**——尺寸是用原生 `resize: both` 拉的，瀏覽器直接改 inline style，React state 完全不知道，所以既沒寫進 localStorage，下一次 re-render 還會用舊的 width/height 把它彈回去。加 `ResizeObserver` 把量到的尺寸餵回 `editorRect`（寫入 localStorage 是既有的 effect 順帶完成）。還原也要考慮視窗：在大螢幕存的 rect 直接套用會讓視窗大半跑到畫面外，那等於沒記——這段邏輯抽成純函式檔 `detachedEditorRect.ts`（夾進視窗、不小於可用尺寸、標題列必須留在畫面內），parse 遇到殘缺或非數值就退回預設而不是把視窗放到 `NaN`。**踩到一個值得記的坑**：observer 比較的 `offsetWidth` 是 border-box 量測，而 style 設的是 width——在 content-box 下兩者每次 render 都差 border 的 2px，被當成 resize 寫回去，視窗每次重繪就長大 2px（實測 reload 後 1123→1165）。因此在該視窗顯式寫上 `box-sizing: border-box`，不依賴全域 reset。**驗證**：Chromium 實跑——拖右下角把視窗放大會存下新尺寸，reload 回來**完全同尺寸**（修正前每次 render 長 2px），observer 只在拖曳過程中各步觸發、之後停止（沒有迴圈），另外把視窗縮到 520x380 時，存著 1083x603 的視窗會被縮成 504x364 並留在畫面內。前端 1031/1031（新增 7 組：原尺寸還原、大螢幕存的 rect 縮進小視窗、不低於可用尺寸、視窗比最小尺寸還小、殘缺/壞掉的儲存值退回預設、非數值拒絕、預設值本身也要能塞進小視窗）、tsc＋vite build。 | feat/animation-input-capture（已 merge 回 master） |
| 2026-08-17 | 修完暫停指示還是會在動畫播到一半冒出來——第二輪（使用者回報，並指出關鍵線索「**實際時間超過動畫時長時** pause 就會出現」）：前一輪把指示改成 `isPlaying \|\| isExtendingAnimation` 只修到「有語音、動畫較長」那條路。實際上頁面的**兩個時鐘都在動畫的名目 `duration` 結束**：有語音的走延長計時器，沒語音的走 `startAnimationOnlyTimer`——而後者**從頭到尾不會設 `isExtendingAnimation`**，所以前一輪對它完全無效。而互動動畫用自己的時鐘跑，時間一定會超過 `duration`，於是兩個時鐘都停了、`isPlaying` 是 false，畫面卻還在動。指示改用新的 `isPlaybackIndicatorActive()`，聯集再加上「互動動畫仍握著輸入」（沿用 `hasActiveCustomScriptCapture()` 與它的訂閱，訂閱掛上時先同步一次目前狀態——它只在狀態翻轉時通知）。**刻意與餵給 renderer 的 `slideAnimationPlaying` 分成兩個值**：互動期間投影片時間軸確實是停的，若讓 GSAP timeline 自己往前跑，而 `currentTime` 站在原地不動，就會與 drift 校正互相拉扯（每次偏差超過 0.3 秒被 seek 回來，畫面抖動）。前端 1024/1024（新增 1 組，同時斷言互動期間 `isSlidePlaybackActive` 仍為 false——兩個意思必須分開）、tsc＋vite build。 | feat/animation-input-capture（已 merge 回 master） |
| 2026-08-17 | 修正「語音一播完就冒出暫停圖示，但動畫還沒播完」（使用者回報）：頁面長度是 `max(語音長度, 動畫時間軸總長)`，動畫較長時 `handleEnded` 會在語音結束的**當下**把 `isPlaying` 設為 false，然後把剩下的時間交給延長計時器——投影片還在動。問題是**所有顯示播放狀態的 UI 都讀 `isPlaying`**：播放/暫停按鈕、全螢幕的「已暫停」指示、一般面板右上角的暫停圓標，於是它們在動畫還在跑的時候宣告已暫停。這裡有兩個不同的「正在播放」被混用了：語音在播 vs. 畫面在動。`slideAnimationPlaying`（`isPlaying || isExtendingAnimation`）其實早就存在、也正確地餵給了 renderer（所以動畫本身會繼續跑），但**只餵給 renderer**，三處 UI 都沒用到。三處改用它，並把這個聯集抽成 `isSlidePlaybackActive()` 放到 `playbackReadiness.ts`（與其他播放就緒規則放在一起），讓兩種「playing」有各自的名字、不會再被誰順手寫回 `isPlaying`。按鈕行為本來就已經正確（`playPause` 在延長期間視為提前結束延長＝暫停），所以只有圖示與標籤要改。前端 1023/1023（新增 1 組）、tsc＋vite build。 | feat/animation-input-capture（已 merge 回 master） |
| 2026-08-17 | 動畫對話框改用 portal 疊到最上層＋產生的動畫程式碼跟隨輸出語言（使用者回報＋截圖）：(1) **z-index**：對話框明明寫著 `z-[1100]`、註解也說「要高於播放頁 header（z-[1000]）」，截圖卻是被 header 與右側欄蓋住。原因不在數字而在 **stacking context**——編輯區在「獨立視窗」模式下是 `fixed z-[135]`（`PlayPageSlidePanel`），那建立了一個新的 stacking context，對話框寫多大的 z-index 都只在那個 context 內比較，整塊仍以 135 去跟 header 的 1000 比，所以永遠輸。把兩個對話框（自訂腳本對話框、焦點放大檢視）改用 `createPortal` 畫到 `document.body`，脫離該 context 才真的是最上層；動畫版面刻意不用瀏覽器原生全螢幕（只是一層 CSS 覆蓋層），所以 portal 到 body 在全螢幕下同樣看得到。(2) **輸出語言**：`animationCustomScript.ts` 先前被輸出語言守衛列為豁免，理由是「產生的是 JavaScript 不是給人讀的散文」——但那段 JavaScript **會在畫面上畫出觀眾要讀的字**（座標軸標籤、說明、提示語如「按任意鍵繼續」、結果文字），而且和烘進圖片裡的文字一樣，使用者事後改不掉、只能重新產生。新增 `animationTextLanguageRule()`，規劃階段與程式碼階段兩個提示詞都帶入 deck 的輸出語言（手動對話框與 auto-focus 自動產生兩條路都涵蓋），並明確排除變數/函式名稱（照英文命名慣例，改成中文只會更難手改）。守衛的豁免清單移除這兩個檔案（`animationAutoFocus.ts` 因為要把語言轉交給程式碼產生器，現在也查了設定），未來新的呼叫點漏掉語言會直接讓測試失敗，而不是等某個使用者發現英文簡報裡冒出中文。後端 content-language 8/8（新增 1 組）、prompt-language 4/4、animationCustomScript 8/8、animation-custom-script 9/9，前端 1022/1022，前後端 tsc＋vite build。 | feat/animation-input-capture（已 merge 回 master） |
| 2026-08-17 | 互動進行中不自動換頁＋修正「在畫面上」的判定（使用者提問時發現）：使用者問「語音一定比動畫短，甚至根本沒有，那這頁應該算還沒播完吧？動畫的時間是拿來做什麼的？」——前半答案是既有機制已經對了（頁面長度取 `max(語音長度, 動畫時間軸總長)`：語音先播完就用計時器延長到動畫跑完，完全沒有語音則整頁改用計時器當播放引擎），後半也如他所推測：`duration` 對 custom-script 就是**速度旋鈕**，腳本內部拿 `Math.min(t/api.duration, 1)` 算 0~1 的一輪進度，實際要幾秒由 `duration` 決定。**但這個模型被互動動畫打破**，而且是上一輪改動留下的缺口：互動要跑多久由觀眾決定、一定超過 `duration`，頁面卻仍用 `duration` 算長度，於是自動換頁會在觀眾玩到一半時把頁面換走。改為 `runPageEndedAdvance()` 在還有動畫握著輸入時不前進、只記下「這頁想換頁了」，等動畫 `releaseKeys()`/`releasePointer()`（或效果離開畫面）再補做——**release 於是成為動畫說「我玩完了」的訊號**，提示詞也明講不呼叫的後果是這頁不會自己往下。順帶修掉一個會讓症狀重演的判定錯誤：capture 用的「效果是否在畫面上」原本算成 `start + duration + exitDuration`，但 `buildGsapTimeline` **只有設了 `exitDuration` 才加淡出**，沒設的 custom-script 會一直顯示到換頁——所以一個仍然顯示在畫面上、觀眾正在操作的互動動畫，會在名目長度一到就停止收到輸入（而沒設 `exitDuration` 正是常見情況）。改用 `customScriptVisibleUntilSeconds()`，沒有 `exitDuration` 時回傳 `Infinity`，與時間軸實際行為一致。前端 1022/1022（新增 6 組：互動進行中的判定含 pointer-only、訂閱只在狀態翻轉時通知、效果離開畫面即結束互動、無 exitDuration 視為永遠可見）、前後端 tsc＋vite build。 | feat/animation-input-capture（已 merge 回 master） |
| 2026-08-17 | 修正「互動動畫播到一半就停」（使用者回報，同一頁）：階段時間是從 host 送的 `t` 來的，而那個 `t` 有**兩個上限，對互動內容都是錯的**——(1) 它被 `Math.min(customScriptDurationSeconds(effect), ...)` 夾在效果長度內（這頁 30 秒），但互動要花多久**取決於觀眾按多快**，一定會超過；一超過 `t` 就凍結，動畫停在半路，症狀完全吻合（那頁 8 個階段、每段 3.75 秒，只要觀眾每段多想兩秒，第五段左右就到頂了）；(2) 它來自 `currentTime`，**投影片一暫停就完全不動**——而觀眾常常正是為了互動才按暫停。原本的夾住邏輯對非互動動畫是對的（播完維持最後一格畫面），錯在把互動內容也套進同一個時間模型。改為：**動畫一旦宣告任何 capture，sandbox 就切換到自己的 rAF 本地時鐘**，host 的 `sync` 只剩兩個用途——設定初始基準、以及在觀眾**倒退**時跟著回去；倒退的判定要比對 **host 自己的前一個 `t`**（夾在上限不動的 `t` 距離本地時鐘已經很遠，但那不是倒退，用「比本地時鐘小」判會每幀誤判成倒退、把動畫一直拉回 30 秒）。沒有宣告 capture 的動畫維持原本由 host 驅動的行為，零影響。單幀時間差夾在 0.5 秒，分頁切回來時不會一次跳過一大段。提示詞補上這個契約：互動動畫**不可以**假設 `t` 不超過 `api.duration`、也不要拿 `t / api.duration` 當整體進度，要記下每個階段開始時的 `t` 再算差值。**驗證**：用那一頁真實的動畫程式碼在 Chromium 照簡報者的方式玩完整輪（每段等它跑完再按鍵）——第 7、8 階段時 host 時鐘已經釘死在 30，動畫自己的 `t` 仍走到 31.6 與 36.0，動畫沒有停；另外單獨驗證暫停播放後動畫仍持續前進（host `t` 停在 1.5，動畫 1.53→3.55→5.55）。前端 1017/1017（新增 5 組時鐘測試：非互動走 host 時鐘、互動在 clamp＋暫停後仍前進、倒退時跟隨、背景分頁不跳一大段、release 後交還 host 時鐘）、前後端 tsc＋vite build。 | feat/animation-input-capture（已 merge 回 master） |
| 2026-08-17 | 修正「按鍵還是收不到」（使用者回報，deck `dyp31H5T0t` 第一頁）：實際去讀那一頁存下來的動畫程式碼，**一行就看到了**——它寫的是 `api.captureKeys(["*"])`，而我的實作只把**裸字串** `"*"` 當萬用字元，包在陣列裡的就被當成「一個名字叫 `*` 的按鍵」，於是宣告了全部等於一個都沒宣告，播放器照常翻頁。這是**我的 API 設計問題不是動畫寫錯**：參數本來就是按鍵陣列，提示詞又寫「也可傳 `"*"`」，寫成 `["*"]` 至少和 `"*"` 一樣自然。sandbox 與 host **兩端都**正規化（sandbox 管新的宣告，host 管已經存起來的舊動畫），單一按鍵也允許用裸字串（且不會被拆成一個個字元），提示詞改成明講兩種寫法皆可。順帶修掉另一個會造成同樣症狀、但還沒被誰踩到的缺陷：宣告只在 iframe 載入時送一次並存在「註冊」上，而 frame 會在**播放中**被重新註冊（`currentAnimationSpec` 會在字幕時間軸重算時換掉物件參照，例如音檔 metadata 到齊那一刻），iframe 不會因此重載、也就不會再宣告一次——動畫會在簡報中途悄悄失去按鍵而且再也拿不回來。改為宣告以 effect 為單位**記憶**、套用到該 effect 的每一個 frame（面板/全螢幕兩份只有一份是宣告來源），換頁時清掉（effect id 只在頁內唯一），且動畫載入後**一定**宣告一次（即使什麼都不要），新動畫才能覆蓋前一個同 id 效果留下的記憶。另加 debugLog 診斷（`localStorage.makeslide.debug='1'`）：印出 frame 是否註冊、是否在畫面上、宣告了什麼、每個按鍵去了哪裡——「動畫收不到按鍵」時要問的就是這幾件事。**驗證**：用**那一頁真實的 19 KB 動畫程式碼**在 Chromium 實跑——按一次 ArrowRight，動畫的 `onKey` 收到 2 次（keydown＋keyup）、播放器收到 0 次，且在模擬過「播放中重新註冊」之後仍然有效（先前這一步就會讓宣告消失）。前端 1012/1012（新增 5 組：陣列萬用字元、裸字串單鍵、重新註冊保留宣告、換頁清除、宣告套用到同 effect 的所有 frame）、tsc＋vite build。 | feat/animation-input-capture（已 merge 回 master） |
| 2026-08-17 | 讓自訂腳本動畫在播放器**之前**收到鍵盤／滑鼠事件（使用者要求）：原本動畫**一個事件都收不到**，而且是三層各自擋住——overlay 一律 `pointer-events: none`（滑鼠根本不會落到 iframe 上）、sandboxed iframe 永遠不會取得焦點（鍵盤事件不會送進去）、`PlayPage` 又在 `window` 的 capture 階段就把 `keydown` 接走。所以 AI 產生的程式碼裡任何 `addEventListener('keydown'/'click')` 都是死碼，寫了也不會執行。**關鍵設計取捨是「宣告式」而不是「先轉發再問它用了沒」**：host 必須**同步**決定要不要 `preventDefault()` 並跳過自己的快捷鍵，但 `postMessage` 是非同步的、跨 opaque origin 也沒有同步詢問的辦法，「轉發後等回覆」在瀏覽器裡做不到。改為動畫用 `api.captureKeys([...])`／`api.capturePointer()` **事先宣告**要處理什麼，sandbox 把宣告 post 回 host，host 每次事件同步查表：命中就轉發進 iframe 並吃掉事件，沒命中就完全不碰——**沒有宣告任何 capture 的動畫行為與改動前一模一樣**，這是「零回歸」的來源。四個細節值得記：(1) **`Escape` 是播放器保留鍵**，連宣告 `'*'` 也拿不到，否則一個寫壞的動畫可以把簡報者鎖在全螢幕裡出不來；(2) 宣告**只在該效果顯示於畫面上的期間有效**（依 `currentTime` 是否落在效果區間），否則一個早就淡出的動畫會佔住方向鍵直到換頁；(3) 滑鼠座標要換算成 iframe 內部座標——舞台在一般面板/全螢幕/編輯器預覽的 CSS 縮放各不相同，直接送 `clientX/clientY` 會對不上腳本畫出來的東西（驗證時舞台 `scale(0.5)`，點在視窗 (200,112) 要變成框內 (200,111.5)）；(4) 鍵盤**廣播給同一個 effect 的所有 frame**，因為一般面板與全螢幕各掛一個 `SlideRenderer`，同一效果可能同時存在兩份，只送一份會讓兩邊動畫狀態分歧。編輯器的即時預覽也接上同一套（互動動畫若在預覽裡試不了，就只能存檔回播放頁才知道按鍵有沒有效）。程式碼產生器的提示詞新增「互動輸入」段落（API 說明、`Escape` 保留、自行 `addEventListener` 收不到事件、只宣告真正用到的按鍵——被宣告的按鍵會讓翻頁/播放暫停在該效果顯示期間失效），規劃階段的提示詞也要求把「要處理哪些按鍵/滑鼠操作」寫進實作步驟。**驗證**：Chromium 實跑（單元測試用假物件驗不到真正的事件流與跨 origin 轉發）——動畫宣告 ArrowRight/a＋pointer＋wheel 時，播放器只看到 ArrowLeft、Escape 與框外的 click，動畫則收到宣告的按鍵（含 keyup）、框內的 pointermove/down/up/click 與 wheel，座標正確還原縮放。前端 1007/1007（新增 30 組：`customScriptInput` 21 組、sandbox runtime 9 組——後者用 stub window/document/parent **真的執行**產生出來的 bootstrap，不只比對字串）、後端 animation-custom-script 9/9、animationCustomScript 8/8、animationSpecValidate 15/15、animationConstantsConsistency 10/10、prompt-language 3/3、前後端 tsc＋vite build。（`page-animation`／`mcp-animation` 兩支在這台機器上跑不完，在 master 上同樣跑不完，與本次改動無關。） | feat/animation-input-capture（已 merge 回 master） |
| 2026-08-17 | 把守衛列出的**11 處**輸出語言缺口全部補完（使用者要求「全部改一下」）：測驗生成、投票生成、`detail.ts` 的即時投票題、tutor quiz 的主題清單與回合評語、作文評分評語、notebook 生成、React 頁生成、upload 的兩個大綱 prompt、YouTube 字幕轉大綱。幾個值得記的細節：tutor quiz 的兩個 prompt 是**模組層常數**（無法在建立時取得語言），改成接受語言的函式，其中評語 prompt 還直接寫著「summary 用**繁體中文**兩到三句話」；notebook 只有 markdown 說明跟隨語言，程式碼與識別字不動；React 頁生成的「文字」就是投影片上的字，補第 10 條規則；upload 與 YouTube 兩處大綱用 `outlineLanguageRule`（title/bullets 會變成投影片文字，語言錯了也會把錯的語言交給圖片模型去畫）；兩個品質檢查另外還寫著「（中文，不超過 60 字）」——那不只指定語言，還把**長度單位**釘死成字元數，對英文應該是 words，一併改成跟隨語言。四個呼叫點維持豁免並註明理由（座標、圖片 OCR、產生的 JavaScript、底層 plumbing）。掃完之後守衛的已知缺口清單**清空**並保持空的：新的呼叫點若忽略設定會直接讓測試失敗，而不是等某個使用者發現輸出語言不對——先前修掉的三個全是那樣被發現的。驗證：兩種設定下各處 prompt 的實際輸出都正確切換（繁體中文／英文（English））。後端 prompt-language 3/3、quizzes 30/30、tutor-quiz 26/26、detail-permission 92/92、upload-pipeline-control-permission 17/17、image-quality 4/4、script-quality 5/5、generate-poll 5/5、generate-quiz-question 3/3、notebook-generate 3/3、reactSlide 22/22、collections 2/2、course-package 3/3、sync-attendees 7/7、page-ask 7/7、page-chat-tools 5/5、tsc | fix/all-prompt-languages（已 merge 回 master） |
| 2026-08-17 | 「從大綱新增多頁投影片」跟隨輸出語言＋把語言守衛改成結構性檢查（使用者回報＋截圖：英文簡報卻產生中文大綱與中文回覆）：那兩處 prompt 的角色句是「你是簡報**內容企劃**助理」「你是簡報**大綱規劃**助理」——**完全沒有提到語言**，所以先前那個比對「你是…繁體中文…助理」的守衛看不到它們，這也是它們躲過上一次全面清掃的原因。兩處都補上 `outlineLanguageRule`（與 pipeline 自己的大綱步驟同一條規則：title 與 bullets 會變成投影片上的文字，語言錯了也會把錯的語言交給圖片模型去畫）；對話的 `assistant_message` 同樣跟隨設定——那是**原樣顯示給使用者**的回覆，英文簡報不該用中文向他描述大綱。**更有價值的是守衛的改法**：比對某一種措辭本來就不可能守得住，改為問一個結構性的問題——「這個呼叫 LLM 的檔案到底有沒有查過 `contentLanguage`」。它立刻找出**另外 11 個**舊檢查完全看不到的呼叫點（測驗生成、投票生成、兩個品質檢查、notebook 生成、React 頁生成、YouTube 字幕轉大綱等）。其中 4 個確認真的不適用（座標、圖片 OCR、產生的 JavaScript、底層 plumbing），其餘列為**已知缺口**並由測試印出來，讓這筆債看得見，而不是像目前為止修掉的三個那樣、一次一個使用者回報才浮現。後端 prompt-language 3/3（+1 結構性檢查）、add-pages-metadata-resync 2/2、add-pages-orphan-recovery 5/5、add-pages-permission 13/13、tsc | fix/outline-output-language（已 merge 回 master） |
| 2026-08-17 | 修正本頁問答「串流開了卻什麼都沒送」（使用者回報「好像沒有完全修好」＋截圖顯示 `Stream ended without an answer`，以及助教反問「哪個標示區域」像是沒收到區域）：先確認後端進程確實是新版（18 秒前才被 tsx watch 重載），再用 **curl 對執行中的服務**重現——SSE 標頭正確、**body 完全空**。根因是可寫判斷：`sendEvent` 依一個由 `request.raw` 的 `'close'` 設定的旗標決定要不要寫，而那個旗標在客戶端根本還在的時候就被設起來了（這個 handler 在 hijack 之後**立刻 await 讀檔**，等第一個事件該送時旗標早已為真）。於是串流開啟、整個模型呼叫期間保持沉默、然後結束——沒有任何東西失敗，也沒有任何東西被送出。改為直接問**回應本身**（`!writableEnded && !destroyed`），而不是從請求端的事件去推論。（`/ask` 也用請求端旗標，但它同時拿它驅動 AbortController、而且**實測是正常的**，所以不依推測去改它。）另外補上 SSE 的開場與每 15 秒心跳註解幀：這裡在模型跑完前不會送出任何東西（有工具時十幾秒起跳），而長時間沒有位元組的連線可能被中間層關掉，抵達瀏覽器同樣是「串流沒有答案」。**`app.inject` 重現不了這個**（沒有真實 socket），所以用執行中的服務端到端驗證：一般提問回得到 `done`；「請把這一頁的逐字稿改得更精簡」依序送出 `tool` 與帶著提案的 `done`（2059→1859 字）；而該頁逐字稿檔案的 md5 前後完全相同——提案仍然什麼都不寫。後端 page-chat-tools 5/5、image-edit-region 7/7、page-ask 7/7、tsc | fix/chat-sse-silent-stream（已 merge 回 master） |
| 2026-08-17 | 有拉區域時改圖走 inpaint、沒有才整張重繪（使用者要求）：被要求「把標示區域中的線重畫」時，工具跑的是整張重繪、根本沒看到選取框——那個框是**瀏覽器端的狀態**，後端不知道。整張重繪會把畫面上其他每一個東西都換成略有不同的版本，而使用者沒有要求那些改變。改為：選取框跟著提問一起送到後端，工具依它決定路徑（有可用區域→inpaint、否則→整張重繪），遮罩在後端依四個比例產生（「修改圖片」按鈕維持它自己那條前端遮罩的路）。區域要**真的是區域**才採用：一次誤點產生的 2×2px 框會把編輯遮到什麼都不剩、模型原樣回傳，那是等了很久又付了錢卻看不到任何變化；超出投影片範圍的框直接拒絕而不是默默夾回去，因為那來自量錯的拖曳，夾回去會很有自信地改到錯的地方。遮罩有一個**看起來什麼事都沒有**的失敗模式：`dest-out` 是用來源的 alpha 去減目標，挖洞的矩形必須**不透明**——用透明矩形會什麼都減不掉、遮罩沒有洞，模型於是重畫整張圖，症狀正好就是「區域被忽略」。以直接斷言像素 alpha 的測試把關（與 2026-08-14 那次踩到同一個坑時的做法一致）。後端 image-edit-region 7/7（新）、page-chat-tools 5/5、tutor-proposal-tools 6/6、page-ask 7/7，前端 977/977，前後端 tsc＋vite build | feat/chat-image-region（已 merge 回 master） |
| 2026-08-17 | 修正「下了改圖指令後跑了一陣子、什麼都沒產生也沒錯誤」（使用者回報）：先查磁碟——**候選圖確實產生了**（時間就在回報前幾分鐘），所以工具成功、問題在後面。寫一個先前缺的測試（既有案例只斷言過 `proposals: []`，從來沒有涵蓋「提案真的抵達回應」）立刻失敗在 `no done frame`，正是回報的症狀。兩個缺陷：(1) **提案被丟掉**——工具成功之後模型還要產出 `{answer:"…"}`，那一則若不符 schema 就整個請求拋錯，catch 只送 error，把**已經生成也已經付過錢**的候選圖一起丟了。改為：失敗時若已有提案，仍以空 answer 送出 `done`；有結果卻沒有描述它的句子，好過帳單有了而什麼都沒有。(2) **錯誤看不見**——面板用 `text-rose-300`（淺粉紅）畫在白底上、又放在輸入框下方遠離視線，所以使用者說「似乎也沒有顯示錯誤」；改為深紅字＋淡底色＋外框（深色模式反過來）。後端 page-chat-tools 5/5（+2：提案是否抵達 done、以及最終答案解析失敗時提案是否倖存），前端 977/977，前後端 tsc＋vite build | fix/chat-proposal-lost（已 merge 回 master） |
| 2026-08-17 | 本頁問答顯示「圖片產生中」動畫與完成後的縮圖（使用者要求）：產生圖片要十幾秒，而本頁問答原本是**單一 JSON 回應**——從送出問題到答案回來之間，對話區什麼都沒有，與當掉無法區分。改為 SSE（沿用 `/ask` 既有的形狀：工具開始送 `tool`、完成送 `done` 並帶答案與提案）；`callChatJSON` 補上串流版本早就有的 `onToolCall`，路由才看得到「工具開始了」這件事。面板顯示一個轉圈動畫並**指名正在跑什麼**（圖片／逐字稿／一般思考中），圖片提案完成後在卡片裡顯示候選圖縮圖——與「修改圖片」流程顯示的是同一張、點下去也是同一個預覽對話框。驗證：`page-chat-tools` +1，斷言 `tool` 事件**排在答案之前**（與結果同時抵達的進度不算進度）；既有那則改為解析串流 body。後端 page-chat-tools 3/3、page-ask 7/7、tool-reasoning-retry 3/3、ai-tools 6/6，前端 977/977，前後端 tsc＋vite build | feat/page-chat-progress（已 merge 回 master） |
| 2026-08-17 | 採用逐字稿提案時一併重新產生語音（使用者要求）：接受改寫後的逐字稿卻留著舊語音，等於那一頁的旁白還在唸舊的字——而這種不一致只有在**播放時**才會發現，是最糟的發現地點。改為走 `regeneratePageAudio`（存稿＋合成一次完成），也就是逐字稿編輯器自己儲存時走的同一條路，讓「逐字稿進到頁面」只有一種方式而不是兩種會各自漂移的方式。沒有設定 TTS 時退回只存逐字稿（與逐字稿編輯器的取捨一致）——直接拒絕這次編輯會讓沒有 TTS key 的簡報連改個用字都不行；留下舊語音是**看得見**的不一致，使用者可以處理，比悄悄不一致好。patch 對話框在按下之前就說明會發生哪一種，因為採用現在要花一點時間也會用掉一次 TTS 呼叫。前端 977/977、前後端 tsc＋vite build | master（直接修） |
| 2026-08-17 | 導師問答改回唯讀＋**修好「工具從來沒真的執行過」**（使用者要求移除導師問答的編輯工具「不希望學生不小心做了修改」；追查時發現更大的問題）：(1) `/ask` 是**學生的**面板——任何看得到這份簡報的人都進得去，包含分享連結——所以會主動說「我幫你改好了要套用嗎」的工具不該在那裡，學生隨手一按就會改到老師的教材。編輯能力只留在本頁問答且仍要通過 `canEditPdf`；一併移除 SSE 的 `proposal` 事件、導師面板的提案卡片、以及已無呼叫者的 `streamChatText.onToolResult`。(2) 使用者接著回報「本頁問答的工具還是沒作用」，用**真實模型實測**（stub 掉 handler，只花一次 chat completion、不產圖）確認模型三種問法都不呼叫工具，再從 log 找到根因：**gpt-5.6 拒絕在推理開啟時使用 function tools**（`400 Function tools with reasoning_effort are not supported for gpt-5.6 in /v1/chat/completions … or set reasoning_effort to 'none'`），而既有處理是**靜默退回無工具生成**——於是這個部署裡**所有**工具呼叫從來沒有執行過，包含導師問答查別頁那些既有的唯讀工具（`get_page_text` 等），唯一的痕跡是每次一行 warn。修法：遇到這個特定拒絕時**帶 `reasoning_effort: 'none'` 重試一次**再退回；以訊息文字比對，因為送這個錯的 provider 用不同的狀態碼、也沒有機器可讀的 code，而無關的錯誤（rate limit、schema 拒絕、`reasoning_effort must be one of…`）不會匹配，才不會把真正的失敗重試成看不見。驗證：修正前三種問法全部「(none)」；修正後「請把這一頁的逐字稿改得更精簡」與英文版都確實呼叫 `propose_script_edit`（語意含糊的 "please modify script to include it" 仍不觸發——"it" 沒有指涉，不動作才是對的）。後端 tool-reasoning-retry 3/3（新）、page-chat-tools 2/2、page-ask 7/7、tutor-proposal-tools 6/6、ai-tools 6/6，前端 977/977，前後端 tsc | fix/tutor-readonly-and-tool-reasoning（已 merge 回 master） |
| 2026-08-17 | 修正改圖／改逐字稿工具接錯面板（使用者回報：本頁問答裡工具沒作用、仍只用文字回答）：工具加在了 `/ask`（**導師問答**）上，但使用者從一開始要的就是**本頁問答**（`/chat`）——而那個面板自己的說明還寫著「此面板用於 AI 問答與生圖／改圖」。我把「本頁問答」讀成了「這一頁的問答」，結果做在隔壁的分頁上，使用者打字的那個面板一個工具都沒有。修法：`callChatJSON` 本來就支援 function calling，所以把同一組工具（含同樣的編輯權限閘門）交給它即可；它的非串流工具迴圈原本不回報 proposal，補上與串流版相同的 `onToolResult`，路由收集後與答案一起回傳（這個端點是單一 JSON 回應而不是 SSE）。UI 完全重用已經做好的提案卡片、圖片預覽對話框與逐字稿 patch viewer；`TutorProposal` 移到 `types.ts` 讓兩個面板共用一份定義。驗證：新增 `page-chat-tools` 2/2——直接斷言**模型真的被交付了這兩個工具**（而且是加在唯讀工具之外、不是取代），以及沒有編輯權的觀看者不會被交付。後端 tutor-proposal-tools 6/6、page-ask 7/7、prompt-language 2/2，前端 977/977，前後端 tsc＋vite build | fix/page-chat-edit-tools（已 merge 回 master） |
| 2026-08-17 | 把**另外九個** AI prompt 寫死的繁體中文改為跟隨輸出語言（使用者回報＋截圖：「本頁問答」面板用中文回答英文簡報）：那個面板的 system prompt 開頭是「你是繁體中文簡報與逐字稿助理」、完全沒讀設定——與先前修好的導師問答是同一個缺陷、不同面板。這次**不只修被指出的那一個**，而是掃過整個 `src/` 找同一個形狀，找到九處：本頁問答、測驗生成、測驗編輯、課堂助教的回答與問題摘要、簡報摘要、課程包、教學建議。全部改為經共用的 `assistantLanguage()` 取得語言名並在 prompt **頭尾各講一次**（結尾一行不夠——上面那些規則本身就是中文寫的，`tutorRoleLine` 已經學過這個教訓）。兩處查證後**維持原樣**：標題生成與大綱生成本來就有中英分支。使用者同時要求的「改圖與改逐字稿也要考慮輸出語言」查證後**本來就成立**——那兩個提案工具是從既有路由抽取的，而那些路由本來就有傳 `contentLanguage`（圖片經 `imageTextLanguageRule` 到達圖片模型、逐字稿經 `buildRewriteScriptUserPrompt`）。新增 `prompt-language.test.ts`：掃描 `src/` 找「你是（一位）繁體中文…助理／助教／顧問」這個一再重複的角色句形狀，並且**同時把該 pattern 釘在幾個已知的壞字串上**，避免日後被「整理」成一個在它該抓的那些行上反而通過的版本。加這個守衛的理由是：這已經是第三次被發現，而每一次都是使用者看到輸出語言不對才回報的——這種錯誤不會讓任何東西失敗，所以沒有測試會抓到。後端 prompt-language 2/2（新）、tutor-language 4/4、page-ask 7/7、tutor-answer 8/8、tutor-proposal-tools 6/6、quizzes 30/30、collections 2/2、course-package 3/3、sync-attendees 7/7、tsc | fix/prompt-output-language（已 merge 回 master） |
| 2026-08-17 | 本頁問答新增「改圖」與「改逐字稿」工具（使用者要求，規格書 [`docs/tutor-edit-tools.md`](docs/tutor-edit-tools.md)）：AI 導師可以在使用者剛描述完問題的地方直接提出修改，但**它不改，它提案**——每個工具產生一份待確認的東西，使用者檢視後才決定。這條界線比工具本身重要：模型會把「這張圖好像少了箭頭」當成指令而不是觀察，而錯的那次代價是使用者的內容被覆蓋。因此頁面的 JPG 與 `.script.txt` 在按下套用前一個位元都不會變——改圖工具寫的是**候選檔**（與「修改圖片」按鈕產生的是同一種東西），改逐字稿工具連檔案都不寫。`propose_page_image_edit` 走既有的 regenerate-image 路徑並回傳候選；**一次問答最多一張**，上限寫在 handler 而不是 prompt 裡（每次呼叫是真的花錢，而且是模型自己決定要花的），而且計數在生成**之前**就遞增，所以呼叫失敗也不能拿來繞過上限。`propose_script_edit` 改寫逐字稿並連同**當下的原文**一起回傳——diff 必須對照「提案產生當時」的內容，因為使用者可能在讀的時候自己改過。兩個工具都不在 `getReadonlyAiTools()` 裡，而且只有在提問者有編輯權時才給：對唯讀觀看者提出他拿不到的修改是沒有意義的。套用一律重用既有路徑：圖片開的是「修改圖片」按鈕本來就會開的預覽對話框（只有一條套用路徑，不會漂移），逐字稿則進**patch viewer**——用 diff 而不是只給新文字，因為「這是更好的逐字稿」不看變更根本無從判斷，而逐字稿通常是使用者已經調過的文字。逐行 diff 自己寫（約 60 行 LCS）而不引入套件：輸入是幾百字的短文。為了不重複實作而做了兩個重構：改寫 prompt 移到 `services/scriptRewritePrompt.ts`、候選圖生成移到 `services/pageEditProposals.ts`（regenerate-image 路由改為呼叫它）——第二份實作會安靜地漏掉圖表參考與簡報風格提示。驗證：新增 `tutor-proposal-tools` 6/6（工具集分離／成本警語／上限真的擋得住第二次／缺少參數與簡報時的拒絕／OpenAI function 形狀）與 `textDiff` 6/6（含空字串、整段重寫、只有尾端空白不算變更）；重構由既有的 figure-reference／image-edit-error／image-edit-timeout／image-client-provider／page-ask／tutor-answer 套件把關，全綠。前端 977/977、前後端 tsc＋vite build。**未實跑**：真的讓模型呼叫這兩個工具會產生圖片生成費用並改動使用者的簡報 | feat/tutor-edit-tools（已 merge 回 master） |
| 2026-08-17 | 修正分頁後的重生工作在步驟 1/3 失敗「圖檔提示詞不可為空」（使用者回報＋截圖）：分頁傳的是 `images: { prompt: '' }`，而 `runRegenerateImages` 對空字串直接擲錯。空字串錯了兩層——那個欄位是**使用者的調整要求**，會被接在一個「已經含有該頁大綱」的 prompt 後面，而且該步驟是**編輯該頁既有的圖片**而不是從零畫。對一個剛被重新規劃的頁面來說，什麼都不說會讓圖片繼續呈現已經搬到另一頁的概念；改為明講「這一頁已重新規劃，請依上面的大綱重畫整張投影片，移除不再屬於它的內容，保留簡報的視覺風格」。`scripts` 的 prompt 是選填、`audio` 沒有這個檢查，所以只有圖片這一步會這樣失敗。常數移到 `pageSplit.ts` 與其他分頁邏輯放在一起並加測試釘住（非空、且確實在講重新規劃）。後端 pageSplit 6/6、page-split-endpoint 4/4、tsc。**未實跑完整重生**——那會實際產生兩張圖與兩段語音的費用並改動使用者的簡報 | master（直接修） |
| 2026-08-16 | 修正「分成兩頁」實機呼叫 LLM 失敗（使用者貼 log）：兩次嘗試都在 `first`／`second` 為 undefined 上驗證失敗。根因是 **prompt 從來沒有寫出 JSON 的結構**——`callChatJSON` 用的是 `response_format: json_object`，那只保證回傳**合法**的 JSON，對鍵名毫無約束；zod schema 只在事後驗證。於是模型回了格式正確、但用它自己取的鍵名的 JSON，兩次都被擋下。這個 repo 其他的 `callChatJSON` 呼叫（如 `animationAutoFocus`）都在 prompt 裡明講格式，只有這個漏了。**mock 掉 LLM 的端點測試不可能抓到這個**——回應是測試自己給的；因此改用真實模型對出問題的那一頁（lxoM4nhHm2 第 5 頁 Newton's method）實跑驗證，現在回傳正確結構並分成「Gradient, Curvature, and the Hessian」與「Deriving the Newton Step」，各有自己的標題與能獨立讀懂的重點。另補一個測試把 JSON 格式範例釘在 prompt 裡。後端 pageSplit 5/5、tsc | master（直接修） |
| 2026-08-16 | 「分成兩頁」改為**重新規劃**而不是硬切（使用者要求，接續下一列）：硬切逐字稿看起來像分頁，實際上留下兩頁「各自都是照一頁寫的」——第一頁講到一半停住、第二頁沒有開場就接下去，而且圖片還是原來那張、上面畫著全部的概念。改為：LLM 為兩頁**各寫一份新的大綱**（標題＋要點），以 pipeline 既有的格式（`Slide N: 標題` ＋ 條列）寫進兩頁的 text，然後呼叫**既有的批次重生 job**（`startRegenerateJob`，指定這兩個頁碼）依新大綱重新產生圖片、逐字稿與語音——不另寫一套流程。由此而來、且刻意如此的幾件事：舊逐字稿在兩頁都**清空**而非分配（它是為一個不再存在的頁寫的，其中一半哪一頁都不屬於）；動畫 spec **整個刪掉**而不是過濾（效果綁在即將被重寫的逐字稿句子索引上，留任何一個都會在錯的句子上觸發）；舊圖先留著當佔位，工作跑完才被換掉，縮圖列不會中途變成空白方框；語音只在 TTS 真的有設定時才要求，否則整個 job 會倒在一個這台機器跑不了的步驟上，連帶把圖片與逐字稿的重生一起拖垮。前端把**伺服器啟動的 job 接手**進既有的重生 hook——沒有這一步進度橫幅不會出現、簡報也不會隨完成而更新，兩頁就只是共用一張圖、沒有語音、也沒有任何說明，症狀與「分頁壞掉」一模一樣。若簡報已有另一個重生工作在跑，分頁**仍然完成**並明說要稍後自己重生，而不是把分頁回滾。驗證：`page-split-endpoint` 斷言兩份大綱都以 pipeline 的格式寫入且頁碼是新位置、兩頁逐字稿清空、圖片複製、audio 清掉、狀態回到 `text_ready`；半邊只有空白字元的大綱會被拒絕且不留下任何東西。後端 pageSplit 4/4、page-split-endpoint 4/4，前端 971/971，前後端 tsc＋vite build | feat/split-page-replan（已 merge 回 master） |
| 2026-08-16 | 新增「分成兩頁」：一頁概念太多時把它拆成兩頁（使用者要求）：切點由 AI 決定——「講到哪裡不再是同一件事」是關於語意的判斷，取逐字稿中點會切在句子的半途。新增 [`services/pageSplit.ts`](backend/src/services/pageSplit.ts)（規劃切點，純函式易測）與 `POST /api/pdfs/:id/pages/:n/split`（實際搬動資料，與重新編號同一個 transaction）。各半邊拿到什麼、以及為什麼：**逐字稿**依選定的句子切開、**絕不重寫**（prompt 明講，程式只依回傳的索引切）；**投影片文字**由模型依同一條概念界線分配；**圖片**新頁先複製一份，兩頁都不會是空白，而且「同一張圖出現兩次」是看得出來的誠實狀態——自動生成兩張新圖等於替使用者花他沒要求花的錢；**語音兩頁都清掉**，因為逐字稿改了之後那段旁白哪一半都不符，留著會得到「播放的內容跟頁面上的字不一樣」的頁；**動畫**把綁在「已經搬到新頁的句子」上的效果從原頁移除——`startTrigger.line` 會指到現在坐在那個索引的另一句話，變成在錯的字上觸發，那比不觸發更難發現。模型若回「全部留在第一頁」等於拒絕分割，數值會被夾回真的能產生兩頁的範圍，而不是照做出一個空白頁讓使用者自己去發現並刪除。React 與 notebook 頁不提供（按鈕停用）：React 的畫面是程式碼、notebook 是文件，都無法在句子處對半分，硬分會產生兩頁共用同一份程式碼、假裝彼此不同。驗證：新增 `pageSplit.test.ts` 4 項（prompt 編號句子／輸出語言／動畫效果過濾）與 `page-split-endpoint.test.ts` 4 項——後者實際打端點，斷言重新編號正確（原本的第 2 頁往後移而不是被覆蓋）、逐字稿與文字各自落在正確的半邊、新頁的圖是複製來的、兩頁的 audio 都被清空、單句頁面回 409 CANNOT_SPLIT 且不建任何東西、React 頁回 409。前端 971/971、前後端 tsc＋vite build | feat/split-page（已 merge 回 master） |
| 2026-08-16 | React 頁的畫布改為「跟隨該份簡報」，不再固定 1920×1080（使用者發現：同一份簡報第 4 頁與第 5 頁解析度不同，並問是不是 React 頁造成的——是）：頁面圖片以 `size: '1536x1024'`（3:2，圖片模型的原生 landscape 尺寸）生成，而 React 畫布寫死 16:9，兩個常數比例不同。實測該份簡報：第 1–4、6–7 頁 `static-image` 1536×1024，第 5 頁 `react` **1920×1080**。這也是使用者最早說「React 頁看起來過大」的真正來源之一——3:2 的圖在寬螢幕上左右留黑邊、16:9 的 React 頁填得更滿，那部分不是縮放 bug 而是形狀本來就不同。而且**烘焙會把這個尺寸寫死進該頁 JPG**，fusion 轉回圖片頁也會留著，所以問題會擴散：16 份含 React 頁的簡報已有 2 份比例不一致，其中一份的 1920×1080 那組還混著 `gsap-image`（＝被 fusion 轉回去的頁）。修法：新增 [`services/deckCanvas.ts`](backend/src/services/deckCanvas.ts)，由該簡報既有頁面圖片的**眾數尺寸**推導畫布——取眾數而不是第一頁，是因為簡報裡本來就可能夾著一兩張用舊畫布烘焙過的頁面，它們不該重新定義整份簡報；尺寸明顯不合理的（過小的佔位圖、過大的掃描檔）不給票，全部不可讀才退回 1920×1080；結果快取 60 秒（編輯器每次載入頁面都會問，而它只在圖片重生時才變）。畫布串到所有假設過它的地方：烘焙文件與截圖 clip、沙箱文件與拖曳的座標換算、外框的 fit scale 與 aspectRatio、以及**生成 prompt 的第 5 條**（原本明講 1920×1080，等於叫模型照錯的形狀排版）。驗證：對回報的那份簡報實跑——推導出 1536×1024，重新烘焙第 5 頁後 JPG 從 1920×1080 變成 1536×1024，與鄰頁一致（跑完已還原原始 JPG，那是診斷不是改使用者的資料）。既有 React 頁若用百分比排版會自動適應，用固定 px 的會偏移，且只有在下次編輯時才會重新烘焙。後端 deckCanvas 5/5（新）、reactSlideBake 9、reactSlide 22、pageOverlayAndFusion 14、reactSlideEdit 22，前端 971/971（+2），前後端 tsc＋vite build。設計文件 §3.2「固定 1920×1080 畫布」已補上這次的更新說明 | feat/deck-native-canvas（已 merge 回 master） |
| 2026-08-16 | 修正 AI 導師不跟隨「輸出語言」設定（使用者回報：帳號設定 `CONTENT_LANGUAGE=en`、用英文提問，導師卻用繁體中文回答）：`/ask` 的 system prompt 第一句寫死「你是繁體中文課堂 AI 導師」，而且**整條路徑從來沒有讀過 `contentLanguage`**。這正是 [`contentLanguage.ts`](backend/src/services/contentLanguage.ts) 檔頭描述的那個坑（「prompt 用繁體中文寫、內容也在講繁體中文」）在唯一漏掉的地方留下的殘留。修法：語言字串一律放回 `contentLanguage.ts`（新增 `tutorRoleLine`／`tutorLanguageInstruction`／`tutorNoAnswerFallback`），端點改讀 `getRuntimeAiSettings().contentLanguage`。語言**在 prompt 頭尾各講一次**——該檔案已經記取過教訓：一堆中文規則之後只附加一行【輸出語言】會輸，所以角色句直接換成該語言的版本，結尾再重申一次並說明「上面的中文規則只描述格式」。另外兩個會強制中文的地方一併修掉：(1) prompt 裡「請直接明確回答『找不到相關資訊』」是在指定**要模型輸出的字面中文**，改為「請說明找不到（用你回答的語言表達即可）」；(2) `TUTOR_NO_ANSWER_FALLBACK` 是**原樣顯示給學生**的文字（不是 prompt），改為依語言，否則英文對話會以中文句子收尾。驗證：新增 `tutor-language.test.ts` 4 項（角色句／結尾重申／fallback 語言／英文版不得含中文），並在 `page-ask.test.ts` 加一項**端點層級**測試——實際打 `/ask`、攔截送給模型的 messages，斷言 en 與 zh-TW 兩個方向的 system prompt 都正確，且不再指定中文字面。測試過程踩到兩個坑：設定是**每帳號快取**的，改 `process.env` 或對「當前帳號」設定都到不了那個請求所服務的帳號，要用 `accountIdFromOwnerSub(OWNER_SUB)`；`page-ask` 套件需要 `--test-force-exit`。後端 page-ask 7/7、tutor-answer 8/8（+1）、tutor-language 4/4（新）、ask-verbosity 4/4、tsc | master（直接修） |
| 2026-08-16 | 修正 React 頁在超寬螢幕（32:9）被放大、與其他頁大小不一致（使用者回報，接續上一列；上一列的 `max-height` 修好了「外框溢出」，但這是另一個症狀）：`slideFitScale` 沒有上限，於是在 5120×1440 這種螢幕上 `min(5120/1920, 1440/1080) = 1.33` → React 頁渲染成 2560×1440，而**同一份簡報的圖片頁只有 1920×1080**——圖片頁是 `<img>` ＋ `object-contain`，它會縮小以 fit 但**永遠不會放大超過自己的像素**。這正是「react page 和一般頁的解析度不同、變得過大」。修法：`slideFitScale` 加上 1:1 上限，與圖片路徑一致。驗證（真實 `SlideRenderer` ＋ app 實際編譯出的 CSS，圖片頁與 React 頁並排比較）：3840×1080／5120×1440／3440×1440／2560×1080 四種超寬比例下兩者現在都是 1920×1080 完全一致；一般編輯面板（有 `maxHeight`）不受影響。**診斷過程的教訓**：前一輪我的重現漏了載入 tailwind CSS，全螢幕根容器的 `fixed inset-0 flex items-center justify-center` 因此完全沒生效——那個測試量的是 app 從來不存在的佈局，一路顯示「通過」，害我以為修正無效而往錯方向找了兩輪；改載 `dist` 裡真正編譯出來的 CSS 之後，立刻重現出使用者截圖的確切數字（1024×511 視窗下 React 頁 1024×576、上下各切 33px）。另外一次 `git stash` 在工作區乾淨時什麼都沒存，讓「修正前」的對照其實跑的是修正後——要對照舊版得用 `git checkout <sha>~1 -- <檔案>`。前端 969/969（+1）、tsc＋vite build | master（直接修） |
| 2026-08-16 | 修正 React 頁在全螢幕時過大、四邊被切掉（使用者回報）：根因是 `ReactSlideFrame` 的外框用 `width: 100%` ＋ `aspect-ratio: 1920/1080`——**高度由寬度推導出來，完全不看還有多少高度可用**。於是任何「投影片區域比 16:9 矮」的情況（也就是 16:9 螢幕扣掉工具列之後的每一種）外框都會長得比視窗高，被 `overflow: hidden` 裁掉。圖片頁沒有這個問題，因為 `object-contain` 本來就會同時對兩軸 fit。更糟的是元件裡那個「兩軸取小」的 `scale` 在這個情況下**是失效的**：它量的是外框自己的 rect，而外框高度恆等於寬度×9/16，所以 `height/1080` 與 `width/1920` 永遠相等，`Math.min` 取不到任何東西。修法是外框加 `max-height: 100%`（呼叫端有給 `maxHeight` 時仍以它為準）：外框因此停在被分配到的空間內，兩軸縮放才有真實的高度可量，投影片縮小並置中。刻意選這個而不是改成 `height: 100%` 填滿——沒有祖先具有確定高度時，百分比 `max-height` 只是解析不出來、退回原本的行為，而 `height: 100%` 會直接塌成 0。順手把外框樣式與縮放抽成純函式 `slideFrameBoxStyle()`／`slideFitScale()` 好讓修法本身測得到。診斷與驗證都用真實 Chrome＋真實元件跑：修正前 1920×1080 與 1366×768 兩種 16:9 視窗下投影片各被切 28px、修正後三種視窗（含 1680×1050）全部完整落在可用區內（1920×1024 → 投影片 1820×1024）；過程中還先排除了自己重現腳本的一個 bug——同一個 page 重複 `setContent` 會拿到第一次的佈局，讓四種方案看起來一模一樣。前端 968/968（+2）、tsc＋vite build | master（直接修，分支已 merge） |
| 2026-08-16 | 連結加上底線（使用者要求）：帶 `data-ms-href` 的元素加底線表示它是連結（`text-underline-offset: 0.15em`——投影片字級大，貼著文字的底線太擠）。刻意**不改顏色**：投影片的色盤是作者的，強制藍色會蓋掉他的設計，底線用 `currentColor` 是仍讀得出「這是連結」的最小標記；寫成 stylesheet 規則而非 inline，所以作者自己在 JSX 裡寫 `textDecoration: 'none'` 仍然優先（實測確認）。**沙箱與烘焙用同一條規則**——畫面上有底線、匯出的 JPG 沒有，正是烘焙存在的意義所要防的不一致，加測試同時比對兩份原始碼都帶著它。驗證：真實 Chrome 量 computed style（有連結 underline+pointer／無連結 none+auto／作者關掉 none+pointer 三種都對），再對真實烘焙出來的 JPG 量像素——有連結那行底下有 **180px 連續**深色（正好是三個 60px 中文字寬），無連結與作者關掉的兩行只有 39/45px（單一筆畫，非連續底線）。順帶修正規格 §4.3 一句與實作不符的描述（原寫「加上 cursor: pointer 與一個焦點外框」，焦點外框從未實作）。後端 pageOverlayAndFusion 14/14（+1）、reactSlideBake 9、reactSlide 22，前端 966/966、前後端 tsc＋vite build | feat/page-overlay-and-fusion（已 merge 回 master） |
| 2026-08-16 | 加上移動元素的功能（使用者要求，接續上一列）：在點選模式下直接**拖曳**元素即可移動，方向鍵微調 1px、Shift＋方向鍵 10px。座標不需要換算——iframe 本身固定 1920×1080、縮放是套在元素上的 `transform`，所以沙箱內的指標位移**就是**畫布位移。三個各自都會無聲失敗的點：(1) **只有絕對定位的元素能拖**，其餘寫 `left`/`top` 上去要嘛沒效果（static）要嘛相對版面位置偏移（relative），兩種都會讀成「拖曳壞了」——這些改為在面板上說明原因並提供「改成可自由擺放」（把當前矩形寫成 `position:absolute`＋`left`/`top`，按下去的瞬間畫面不會跳，因為使用者要的是「能移動它」不是「移動它」）；(2) **單位跟著元素走**，拖曳前先讀 inline 的 `left`/`top` 用的是 px 還是 %，算完換算回同一種，否則一份用百分比寫的投影片只要有人微調過一次就會變成像素；(3) **位移超過 3px 才算拖曳**，否則每一次「點一下選取它」都會順手把元素挪動一兩個像素。移動走與其他面板調整同一條路（先進未儲存 config、按儲存才寫進 JSX），所以拖過頭直接拖回來就好，不會每動一次就重編譯。**順帶修掉上一列留下的一個 bug：連結其實從來點不到**——沙箱 iframe 在非點選模式下是 `pointer-events: none`，加上手寫畫布那層覆蓋整個舞台，點擊根本進不了沙箱；新增 `interactive`，由呼叫端在「沒有別人要這個點擊」時（非點選、非手寫、非區域選取）且**該頁程式碼真的含 `data-ms-href`** 才開，否則一般 React 頁會白白把點擊從手寫畫布手上搶走。**另修一個既有的無聲漂移**：前端的可編輯 CSS 白名單少了後端 2026-08-15 補上的 `position`/`left`/`top`/`right`/`bottom`/`white-space`/`overflow`，前端 `normalizeStyleOverrides` 會在送出前把 `left`/`top` 丟掉，於是拖曳會看起來完全沒作用；補齊後加了一個**直接讀前端原始碼比對兩份清單**的測試，因為這種漂移兩個方向都不會有東西壞掉、也不會有任何回報（有一個前端斷言拿 `position` 當「會被擋掉」的例子，那個斷言只是因為漂移才成立，一併改掉）。驗證：真實 Chrome 實跑——px 元素拖曳精確落在 350px/380px、% 元素拖完仍是 20%/20%（單位保留）、版面排列的元素完全不動、拖曳後點擊仍能選取（門檻沒吃掉點擊）、方向鍵 +1 與 Shift +10 都對、`movable:false` 與 rect 正確回報。前端 966/966（+2）、後端 pageOverlayAndFusion 13/13（+1）、reactSlide 22／edit 22／bake 9、前後端 tsc＋vite build | feat/page-overlay-and-fusion（已 merge 回 master） |
| 2026-08-16 | 往頁面加入額外的文字與圖片＋融合回圖片頁（使用者先要求評估「一般頁面加這個功能」vs「加東西時自動轉 React 頁」，選定顯式轉換，並提出把「轉回圖片頁」當成 fusion；規格書 [`docs/page-overlay-and-fusion.md`](docs/page-overlay-and-fusion.md)）：評估的關鍵事實是**所有匯出只吃 `<page_uid>.jpg`**（[pptx.ts:67](backend/src/routes/pdfs/pptx.ts#L67)、[generateVideo.ts:102](backend/src/worker/steps/generateVideo.ts#L102)），因此加上去的東西要嘛進 JPG、要嘛只活在播放器裡——GSAP 的 `text-callout`／`overlay-image` 正是後者（既有缺陷，未在本輪修）。否決自動轉換：它的三個後果都是「畫面上正常、結果錯了」。改為顯式轉換，並依使用者追加要求把三件事做硬：(1) **烘焙不可用時不允許進入 React 模式**（`PUT` 對非 React 頁回 424；已經是 React 頁的存檔不擋，否則環境問題會變成既有頁面全部不能編輯）；(2) **fusion 烘焙失敗不再照樣轉換**——原本只記一行 warn 就還原，結果是頁面顯示轉換前的舊圖、使用者加的東西全部從畫面消失，改為維持 React 頁並讓前端問「重試／放棄更改仍轉回／留在 React」；(3) 轉換前回報會停播的 GSAP 效果數。圖片走**檔案＋`MS_ASSET("asset-….png")`**：不存 URL（換部署位址就失效）、不存 data URL（超過程式碼長度上限），沙箱與烘焙各自解析；兩邊都用**內嵌 data URL**，因為沙箱是 opaque origin、它自己發的請求不帶 session cookie 會 403（這正是背景圖當初被移到沙箱外的原因，差點又踩一次）。上傳以 sharp 實際解碼驗證（副檔名不是證據），SVG 一律拒絕（沙箱以 `<img>` 載入，而 SVG 是可帶腳本的文件）。連結做成元素上的 `data-ms-href` 而非包一層 `<a>`：沙箱沒有 `allow-popups`／`allow-top-navigation`，`<a>` 點下去會被瀏覽器直接擋掉，改為 postMessage 給父視窗、父視窗再驗一次 http(s) 才 `window.open(…, 'noopener')`（沙箱端的檢查防意外，父視窗端的才防惡意——程式碼可以手改）。順帶把採用頁面圖當背景的遮罩預設從 0.35 改為 0（那個值是為「整頁重寫」設計的，只加一個註記卻讓整頁變暗）。另修一個會讓功能無聲失效的點：烘焙原本只等元件產出 DOM，但 `<img>` 有 DOM 與有像素是兩回事，截圖會**時有時無**地漏掉圖片——加 `whenImagesSettled`（load/error 都算，5 秒上限，一張載不出的圖不該讓整份簡報匯不出去）。驗證：真實 Chrome 跑沙箱——`MS_ASSET` 已知素材回 data URL、未知回 ''、圖片實際載入渲染（naturalWidth 2／寬 200px）、真實點擊送出 `ms-slide-link` 且 iframe 未導航；真實烘焙的 JPG 在圖片位置量到紅色像素（255,0,0）、外側白色——即「加的圖確實進得了匯出」。後端新增 `pageOverlayAndFusion.test.ts` 12/12，既有 reactSlide 22／edit 22／bake 9／textExtract 23／deckOutline 4／textDetect 3 全綠，前端 964/964（+3）、前後端 tsc＋vite build。`reactSlideImportRecompile` 與 `react-slide-conversion` 兩個測試檔在**未改動的 master 上也會超時**，屬既有狀況 | feat/page-overlay-and-fusion（已 merge 回 master） |
...
| 2026-08-12 | React 投影片頁第二輪（使用者要求）：(1) 把「轉成 Notebook」按鈕換成**更改頁面類別對話框**——頁面型別變成三種之後，單向的「轉成 X」按鈕就不成立了，使用者要問的是「這一頁該是什麼」，而答案必須包含換回來；三個方向各自呼叫該型別自己的端點，過程中不刪任何東西（JPG、`.ipynb`、React 程式碼都留著），切成 React 頁存的是該頁已有的程式碼（沒有就用預設骨架），因此切換本身不花 LLM。(2) **重生時 React 頁重寫程式碼而不是重畫圖**：這種頁面的畫面就是程式碼，重畫 JPG 等於花一次圖片費用產生一張這頁不會顯示的檔案、真正的投影片卻原封不動；重寫餵的是簡報大綱（每頁一行、★標記本頁）＋本頁完整逐字稿＋使用者提示詞，大綱是必要的，否則每頁都會被當成整份簡報唯一的一頁來寫；刻意不把舊程式碼餵回去，因為重生的語意是「依現在的逐字稿重做」。(3) 修好使用者回報的**「React 頁畫面看不到」**：資料、API、沙箱全部正常，問題在版面——圖片頁外框是 `inline-block`（讓外框貼齊圖片），而 iframe 容器要 `width:100%`，互相等待的結果是寬度 0，投影片「在 DOM 裡、畫面上看不到」；改為明確 `display:block`＋寬度，縮放同時看兩軸（只看寬度的話播放器一限高就溢出被裁）並置中。驗證：用 Playwright ＋系統 Chrome 開實際簡報第 13 頁，確認修正前空白、修正後正確顯示；前端 tsc＋vite build＋938/938、後端 tsc＋reactSlide 兩檔 25/25（新增 4）。註：後端完整套件在本機會實際去打 OpenAI（測試金鑰 401 後重試）而長時間卡住，屬既有狀況，故只跑相關子集 | feat/react-slide-pages |
| 2026-08-12 | React 頁點選模式浮動「元素編輯」面板（使用者要求）：點選模式一開就浮出置頂面板，點到的元素直接在裡面改文字與 CSS。做成可拖曳、可收合的面板而不是 modal——modal 會蓋住使用者正在點的那張投影片，而每一次調整都要對著投影片本身判斷。面板依常用程度排序（文字 ＋ color／background-color／font-size／font-weight／text-align／padding／border-radius／opacity，各給合適的控制項 ＋ 其他已調整屬性 ＋ 任意屬性下拉），而不是把 31 個白名單屬性全攤開——那種面板沒有人會讀。每個控制項顯示元素的**實際值**（有覆寫用覆寫、否則用沙箱回報的 computed），只有真正動過的才寫回設定，已覆寫者以 `•` 標示可單獨還原；編輯區抽成共用元件，分頁內與浮動面板共用同一份，不會走針。色票需要把 computed 的 `rgb(...)` 轉成 `#rrggbb`（`<input type="color">` 只吃 hex，餵 rgb 會靜默變黑），全透明與漸層則不顯示色票——假的黑色方塊比沒有更糟。驗證：Playwright ＋系統 Chrome 實跑——開點選模式→面板浮出→點標題→面板顯示 `<h1> · 0/0` 與各屬性→改 color 為 #ff3366→沙箱內 h1 的 inline style 立刻變成 `color: rgb(255,51,102) !important`、畫面上標題同步變色；前端 tsc＋vite build＋946/946（新增 8） | feat/react-slide-pages |
| 2026-08-12 | 修好使用者回報的「click 元素後元素編輯面板沒有改變」：先在沙箱與父視窗兩端各裝探針，確認 trusted 的真實點擊進得到 iframe、`ms-slide-select` 也確實送達父視窗——所以問題不是點擊本身，而是三個各自都會讓它**無聲失效**的環節（失效時畫面毫無回饋，看起來就跟壞掉一樣）：(1) 點到沒有 `data-ms-path` 的地方（卡片留白、段落裡的 `<strong>`）原本直接忽略，改用 `closest('[data-ms-path]')` 選最近的可選元素；(2)「開啟點選模式」靠 postMessage 推進沙箱，iframe 若比父層 listener 早載入完成那則訊息就沒人接，之後點什麼都沒用——初始狀態改為同時烘進 srcdoc 的 `<body class="ms-inspect">`，且 iframe `onLoad` 也標記 ready，不再只依賴沙箱送上來的 ready；(3) 手寫標註畫布與選取框覆蓋整個舞台，點選模式開啟時整層改 `pointer-events: none`。另外全螢幕的 SlideRenderer 根本沒接 `inspect`/`onSelect`（面板浮得出來但點了沒反應），面板 z-index 也提高到全螢幕覆蓋層之上。驗證：Playwright 實跑——點標題選到 `<h1> · 0/0`、點投影片空白處選到最外層容器 `path 0`（修正前完全沒反應）；前端 tsc＋vite build＋948/948（新增 2） | feat/react-slide-pages |
| 2026-08-13 | React 頁三則使用者回報的修正＋一個新功能：(1) **AI 背景圖產生了卻沒顯示**——檔案有、生成成功，但背景圖端點要 session cookie，而沙箱是 opaque origin：`about:srcdoc` 無法解析相對網址，給絕對網址也是跨站子資源請求、不帶 cookie，於是 403、默默沒有背景；改成背景畫在**沙箱外面**（父文件、iframe 之下），沙箱在有背景時保持透明，`hasSlideBackground()` 集中判斷「有沒有背景可畫」（選了圖片但還沒產生／選了純色卻沒顏色都算沒有，否則會用透明蓋掉主題底色）。(2) **面板配色沒跟主題**——原本混用硬編碼 `slate-*` 與語意 token，只有一種主題下正常，屬性標籤在另一種主題淡到看不見；改為只用 `surface`／`text`／`muted`／`border`／`primary`／`danger`，對比由既有 `contrastRatio.test.ts` 把關。(3) **編輯區可分離成浮動視窗**（新功能）：分頁列最右加 `⧉`，整區變成可拖曳、可縮放的視窗，視窗標題列與分頁列各有 `⤓` 靠回原位，原位置留可點擊提示；位置大小存 localStorage，因為合適的擺放取決於使用者的螢幕。驗證：真實瀏覽器——背景圖請求 200 且投影片渲染在圖上、面板在 light/dark 各截一張確認可讀、分離後拖曳 (80,120)→(280,217) 並成功靠回；前端 tsc＋vite build＋950/950（新增 4） | feat/react-slide-pages |
| 2026-08-13 | 修好「點選元素編輯又不行了」：一般狀態與編輯區分離狀態下實測點擊都正常，所以問題不在點擊，而在**點選模式會被分頁切換偷偷關掉**——`inspect` 綁在「React 分頁是否開著」上（強制關閉的 effect ＋ 面板顯示條件 ＋ 傳給 renderer 的 prop 共三處），於是切去看一眼逐字稿、或任何改到 editTab 的操作（在分離出去的浮動編輯視窗裡尤其容易發生），點選模式就無聲關閉，體感就是「這功能隨時會莫名其妙失效」。使用者明確開的東西應該開到他自己關掉為止：改成只有「這一頁不是 React 頁」才強制關閉（那時沒有沙箱可點），換頁只清掉選取。驗證：重現該情境——開點選模式→切到逐字稿分頁→面板仍在→點投影片仍選得到元素（修正前面板會消失且點擊無效）；前端 tsc＋vite build＋950/950 | feat/react-slide-pages |
| 2026-08-13 | 修好使用者回報的「還是不行」：點選元素在 docked 狀態一切正常，問題出在**編輯區分離之後**——浮動視窗預設開在 (80,120)、900×600，正好蓋住整張投影片，於是每一次點擊都被它接走（`elementsFromPoint` 最上層是那個 `<section>` 而不是 iframe），畫面上完全沒有任何跡象說明為什麼點不到。兩個修正：(1) 分離時**從原地浮起**——以編輯區當下的位置與寬度作為初始值，並夾在畫面內、下方空間不足時往上移，視窗因此出現在投影片下方（實測 y 從 120 變成 656）而不是壓在上面；(2) 開啟點選模式時若視窗仍與投影片重疊，自動把它移到投影片下緣——使用者剛按下「點選模式」，意圖毫無疑問，沒有道理讓他去跟一個會吃掉所有點擊的視窗搏鬥。驗證：以「會遮擋的既有位置」重現失敗（sandbox 收不到任何點擊）→ 修正後同一情境點擊命中 iframe、面板顯示選取；分離預設位置、拖曳與靠回原位皆重測通過；前端 tsc＋vite build＋950/950 | feat/react-slide-pages |
| 2026-08-13 | 依使用者回報的兩個線索（滑過投影片**有**藍色外框、但元素編輯面板**完全沒出現**）定位：藍框來自沙箱內的 hover CSS，代表點選模式確實開著、指標事件也通得到 iframe，所以問題不在點擊而在**面板沒被畫出來**。兩個修正：(1) 面板改用 `createPortal(..., document.body)`——`position: fixed` 其實是相對於「最近一個有 transform／filter／backdrop-filter 的祖先」而非視窗，只要面板上方任何一層有這類屬性（對話框背景、轉場動畫都會），視窗就被定位進那個元素的座標系，可能整個跑到畫面外，而畫面上不會有任何線索；(2) 拿掉「這一頁不是 React 頁就不顯示面板」的條件——它會產生一模一樣的症狀（模式開著卻沒有視窗證明它開著），改成一律顯示並在面板內用一行說明。另外面板現在會自報「第 N 頁 · 頁面型別：react」，把「系統認為現在是什麼狀態」直接放到畫面上，下次不必再靠猜。驗證：分離＋遮擋情境實跑，面板存在且點擊後內容為「第 13 頁 · 頁面型別：react ／ `<h1> · 0/0` ／ 各屬性控制項」；前端 tsc＋vite build＋950/950 | feat/react-slide-pages |
| 2026-08-13 | 針對「點了元素面板不變」再修一輪，並把診斷做進畫面：先用 Playwright 逐一排除——一般版面、編輯區分離、分離視窗壓在投影片上、CPU 降速 20 倍（想逼出 React 18 排程 commit 落在 rAF 之後的競態）四種情境全部都選得到元素，等於**我這台機器重現不出來**，那就不該再靠猜。於是兩手並進：(1) 穩健性——路徑指派與覆寫套用改由 `MutationObserver` 在元件 DOM 變動時重跑（原本只在單一 `requestAnimationFrame` 跑一次；真的踩到時症狀正是「hover 有藍框但點擊無效、既有覆寫也沒套上」），點擊當下若仍無路徑再補一次；(2) 可觀測性——沙箱回報 `pathCount` 與最後點到的 tag，面板顯示「可點選元素：N · 最後點到：tag」，因為「N=0」與「點到了但找不到可選祖先」是兩種不同故障、症狀卻相同，現在一張截圖就能分辨。另外面板先前已改 portal 到 `<body>` 並一律顯示。驗證：降速 20 倍實跑，面板顯示「可點選元素：6 · 最後點到：h1」且選取成功；前端 tsc＋vite build＋950/950 | feat/react-slide-pages |
| 2026-08-13 | 補上 React 投影片頁最大的缺口：**把 React 畫面烘焙回頁面的 JPG**。匯出 PDF/PPTX/影片/SCORM、縮圖列與封面吃的都是 `<page_uid>.jpg`，在此之前它們拿到的是「這一頁還沒變成 React 頁之前」的舊圖——畫面上對、發出去的每一個檔案都錯，而且沒有任何地方會提醒。改用無頭瀏覽器渲染 1920×1080 JPEG 寫回 `image_path` 並重建縮圖，其餘功能因此**一行都不用改**。幾個判斷：用真的瀏覽器而不是 SSR/canvas 近似，因為投影片是真的 HTML/CSS 由真的排版引擎排出來的，近似會產生一張與使用者核可過的畫面不同的圖，那比圖有點舊更糟；烘焙文件是沙箱的**靜態版**（同畫布、同 token、同背景與覆寫，但沒有 inspector／postMessage／hover 外框，那些不屬於一張圖片）；React UMD 與背景圖都內嵌進文件（`setContent` 沒有 origin 可解析相對網址，而烘焙也沒有 session 去打需要驗證的背景圖端點，離線機器同樣能烘）；截圖等 `__msSlideReady`，因為 React 18 的 commit 是排程的、第一幀截圖會拍到空白頁。用 `playwright-core` ＋ 機器上的 Chrome（不下載瀏覽器），沒有瀏覽器的部署得到 424 `BAKE_UNAVAILABLE` 與明確訊息而不是堆疊——烘焙是加分項，沒有它頁面照樣渲染給觀眾看。存程式碼／存設定／AI 生成／生成背景後在背景自動烘焙（同一頁同時只跑一個、失敗只記 log 不影響存檔，讓存檔因為截圖失敗而失敗是更差的交易），另有「立即更新這一頁的圖片」按鈕。驗證：對實際第 13 頁烘焙產出 1920×1080、127KB 的 JPEG，背景圖、遮罩、主題字型與使用者的兩筆覆寫（青色首行、手動加的文字）全部正確重現，耗時約 1.9 秒；後端 tsc＋烘焙測試 9/9（新增）、前端 tsc＋vite build＋950/950 | feat/react-slide-bake |
| 2026-08-13 | 讓 React 頁的兩個轉換方向都不再讓畫面消失（使用者要求）：(1) **轉成 React 頁**時把原本的 JPG 複製成背景（`mode:'image'` ＋ 0.35 遮罩），否則轉換在視覺上等於把投影片清空——檔案還在、匯出讀得到，但畫面上原本的東西全不見了；刻意是**複製**而不是引用頁面 JPG，因為烘焙會把渲染結果寫回同一個 JPG，背景若指向它，每烘焙一次就把上一次的結果再疊進去一層。使用者若已自己選過背景就完全不動。(2) **轉回一般投影片**時先烘焙再還原（順序不能反：烘焙需要該頁仍是 React 頁），讓那張圖是使用者剛看到的 React 畫面而非變成 React 頁之前的舊圖；烘焙失敗只記 log，因為使用者要的是轉換本身。過程中順手釐清測試邊界：測試套件預設不啟動瀏覽器（`MAKESLIDE_TEST_ALLOW_BROWSER=1` 才實跑——一個 fire-and-forget 的瀏覽器會活過斷言、讓測試進程不結束，也讓套件相依於主機裝了什麼），自動烘焙另可用 `REACT_SLIDE_AUTO_BAKE=0` 關閉。驗證：新增 `react-slide-conversion.test.ts` 以 `app.inject` 建拋棄式簡報實測三件事——轉成 React 會產生背景快照且帶遮罩、既有背景不被覆寫、轉回時轉換必定成功（3/3）；前端 tsc＋vite build＋950/950 | feat/react-slide-conversion-visuals |
| 2026-08-14 | React 頁的「修圖套用到背景」與「區域文字轉 React 文字」（使用者要求，先寫規格書 [`docs/react-slide-image-to-text.md`](docs/react-slide-image-to-text.md) 再依規格實作）：(1) **修圖套用改寫到背景圖**——React 頁的 `<page_uid>.jpg` 是烘焙產物，下一次存檔就會被蓋掉，修好的圖會無聲消失；同時 inpaint 的**來源**在 React 頁也改讀背景圖，否則是在修一張已經含有 React 文字層的烘焙結果，修完再當背景，文字會出現兩次。(2) **區域文字轉 React 文字**：辨識選區的文字與樣式做成文字層，並把那塊像素從背景圖抹掉。文字**存成 `config.textLayers` 而不是寫進 `.slide.jsx`**——插入 JSX 要改動 LLM 寫的原始碼（脆弱），還會讓既有元素路徑位移、使所有覆寫失效，而且重新生成程式碼時會整段消失；當成 config 則像背景一樣活過重生，每一層都能單獨編輯或刪除。字級由**區域幾何**夾制（高度÷行數）：模型對字級的估計不可靠，而失敗是不對稱的——太大會溢出蓋到旁邊並被裁掉，太小只是留白多一點且一個控制項就能修，所以寧可略小。字型只能對應到主題的三種字體（heading/body/mono），因為我們沒有原圖的字型檔、沙箱也不能載外部字型——規格裡明講這個限制。辨識與抹圖**分開回報**（`erase: done/skipped/failed`）：辨識快又便宜、結果本身有價值，抹圖是生成式呼叫且會失敗，失敗時文字層保留、畫面上會看到兩份文字，這件事必須明說而不是回報成功；抹圖前備份背景，復原是一個按鈕。實作時踩到一個非直覺的坑：mask 的 `dest-out` 是用**來源**的 alpha 去減目標，挖洞的矩形必須是**不透明**的——用透明矩形會什麼都不減、mask 沒有洞、抹圖變成重畫整張背景；已用直接斷言像素 alpha 的測試把關。驗證：新增 `reactSlideTextExtract.test.ts`（11 項，含 mask 像素、字級夾制的四種情況、危險圖層被丟棄），React 頁相關測試序列跑 48/48、前端 tsc＋vite build＋950/950 | feat/react-slide-image-to-text |
| 2026-08-14 | 修好使用者回報的「第 14 頁產生失敗：`audiocpp_cli failed: Qwen3 custom voice prefill requires speaker`」：症狀的關鍵是**只有那一頁失敗**，同一份簡報其他頁都合成得出來。原因不在 audio.cpp 而在聲音回退鏈——`resolveSpeakerVoice` 對**沒有 `Speaker N:` 前綴的段落**（第 14 頁的講稿是單人旁白 `This is a test`）第一行就 `return deckVoice` 直接結束，完全跳過講者設定；而這份簡報的 `tts_voice` 是空的（另一份是殘留的 Gemini `Kore`，對本機模型不可用一樣會被丟掉），於是這一段帶著空聲音進到合成。其他頁有 `Speaker 1:`／`Speaker 2:`，回退鏈走得到全域的 `vivian`／`ryan`，所以正常。Qwen3-TTS CustomVoice 沒有預設講者，少了 `--speaker` 就以那句英文中斷，而訊息裡既沒有頁碼也沒有「是哪個設定」。修法分兩層，因為只做一層都還留著「帶著空聲音抵達合成」的路徑：(1) 無前綴段落在簡報聲音為空或是別家音色名時，往下退到**講者 1**（簡報的、再來是全域的）——單人旁白就是這份簡報的主講者，而講者 2 不會被借用；(2) `audioCppEffectiveVoice` 在 `qwen3_tts` 家族仍然什麼都沒有時補上內建講者 `vivian` 並記一筆 warn，放在 `synthesizeAudioCppSpeech` 這個唯一入口，因此 server 模式（同樣要 speaker）自動一起適用。順手把散在四處的 `startsWith('qwen3_tts')` 收成 `isQwen3TtsFamily`。驗證：先用本機真實 CustomVoice 套件重現——不帶 `--speaker` 直接跑 CLI 得到與回報一字不差的錯誤，修正後以同樣的空聲音、同樣的文字實跑 `synthesizeAudioCppSpeech()` 產出 73 KB 音檔並印出 fallback 警告；後端 tsc、audiocpp 與 synthesize-audio 四檔 129/129（新增 2）。文件 `docs/audiocpp-local-tts.md` §9 一併補上這兩條規則 | fix/audiocpp-missing-speaker-voice（已 merge 回 master 與 feat/react-slide-image-to-text） |
| 2026-08-14 | 修好使用者回報的「輸出語言設成英文，但圖片和逐字稿都還是中文」：設定本身沒問題——帳號的 `CONTENT_LANGUAGE=en` 確實存下來也讀得到，證據是同一份簡報的**標題是英文**（標題那條路徑沒有相衝突的指示）。輸的是底下的 prompt：逐字稿的模板是「用中文寫、內容也在講中文」——開頭 `你是一位專業的中文簡報講師`、編號規則裡再寫一次 `使用繁體中文`、還要求 `台灣用語`，而每一頁的 user message 更直接寫著 `輸出語言：zh-TW（繁體中文）`，那個值來自 `OPENAI_SCRIPT_LANGUAGE`（operator 層級的 env，與帳號設定無關）。在這一整套後面補一行【輸出語言】是贏不了的，何況 user message 離任務更近、衝突更直接。圖片那邊則是**完全沒有語言規則**：`buildImagePrompt` 整份是中文，頁面文字也是中文，生圖模型自然照抄。修法是把語言變成模板變數，並讓所有組 prompt 的地方都從同一處（`services/contentLanguage.ts`）取值：(1) 8 個逐字稿模板改用 `{{language}}`，中文專屬的規則（轉場詞、台灣用語）移進 `{{language_notes}}`——英文時它說的是相反的事，並註明上面規則裡的中文例子只示範**格式**；刻意維持單一模板而不是中英各一份，因為那些例子教的是標記與 JSON 形狀，不隨語言改變。(2) 每頁 user message 與單頁改寫改用帳號設定；`OPENAI_SCRIPT_LANGUAGE` 直接移除，因為除了一行開機 log 之外已經沒有人讀它。(3) 英文另外補上「字數指的是字元數不是單字數」——否則模型看到「約 350 字」會寫出 350 words，長度差好幾倍，該頁語音會蓋過投影片。(4) 整份重寫（deck rewrite）原本一個字都沒提語言，而它會把每一頁重寫一遍，等於把生成階段的成果又推回中文。(5) 圖片：`buildImagePrompt` 收語言並把規則放在它所管的頁面文字**之前**，大綱步驟（title/bullets，正是後來被引用成頁面文字的東西）也在前一步就先定語言。驗證：用這個帳號的真實設定（`CONTENT_LANGUAGE=en`、gpt-4o-mini）實跑一次——餵中文頁面文字、拿回英文逐字稿且語氣標記格式正確；後端 tsc、新增 `content-language.test.ts` 7 項（模板不得再自己寫死語言、兩種語言都要 render 乾淨、system 與 user message 必須一致）加上 image/prompt-template 相關 36/36。注意：圖片端只驗到 prompt 內容，沒有實際生圖（會產生費用並覆蓋既有圖片）；另 `scripts-txt`、`prompt-text-limit` 兩個測試檔在本機會實際打 API 而卡住，修改前後皆然 | fix/content-language-en（已 merge 回 master 與 feat/react-slide-image-to-text） |
| 2026-08-14 | 修好使用者回報的「點到的元素又不能編輯了」——這是前一次改動造成的回歸，而且是**面板自己的診斷抓到的**：畫面上寫著「可點選元素：6 · 最後點到：div!」，結尾的 `!` 正是「點到了但找不到可選祖先」。原因是新加的文字層容器 `#ms-text-layers` 覆蓋整張畫布（`position:absolute; inset:0`）且疊在元件之上，於是每一次點擊都落在它身上。修法：容器 `pointer-events: none`，只有裡面的文字層本身 `pointer-events: auto`。順帶一提，這次從回報到定位只花一次來回——上一輪加進面板的自我回報（可點選元素數／最後點到什麼）正是為此存在。驗證：Playwright 實跑，點標題選到 `<h1> · 0/0`、點空白處選到最外層容器；新增測試直接斷言沙箱文件裡的 `pointer-events` 規則；前端 950→952/952 | feat/react-slide-image-to-text |
| 2026-08-15 | 修好使用者回報的 `400 Unsupported parameter: 'max_tokens' is not supported with this model. Use 'max_completion_tokens' instead.`（發生在 React 頁的「區域文字轉 React 文字」，但其實**每一個 LLM 呼叫**都會中）：挑選這兩個參數的判斷寫的是 `startsWith('gpt-5.5')`，而這個帳號的模型是 **gpt-5.6**——一個小數點的版本差就掉出唯一被認得的字串，於是請求在送出前就注定拿到 400。同一個函式也決定要不要送 `temperature`（GPT-5 家族一律拒絕），所以那會是下一個 400。修法分兩層，因為「列舉模型名稱」這件事保證還會再過期，而這個 bug 正是它過期的樣子：(1) 改成**依家族**比對（`gpt-[5-9]…` 與 o 系列 `o1`/`o3`…），並先去掉供應商前綴，讓 OpenRouter 那種 `openai/gpt-5.6` 也認得；(2) **從 400 本身學**——錯誤裡指名 `max_tokens`／`max_completion_tokens`／`temperature` 時，就記下相反的形狀並把同一個請求重送一次，每個模型只學一次（快取在行程內），因此未來冒出來的新家族代價是「一次多餘的呼叫」而不是「整個功能停擺」；JSON 與串流兩條路徑都套用，串流的修復發生在第一個 token 送出之前，呼叫端看不到。順手修掉另一個雜訊：`fetch` 會自行解掉它認得的 Content-Encoding 但**保留 header**，所以回應可能寫著 `br` 卻交給我們純 JSON，手動的 brotli 補救因此每一次呼叫都失敗並印一則 WARN——body 已經是 JSON 就不再嘗試（真的沒解碼的還是走補救，真的失敗仍會警告）。驗證：用這個帳號真實的 gpt-5.6 實跑——形狀解析為 `max_completion_tokens` 且不送 temperature，實際呼叫正常回覆（修正前是 400），brotli 警告也不再出現；後端 tsc、新增 `chat-param-shape.test.ts` 6 項（家族比對、舊模型不受影響、從 400 學一次且不重複學、無關錯誤不誤判、實際送出的參數、修復後重送）、`ai-tool-loop` 3/3 | fix/gpt5-max-completion-tokens |
| 2026-08-15 | React 編輯模式的「刪除元素」（使用者要求：可以用 Del 鍵，也可以用元素編輯器上的按鈕）：原本元素編輯只能改「說什麼」和「長什麼樣」，不能把它拿掉——生成器多給的一個標題或一個空容器，只能回去改程式碼，而那正是覆寫機制存在的目的。因此刪除做成**另一種覆寫**（`hidden: true`）而不是改 JSX：從原始碼刪等於改寫 LLM 寫的程式、會讓後面每一個元素路徑位移（使用者其他的調整全部失效），而且重新生成時元素又會回來；當成覆寫則像背景一樣活過重生，而且是**一筆可以還原的紀錄**。刪掉的元素在播放與烘焙（也就是匯出 PDF/PPTX/影片實際用的那張圖）裡是真的不見，但**在點選模式下仍以淡色虛線顯示**——否則點錯了就只剩覆寫清單能救，元素本身永遠選不回來。這也決定了實作方式：旗標放在 attribute 上、由點選模式的規則以特異性壓過它，而不是用 inline `display:none !important`（那個沒有任何樣式規則蓋得過）。三個入口共用同一個動作 `deleteReactSelection()`，免得它們對「現在選到的是文字層還是元素」各有答案：(1) **沙箱裡按 Del**——點投影片元素會把焦點放進 iframe，而 opaque origin 讓父視窗看不到它的鍵盤事件，所以沙箱自己攔下來再 postMessage，否則「選完馬上按 Del」這個最自然的動作完全沒反應；(2) **應用層按 Del**（焦點在面板上時）；(3) 元素編輯器裡的 🗑 按鈕，刪除後變成「還原這個元素」。兩個 handler 都不會從 input／textarea／select／contenteditable 手上搶走這個鍵。另外修掉一個順手發現的問題：`hidden` 若用 `z.boolean()` 嚴格驗證，一筆手改壞的值會讓**整份 config**退回預設（連背景、文字層、其他覆寫一起消失），改成與 `styles` 一樣寬鬆、在 sanitize 時才收斂成真布林。驗證：用真實 Chrome 跑實際的沙箱文件——點選模式關閉時元素 computed 為 `display:none` 且高度 0；開啟時可見、opacity 0.28，且 `elementFromPoint` 仍命中它的路徑（代表選得回來、還原得了）；沙箱內按 Del 確實送出 `ms-slide-delete-request`；在輸入框裡按 Del 什麼都不送。前端 tsc＋958/958（新增 6）、後端 tsc＋reactSlide/bake/text-extract 44/44（新增 1）。註：`react-slide-conversion.test.ts` 在本機會逾時，修改前後皆然 | feat/react-slide-delete-element |
| 2026-08-15 | 改成「所有修改都同步進 JSX 程式」（使用者要求：不要再維護額外的覆寫層；程式碼是所有修改最終儲存的位置，LLM 重生時元素不見就是不見，靠版本系統還原）：先確認可行性關鍵——這台機器上實際存在的 20 個 `.slide.jsx` **全部是靜態 JSX 樹**（沒有 `.map()`、條件渲染、fragment、自訂子元件），而「DOM 位置路徑 ↔ 原始碼節點」的對應只有在靜態樹上才成立。即使如此仍**換掉定位方式**：儲存時為每個 JSX 元素注入 `data-ms-id`，runtime 改用 id 而不是位置路徑——位置路徑只在程式碼沒動過時有意義，而現在每一次編輯都會動程式碼。改寫採**來源片段替換**而不是從 AST 重新輸出：一個字的修改必須讀起來就是一個字的 diff（對手改程式碼的人如此，對現在要靠它當還原機制的版本歷史更是如此），文字替換也刻意保留前後縮排。新增 `PATCH .../react-slide/edits`：一批修改對同一次 parse 解析出的位置一起套用，因此兩個修改不會互相位移；對不到元素的修改**回報在 `skipped` 而不是靜默丟掉**（面板還顯示著、儲存卻悄悄沒生效，比明講失敗更糟）。編輯過程仍維持現在的手感——拖滑桿即時改 DOM、不重編譯，但那份東西降級為**未儲存的暫存**，按儲存時寫進程式碼並清空。安全性上這是使用者輸入第一次變成**程式碼**而不是資料，所以除了既有的 CSS 白名單與安全值檢查，寫進 JS 字串字面量時再轉義一次（含 U+2028/U+2029）。同時補上 `.slide.jsx` 的版本歷史（history／versions／restore，沿用既有的版本對話框，多認一種類型）——這是使用者「靠版本系統還原」的前提，先前**並不存在**（存程式碼的路徑根本沒有 commit，版本 API 也只涵蓋圖片與逐字稿）；還原會重新編譯，因為沙箱實際跑的是 `.slide.js`。驗證：對這台機器上全部 20 個真實 `.slide.jsx` 各跑一次「改文字＋改樣式＋刪元素」，20/20 改寫後都能重新編譯且結果正確；再用真實 Chrome 跑完整迴圈——原始碼裡的 id 確實傳到 DOM、點擊解析得到該 id、改寫套用、重新編譯後畫面顯示新文字與新顏色。後端 tsc＋reactSlide 系列 55/55（新增 13）、前端 tsc＋958/958。**尚未搬進 JSX**：從背景圖抽出來的文字層仍是 config、仍畫在自己的圖層 | feat/react-slide-edits-in-jsx |
| 2026-08-15 | start.sh 改成「差異放 .env、腳本人人共用」（使用者要求）：起因是這台機器上的 start.sh 被改過才跑得動（`set -euo pipefail` 被註解掉），而**改過的 start.sh 沒辦法跟著 git 走**——每次 pull 都衝突，別人也看不到這台機器到底不一樣在哪。先查根因：在嚴格模式下實跑 `--backend-only`（含 audio.cpp 檢查、Jupyter、後端 listening）與 `--frontend-only`（Vite 起來）兩條路徑都**沒有中斷**，也就是那一行現在已經不需要註解掉了。改法是把所有「這台機器不一樣」的東西變成 .env 設定：`START_MODE`／`START_PORT`／`START_DEV`／`START_HTTPS`（含三個憑證路徑）／`START_FRONTEND_BUILD_WATCH`／`START_SKIP_NVM`／`START_STRICT`，優先序是環境變數 → .env → 內建預設，而命令列參數壓過三者（機器可以有自己的預設，又不失去臨時改一次的能力）。嚴格模式恢復預設開啟並改成**設定**而不是「自己去註解掉那一行」——只有在這台機器 source 進來的腳本（某些 nvm/conda 版本）本身與 `set -u` 衝突時才需要關。`read_env_var` 因此要前移到檔案最上面（預設值都依賴它），後面那份重複定義刪掉。驗證：`--help` 列出這些設定；把 `START_MODE=frontend`／`START_PORT=8899` 寫進 .env 後實跑，確實以 frontend 模式跑在 8899；再確認 `--frontend-only` 仍壓得過 .env 裡的 `START_MODE=backend` | feat/start-sh-env-driven（已 merge 回 master） |
| 2026-08-15 | 把「背景圖抽出來的文字層」也產生進 JSX（使用者要求，接續上一輪）：這是最後一個還活在程式碼旁邊的東西——辨識出來的文字原本是 config 裡的一筆、畫在自己的圖層上，JSX 根本不知道它存在。現在直接插進根元素，成為一個帶 `data-ms-id` 的絕對定位 `<div>`，從出現的第一刻起就跟其他元素一樣可編輯、可刪除、進得了版本歷史。設計文件當初選 config 的理由（重生時保留、不影響元素路徑）在新模型下不再成立：程式碼是所有修改最終儲存的位置，重生時元素不見就是不見，要回去用版本歷史（文件已補上這段更新）。定位屬性（position／left／top／right／bottom／white-space／overflow）因此加進可編輯 CSS 白名單——移動那段文字現在就是在改這些；有兩個測試剛好拿 `position` 當「會被擋掉」的例子，改用仍會被擋的屬性。既有已經存下來的 `textLayers` 仍會渲染（相容），只是不再產生新的。安全性照舊：插入的文字走 JSX 文字轉義，每個樣式值走白名單＋安全值檢查＋JS 字串字面量轉義。驗證：真實 Chrome 比對新舊兩種做法的實際幾何——完全一致（192,216 768×129.59），顏色、字級、字重也相同，且帶著 `data-ms-id` 立刻就選得到；後端 tsc＋reactSlide 系列 58/58（新增 3）、前端 tsc＋958/958 | feat/react-slide-text-layers-in-jsx（已 merge 回 master） |
| 2026-08-15 | 修好使用者回報的「去除文字時會把整頁的文字都從圖中去除，而框外的文字又沒有被 JSX 加回去」：兩個問題疊在一起，缺一不可。(1) `images.edit` 在 gpt-image 這類模型上是**重新生成整張圖**——mask 只是「哪裡該改」的提示，其餘部分照樣被重畫，於是框外的文字一起消失，而那些字並沒有被抽成 JSX，等於整段不見。(2) 頁面是 16:9、模型輸出是 3:2，把整頁塞進去會 letterbox（1536×1024 下上下各 80px 白邊），而 mask 的洞卻是把兩軸各自等比縮放算出來的——洞根本不在使用者框的位置。修法：送給模型的改成**框附近、已經符合模型長寬比的一塊裁切**（帶 padding，讓模型看得到周圍背景才有東西可以延續），因此沒有 letterbox、洞正好落在選取範圍上；拿回來之後**只把框內那個矩形合成回去**，框外一個位元都不會被寫到——這個保證是結構性的，而不是「拜託模型不要動」。驗證：拿純紅色當作模型輸出、白底頁面實測——框內被重畫，框的四個邊界外側、模型也看得到的 padding 區、以及遠端角落全部仍是白的；另外斷言 context 一定包含選取範圍且不超出圖片（含貼齊角落與整頁選取），以及 mask 的洞是相對裁切而不是相對整頁定位。後端 tsc＋reactSlide 系列 61/61（新增 3） | fix/erase-only-inside-region（已 merge 回 master） |
| 2026-08-15 | 追查使用者回報的「背景上的文字並沒有移除」，發現**背景其實移除成功了**，錯的是後續：先比對抹除前後的背景圖，只有使用者框的那張卡片變空白，右邊圖表、其他三張卡片、標題、底部重點全部原封不動（上一輪「只抹框內」的修正有效）；再看該頁 JSX，抽出來的文字確實以絕對定位 `<div>` 進去了。真正的問題是**字級**：抽出來的文字用「區域高度 ÷ 模型回報的行數」定字級，從來沒有問過「一行在這個寬度放不放得下」——模型說兩行、實際換行成三行，於是溢出被裁，畫面上看起來就像「原本的文字還在，只是變醜了」，難怪會被當成沒有移除。改成**照瀏覽器的方式模擬換行**（CJK 可任意處斷行、尊重模型給的換行符）數出真實行數，再二分搜尋出高度塞得下的最大字級；字寬用量出來的值而不是理論值——這個字重下 CJK 大約佔 1.25em 而不是 1em，差的正好就是「兩行變三行」那一點。刻意往小的方向取整：字太小只是留白多一點、一個控制項就能修，字太大是直接被裁掉。驗證：用該頁實際抽出的兩塊重算，30px→25px、36px→29px，再用無頭瀏覽器把整頁渲染出來，兩塊都完整落在卡片內、與旁邊未動過的卡片排版一致；沒有傳文字/寬度的呼叫端維持原行為。後端 tsc＋reactSlideTextExtract 16/16（新增 2） | fix/extracted-text-fits-region（已 merge 回 master） |
| 2026-08-15 | 「轉換完後背景要即時替換，兩者無法同時產生就等都好了再顯示」（使用者要求）：查下去發現這其實也是上一輪那個誤會的第二個成因——背景圖是從一個**永遠不變的 URL** 提供的，靠 config 的 `updated_at` 當 cache buster，而抹除只寫了檔案、**沒有動 `updated_at`**。瀏覽器因此完全沒有理由重抓，畫面上留著舊背景（字都還在），旁邊又擺著剛抽出來的文字，看起來就是「抹除根本沒作用」。（我上一輪用 bake 直接讀檔渲染，所以看到的是乾淨的，這也是為什麼兩邊看到的不一樣。）修法兩半：(1) 抹除成功後在 config 上蓋 `updated_at` 並寫回，讓 URL 跟著圖一起變；(2) 前端**等新背景解碼完成**再套用新的程式碼與 config，讓「重畫過的背景」與「從它裡面抽出來的文字」在同一步出現——辨識與抹除是兩個各自會成功或失敗的操作，誰先到就先套用的話，就會看到抽出來的文字疊在還有同樣文字的背景上。這個等待在失敗時照樣往下走、且最多 8 秒：它的作用只是讓切換看起來乾淨，不能讓一張載不動的圖把整筆修改卡住。另補一個測試釘住 cache key——它掉了不會有任何錯誤訊息，症狀是「抹除壞掉」而不是「圖片過期」。後端 tsc＋reactSlide 系列 63/63、前端 tsc＋959/959（新增 1） | fix/background-swap-after-extract（已 merge 回 master） |
| 2026-08-15 | 「產生文字的換行要轉成 HTML 換行；調整字型大小時也要依換行位置調整，才能和原圖儘量一致」（使用者要求）：兩件事其實是同一個症狀的兩半。(1) **換行被吃掉**——JSX 裡文字中的換行是「空白」，`<div>a\nb</div>` 編譯出來就是 `"a b"`，所以模型辨識出來的斷行在到達畫面之前就被折成空格，整段變成一行流下來（`white-space: pre-wrap` 救不了，因為換行在編譯階段就沒了）。改成輸出 `<br />`，那是唯一能在編譯後仍然是換行的形式；同時讓「children 是文字＋`<br>`」仍然算純文字可編輯——否則剛抽出來的那一塊反而會變成面板唯一拒絕編輯的東西——而沙箱回報文字時把 `<br>` 讀回 `\n`，這樣編輯一次不會把版面默默壓平。(2) **字級要照著換行算**——原本是貪婪重排文字、再看什麼形狀塞得下，那會允許更大的字級卻排出跟原圖不一樣的形狀，正好與「儘量一致」相反。改成**每一行都必須以一行的身分放得下**：字級由最長那一行的寬度、以及該行數所需的高度共同決定。驗證：對實際簡報中抽出的兩塊重算，30px→24px、36px→23px，並用無頭瀏覽器渲染整頁——兩塊都在原本的位置斷行，字級與旁邊沒動過的卡片一致。後端 tsc＋reactSlide 系列 67/67（新增 4）、前端 tsc＋959/959 | fix/extracted-text-line-breaks（已 merge 回 master） |
| 2026-08-15 | 修好使用者回報的「找出來的字都不見了，是不是 render 有問題」——是我上一輪改壞的：沙箱 runtime 寫在 template literal 裡，我在其中寫了單層逸出的換行，於是沙箱收到的是**字串中間真的斷行**，該字串提早結束、**整個 runtime script 語法錯誤**，結果什麼都不渲染（背景是父層畫的所以還在，看起來就像「投影片內容整個消失」）。這種錯誤的症狀是空白畫面而不是錯誤訊息，從截圖幾乎看不出來，而且**這是第二次**（上次是註解裡的反引號），所以補了一個測試直接用 `new Function` 去 parse runtime（只編譯、不執行）。驗證：用真實那一頁跑完整沙箱文件，五個元素全部回來、沒有 pageerror | fix/sandbox-runtime-escape（已 merge 回 master） |
| 2026-08-15 | 讓抽出來的文字連顏色、粗細都貼近原圖（使用者要求）：原本每一塊抽出來的字都是同一種通用深灰、而且一律 700 粗體，所以永遠比它取代的那張圖更重、顏色也不對。**顏色改成從裁切圖量出來**而不是問模型——像素就在手上、確切知道墨色是什麼，模型只是回報一個印象。作法：出現最多的顏色當背景，亮度離它最遠且數量足夠的當墨色，再往同方向以較低門檻找一次「筆芯」——文字有反鋸齒，介於墨色與背景之間的過渡色比筆畫本身還多，只做一輪會停在邊緣的中間色。取樣用最近鄰縮放，因為任何平滑 kernel 都會在筆畫邊緣製造出新的顏色（包括比墨色更深的 overshoot），那正好會被「離背景最遠」選中。區域裡沒有東西突出於背景時回 null，維持模型的答案。**字重**則改 prompt：明說預設是 400，只有筆畫明顯比內文更粗或明顯是標題／強調時才回 600/700，並明講不要因為圖片被放大、模糊或有反鋸齒就判成粗體。驗證：對已知墨色的合成圖精確還原（#db2060、#172638），空白圖回 null；並用實際簡報中抽過的四個區域比對（模型說 #172638 的那塊，取樣得到圖中真實存在的 #1c1d22）。後端 tsc＋reactSlideTextExtract 19/19（新增 2） | feat/extracted-text-color-weight（已 merge 回 master） |
| 2026-08-15 | 修好使用者回報的「剛才最後一個轉換，文字偏小」：字寬估計寫的是 CJK 佔 1.25em，實際**在沙箱自己的字型裡量出來是 1.00**（字重 700 也一樣），拉丁字平均 0.55——所以每一塊抽出來的字都被壓小約五分之一。1.25 是把一個 bug 誤判成另一個 bug 的產物：當時換行還會被折成空格，文字因此連成一長串、提早換行，看起來就像字比實際寬。換行修好之後，那個保守係數就只剩下「無故縮小」。保留 2% 餘裕，讓剛好等於區域寬度的一行不會因為捨入而換行。驗證：用該簡報真實的主題字型在 Chrome 裡實測字寬，再端到端確認——實際簡報上抽出的四塊字級變成 29/29/25/25px（模型原本說 30/36/27/26），渲染出來每一塊都在原本的位置斷行、字級與周圍卡片相稱。後端 tsc＋reactSlideTextExtract 19/19 | fix/extracted-text-size-measured（已 merge 回 master） |
| 2026-08-15 | 用 PP-OCRv6 做文字框偵測的可行性實測，並讓一行內的多種顏色各自成為獨立元素（使用者要求；先前的分析報告：專用偵測模型比 VLM 高 39.4 個百分點）。(1) **偵測實測**：用 `ppu-paddle-ocr`（PP-OCRv6、ONNX 跑在 Node，不需 Python）對實際投影片背景圖跑 `detect()`——每頁約 20 個框、300～500ms，框到**行級**而且很緊：標題兩行分開、每張卡片的每一行、圖表的軸標籤與註解框都各自成框，明顯優於使用者手動拖的大框。同時證實一件事：它也會框出圖表的座標軸數字，那些不該轉成文字，所以報告裡「一鍵框出全部、使用者逐一確認」是必要的，不能全自動。(2) **一行多色**：`sampleTextColorRuns` 逐欄取離背景最遠的像素當該欄墨色、相鄰同色合併成段；字間空白繼承所在段（否則一個空格就把詞組切兩半），段與段首尾相接鋪滿整行，比筆畫還細的段丟掉（那是兩色交界的反鋸齒）。(3) **每段各自成為元素**而不是共用元素裡的 `<span>`：元素才有 id，才能單獨選取、改樣式、刪除；span 只能改程式碼，而且會讓整塊因為「含 markup」被文字編輯拒絕。`splitTextByColorRuns` 用與字級計算同一套字寬把像素位置對回字串，捨入的餘數併進最後一段，不會掉字；多行文字不切（換行會重設 x，跨行無法對應）。驗證：合成的「深色／藍／深色」一行回來正好三段且藍色落在畫的位置、單色一段、空白零段；真實投影片上「Data fitting 與 linear regression」讀到 #1762d0。後端 tsc＋reactSlide 系列 73/73（新增 4）、前端 tsc | feat/text-color-runs + feat/color-runs-as-elements（已 merge 回 master） |
| 2026-08-15 | 接上自動框選（使用者要求）：抽文字原本每一段都要自己拖框，現在由 PP-OCRv6 的偵測模型（ONNX、不需 Python）找出來——實測一頁約 12 個框、約一秒，貼著每一行。刻意**只做偵測、不做辨識**：文字與樣式仍走既有的抽取端點、一次一塊，因為偵測會把**所有**文字都找出來（包括圖表座標軸標籤，那些屬於圖的一部分），必須由使用者挑；自動全轉的話，收拾要一個元素一個元素來。行框會先合併成段落（左緣相近、行高相近、行距小於一行），因為使用者要抽的單位是段落，而段落內的換行正是抽取端用 `<br />` 重現的東西；合併時**行高要另外記**——兩行一合併，框的高度就變成整段的高度，再拿它去比第三行必然不合。模型延遲載入並快取，失敗回 424 並說明「第一次使用需要連外，你仍然可以自己拉框」，與 audio.cpp 的安裝是同一種取捨。已知限制：合併規則在圖表區會過度合併（軸標籤垂直排列、左緣與大小都相近），使用者挑選時避開即可。後端 tsc＋reactSlide 系列 67/67（新增 3）、前端 tsc＋961/961 | feat/auto-detect-text-regions（已 merge 回 master） |
| 2026-08-15 | 把抽文字改成「先全部挑好、一次轉換」（使用者要求）：原本是拉一個框、轉一次、等一次，重複到做完。現在偵測會把整頁的文字框都找出來，使用者挑一組，**兩邊操作同一份選取**——編輯區的按鍵與畫在投影片上的框是同一個動作，點框等於點它的按鍵。幾個決定：偵測順便**辨識文字**，所以每個按鍵顯示前五個字（座標沒辦法讓人認出那是哪一塊）；**寬度太小的框預設不選**，投影片上那幾乎都是圖表座標軸與頁面裝飾，屬於圖的一部分——但那只是建議，每個框都能 toggle；**一次請求轉換全部**，因為每個框各自轉換的話會各自重新編譯、各自改寫檔案，十個框就是十次編譯、十次 commit、十次可能停在轉一半的狀態；改成逐框辨識、一起寫進 JSX、之後再逐框抹背景（一個抹除失敗不會拖累其他），背景等全部寫完才換上，部分失敗會如實回報 `partial` 而不是報成功。後端 tsc＋reactSlide 系列 45/45、前端 tsc＋961/961 | feat/batch-text-selection（已 merge 回 master） |
| 2026-08-16 | 把 React 編輯區重構成分頁式介面（使用者要求：每個區塊變成一個頁面，相關的放一起）：原本七個區段疊在同一個捲動欄裡——產生程式碼、換主題、換背景、抽文字、點選元素、看原始碼、烘焙圖片——每加一個功能就更長，想用的那個永遠在捲軸下面。改成五頁，順序照實際工作流程：**產生**、**外觀**（主題＋背景）、**文字**（偵測、挑選、批次轉換）、**元素**（點選模式與元素編輯器）、**程式碼**（原始碼、版本歷史、輸出圖片）。只動分組，每個區段都是原本那段程式碼搬到分頁底下；狀態沒有跟著切分，所以切到別頁再切回來，輸入的東西還在。驗證：前端 tsc＋961/961。註：Playwright 端到端只驗到「app 首頁載入正常、無 JS 錯誤」——這個部署的播放頁需要 Google OAuth，自動化過不了登入，分頁本身沒能在真實瀏覽器裡點過 | feat/react-tab-pages（已 merge 回 master） |
| 2026-08-16 | 互動 React 頁（`react-app`）＋ 外部 coding agent 迴圈的設計文件（使用者要求；只寫文件，未實作）：使用者要的是「一頁裡跑一個可互動的小程式」，而且架構上不在 MakeSlide 內建 coding agent，改由外部 agent（Claude Code）透過 MCP 送程式進來試跑、依結果修正。文件定調的核心是**執行在 client、驗證在 server**——互動頁跑在觀眾自己的瀏覽器裡，100 個觀眾就是 100 台機器各跑一個 iframe，伺服器只送出一份字串，所以「多人時的 scalability」在這個模型下不成立；伺服器端的 headless 瀏覽器因此不是執行環境，而是**存檔前的准入檢查**，而那也是無窮迴圈唯一擋得住的地方（CSP 管得住外連、管不住 `while(true)`，而 sandboxed iframe 與宿主是否同 process 不可依賴，必須假設一個寫壞的頁面會凍住觀眾整個分頁）。隔離維持既有的 opaque-origin iframe（另評估了獨立 sandbox 網域、Web Worker＋DOM 橋接、宣告式 DSL 三案，前兩者成本不成比例，第三案留作互補的 tier 1），並補上盤點時發現**全站完全沒有**的 CSP：`connect-src 'none'` 是重點，它讓「觀眾輸入資料按按鈕計算」這種頁面無法把輸入送到外部——sandbox 屬性擋的是讀取宿主，擋不住往外送。既有 regex deny list 降級為存檔時的 UX 提示（`window['fet'+'ch']` 繞得過 regex，繞不過 CSP）；`'unsafe-eval'` 必須保留（runtime 用 `new Function`），但 CSP 各 directive 獨立，不影響 connect-src。另盤點出互動真正的障礙不在安全而在三件既有設計：播放時 iframe 被寫死 `pointer-events: none`、進出全螢幕 remount 會清掉觀眾輸入的狀態（notebook 的單例＋reparenting 是先例，但 iframe reparenting 多半會 reload，需實測）、覆寫的 `MutationObserver` 會把互動改動還原且 `!important` 會殺死 CSS 動畫。分五階段落地，其中「harness ＋ 配額／併發」必須同批做——烘焙目前沒有任何 semaphore，沒有配額的 harness 本身就是 DoS 面 | docs/react-app-design（文件，已 merge 回 master）；TODO.md 更新於 master |
| 2026-08-16 | 修掉 ZIP 匯入挾帶程式碼的漏洞（查證互動 React 頁設計文件的待決事項時發現，與互動頁無關、是當時就存在的問題，因此獨立成一個分支先修）：匯出把 `.slide.jsx` 與 `.slide.js` 都打包、匯入把整個目錄原樣 `fs.cp` 複製，而讀取路徑**只在 compiled 缺失時**才重新編譯——於是一份手工構造的 ZIP 可以讓 `.slide.jsx` 看起來人畜無害、`.slide.js` 是完全不同的東西，而沙箱執行的正是後者；存檔時的 deny list 只對 `PUT` 進來的原始碼跑，完全碰不到這條路徑，而在 CSP 尚未補上之前那段程式碼還能自由外連（把觀眾看到與輸入的內容送出去）。改為匯入時一律從各頁自己的 `.slide.jsx` 重新編譯，走與正常存檔相同的 `validateAndCompileReactSlide`（長度上限、deny list、`window.SlideComponent` 契約檢查一次到位），因此匯入後在沙箱裡跑的東西，與這份原始碼在本站存檔會得到的結果完全一致；原始碼編不過的頁面則把過期的 `.slide.js` **刪除**而非留著——留著就是這個漏洞本身，而該頁仍保有原始碼（可修）與 JPG（畫面仍有東西可看），讀取路徑之後按需重編。`react_slide_path` 來自 ZIP，故一律走 `safeJoinPdfPath` 解析而非直接信任。另修掉 `react-slides.json` 沒被列入 `SIDECAR_FILES` 的疏漏（其餘 sidecar 都在，導致這個中繼檔被原樣複製進新簡報的儲存目錄）。新增 `reactSlideImportRecompile.test.ts` 3 個測試：竄改的 `.slide.js` 被原始碼取代、原始碼過不了 deny list 時不留下任何編譯檔、編不過的頁被拒且不存在的頁號只跳過不拋錯。驗證：後端 tsc；新測試 3/3；匯出匯入與 React 相關回歸（export-import-notebook／zip-interactive／zip-sources／react-slide-conversion／本檔）12/12，reactSlide＋reactSlideEdit 純函式 44/44，0 失敗。（附帶記錄：這幾個整合測試檔即使在未修改的 master 上也不會自然退出，`timeout` 收掉是既有行為，判讀時要看 TAP 的 ok 行而非 exit code） | fix/react-slide-import-recompile（已 merge 回 master） |
| 2026-08-16 | 互動 React 頁設計文件補上「原始碼的形狀」一節（使用者提問：MCP 傳進來的是編譯過的還是原始碼？程式都在一個大檔案裡對比較大的程式不方便，可以用目錄嗎）：(1) 釐清一個頁面在磁碟上是 `.slide.jsx`＋`.slide.js` 一對，`PUT` 的 body 只有 `code`／`config`、沒有 `compiled` 欄位，所以 agent 送的一律是原始碼、編譯只在後端做——這正是 agent 迴圈需要的形狀；但**存進去的原始碼不等於 agent 送出的原始碼**，`writeReactSlideForPage` 會先跑 `ensureElementIds` 補上 `data-ms-id` 再重編，`set` 後再 `get` 會多出一批 id 屬性，這必須寫進 `get_react_slide_contract()`，否則 agent 會以為自己的程式碼被竄改。(2) 新增多檔目錄模式 `pages/<uid>.slide/`：與單檔互斥、不需要新 DB 欄位，匯出／匯入／版控天然支援；關鍵性質是 **bundle 只是編譯期的事、執行期完全不變**，沙箱與烘焙拿到的仍是單一 IIFE 字串。編譯改用 esbuild `build({bundle:true})`，但整個目錄先讀進記憶體、plugin 攔截 resolve/load 只在 map 裡查，**路徑穿越因此不是「被規則擋下」而是根本不存在**（與 CSP 同一個思路：限制環境而非檢查字串）；deny list 只放寬到相對路徑 import／export，且改成對 bundle 後的結果跑。多檔只開放給 `react-app`——`ensureElementIds` 與元素編輯靠單一字串與 id 定位，而 `react-app` 本來就關閉覆寫，這個問題在它身上不存在，多檔因此不必先解一個它不需要解的題。(3) §9 匯入那列由「待查證」改為「已修正」並補上與 CSP 互為縱深的說明，§12 待決事項移除已解決項、新增「多檔是否放行 .css/.json」。章節重新編號（原 §7 起後移一節）並核對全部交叉引用 | docs/react-app-design（文件，已 merge 回 master）；TODO.md 更新於 master |
| 2026-08-30 | （使用者回報）簡報 Thy4vQfYRC 只產出一頁，追查後確認不是 PDF 解析的問題——`extract_figures` 記到 11 頁、抽出 8 張圖，`source.txt` 54KB 完整帶著 `[[PDF_PAGE_1..11]]` 標記；壞在 `split_text`，該階段 0ms 就回 `pages:1`，代表根本沒走到 LLM 大綱、而是命中了 `splitBySlideMarkers()` 的「原文已含 Slide 標記」捷徑。原因是 marker regex 把頁碼與分隔符都設成可選，於是 `slide` 後面只要接一個連字號就算標記，論文內文那行 `slide-level labels are available for this dataset.` （這篇正是 whole slide image 的病理切片論文，全文滿是 slide）被當成整份文件唯一的標記。後果有兩個，第二個更嚴重：切出的頁數塌成 1，而且切分只從第一個標記往後取，該行之前的 526 行（論文前 60%，含標題、摘要、方法）被無聲丟棄。修法是把單一寬鬆 regex 拆成兩條明確的：有編號的 `Slide 1` / `## Slide 1: 標題` / `Slide 2 - 標題`，以及無編號的 `#Slide: 標題`——後者現在強制要有冒號，連字號不再算數。另加兩道保護把誤判從資料遺失降級成 fallback：第一個標記之前被丟棄的前言同時超過 200 字與全文 20% 時，以及只有單一標記卻切出超過 3000 字的頁時，一律回傳 `[]`，讓呼叫端自然落回 LLM 大綱流程。新增 6 個回歸測試（合法變體仍可辨識、英文散文不再誤判、兩道保護各自生效、短前言不受影響、以及端到端確認這類論文會走大綱流程且前段內容存活）。驗證：後端 tsc 全綠；split-text-with-llm 11/11，addPagesOutline 7/7、page-split-endpoint 4/4、pageSplit 6/6、prompt-text-limit 4/4、prompt-language 3/3、upload-slide 4/4、subtitleSplitConsistency 2/2，合計 41/41、0 失敗 | fix/slide-marker-false-positive |
| 2026-08-30 | （使用者回報）簡報 Iep9jo9kCM 產出 319 頁。查 LLM 請求記錄發現真正想要的大綱其實產生過兩次卻都被丟掉：`pdf-fulltext-outline` 兩次都 `finish_reason: stop`、JSON 完整、內容良好，分別是 23 張與 22 張，卻雙雙被 `OutlineSchema` 的 `.max(20)` 擋下。關鍵矛盾是這個上限從來沒寫進提示詞——system prompt 只講「每頁 2~6 點重點」，沒有任何張數約束，模型沒有理由猜到 20 這個數字，而 39 頁的 ICLR 論文規劃 22~23 張本來就合理。驗證失敗後 `buildOutlineFromFullText` 回 null，掉進 Strategy 3 的 chunk 流程，把 109,874 字元以 1800 字元切成 63 個 chunk、每個獨立展開約 5 頁，且全程沒有任何頁數上限，於是滾成 319 頁（代價是兩次白費的大綱呼叫 74k tokens／91 秒，加上 12 分鐘的 chunk 切分）。修法是讓張數由原文份量推導而不是用 schema 硬猜：新增 `splitByPdfPageMarkers()`（依 `[[PDF_PAGE_N]]` 切出每頁），以及 `estimateSourcePageCount()`（優先數標記，沒有標記才用字元密度換算）與 `slideCountRangeForSourcePages()`（每頁原文約 0.5~1 張投影片，clamp 到 3~60）。算出的範圍寫進 system 與 user 兩邊的提示詞，模型終於知道自己被什麼標準衡量；`maxTokens` 改成隨範圍放大（原本固定 6400，實測 23 張就用掉 5571，39 張必爆），schema 的上限只留作失控保護。chunk 路徑同樣處理：chunk 從 1800 放大到 12000 字元、沿頁邊界切（每個 chunk 都是完整整數頁），每個 chunk 再依自己涵蓋的頁數算出範圍——同一份文件從 63 個 chunk／319 頁降到 10 個 chunk／30~50 張。新增 9 個測試（範圍換算與 clamp 邊界、兩種頁數估算、提示詞確實帶上範圍與放大的 maxTokens、22~23 張大綱不再被拒且不落入 chunk、chunk 沿頁邊界切且各自帶範圍，以及 `splitByPdfPageMarkers` 的 3 個）。驗證：後端 tsc 全綠；split-text-with-llm 10/10、pdfPageMarkers 9/9、pdf-page-markers 6/6、addPagesOutline 7/7、page-split-endpoint 4/4、pageSplit 6/6、prompt-text-limit 4/4、prompt-language 3/3、upload-slide 4/4、subtitleSplitConsistency 2/2、page-chat-tools 5/5，合計 60/60、0 失敗 | feat/outline-slide-count-range |
| 2026-08-31 | （使用者回報）簡報 wSLE3trkxO 只產生三頁、且以 `400 Invalid 'prompt': string too long`（上限 32000、實得 51005）整份失敗。查下來不是新問題，是同一份 whole slide imaging 論文第二次踩到 slide marker 誤判：`split_text` 只花 2ms 就回 `pages:3`，代表又走了不呼叫 LLM 的 marker 捷徑。三頁的起始行全部來自參考文獻——`... for whole slide image classification ...` 在 whole 之後換行，下一行就以 `slide` 開頭，而舊 regex 連分隔符都是可選的，`slide` 後面接任何東西都算標記。三頁字元數 8,359／1,731／51,547 極度不均，最後一頁吞下剩餘全文，再被整段送進圖片生成 prompt 而爆掉。使用者指出更根本的解法：選「一般文件 AI 分頁」時本來就該跳過文字 slide 分頁、直接用 LLM 分頁。因此在收緊 regex（見同分支前一個 commit）之外，新增 `SplitTextOptions.allowSlideMarkers`，由 pipeline 對 document-mode PDF 傳 false——判斷條件是 source.pdf 與 source.txt 並存，worker 本來就用這個區分模式，不需要新欄位。一般文件是連續散文，任何長得像標記的行都是內文而非分頁指示；使用者自己手寫、以 .txt 匯入的 `Slide 1:` 大綱則保留捷徑，照樣不呼叫 LLM。`split_text` 的 stage metadata 補上 `document_mode` 方便日後判讀。新增 2 個測試（一般文件跳過捷徑並交給 LLM、手寫大綱仍照標記切且零次 LLM 呼叫）。驗證：後端 tsc 全綠；split-text-with-llm 13/13、pageSplit 6/6、page-split-endpoint 4/4、addPagesOutline 7/7、pdfPageMarkers 6/6、upload-slide 4/4、prompt-text-limit 4/4，合計 44/44、0 失敗。附帶記錄：`buildImagePrompt()` 對 pageText／pageScript 沒有任何長度上限，只要有一頁異常長就會讓整份簡報 failed（前兩頁其實已產圖成功），這是尚未處理的獨立缺陷 | fix/slide-marker-false-positive |
| 2026-08-31 | （使用者回報）圖片生成跳出 `400 Invalid 'prompt': string too long`（上限 32000、實得 51005）。這是分頁誤判的下游後果，但本身是個獨立缺陷：`buildImagePrompt()` 把每個欄位原樣塞進 prompt，沒有任何長度上限，所以只要有一頁文字異常長就會讓**整份**簡報 failed——實際情況是前兩頁的圖都已經產好了，卻被第三頁拖著一起死。修法是在 `buildImagePrompt()` 內設兩層上限：每個引用區塊（pageText／pageScript／figureNotes／textBody／使用者與整份調整需求）各截到 6000 字元，組完後整串再截到 `IMAGE_PROMPT_MAX_CHARS`（28000，替呼叫端事後附加的 `[Context]` 留餘裕）。被截掉的一定是引用材料，通用規則與語言規則不會被動到，截斷處補上「（內容過長，其餘已省略）」讓模型知道原文還沒完。6000 比一頁投影片正常的一兩百字高兩個數量級，正常簡報完全不受影響；異常頁變成畫得比較泛泛，而不是把其他頁一起拖下水。防護寫在函式內，pipeline／regenerate／page-operations／pageEditProposals 四個呼叫端一次覆蓋。新增 3 個測試（超長 pageText 被截斷且規則存活、每個區塊都超長時整串仍有界、正常長度不被動到）。驗證：後端 tsc 全綠；imagePromptTemplates 12/12、image-prompt-templates 2/2 等 14 個相關測試檔合計 84/84、0 失敗 | fix/image-prompt-length-guard |
| 2026-08-31 | 把上述三個分支整合並 merge 回 master，再同步到 worktree/demo16。整合順序：先在 worktree 上把 `feat/outline-slide-count-range` merge 進 `fix/image-prompt-length-guard`（唯一衝突是兩邊都往 `split-text-with-llm.test.ts` 檔尾追加測試，解法是兩邊全留；原始碼因為改的是 `splitTextWithLlm.ts` 的不同區域而自動合併），在 worktree 上跑完測試確認無誤後，master 才做一次無衝突的 merge（`4369eba3`），最後 demo16 merge master（`cf031667`）。現在 `git diff master worktree/demo16` 為空。驗證：整合後 tsc 全綠、相關測試 84/84，demo16 上再驗 60/60，0 失敗。**待處理**：`b5645e2c` 意外挾帶了工作區既有的 package-lock.json 變更（368 行，新增 transitive 依賴 `@techstark/opencv-js`），已隨 merge 進入 master；package.json 未變動，需確認這份 lock 漂移是否該保留 | master（4369eba3）／worktree/demo16（cf031667） |
| 2026-08-31 | （使用者需求）把每份簡報的分享狀況顯示在列表中。原本列表完全看不出「誰看得到這份簡報」——`visibility` 只以 hover 才浮現的複製連結按鈕間接透露，而 `pdf_shares` 的分享連結與 `pdf_permissions` 的個別授權要點進簡報才知道。後端在 `GET /api/pdfs` 每筆加四個數字：有效分享連結、已過期連結、被個別授權的使用者數、群組數，由新的 `getPdfShareSummaries()` 以兩個 GROUP BY 一次撈完（`pdf_shares` 與 `pdf_permissions` 都是小表，逐筆查會變成 N+1）。過期判斷沿用 share token 檢查的同一條比較，所以算成「有效」的連結就是真的還打得開的。關鍵設計：這四個欄位**只填給擁有者**——被授權讀取的人不該知道這份簡報還分享給了誰；因此 `undefined` 是「不是我的簡報」、`0` 才是「確實沒分享」，兩者在 UI 上意義不同。前端 grid 與 list 兩種檢視都接上共用的 `ShareStatusBadges`，顯示 🌐 公開／🔗 連結數／👤 人數／👥 群組數／⌛ 已過期；判斷邏輯抽成純函式 `shareStatusBadges.ts`（照 `pdfCardCover.ts` 的既有模式）方便測試，元件只負責把 kind 對到圖示、顏色與 i18n key。只有過期連結的簡報視為未分享，但仍顯示過期徽章讓擁有者知道連結為何失效；list 檢視多顯示「未分享」灰徽章，卡片檢視較擠則留白。zh-TW 與 en 各補 14 個字串。新增 11 個測試（後端 5：計數含過期／未分享不入 Map／擁有者拿得到／未分享回 0 而非 undefined／非擁有者拿不到；前端 6：非自己的簡報不渲染、未分享的兩種模式、public 與 public_editable 之分、連結/人/群組的順序、過期排最後、只有過期連結的情況）。驗證：前後端 tsc 全綠；後端 pdf-list-share-status 5/5、pdf-list-owner 6/6、share 6/6、share-expiry 3/3、pdf-permissions-api 6/6、permissions 6/6、detail-permission 92/92，合計 124/124；前端完整測試 1065/1065，0 失敗 | feat/list-share-status |
| 2026-09-03 | （使用者回報）全螢幕時投票與頁面評論的圖示會重疊。頂端同一條水平線上原本有三組各自 `absolute` 釘死座標的疊層（中央徽章列 `left-1/2 top-4`、投票鈕與版面工具列 `right-4 top-4`、follower 提問鈕 `left-4 top-4`），沒有任何機制讓它們互相讓開，視窗一窄或工具列一長右側那組就壓到中央徽章上——會不會重疊只看當下視窗寬度。改成把三組收進同一個 `absolute inset-x-0 top-0` 的三欄格線 `grid-cols-[1fr_auto_1fr]`（左：提問；中：🗳／📝／💬 指示徽章；右：投票鈕＋版面工具列＋離開），欄與欄只會互相推擠而不會互相覆蓋，任何寬度都成立；容器維持 `pointer-events-none` 以免擋住底下全高的上一頁／下一頁點擊區與播放／暫停切換，互動元素各自 `pointer-events-auto`。新增 `fullscreenTopBar.test.ts` 3 項原始碼層級守門斷言（比照 `floatingLayerOrder.test.ts`，因為沒有渲染測試環境、幾何重疊又隨視窗寬度而變）。驗證：`tsc --noEmit`、`vite build`、前端全套 1068/1068 通過 | fix/fullscreen-top-overlay-collision |
| 2026-09-03 | 把 `fix/fullscreen-top-overlay-collision`（全螢幕頂端三欄格線）以 `--no-ff` merge 回 master，再把 `worktree/demo16` fast-forward 到同一個 commit（該 worktree 無自己的提交，故無需解衝突） | master／worktree/demo16 |
...
| 2026-09-05 | （使用者要求，承上）剪下區域的動畫改由 AI 決定時機與位置：視覺 LLM 看抹除後的頁面、每塊剪下圖與逐字稿句子，逐塊挑出對應句子（效果改用 `startTrigger: transcript-line, anchor: start`，該句開始時淡入）與最合適的顯示位置（高度依圖片長寬比、框推回頁內；原位優先，明顯更好才搬）。沒有 LLM 或模型失敗退回原位＋依序出現。placer 注入、測試以假 placer 驗證；面板逐塊顯示所選句子與位置。後端 6/6、前端 1119/1119。以 `--no-ff` merge 回 master 並 fast-forward `worktree/demo16`，並重新建置 demo16 的前端靜態檔 | feat/cutout-placement → master／worktree/demo16 |
| 2026-09-05 | （使用者要求，承上）剪下區域可自動偵測：像素分析（背景色估計→色距遮罩→膨脹→連通元件→合併→尺寸過濾）找候選框，有 LLM 時再由視覺模型把編號框分組成單位、剔除裝飾、寫說明——模型只回編號不回座標。結果填進區域清單並開啟框選模式供確認，說明成為素材 caption。後端 5/5、前端 1120/1120。以 `--no-ff` merge 回 master、fast-forward `worktree/demo16` 並重建其前端 | feat/cutout-detect → master／worktree/demo16 |
| 2026-09-05 | （使用者回報）demo16 的 `vite build --watch` 以 `ENOSPC: System limit for number of file watchers reached` 死掉（watch 到 `node_modules/katex/dist/fonts/*.woff`）。查證：機器 `fs.inotify.max_user_watches` 是 65536、已用約 65000——最大佔用者是 VS Code server（約 47000）與一個 Chrome/Electron 的 Node 服務（約 15000），vite 只是受害者；而 rollup watch 每次重建先清空 `dist/`，中途死掉就留下沒有 `index.html` 的 dist，正是稍早「Route not found」的原因。修法（[vite.config.ts](frontend/vite.config.ts)）：一個 `configResolved` 外掛在 watch 模式設定 `build.watch.exclude` 與 `emptyOutDir: false`——必須在 hook 裡做，因為 CLI 的 `--watch` 進來是 `build.watch: true`，會整個蓋掉設定檔裡寫的物件；排除用 RegExp 而非 glob，因為 Vite 登錄 CSS `url()` 素材（KaTeX 字型）用的是相對根目錄的 `../node_modules/...`，`**` glob 不吃開頭的 `..`（picomatch 視為 dot segment），glob 版實測仍會去 watch 字型。實測 watcher 從撞上上限變成只用約 400 個 watch。系統面仍建議提高上限：`sudo sysctl -w fs.inotify.max_user_watches=524288` 並寫入 `/etc/sysctl.d/99-inotify.conf` | fix/vite-watch-inotify → master／worktree/demo16 |
| 2026-09-05 | （使用者回報）`TNQ62wZM_z` 第 9–12 頁自動偵測的區域「都選太大了」（只有第 10 頁還行）。查證：第一版只有「膨脹＋合併」，卡片式密排版面整頁黏成一塊——第 11 頁一框 94×77%、第 12 頁一框 93×75%、第 9 頁因超過 85% 被整個丟掉、回 0 個。改為**遞迴 XY-cut**：大塊在內部最寬的空白帶切開（塊內重新估背景色，卡片淺底當背景；橫向帶容許細箭頭穿過、直向帶須全空；4–12% 的小塊只在寬帶才切，免得條列被切成行；墨密的照片不切），再把緊貼大塊、寬度對得上的薄條（座標軸標籤、圖說）吸回去。實測四頁各得 8–13 個貼合卡片與圖塊的框；第 9 頁從 0 個變 10 個。測試 7/7（新增合成卡片版面與吸附兩項）。demo16 的後端以 tsx watch 執行，同步後自動重載 | fix/cutout-detect-xycut → master／worktree/demo16 |
| 2026-09-05 | （使用者要求）重生對話框新增「✂️ 自動剪下區域並產生動畫」：重生工作多一個 `cutout` 步驤（排最後），逐頁偵測→裁下→抹除→依講稿定時定位；快照含圖片與動畫規格可回復；逐頁容錯。三個模型協作者集中於 `cutoutDeps.ts`；worker 以動態 import 避開 `page-operations → regenerate → pageCutouts → reactSlide` 的循環引用。後端新測試 2/2、既有重生測試 17/17；前端 1120/1120。以 `--no-ff` merge 回 master、fast-forward `worktree/demo16` 並重建其前端 | feat/regenerate-cutouts → master／worktree/demo16 |
...
| 2026-09-06 | （使用者要求，承上）動畫摘要列：拿掉編號、位置迷你圖放大到 72×40，插入圖片效果直接顯示圖片本身（72×44）取代迷你圖，位置改放滑過提示。守門測試補 2 項；前端 `tsc`＋`vite build` 通過。merge 回 master、fast-forward `worktree/demo16` 並重建其前端 | feat/animation-summary-thumbs → master／worktree/demo16 |
| 2026-09-06 | （使用者要求，承上）動畫摘要列：hover 不再顯示位置文字，改彈出預覽——插入圖片顯示圖片本身、其他效果顯示標示框底下的投影片切圖（CSS 從頁面圖裁出，有剪下的頁用未剪縮圖）；清單改依解析後的開始時間排序（同時間依規格順序），移除 ▲▼ 手動排序。純函式 `cropStyleForBox` 1 項測試、守門 1 項；前端 1137/1137。過程中曾因腳本誤把 `AnimationEditorTab.tsx` 清空，已從 git 還原後重做，未進入任何 commit | feat/animation-summary-hover-sort → master／worktree/demo16 |
| 2026-09-06 | （使用者回報）插入圖片 hover 的預覽太小：彈出寬度從 320px 改為 `min(640px, 60vw)`、圖片最高 70vh，切圖預覽同寬。已確認 Tailwind 有產出該任意值樣式。merge 回 master、fast-forward `worktree/demo16` 並重建其前端 | fix/animation-hover-preview-size → master／worktree/demo16 |
...
| 2026-09-09 | （使用者要求）MCP 也能改頁面型別與 React 頁的程式碼。使用者問「有沒有功能可以改頁面類別、並在是 react 時直接改其中的 script」，查出 UI 與 REST API 都有、**MCP 一個都沒有**：53 個工具裡 notebook 方向齊全，React 方向整個缺席。新增四個工具（`get_page_react_slide`／`set_page_react_slide`／`generate_page_react_slide`／`convert_react_page_to_slide`），全部建立在 React 分頁本來就在用的 REST 端點上（那條路上有權限檢查與烘焙守門，繞過去等於關掉它們），ChatGPT 的 HTTP 傳輸沿用同一份 `TOOLS` 因此一起有了。兩件不講清楚 agent 就會做錯的事寫進了工具回應：從沒當過 React 頁的頁面也讀得到程式碼（後端回骨架），所以回應開頭必須講出真正的型別；轉回圖片時的烘焙就是把加上去的東西變成像素的那一步，失敗不吞掉，`force` 的描述直接寫明後果是退回舊圖。程式碼契約（`window.SlideComponent`、不可 import／export、60000 字）在送出前先擋一次——那一行的缺漏從編譯錯誤看不出來。長度上限是複製而非 import（`mcp-server.ts` 必須維持零依賴可單檔執行），另加守門測試比對後端實際值。文件補上「React 投影片頁面」一節並把過時的「47 個工具」更正為 53。測試 14 條，13 條先確認改動前會失敗；進入 React 模式需要瀏覽器而測試環境停用烘焙，該條依環境斷言正確行為並**兩個分支都實跑過**。後端 tsc 全綠、MCP 相關 133/133、完整套件 2171 項 26 失敗且與 master 基線逐字相同。已以 `--no-ff` merge 回 master。線上要等下次重啟 `start.sh` 才會有這四個工具 | feat/mcp-react-slide-tools → master |
| 2026-09-12 | （使用者要求）把 OpenAI 圖片生成模型從 `gpt-image-2` 換成 `gpt-image-2.5`。實際生效的是 `.env` 的 `OPENAI_IMAGE_MODEL`（它會蓋過 config 預設值），所以三處一起改：`backend/src/config.ts` 的 zod 預設值、`.env.example`，以及本機不進版控的 `.env`；`aiSettings.ts` 裡舉例用的模型名稱註解也一併更新。cgu-air／openrouter 若有設定自己的圖片模型名稱則不受影響，沒設定時會跟著改用新名稱。沒有動 `images.edit` 的尺寸（1536x1024）等參數。後端 tsc 全綠。線上要等下次重啟 `start.sh` 才會用新模型 | chore/gpt-image-2.5 → master |
| 2026-09-12 | （使用者要求）用單元測試實際驗證圖片模型，結果**推翻了上一筆**：OpenAI 回 `400 The model 'gpt-image-2.5' does not exist.`，重啟後產圖與 inpaint 會全部失敗。新增 `backend/test/live-image-model.test.ts`，會真的打 API 並計費，所以預設略過，要設 `MAKESLIDE_LIVE_IMAGE_TEST=1` 才跑；`MAKESLIDE_LIVE_IMAGE_ACCOUNT` 指定用哪個帳號的 key 與路由（和產投影片一樣走 `getImageClient`），`MAKESLIDE_LIVE_IMAGE_PROVIDER=openai` 則可繞過路由直接打 OpenAI。測兩件事：`images.generate` 1536x1024，以及帶 mask 的 `images.edit`（inpaint 走的路徑），兩者都驗證回傳圖片的尺寸。查過程還發現：帳號 111891044144240617135 的 LLM provider 是 cgu-air，所以圖片也送 CGU Air，但那裡回 `401 Invalid API key`——**這跟模型無關，是那把 CGU Air key 本身無效**，這次沒有處理。直接打 OpenAI 時，對照組 `gpt-image-2` 兩項通過，證明測試與 key 都沒問題；模型清單裡只有 `gpt-image-2.5-flare`／`gpt-image-2.5-sunburst` 兩個變體，實測兩者都通過（flare 約 9 秒、sunburst 約 12–14 秒），使用者選了 **flare**。`config.ts` 預設值、`.env.example`、本機 `.env`、`aiSettings.ts` 註解都改成 `gpt-image-2.5-flare`，改完不設覆寫變數再跑一次，兩項通過；後端 tsc 全綠 | test/live-gpt-image-2.5 → master |
| 2026-09-12 | （使用者要求）試 CGU Air 能不能用新的圖片模型，結果發現**上一筆的改動會讓所有 CGU Air 帳號無法產圖**：CGU Air 帳號沒填 `CGU_AIR_IMAGE_MODEL` 時沿用 `OPENAI_IMAGE_MODEL`，而 CGU Air 的 `/models` 最新只到 `gpt-image-2`，送 `gpt-image-2.5-flare` 會得到 `500 OpenAI pricing is not configured for model`。這台機器上三個真實帳號都用 cgu-air、而且都沒填圖片模型，重啟後會一起壞掉。修法（使用者選的）：新增 `CGU_AIR_DEFAULT_IMAGE_MODEL = 'gpt-image-2'`，`providerImageModel` 對 cgu-air 改成用這個值當後備，不再沿用 OpenAI 的模型名稱；OpenRouter 的後備規則不變。設定頁 CGU Air 圖片模型欄位的 placeholder／說明（中英文）改成「留空則用 gpt-image-2」。`image-client-provider.test.ts` 的後備案例改成斷言 `gpt-image-2`（修改前會拿到 `gpt-image-2.5-flare`），7/7 通過；前後端 tsc 全綠。實測（使用者同意用帳號 112007433750624192225 的 CGU Air key）：產圖與 inpaint 都通過，路由到 `provider=cgu-air model=gpt-image-2`。另外兩件事：帳號 111891044144240617135 的 CGU Air key 無效（`/models`、chat 都 401），這次沒處理；該帳號的 `LLM_PROVIDER` 在測試期間被改成 `openai`（不是這次工作改的） | fix/cgu-air-default-image-model → master |
| 2026-09-12 | （使用者要求）AI 導師加一個勾選框，讓學生可以要求使用教材以外的資訊。原本 system prompt 有一條【禁止杜撰】寫死「嚴禁引入教材以外的知識」，所以單加一個欄位沒用，要換掉的是那條規則本身。做法：把那段抽成 `routes/pdfs/askKnowledgeScope.ts` 的 `askKnowledgeScopeInstruction()`，接在原本的位置。**不勾選**時回傳原文，已比對與 master 逐字相同（139 字）；**勾選**時改成放寬版：以教材為主、教材沒涵蓋才用一般知識補充，教材外的內容必須逐處標示「（教材外補充）」，與教材說法不同時要指出差異，仍不准編造數據／引用／文獻／網址。放寬的是資訊來源，不是准許亂講。導師本來就沒有上網工具，所以「教材以外」指的是模型自己的知識。API 多一個選填欄位 `allowOutsideKnowledge: boolean`，沒勾時前端不送，request 與改動前相同。前端在「回答長度」下方加勾選框，預設不勾、每次開頁重設（與回答長度一致，不存 localStorage），生成中停用，滑鼠停留有說明，中英文字串都有。測試：後端 page-ask 系列 17/17（新增 3：勾選時換規則且保留引用／長度規則、`false` 與省略時 prompt 完全相同、非 boolean 回 400）；前端新增 `api.page-ask.test.ts` 檢查 request body（`askPageQuestion` 在位置參數中間插了新參數，最容易悄悄接錯）；前後端 tsc 全綠。實際打模型（gpt-5.6）問教材沒提到的「C4 與 CAM 植物差異」：不勾選時明確說教材沒涵蓋；勾選時用一般知識回答並逐段標示教材外補充。畫面沒有實際開瀏覽器看過 | feat/tutor-allow-outside-knowledge → master |
| 2026-09-12 | （使用者要求）加入「清除圖片」，用來強迫 AI 重新生圖、忽略原圖。關鍵不是加一個按鈕，而是既有的重生本質上是對現有圖片做 `images.edit`，設計上就會保留原本的版型——想重畫時這正好是錯的行為，而且靠提示詞叫模型別看它並不可靠。查出兩條重生路徑（`services/pageEditProposals.ts` 的單頁重生、`worker/regenerate.ts` 的整份重生）原本就有「底圖不存在就退回純文字生圖」的後備，所以把圖片刪掉本身就是「忽略原圖」的開關，不必另外加參數分岔。新增 `POST /api/pdfs/:id/pages/:n/clear-image`：刪掉頁面圖、縮圖、底圖（第 1 頁連封面與封面縮圖），`image_path` 設為 NULL（詳細 API 就會回報這頁沒有圖），使 cutout 歷史失效，並 commit 刪除（`git add` 對已刪檔案會記錄刪除，舊圖仍留在版本歷史）。刪之前先把原圖複製成該頁的候選圖，回傳 `candidate_image_url`，前端把它貼回聊天區，於是「救回原圖」就是既有的套用候選圖那一鍵（使用者選的做法）。刻意保留 `elements.json`：那是使用者自己加的元素、不是 AI 畫的，下一張圖會重新合成在它底下。React 頁面回 409——那裡的 JPG 是程式碼的烘焙結果，下次存檔就會重生，清掉毫無意義。清除後不自動重生（使用者選的），讓人可以先改提示詞。UI：問答區「修改圖片」旁新增「清除圖片」，只在該頁真的有圖時出現，按下先跳確認。另外修掉一個順帶的謊話：頁面沒有圖時畫面一律顯示「圖片產生中…」，但簡報已完成時根本沒有東西在跑，改成顯示「這一頁目前沒有圖片」。測試：後端新增 `clear-page-image.test.ts` 6 條（檔案與封面真的被刪、候選圖逐位元等於原圖且可經既有路由取回、**清除後重生從 edit 變成 generate**、重複清除回報沒有東西可清、React 頁 409 且不動任何檔案、未知頁 404）；前端新增 `clearImageWiring.test.ts` 4 條（確認框、reloadDetail、候選圖貼回聊天、按鈕出現條件、佔位文字、兩種語言字串齊全）。相關後端測試 30+28 全過、前端 1161 全過、前後端 tsc 全綠。畫面沒有實際開瀏覽器看過；MCP 尚未提供對應工具 | feat/clear-page-image → master |
| 2026-09-12 | （使用者要求）查「重生的圖為什麼常常和原圖不同風格、而且偏暗」，查出三個各自獨立的原因並分三個分支修掉。**(1) 暗色系的真兇在前端**：`useImageStyle.ts` 與 `PlayPage.tsx` 把簡報風格的初始值寫死成「簡潔商業風格，**以深色系為主**，文字清晰對比，版面留白充足」，而這個值每次重生都會以「整份圖片風格（固定套用）」送出，只有簡報自己存過風格時才會被覆蓋——資料庫裡 132 份簡報有 122 份（92%）沒存過。初始生成在沒設定時用的是後端的 `IMAGE_PROMPT_TEMPLATES[0]`（academic minimalist，soft neutral background，淺色），於是「初始淺、重生暗」。改成預設空字串，且沒設定時整段風格區塊不送（原本會送「(無)」，仍是模型會讀的一行），讓後端套用與初始生成相同的預設。單頁與整份重生兩條路徑都改。**(2) 風格漂移**：`pageEditProposals.proposePageImageEdit` 的 `stylePrompt` 寫死 `IMAGE_PROMPT_TEMPLATES[0]`，從不讀該簡報的 `image_style_prompt`，但初始生成與整份重生都讀——於是有設定風格的簡報，提示詞裡會同時出現「academic minimalist」和簡報自己的風格互打；而 MCP 工具與 AI 導師改圖提案這種沒有播放頁在後面的呼叫，根本拿不到簡報風格。抽出共用的 `deckImageStylePrompt()`，三條路徑都改用它。**(3) 順帶發現版本歷史在這台機器上根本沒運作**：`ensurePresentationRepo` 用 `git init -b main`，而 `-b` 要 git 2.28 以上，這台是 2.25，整個 init 直接拋錯；因為這裡的錯誤刻意設計成非致命，結果是「什麼都沒發生」——130 份簡報只有 2 份有 `.git`，圖片/逐字稿版本歷史、還原、GitHub 同步全部等於不存在。改成 `git init` + `git symbolic-ref HEAD refs/heads/main`，失敗改記 error（症狀是沉默，warn 沒人看得到），並修掉 `presentation-git-merge-conflict.test.ts` 自己 setup 裡同樣的旗標（那條測試因此從既有失敗變成通過）。驗證：新增前端 `imageStyleDefault.test.ts` 3 條、後端 `regenerate-uses-deck-style.test.ts` 3 條、`presentation-git-init.test.ts` 2 條，全部先確認改動前會失敗；前端 1168 全過，後端圖片相關 20/20、git 相關 45/45（另 3 條 react-slide-conversion 失敗在 master 上同樣失敗，屬既有基線）；前後端 tsc 全綠。查證過程中我誤刪了 `storage/dyp31H5T0t/pages/009.candidate.{neHNx1DXEF,SC-bseFPn_}.jpg` 兩個不是我產生的候選圖（清暫存檔時用「20 分鐘內修改」當條件），已告知使用者；因為第 (3) 點，那兩個檔案沒有版本歷史可還原 | fix/image-style-dark-default＋fix/regen-uses-deck-style＋fix/git-init-old-git → master |
| 2026-09-12 | （使用者要求）讓 AI agent 透過 MCP 把**帶動畫的 pptx** 搬進 makeslide：靜態頁變圖片頁、動畫頁變成可用上下鍵逐步播放的 React 頁，每一步配自己的旁白語音。設計寫在 `docs/pptx-animated-import-design.md`，分七期各自一個分支。**關鍵發現**：pptx 的動畫在 OOXML 裡是 `p:timing` 下的一串 click step，每步的 `p:spTgt/@spid` 指名這一下要讓哪些 shape 出現或消失——所以「第 k 步的畫面」＝把後面才出現的 shape 從 slide XML 切掉之後，交給 LibreOffice 算出來的那張圖，**還原度是原檔等級，完全不用 AI 猜版面**，AI 只負責它真正擅長的事：寫旁白。shape 的切除是位元組手術而非 DOM 重寫（namespace、屬性順序、`r:embed`、`p:extLst` 全部原封不動，任何差異都會變成畫面差異）。(1) `feat/pptx-parse`：自寫 XML 掃描器＋解析投影片順序／click step／文字（12 測試，含真檔 26 頁／17 動畫頁／136 frame）。(2) `feat/pptx-frames`：LibreOffice 批次轉檔＋pdftoppm 出圖（3 測試，真的跑 LibreOffice，驗證每一步畫的比前一步多）；最後一步與靜態頁共用原檔的單次轉檔，136 frame 只需 110 次變體轉換。(3) `feat/react-slide-steps`：新增 `<uid>.steps.json` 與沙箱的步驟顯示機制——React 投影片契約要求元件是靜態的（重繪會洗掉使用者的文字覆寫），所以**步驟不能用 React state**，改由沙箱執行環境依 `data-ms-step-layer` 屬性顯示／隱藏，步驟經 postMessage 傳入，和主題、覆寫走同一條路（後端 8＋前端 2 測試）。(4) `feat/step-playback`：上下鍵逐步（全螢幕與一般模式都可用）、每步播自己的語音、播完自動進下一步；暫停時只換畫面不出聲；沒有旁白的步驟是靜音而不是回頭播整頁（8 測試）。(5) `feat/pptx-import`：`POST /api/pdfs/from-pptx` 立刻回傳並在背景轉檔，含狀態端點；每頁仍保留一張完成畫面的 JPG，讓縮圖／匯出／沙箱失敗時的退路都不必知道步驟的存在；步驟畫面存成 WebP 頁面素材（同一份文件要把所有 frame 以 data URL 帶進沙箱，WebP 大約省一半）（4 測試）。(6) `feat/pptx-narration`：模型**不看圖**，而是被告知每一下點擊讓哪些字出現（這個資訊 pptx 本身就有），逐步寫旁白再逐步 TTS；每步語音重用整條 TTS 路徑（供應商選擇、語氣標記、響度、故障切換）(4 測試)。(7) `feat/mcp-pptx`：`upload_pptx`／`get_pptx_import_status`／`narrate_pptx_steps`／`get_page_steps` 四個工具＋旁白端點與文件（5 測試）。**實測**（使用者指定的 `computational Graph (2).pptx`）：真的跑完整匯入，26 頁、17 個動畫頁、127 步，234 秒；抽查第 3 頁的第 0／3／8 步畫面與原檔逐步一致；第 3 頁用真模型產生 9 步旁白＋語音共 50 秒，內容確實跟著動畫走（第 1 步講輸入 a、b、c，第 2 步講 b×c 得到 u……）。相關測試 57/60 通過（3 個失敗是既有基線：react-slide-conversion 需要瀏覽器烘焙，master 上同樣失敗）、前端 1178 全過、前後端 tsc 全綠。**尚未做**：整份簡報的旁白還沒跑（只跑了第 3 頁，其餘 25 頁要花 LLM／TTS 費用，待使用者決定）；瀏覽器裡的實際播放尚未親眼驗證；MCP 工具尚未透過真的 MCP client 端對端跑過 | feat/pptx-parse＋feat/pptx-frames＋feat/react-slide-steps＋feat/step-playback＋feat/pptx-import＋feat/pptx-narration＋feat/mcp-pptx → master |
| 2026-09-12 | （使用者要求）把 `computational Graph (2).pptx` 整份跑完旁白，並修掉實跑中暴露的三個缺陷。**旁白**：26 頁、136 個步驟全部產生旁白並合成語音，零失敗，1366 秒；抽查第 16 頁（25 步）內容正確，但只揭露無文字圖形的那幾步旁白偏空泛（先天限制：步驟是用「這一下讓哪些字出現」描述的）。**缺陷 1（使用者回報）**：每一段動畫語音播完會自動暫停。原因是瀏覽器在媒體播完時**先送 `pause` 再送 `ended`**，所以進到 ended 處理時狀態早已是暫停，下一步的語音被載入卻從未開始播；換頁那條路徑一直有明確重設播放狀態就是為了這個。修法相同：步驟前進時一併重設播放狀態。順手修掉相關問題：某一步播完後按播放會跳下一頁，改成繼續下一步（2 條回歸測試，其中一條我第一次寫錯——它比對到我自己註解裡引用的程式碼文字，改成先濾掉註解）。**缺陷 2**：匯入完成但還沒配旁白的簡報，頁面停在非終端狀態，而 `recoverOrphanedAddPagesPages()` 會把「已完成簡報裡的非終端頁面」當成重啟殘骸標記失敗——線上舊版 server 在我匯入後幾分鐘內就把第 6–26 頁標成失敗。改用本 repo 既有的終端頁面狀態 `audio_ready`（`from-pages.ts` 對複製頁面同樣處理並有註解說明），旁白階段也不再把已完成的頁面降回 `script_ready`；回歸測試直接呼叫那個掃描驗證不再誤傷。**缺陷 3**：`rescanPendingOnStartup()` 會把所有 `processing` 的簡報重新丟進 PDF 主流程，包含匯入到一半被中斷的 pptx（結果是「找不到 source.pdf」——這份簡報的錯誤訊息正是這句）。改成跳過 pptx 來源並標記為失敗、訊息說明是匯入被重啟中斷請重新上傳；一般 PDF 仍照常重新排入（3 測試）。另外在真的 Chrome 裡驗證沙箱逐步顯示：step 0 只有第 0 層、step 3 顯示 0–3 層、step 8 全部 9 層，畫面與原簡報一致；最重的一頁（25 步）沙箱文件約 1MB、平均每頁 219KB。已把該簡報被誤標的 26 頁狀態與錯誤訊息清乾淨 | fix/step-audio-autoplay＋fix/pptx-import-page-status＋fix/pptx-import-restart-recovery → master |
| 2026-09-12 | （使用者回報）產生的逐字稿是逐頁逐步各自生成的，前後文不連貫，而且顯然沒有理解整份簡報的目標，只是在唸每一步新增了什麼。**原因**：第一版旁白唯一得到的資訊就是「這一下點擊讓哪些字出現」，模型既不知道整份在教什麼，也看不到頁與頁之間的關係。**改成兩階段**：(1) `services/pptx/narrationPlan.ts` 一次讀完整份投影片寫出講稿大綱——整份的目標與脈絡，加上每一頁的目標、要講到的重點、如何承接上一頁。一次而不是逐頁，因為要的正是頁與頁之間的脈絡，逐頁的模型看不到它；模型漏掉的頁面仍會補上空白項目，讓旁白退化成「沒有額外脈絡」而不是壞掉。(2) `stepNarration.ts` 拿著整份目標、這一頁的目標與重點、上一頁與下一頁在講什麼，把講解分配到各步驟：第一步開題並承接上一頁、中間依重點推進、最後一步收尾或帶到下一頁；畫面變化只用來決定「講到這裡時學生看得到什麼」，提示詞明確禁止「這一步顯示…」「畫面出現…」這類描述畫面的句子。**大綱寫進簡報**（使用者追加要求）：每一頁的內容文字下方以 `【講稿大綱】` 標記附上該頁大綱，簡報來源文字開頭附上整份目標與脈絡——所以它是使用者看得到、改得動的大綱，不是藏在提示詞裡的東西；重跑會取代該段而不是疊加，並可用 `reusePlan` 沿用改過的大綱重新產生旁白而不重複付費。新增 7 測試（一次讀完整份、漏頁不壞、規劃失敗仍可產生旁白、大綱寫回不疊加、提示詞帶上下頁脈絡且禁止描述畫面），pptx 相關 53/53 通過、tsc 全綠。**實跑**：大綱 68 秒，內容正確（建圖→前向→數值微分對比連鎖律→反向→走訪順序→Python 動態建圖→總結）；整份重跑 26 頁／136 步全部重寫並重新合成語音，零失敗，1285 秒。抽查跨頁銜接成立：第 3 頁結尾「next, we'll see how a small input change propagates」→ 第 4 頁開頭「Now that we can evaluate the graph forward…as our baseline for measuring sensitivity」→ 第 5 頁「Continuing the perturbation method…」。第 26 頁結尾提到期中考一度被我當成編造，查證後發現投影片上本來就寫著「This will be in the mid term!!!」，是忠實轉述 | feat/narration-from-deck-outline → master |
...
| 2026-09-13 | （使用者回報）PPTX 匯入的第 10 頁「第五步把之前的元素都顯示了出來，而且少了最後一步」。兩個現象同一個根因：`renderFrames.ts` 的省成本捷徑假設「最後一格＝原始檔的樣子」，而這只在沒有退場效果時成立。該頁的點擊序列會用 exit 把前一組說明換掉（`entr 434` →「`exit 434`＋`entr 439`」→「`exit 439`＋`entr 443`」→ `entr 446`），所以全部點完的畫面比原檔少東西；最後一格拿了原檔那張，把兩組早該退場的文字畫回來疊在最後的長算式上，而真正的最後一格從未被渲染。改成由 `hiddenShapeIdsForStep` 回答「這一格有沒有藏起任何形狀」，`canRenderFromOriginal` 只在空集合時才走原檔——靜態頁與沒有 exit 的動畫頁仍享有原本的成本優化。`buildStepSlideXml` 本身一直是對的，錯的是哪些格子可以跳過它，所以只比對 XML 看不出問題、要比對渲染出的圖。測試：純函式四種情況＋用真的 LibreOffice 驗證「有 exit 的投影片全部點完那格墨量必須少於原檔」，第二條先確認修正前會失敗（舊行為下兩張圖墨量完全相同 0.01630738）；pptx 相關套件 5/5 與 31/31，設計文件補上這個陷阱。**既有匯入結果未修正**——該份簡報第 6、10、16 頁需重新匯入才會正確，且旁白會一起重寫，待使用者決定 | fix/pptx-exit-effect-final-frame → master |
| 2026-09-13 | （使用者要求，方案經確認）分步頁旁白太短且長度不可調。先釐清約束是「一段對一步」而非「一句對一步」（每段配一個 step 音檔，畫面前進靠它播完），短的原因是兩個寫死的數字：提示詞「請為每一步寫一到三句」與回來後 `slice(0, 300)` 的硬截斷，只改一個沒用。改成新增 `pdfs.script_chars_per_step`，定義在「每一步」而不是「每一頁」——一頁分 24 步是一段解說分成 24 拍，總長要隨步數成長；退值鏈為每步字數→每頁字數→系統預設，靜態頁維持用每頁字數。長度指示用與一般逐字稿同一組 `scriptCharBounds`／`scriptLengthFor` 產生（英文自動以 words 計），截斷上限改成跟著目標走。另外 `POST /pptx-narration` 與 MCP `narrate_pptx_steps` 加 `pages` 與 `chars_per_step`，不必為了一頁重跑 26 頁；大綱仍涵蓋整份，未指定的頁旁白與語音完全不動。測試 11 條（退值鏈、每步下限可低於每頁下限、送出與省略的差別、請求驗證、截斷跟著目標、提示詞不得再有句數），pptx 72/72、逐字稿與 MCP 24/24。未做：前端沒有 UI（只能走 API／MCP），逐字稿編輯框在分步頁仍是「改了不會被唸」的陷阱（使用者確認方案時列為階段二／三） | feat/step-narration-length → master |
| 2026-09-13 | （使用者要求）把 PPTX 匯入加進前端。後端 `POST /api/pdfs/from-pptx` 早就存在但前端一行都沒呼叫，只能用 MCP 或 curl 匯入 PowerPoint；現在是上傳選單第二項。一個 file input 服務兩種來源，因此 `accept` 與端點都必須跟著選擇走（否則 .pptx 選不到、或被送去 PDF 路由回「這不是 PDF」）。PPTX 不開提示詞對話框——它建立的簡報已經有頁面，問「要怎麼產生」是在問一份已存在的簡報；改成說明接下來會發生什麼（逐步渲染、需要數分鐘、完成後還沒有旁白）。匯入階段寫進 `progress_step`，首頁本來就每 5 秒輪詢，卡片才不會連續幾分鐘只寫「處理中」而與卡住難以分辨。這條路不受 LLM 停用影響。測試 5 條接線守門＋i18n，後端 pptx 套件 18/18 | feat/pptx-import-ui → master |
| 2026-09-13 | （使用者要求）讓使用者能直接改動畫頁的逐字稿並重生語音。新增 `PUT /api/pdfs/:id/pages/:n/steps/:k/script`：寫入某一步並重新合成該步語音——重生是預設而非選項，因為字與聲音不可以各說各話；`voice: false` 留給「先改好幾步再一起配音」且會明講語音仍是舊的。清空一步會連音檔一起移除（否則它會繼續講話），合成失敗保住文字並丟掉過期音檔，頁層級 `script.txt` 跟著更新（匯出／搜尋／AI 導師讀的都是它）。前端把分步頁的「逐字稿」分頁整個換成 StepNarrationPanel——取代而非並排，因為原本的編輯框在這種頁面上什麼都改不到；每一步顯示文字、語音狀態與長度，可「儲存並重生語音」或「只存文字」，另可指定每步字數用 AI 只重寫這一頁（其他頁不動）並把字數存回設定、輪詢進度。測試：後端 8 條（逐步隔離、script.txt 跟上、清空即移除音檔、越界要講出實際步數、權限）＋前端 5 條接線守門，相關套件 117/117 | feat/step-narration-editing → master |
| 2026-09-13 | （使用者要求）AI 改寫要有「額外提示」欄位，且與提示詞不同——下次重生不會續用。兩條改寫路徑都加了只活一次的覆寫：`rewrite-script` 的 `target_chars`，`pptx-narration`／MCP 的 `chars_per_step` 與 `instruction`，都不寫回簡報。一次性提示特別不能進 narration plan（它會存檔並被之後每次重生沿用，一句「這次多舉一個例子」掉進去就變成永久綱要）；提示排在規則最後以蓋過通則，但必須在「請以 JSON 回覆」之前，否則回應不再可解析。前端兩處都有欄位、換頁即清空（為某頁打的提示不該被套到下一頁），並指向提示詞分頁說明哪裡才是每次生效的地方。測試 7 條：帶覆寫呼叫後設定與提示詞原封不動、超範圍要擋下而非夾住、提示在 system prompt 但排在 JSON 行之前、產生 plan 的呼叫拿不到它 | feat/one-off-rewrite-hint → master |
| 2026-09-13 | （使用者回報）重生後第 10 頁每步逐字稿都被切斷。實測每步剛好 300 字元且斷在詞中。兩個疊在一起的缺陷：(1) 截斷數「字元」而提示詞用 `scriptLengthFor` 以「英文字數」要求長度——150 字元目標約 78 words 約 450 字元，於是每步都被切在 300；前一輪把固定 300 改成「目標兩倍」時沒發現這個單位錯配，中文兩種算法恰好一致所以看不出來，英文一路壞著。(2) 輸出預算固定 2000 tokens，24 步的頁塞不下，JSON 被截斷、只有先回來的幾步被寫入。改法：截斷不再負責控制長度（那是提示詞的事），只留防爆底線並**切在句子邊界**（旁白要唸出來，斷在半句就是壞的；整段無句號可切時仍照切），預算改成隨步數×目標字數成長（2000～16000）；寫入 manifest 處的 slice 也換成同一個函式。測試三條先確認在舊 cap 與固定預算下會失敗，相關套件 32/32。**已被切斷的資料沒有自動修復**，需再重生一次 | fix/step-script-truncation → master |
| 2026-09-13 | （使用者回報）改寫逐字稿全程顯示「進行中… 0/0」。兩個原因，而且頁數本來就是錯的計量：進度只在「一頁做完」時回報，而重寫單頁的那一頁正好在最後才完成，中間沒有任何可顯示的值；總數也是在同一刻才算出來，在那之前是 0。改法：總頁數在任何耗時工作之前就先報出來（第一件事其實是擬大綱，那是一次跨整份簡報的模型呼叫，本身就可能跑一分鐘），並帶上階段（planning／writing／speaking）；配音改成逐步回報——那才是時間真正花掉的地方（每步一次 TTS，24 步的頁幾乎所有時間都在這裡），某一步配音失敗仍然推進計數，因為呼叫端看的是進度不是成敗。面板改成講正在做什麼而不是一個在單頁情境下無意義的比例（「正在讀整份簡報、擬定講解大綱…」「第 10 頁：配音中 3/5 步」），整份重寫時才顯示頁數。守門測試 1 條（排大綱前必須先報總數、逐步回呼要接上）；pptx 相關 63/63、前端 31/31 與 build 通過 | fix/narration-progress → master |
| 2026-09-13 | （使用者要求）單頁重生時要逐步更新畫面。原本整個工作結束才一次顯示，而時間幾乎都花在配音（每步一次 TTS），長頁面等於面板看起來卡住好幾分鐘。兩個障礙：(1) manifest 要等**所有步都配完**才寫回，期間沒有新東西可讀——改成每一步寫一次（順帶讓中途中斷也能保住已完成的音檔）；(2) 沒有「只問這一頁」的方法，唯一來源是整份 detail（為了一頁重抓所有頁）——新增 `GET /api/pdfs/:id/pages/:n/steps`。面板在輪詢工作狀態時一併抓它，有較新的就顯示，於是每一列會先長出文字、再長出語音；canonical 的 detail 一回來就丟掉這份即時副本，兩者不會各說一套。單步儲存也直接用回應更新那一列（那一列不該落後於剛剛改它的請求）。文字仍然是整頁一次出現——模型一次回傳所有行，那就是它們存在的時刻；逐步到達的是語音。測試：端點逐步回報且沒有文字的步驟不會借用鄰居的音檔、一般頁回空陣列而不是失敗、manifest 寫入必須在迴圈內、面板優先用即時副本並在 reload 後丟棄。相關套件 123/123 | feat/live-step-updates → master |
| 2026-09-13 | （使用者回報）多頁重生時不會重生 React 分步頁的逐步旁白，反而產生一份不會被使用的整頁逐字稿。查證屬實，而且比表面更嚴重：分步頁的頁層級 `script.txt` **就是各步 join 起來的內容**，而匯出、搜尋、AI 導師讀的都是它——一般的重生會把它整個覆寫，於是簡報「說的」和「顯示的」從此不一致；接著語音階段還會依那份稿子錄一個播放器根本不會用的頁層級音檔，但它的長度仍被算進簡報總長。修法：逐字稿與語音兩個階段都跳過分步頁，判斷依據是**有沒有 step manifest 而不是 `render_type === 'react'`**——本機 44 個 React 頁裡有 27 個不是分步頁，它們本來就用頁層級逐字稿，用 render_type 一刀切會把它們的重生一起停掉。排除發生在「刪除既有逐字稿」之前（跳過卻已經先刪掉，等於照樣毀了要保護的東西）；若選到的頁全是分步頁，工作直接以「請改用逐步旁白重寫」失敗，而不是回報成功卻什麼也沒做。測試 3 條先確認修正前會失敗；同批的 5 條 regenerate 失敗為既有基線（把改動 stash 後重跑逐條相同） | fix/regenerate-skip-step-pages → master |
| 2026-09-13 | （承上，使用者指出分步頁應顯示逐步內容）全螢幕的逐字稿編輯框也有同一個陷阱：它改的是頁層級稿子，分步頁永遠不會唸。改成分步頁唯讀而不是隱藏——它顯示的是各步合起來的內容，授課時仍值得閱讀——並加一行說明指向一般檢視的逐字稿分頁逐步編輯。守門測試 1 條；前端 33/33、build 與 tsc 通過 | fix/fullscreen-step-transcript → master |
| 2026-09-13 | （使用者指正）上一輪把分步頁從重生中排除、選到就整個工作失敗——使用者指出「應該根據不同的頁用不同方式重生，怎麼會直接說是失敗」。確實是用錯的答案回應對的問題：混選一般頁與分步頁是再正常不過的操作，失敗等於要使用者跑兩次、還得自己知道哪幾頁是哪一種（UI 根本沒顯示）。改成兩個階段都各走各的路：逐字稿階段一般頁走 `generateScript`、分步頁走 `narrateImportedDeck(pages, textOnly)`；語音階段一般頁走 `synthesizeAudio`、分步頁走新的 `respeakPageSteps` 逐步錄。逐字稿階段刻意只寫文字——錄音是語音階段的事，在這裡順便配音等於收了使用者沒要求的 TTS 費用。`respeakPageSteps` 只把現有的話再說一次而不是重寫（「重生語音」是 say this again，不是 say something else），每錄好一步就寫回 manifest，中途中斷也保住已錄的部分。分步頁若原始 pptx 已不在就只有那部分失敗並回報，不會拖垮已經成功的一般頁。進度涵蓋兩種頁，只有「完全沒有可唸的東西」才算錯誤。測試 4 條（兩階段的分流、以 manifest 而非 render_type 判斷、分流必須發生在刪除既有逐字稿之前、respeak 不呼叫模型）；同批 5 條 regenerate 失敗與基線逐條相同 | feat/regenerate-mixed-pages → master |
| 2026-09-13 | （使用者要求）匯入 PPT 也要有提示詞／風格對話框，能調字數與分段字數。原本上傳後直接開工，而決定旁白聽起來如何的設定要事後才找得到——通常是第一份旁白寫出來風格不對或太短才發現，那時重寫又是一輪 LLM＋TTS 費用。新增專屬對話框，問的是真正還開放的三件事（不是 PDF 那種「要怎麼產生這份簡報」的提示詞——頁面本來就來自檔案）：講解風格（存成簡報提示詞，之後每次旁白都套用；臨時要求請用重寫時的「這次額外的要求」）、每頁字數、每步字數（整頁總長＝每步字數×步數，所以 24 步的頁自然講比較久）。三者都在建立簡報時就寫入，第一份旁白就照著走而不是先產出一份待修正的草稿。對話框另外提供「匯入完成後自動產生旁白與語音」：旁白是依匯入產生的步驟寫的，只能在頁面畫完之後才開始，而帳號必須在上傳時就記下（那時請求早已結束，而旁白花的是該帳號的模型與 TTS 額度）；沒有可用 LLM 時這個選項不可用。測試：對話框的值要送進上傳、multipart 欄位必須排在檔案之前（路由是從 file handle 讀欄位，排在後面就還不存在）、後端三個欄位都要解析並限制範圍、旁白 job 必須在匯入之後而不是並行啟動。前端 7/7、後端 28/28、build 與兩邊 tsc 通過 | feat/pptx-import-options → master |
| 2026-09-13 | （使用者回報「還是失敗」）截圖上的錯誤訊息其實已經從程式碼移除——查出後端跑的是 17:00 啟動的行程，而修正是 17:13 才寫入。實測證明先前「`tsx watch` 會自動載入」的說法錯誤：touch 一個靜態被 import 的檔案（`worker/regenerate.ts`）等 8 秒，服務 pid 完全不變；touch 進入點 `server.ts` 才會重啟。不是 inotify 名額問題（上限 65536、用量個位數）。也順帶記下判斷「線上跑哪一版」的正確方法：先用 `ss -lntp` 找出真正在聽 port 的行程（ps 裡同名行程可能好幾個且都不是它），再比對它的啟動時間與最新檔案 mtime。已 touch 進入點讓修正生效（新行程 17:25:12，晚於所有今天的改動）。往後每次後端改動都要主動觸發重載 | docs/tsx-watch-note → master |
...
| 2026-09-16 | （使用者回報，承 09-14 未完全修好）進入 React 頁仍先顯示黑色。上一輪的雙緩衝只涵蓋「同一個 frame 換文件」，另外三條路徑都繞過它：(1) 程式碼與素材是兩個獨立請求，而兩者都組成沙盒文件——程式碼先到，就用「新頁程式碼＋上一頁素材」組出文件，分步頁全是圖片，那份文件就是一片深色底，直到素材到了再重建一次；改成兩者 `Promise.all` 一起抓、同一個 tick 生效，編輯時插圖用的素材刷新則跳過載入時已抓過的。(2) 沙盒在 `render()` 後下一個畫格就回報 ready，而正上方的註解早就寫明 React 18 的 commit 可能晚於那一格、分步頁的 webp 解碼更晚，所以等 ready 的換頁照樣換上空白頁；改成等 commit、所有圖片解碼、再一個已繪製的畫格才回報，並以 4 秒封頂避免壞圖卡住，外層的保險逾時也調到比 4 秒長（否則外層先放棄、換上半解碼的頁面）。(3) 從圖片頁進入時 frame 是全新掛載、文件立刻可見，而此時唯一載入的 React 內容是更早某個 React 頁的；改成保留上一個畫面——前一頁是圖片就繼續顯示那張圖（內容到了之後當作隱形 frame 底下的海報）、前一頁是 React 就讓它的 frame 保留原文件，等新頁回報已繪製才一次切換；「已繪製」以沙盒回報為準而不是 iframe 的 load 事件（那時 React 還沒畫任何東西）。守門測試 4 條先確認修正前會失敗；前端廣泛套件 1172/1172。已知限制：第一次開啟簡報就落在 React 頁時沒有上一個畫面可保留 | fix/react-page-entry-black → master |
...
| 2026-09-20 | （使用者要求）有元素的頁面可以「換底圖＋AI 微調元素位置」來美化，且元素維持可編輯。查證後換底圖那一半本來就有，缺排版那一半；依裁示連 AI 生成背景一起做。新服務 `pageElementsBeautify`（背景提示詞要求絕對無文字、視覺模型回傳每個 id 的幾何與文字字級顏色、純函式套用、依底圖亮度修正文字對比），新端點 `/elements/beautify` 與 `/beautify/undo`：動手前先存還原點（底圖＋元素成對），兩半各自降級，失敗以 warnings 回報；模型只准動位置大小與文字字級顏色，文字內容與素材一律沿用。前端在元素分頁加「AI 美化排版」面板與「復原美化」。驗證：拋棄式副本實測生成背景 21 秒、元素被移動並改成深底可見的亮色、文字未被改寫；另一輪只排版再復原可完全回到原狀。測試 12 條，後端 39/39、前端 1143/1143、tsc 與 build 通過 | feat/element-beautify → master |
| 2026-09-20 | （使用者回報＋截圖）AI 導師說它沒有學生所在頁（第 31 頁）的內容，要使用者自己貼上。實測語料為 100,810 字元、上限卻是 14,000，而組法是「依頁序串起來、從尾端截掉」——提示詞裡只有第 1–5 頁，學生所在頁被截光；加上系統提示詞宣稱「所有頁面都給你了」，握有 `get_page_*` 工具的模型也不會去補讀。新服務 `askCorpus`：無條件保留所在頁（即使單頁超過整個預算），再以它為中心向外擴散花預算，依頁碼排序輸出；未附上的頁碼以區間列出並明講要用工具讀、不可向學生索取。驗證：同一份簡報同一頁同一個問題，導師改用第 31 頁內容作答。守門測試 8 條（端點接線 1 條先確認修改前會失敗），ask／tutor 76/76、tsc 通過、已 touch 進入點 | fix/ask-corpus-keeps-current-page → master |
| 2026-09-20 | （使用者回報）第 31 頁有評論，但全螢幕沒有圖示。查證後徽章本身沒壞（重新載入後一般畫面與全螢幕四種版面都會顯示）：`has_comment` 是隨簡報資料來的旗標，而 AI 導師「存成評論」與側邊欄留言表單都只更新局部狀態、沒回寫 detail，所以當場寫的第一則評論要重新載入才看得到；全螢幕更嚴重，因為同一個旗標還決定「要不要去抓評論」，false 就永遠不抓。修法：PlayPage 新增 `setPageHasComment` 就地改 detail，兩個寫入路徑寫完打開旗標、刪到剩零則關掉。實測：送出留言後不重新整理，一般畫面立刻出現 💬、全螢幕顯示「💬1」。守門測試 4 條（3 條先確認修改前會失敗），前端 1147/1147、tsc 與 build 通過 | fix/comment-badge-updates-live → master |
...
| 2026-09-20 | （使用者回報）`rYFD1VwStl` 第 8 頁動畫與逐字稿對不上。查證：`stepNarration` 送給模型的訊息**完全沒有影像**，步驟只用「新出現的文字」描述；重建當時輸入後確認 7 個展開步驟得到同一行「沒有文字的圖形」，模型等於沒看過動畫，只能拿整頁文字＋大綱寫完再平均切段（實際第 4 步是把向量收回去，逐字稿卻在講取對數）。依裁示採「首幀＋每步變化區域」：新服務 `stepFrames` 逐像素比對相鄰影格算出變化外框，並以由紙變墨／由墨變紙的像素數分類新增／收起／替換（先前用整塊平均墨水量會被沒變的公式稀釋，測試抓到後改掉）；影格匯入時就已存在，不必重算。沒有影格或解不開時維持原本純文字描述。驗證：重新產生第 8 頁後，第 6 步寫出畫面上才有的「zero, one, and one」，第 5、7 步也都對上。測試 9 條（接線 1 條先確認修改前會失敗），pptx／step 相關 90/90、tsc 通過、已 touch 進入點。待改善：單一步驟在相距很遠的位置同時變化時外框會涵蓋大片區域，前幾步仍偏向照大綱講 | feat/step-narration-sees-frames → master |
| 2026-09-20 | （使用者回報）`VIUWSnfnsK` 第 8 頁動畫 2–6 步沒有變化，應為文字逐行出現。查證：7 張影格中第 0–5 張位元組相同；原始 pptx 的六次點擊全部指向同一形狀 `spid=159`，只差在 `p:txEl/p:pRg` 的段落編號，而解析器只讀 `@spid`，加上可見度規則是「後面會帶進來的先藏起來」，於是形狀被藏到最後一次點擊。修正：解析段落層級目標，新增 `hiddenParagraphsForStep` 與 `removeParagraphs`（裁掉未揭露的 `a:p`、形狀留著），一行都沒出現的形狀整個移除，`canRenderFromOriginal` 改用 `isFullyBuiltStep`；並把可見度規則改寫成依每個目標的事件清單、由前一次點擊決定，一併修掉「同一個東西被兩次進入效果指到也會藏到最後」的通用形式。附帶好處：每一步揭露的文字變成那一行，旁白模型不再只收到「沒有文字的圖形」。驗證：用真實 pptx 重新推導，文字確實逐行出現。測試 11 條（舊版全失敗），pptx／step 101/101、tsc 通過、已 touch 進入點。注意：該頁最後兩次點擊指向空白段落，所以那兩步仍無變化；既有影格需重新匯入才會更新 | fix/pptx-paragraph-level-builds → master |
...
| 2026-09-22 | （使用者要求）圖片供應商設定：`IMAGE_PROVIDER` 在跟著 LLM／OpenAI（gpt-image-2.5-flare）／Gemini（Nano Banana 2，gemini-3.1-flash-image）／Qwen-Image（qwen-image-2.1，DashScope）之間選；新增 `imageAdapters.ts` 讓 Gemini／Qwen 以 OpenAI Images API 的形狀接進所有呼叫點（遮罩以提示代替）；設定頁選單與欄位；固定供應商不走 LLM 失效轉移。後端 17/17、前端 33/33、`tsc`＋`vite build` 通過。以 `--no-ff` merge 回 master、fast-forward `worktree/demo16` 並重建其前端。未對真實 Gemini／DashScope 實測 | feat/image-provider-setting → master／worktree/demo16 |
| 2026-09-22 | （使用者要求）Qwen-Image-2.1 改為獨立服務：`scripts/qwen-image-server/`（server.py 包官方 Diffusers pipeline、JSON API、遮罩後合成、token、`--stub`；run.sh 可攜、自動檢查安裝 Python／venv／git／CUDA→torch／磁碟／套件）＋文件；後端 `QWEN_IMAGE_BACKEND=local|dashscope` 與本機轉接器；設定頁執行方式選單。後端 9/9＋11/11（含真起 stub 服務的端到端）、前端 33/33、`tsc`＋`vite build`、`run.sh --stub` 通過。以 `--no-ff` merge 回 master、fast-forward `worktree/demo16` 並重建其前端。未在此機載入真模型（8 GB VRAM） | feat/qwen-image-local-service → master／worktree/demo16 |
| 2026-09-22 | （使用者要求）在 conda `qwen` 環境把 `scripts/qwen-image-server` 跑起來。環境原本的 torch 2.14.0 是 cu130 版，本機驅動只到 CUDA 12.8，`cuda.is_available()` 為 False，改裝 torch 2.11.0+cu128；接著載入模型時因缺 torchvision 而 `ImportError`（Qwen3-VL 文字編碼器的 processor 需要）。服務以 `--cpu-offload` 在 A5000 上啟動（權重 31 GB 超過 24 GB VRAM），1024×768 一張約 54 秒，出圖正常。repo 修正：`requirements.txt` 與 `run.sh` 補 torchvision（和 torch 同一個 wheel 來源，torch 已存在時固定在該版本，避免被換成驅動跑不動的版本）並在環境檢查中匯入；`QWEN_IMAGE_VENV` 指向既有環境時改用該環境的 Python 做版本檢查（原本會抓到 PATH 上的 3.8 而中止）；文件補說明。驗證：qwen 環境 `run.sh --check` 通過；全新 venv 只裝 torch 2.11（CPU）後跑 `--check`，確認只補上 torchvision、torch 維持 2.11 | fix/qwen-image-torchvision → master |
