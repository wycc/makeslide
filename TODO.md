...
- [ ] **階段 5：後續加強**（2026-07-08 分析既有 notebook 程式後新增；核心階段 0–4 已完成，以下為體驗／一致性強化）。
    ...
    - [ ] **5b：總播放時長排除 notebook 頁殘留 audio_duration**：一頁轉成 notebook 後其 DB `audio_duration_seconds` 仍在，`regenerate.ts` 重算 `total_audio_duration_seconds` 時會把它算入，與「notebook 頁無語音」（1d）矛盾。應在彙總查詢對 `render_type='notebook'` 的頁視為 0／排除；並考慮轉成 notebook／generate 時觸發重算。純函式 `sumAudioDurationSeconds` 可測。
    - [ ] **5c：AI generate 帶入頁面既有內容作 context**：後端 generate 端點已支援 `context` 參數，但前端 `handleGenerateNotebookForCurrentPage` 未帶。可讀當前頁 script／text 作 context，使 AI 產生的 notebook 更貼合該頁主題。
    - [ ] **5d：notebook cell 增／刪 UI**：目前只能編輯既有 cell，無法新增／刪除 cell。`nbformatModel` 加 `insertCell`／`deleteCell` 純函式（immutable、可測），`NotebookPanel`（editable）command 模式加 `a`/`b`（上／下插入）、`dd`（刪除）快捷鍵或工具列鈕，經 `savePageNotebook` 寫回。
    - [ ] **5e：kernel 執行逾時／連線失敗的使用者提示**：`useJupyterKernel` 執行長時間無回應或連線中斷時，`NotebookPanel` 目前僅有 `runError` 布林；可加逾時偵測與更明確的狀態訊息（連線中／逾時／中斷可重試），提升就地執行的可觀測性。
