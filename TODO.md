...
- [ ] `page-operations.ts` 的 `GET .../image-candidates/:candidateId` 補上讀取權限檢查：同檔案的 `GET /api/pdfs/:id/pages/:n/image-candidates/:candidateId` 直接串流 inpaint 產生的候選圖片檔案，只檢查頁碼是否在範圍內，沒有檢查 PDF 讀取權限；雖然需要知道隨機產生的 `candidateId`，但這只是降低被亂猜的機率，不是真正的存取控制。應補上與其他圖片串流端點一致的讀取權限檢查。
