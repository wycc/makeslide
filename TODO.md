...
- [ ] `versioning.ts` 的 4 個 GET 端點補上讀取權限檢查（風險最高，優先處理）：`backend/src/routes/pdfs/versioning.ts` 的 `GET /api/pdfs/:id/pages/:n/image/history`、`GET .../script/history`、`GET .../script/versions/:hash`、`GET .../image/versions/:hash` 只檢查頁面/版本是否存在，完全沒有讀取權限檢查；同檔案的 POST 還原端點（`image/restore/:hash` 等）已正確使用 `getPdfPermissionRow()` + `canEditPdf()`，證明這是 GET 端點的遺漏而非設計選擇。任何人知道 PDF id 與頁碼，就能讀到別人私有簡報的逐字稿完整歷史與歷史版本圖片。應比照 `figures.ts`/`drawings.ts`/`page-animation.ts`/`quizzes.ts` 的慣例，補上本地 `canReadPdf()`/`getShareToken()`/`hasShareAccess()` helper 並套用到這 4 個 GET 端點，無權限回傳 `403 FORBIDDEN`，並新增測試覆蓋非擁有者對 private 簡報應得 403、擁有者/`public`/分享 token 讀取應正常回應。
