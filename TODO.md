...
- [ ] `page-operations.ts` 的 `GET .../chat-history` 補上讀取權限檢查（風險最高，優先處理）：`backend/src/routes/pdfs/page-operations.ts` 的 `GET /api/pdfs/:id/pages/:n/chat-history` 只檢查頁面是否存在，完全沒有讀取權限檢查；同檔案緊接著的 `DELETE /api/pdfs/:id/pages/:n/chat-history` 已正確使用 `getPdfPermissionRow()` + `canEditPdf()`，證明這是 GET 端點的遺漏。任何人只要知道 PDF id 與頁碼，就能讀到別人簡報裡與 AI 修圖/改逐字稿的完整對話內容，不需要任何額外的 token 或猜測值。應比照既有慣例補上 `canReadPdf()`/`getShareToken()`/`hasShareAccess()` 並套用到此端點，並新增測試覆蓋非擁有者對 private 簡報應得 403、擁有者/`public`/分享 token 讀取應正常回應。
