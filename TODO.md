...
[x] `highlight-box` 動畫效果—pulse 模式：目前 `highlight-box` 是靜態邊框；應新增 `highlightPulse?: boolean` 選項（預設 `false`），當啟用時，GSAP timeline 在效果顯示期間讓邊框 box-shadow 週期性放大/縮小，形成脈動視覺（可在 GSAP yoyo repeat 或自訂 progress 函式中實現），讓重要內容更加吸睛；在 `AnimationEditorTab` 新增 checkbox 控制，並更新後端驗證/序列化、前端 types.ts，及中英文 i18n。 ✓ 完成於 branch: feature/highlight-box-pulse-20260617
...
## 2026-06-18 新增項目 Batch 5
[x] `step-list` 背景色與文字色是否可由 AI 自動生成建議：目前 AI auto-focus 功能（`animationAutoFocus.ts`）生成 `step-list` 效果時不提供 `stepListBgColor`/`stepListTextColor` 欄位，使用者需手動調整；應在 `AUTO_FOCUS_SYSTEM_PROMPT` 的 `step-list` 說明新增建議，讓 AI 在適合深色背景的投影片中可選擇較淺的背景色，並更新 `AutoFocusItemSchema` 允許 `stepListBgColor`/`stepListTextColor` 欄位，`mapAutoFocusResponseToEffects` 中加入對應傳遞邏輯，同時新增 1 個單元測試驗證有顏色時正確傳遞。 ✓ 完成於 branch: feature/step-list-ai-colors-20260617
...
[x] `pointer` 效果形狀選項—新增圓點（dot）模式：目前 `pointer` 效果固定使用 SVG 游標箭頭路徑（`<path d="M4 0 ...`）；應新增 `pointerShape?: 'arrow' | 'dot'` 選項（預設 `'arrow'`），當設定為 `'dot'` 時在投影片上渲染一個圓形指示點（`<circle r="50%">`），不使用 angle 旋轉；在 `AnimationEditorTab.tsx` 新增下拉選單（arrow/dot），並同步更新後端 `AnimationEffect`/`EffectSchema`/序列化、前端 `types.ts`/`SlideRenderer` 及中英文 i18n。 ✓ 完成於 branch: feature/pointer-shape-20260618
...
- 內容: 新增 `formula` 效果圓角半徑控制。後端 `pageAnimation.ts` 新增 `DEFAULT_FORMULA_BORDER_RADIUS = 8`、`MAX_FORMULA_BORDER_RADIUS = 32` 常數，`AnimationEffect` 介面新增 `formulaBorderRadius?: number` 欄位，`EffectSchema` 新增 `z.number().int().min(0).max(32)` 驗證，序列化時以整數夾至合法範圍。前端 `types.ts` 同步新增欄位；`SlideRenderer.tsx` 以 `${effect.formulaBorderRadius ?? 8}px` 取代硬編碼 `'8px'` 的 borderRadius；`AnimationEditorTab.tsx` 在 formula 顏色選擇器 flex 行後新增圓角數字輸入框（range 0-32, step 2）及 px 單位標籤；中英文 i18n 新增 `play.animation.formulaBorderRadius` 翻譯鍵。
...
- 時間: 2026-06-18 14:00:00 +0800
- 分支: feature/pointer-shape-20260618
- 內容: 新增 `pointer` 效果形狀選項（`pointerShape`）。後端 `pageAnimation.ts` 在 `AnimationEffect` 介面新增 `pointerShape?: 'arrow' | 'dot'` 欄位，`EffectSchema` 新增 `z.enum(['arrow', 'dot'])` 驗證，序列化時一併輸出。前端 `types.ts` 同步新增欄位；`SlideRenderer.tsx` 依 `pointerShape ?? 'arrow'` 判斷：`'dot'` 時渲染 `<circle cx="12" cy="12" r="10">`，`'arrow'` 時渲染原始游標路徑；`dot` 模式不套用 angle 旋轉（transform 中去除 `rotate`）；`AnimationEditorTab.tsx` 在指標區塊最上方新增形狀下拉選單（arrow/dot 兩個選項）；中英文 i18n 新增 `play.animation.pointerShape`/`pointerShape.arrow`/`pointerShape.dot` 翻譯鍵。

- 時間: 2026-06-17 06:40:00 +0800
- 分支: feature/step-list-ai-colors-20260617
- 內容: 新增 `step-list` 效果背景色/文字色的 AI 自動建議功能。後端 `animationAutoFocus.ts` 在 `AutoFocusItemSchema` 新增 `stepListBgColor`/`stepListTextColor` Zod 欄位（hex color regex 驗證），引入 `MAX_SHAPE_COLOR_LENGTH`；在系統提示詞 step 6 後新增 6b 說明，指引 AI 判斷投影片背景色系（淺色系建議提供深藍+近白配色，深色系可省略）；`mapAutoFocusResponseToEffects` 在 `step-list` 分支中加入 `stepListBgColor`/`stepListTextColor` 提取與傳遞邏輯（僅在 items 存在且類型確認為 step-list 時才傳遞）；新增 1 個單元測試（ok 94）：驗證有顏色時正確傳遞至 AnimationEffect、無顏色時欄位為 undefined，且結果通過 validateAnimationSpec。

- 時間: 2026-06-17 06:50:00 +0800
- 分支: feature/highlight-box-pulse-20260617
- 內容: 新增 `highlight-box` 效果脈動光暈（pulse）模式。後端 `pageAnimation.ts` 在 `AnimationEffect` 介面新增 `highlightPulse?: boolean` 欄位，`EffectSchema` 新增 `z.boolean().optional()` 驗證，序列化時一併輸出。前端 `types.ts` 同步新增欄位；`buildGsapTimeline.ts` 將 `highlight-box` 從 fallthrough case 分離出獨立 case：當 `highlightPulse` 為 `true` 時，在淡入完成後以 `fromTo` 加入 `boxShadow` 脈動動畫（`yoyo: true, repeat: -1`，period 0.7s，`sine.inOut` easing），在正常光暈與 2.5 倍放大光暈間來回切換（若有 `highlightOuterColor` 外框也同步放大）；`AnimationEditorTab.tsx` 在 `highlightOuterColor` checkbox 下方新增「脈動光暈」checkbox；中英文 i18n 新增 `play.animation.highlightPulse` 翻譯鍵。後端/前端 TypeScript 均通過。
...
- 時間: 2026-06-17 07:20:00 +0800
- 分支: feature/text-callout-align-20260617
- 內容: 新增 `text-callout` 效果文字對齊方式控制。後端 `pageAnimation.ts` 在 `AnimationEffect` 介面新增 `textCalloutAlign?: 'left' | 'center' | 'right'` 欄位，`EffectSchema` 新增 `z.enum(['left', 'center', 'right'])` 驗證，序列化時一併輸出。前端 `types.ts` 同步新增欄位；`SlideRenderer.tsx` 依 `textCalloutAlign ?? 'center'` 計算 `textAlign` style 及 flexbox `justifyContent`（left→flex-start、right→flex-end、center→center）；`AnimationEditorTab.tsx` 在圓角輸入框下方新增對齊下拉選單（靠左/置中/靠右）；中英文 i18n 新增 `play.animation.textCalloutAlign`/`.left`/`.center`/`.right` 翻譯鍵。後端/前端 TypeScript 均通過。
...
[x] Skills 整合至 regenerate 流程：目前技能 prompt 只在初次生成逐字稿（pipeline.ts）時注入，`regenerate.ts` 的 `runRegenerateScripts` 函式在重新生成逐字稿時並未讀取啟用技能；應在 `runRegenerateScripts` 中加入與 pipeline.ts 相同的技能合併邏輯（`getEnabledSkillPrompts` + 合併至 `userPrompt`），確保重新生成時也套用使用者的技能設定。 ✓ 完成於 branch: feature/skills-regenerate-20260617
[x] Skills UI 編輯功能：目前設定頁中的自訂技能只能新增/刪除/啟用停用，無法在建立後修改名稱或指令內容；應在每個自訂技能列表項目加入「編輯」按鈕，點擊後展開行內編輯表單（名稱 input + 指令 textarea + 套用範圍 select + 儲存/取消按鈕），儲存時呼叫 `PATCH /api/skills/:id`，並即時更新列表狀態，不需重新載入頁面。 ✓ 完成於 branch: feature/skills-edit-ui-20260617
[x] `highlight-box` 效果半透明填充色：目前 `highlight-box` 效果只有外框線，框內無任何填充；應新增 `highlightFillColor?: string`（CSS hex，選填，例如 `'#ef444430'` 表示 19% 透明紅色）欄位，讓使用者在動畫編輯器中設定框內的半透明填充顏色；後端同步更新 `AnimationEffect`/`EffectSchema`（同色碼 regex 驗證）/序列化；前端 `types.ts` 新增欄位、`SlideRenderer.tsx` 在 `highlight-box` div 的 `backgroundColor` style 中加入 `highlightFillColor`（預設 `'transparent'`）；`AnimationEditorTab.tsx` 在邊框顏色選擇器後新增一個帶勾選框的顏色選擇器（類似 shapeFillColor 的實作）；中英文 i18n 新增翻譯鍵。 ✓ 完成於 branch: feature/highlight-fill-color-20260617
[x] `pointer` 效果脈動動畫：目前 `pointer` 效果的箭頭/圓點靜態顯示；應新增 `pointerPulse?: boolean`（預設 false），啟用時對 dot 指標使用 GSAP `boxShadow` 脈動動畫（類似 `highlight-box` 的 `highlightPulse`），對 arrow 指標則使用 `scale` yoyo 動畫（1→1.15→1，0.6s，無限循環）；後端同步更新 `AnimationEffect`/`EffectSchema`/序列化；前端同步更新 `types.ts`/`buildGsapTimeline.ts`（在 pointer case 加入條件動畫）/`AnimationEditorTab.tsx`（勾選框）/中英文 i18n。 ✓ 完成於 branch: feature/pointer-pulse-20260617
[x] Manim `animate.shake(m, progress, opts)` 搖晃效果：應新增 `animate.shake(m, progress, opts)` 函式，模擬元素水平搖晃的效果（適合用於強調、警示、出錯場景）；以 `progress` 控制搖晃幅度（progress 0 = 無偏移，0.5 = 最大幅度，1 = 回到原位），透過 `Math.sin(progress * Math.PI * cycles)` 計算水平位移（translateX），opts 支援 `amplitude`（搖晃幅度 px，預設 8）和 `cycles`（搖晃週期數，預設 4）；對非 transform 支援元素退回為 opacity；新增至少 2 個 vm 測試（一個驗證中間 progress 有非零偏移，一個驗證 progress=0 和 progress=1 都為零偏移）。 ✓ 完成於 branch: feature/manim-shake-20260617
[x] `text-callout` 效果邊框顏色：目前 `text-callout` 的方框沒有外框；應新增 `textCalloutBorderColor?: string`（CSS hex，選填）欄位，設定後在方框加上 `border: 2px solid {color}` 樣式；後端同步更新 `AnimationEffect`/`EffectSchema`（色碼 regex 驗證）/序列化；前端 `types.ts` 新增欄位、`SlideRenderer.tsx` 條件加入 `border` style；`AnimationEditorTab.tsx` 加入帶勾選框的顏色選擇器；中英文 i18n 新增翻譯鍵。 ✓ 完成於 branch: feature/text-callout-border-color-20260617
[x] `shape` 效果 rect 圓角半徑客製化：目前 `shape` 的 `rect` 類型固定使用 `rx="6"`（SVG 單位）；應新增 `shapeRectRadius?: number`（SVG 單位，整數，預設 6，範圍 0–24），讓使用者自訂矩形圓角；後端同步更新 `AnimationEffect`/`EffectSchema`（`z.number().int().min(0).max(24)`）/序列化；前端 `types.ts` 新增欄位、`SlideRenderer.tsx` 以 `effect.shapeRectRadius ?? 6` 取代硬編碼 `rx="6"`；`AnimationEditorTab.tsx` 在 shape 設定區塊加入數字輸入框（range 0-24, step 2，僅對 rect 類型顯示）；中英文 i18n 新增翻譯鍵。 ✓ 完成於 branch: feature/shape-rect-radius-20260617

- 時間: 2026-06-17 09:30:00 +0800
- 分支: feature/skills-regenerate-20260617
- 內容: 修正 Skills 未整合至 regenerate 流程的缺口。在 `regenerate.ts` 的 `runRegenerateScripts` 函式中，原先直接使用 `opts.prompt ?? pdfRow.user_prompt` 作為 `userPrompt`，未讀取已啟用的技能 prompt。修正後：引入 `currentAccountId` 與 `getEnabledSkillPrompts`；在設定 `userPrompt` 前呼叫 `getEnabledSkillPrompts(currentAccountId(), 'script')` 取得所有已啟用技能指令，若有技能則以換行合併至基本 userPrompt（過濾空字串後 join），確保重新生成逐字稿時同樣套用使用者的技能設定，與初次生成行為一致。後端 TypeScript 通過。

- 時間: 2026-06-17 09:45:00 +0800
- 分支: feature/skills-edit-ui-20260617
- 內容: 新增自訂技能行內編輯功能。在 `SettingsPage.tsx` 中增加 `editingSkillId`、`editName`、`editPrompt`、`editApplyTo`、`savingSkillId` 五個狀態變數；自訂技能列表中每筆資料新增「編輯」按鈕，點擊後切換為行內表單（名稱 input、指令 textarea、套用範圍 select、儲存/取消按鈕），儲存時呼叫 `updateSkill()` 送出 PATCH 請求，成功後即時更新列表並關閉表單；內建技能不顯示編輯按鈕。中英文 i18n 新增 `settings.skillEdit`/`settings.skillSave`/`settings.skillCancel` 翻譯鍵。前端 TypeScript 通過。

- 時間: 2026-06-17 10:00:00 +0800
- 分支: feature/highlight-fill-color-20260617
- 內容: 新增 `highlight-box` 效果半透明填充色控制。後端 `pageAnimation.ts` 在 `AnimationEffect` 介面新增 `highlightFillColor?: string` 欄位，`EffectSchema` 新增與邊框顏色相同的 hex regex 驗證（最長 9 字元，包含 alpha channel），序列化時一併輸出。前端 `types.ts` 同步新增欄位；`SlideRenderer.tsx` 在 `highlight-box` div style 加入 `backgroundColor: hFill`（預設 `'transparent'`）；`AnimationEditorTab.tsx` 在 highlightPulse 勾選框後新增帶勾選框的顏色選擇器（勾選啟用時預設 `#ef444430`，色彩選擇器只控制前 7 碼，alpha 碼 `30` 保留），中英文 i18n 新增 `play.animation.highlightFillColor`（'Fill color'／'填充顏色'）翻譯鍵。後端/前端 TypeScript 均通過。

- 時間: 2026-06-17 10:30:00 +0800
- 分支: feature/pointer-pulse-20260617
- 內容: 新增 `pointer` 效果脈動動畫控制。後端 `pageAnimation.ts` 在 `AnimationEffect` 介面新增 `pointerPulse?: boolean` 欄位，`EffectSchema` 新增 `z.boolean().optional()` 驗證，序列化時一併輸出。前端 `types.ts` 同步新增欄位；`buildGsapTimeline.ts` 將 `pointer` case 從 spotlight/text-callout/shape/... 的共用 fallthrough 中拆出為獨立 case：淡入後若 `effect.pointerPulse` 為 true，依 `pointerShape` 選擇脈動幅度（dot: scale 1→1.3，arrow: scale 1→1.15），以 `sine.inOut` ease、0.6s 週期、`yoyo: true`、`repeat: -1` 無限循環，並在 `transformOrigin: '50% 50%'` 使縮放以元素中心為基準；exitDuration tween 加入 `scale: 1` 確保退場時回到正常尺寸。`AnimationEditorTab.tsx` 在 pointerSize 輸入框後新增脈動勾選框；中英文 i18n 新增 `play.animation.pointerPulse`（'Pulse animation'／'脈動動畫'）翻譯鍵。前後端 TypeScript 均通過。

- 時間: 2026-06-17 11:00:00 +0800
- 分支: feature/manim-shake-20260617
- 內容: 新增 Manim `animate.shake(m, progress, opts)` 水平搖晃效果。在 `manimHelperScript.ts` 的 animate 物件中新增 `shake` 函式：以 `Math.sin(p * Math.PI * cycles) * amplitude` 計算水平位移，設定至 `m.el.style.transform = 'translateX(...)px'`；整數 cycles 確保 progress=0 和 progress=1 時 sin 值皆為 0，自然回到原位。opts 支援 `amplitude`（預設 8px）和 `cycles`（預設 4 週期）。同步在 `manimHelperScript.test.ts` 新增 2 個 vm 單元測試：一個驗證端點（progress=0 和 progress=1）偏移量 &lt;0.01px，另一個驗證中間 progress=0.125 時偏移量大於 1px，以及自訂 amplitude=20/cycles=2 時峰值接近 20px。34 個測試全數通過。

- 時間: 2026-06-17 11:20:00 +0800
- 分支: feature/text-callout-border-color-20260617
- 內容: 新增 `text-callout` 效果邊框顏色控制。後端 `pageAnimation.ts` 在 `AnimationEffect` 介面新增 `textCalloutBorderColor?: string` 欄位，`EffectSchema` 新增 `z.string().max(9).regex(/^#[0-9a-fA-F]{3,8}$/)` hex 色碼驗證，序列化時一併輸出。前端 `types.ts` 同步新增欄位；`SlideRenderer.tsx` 在 `text-callout` div style 中加入條件性 `border: 2px solid {color}`（未設定時不加入 border 屬性）；`AnimationEditorTab.tsx` 在 textCalloutAlign 下方新增帶勾選框的顏色選擇器（勾選啟用時預設 `#ffffff`，顏色選擇器控制完整 hex）；中英文 i18n 新增 `play.animation.textCalloutBorderColor`（'Border color'／'邊框顏色'）翻譯鍵。前後端 TypeScript 均通過。
...
[x] `spotlight` 效果矩形形狀選項：目前 `spotlight` 效果固定使用圓形聚光燈（`borderRadius: '50%'`）；應新增 `spotlightShape?: 'circle' | 'rect'`（預設 `'circle'`）欄位，`'rect'` 模式移除 borderRadius 50% 改用 `spotlightBorderRadius?: number`（0–32px，預設 8）呈現矩形聚光燈；後端同步更新 `AnimationEffect`/`EffectSchema`（`z.enum(['circle','rect'])`，`z.number().int().min(0).max(32)`）/序列化；前端 `types.ts` 新增欄位；`SlideRenderer.tsx` 依 `spotlightShape` 條件設定 `borderRadius`；`AnimationEditorTab.tsx` 在 spotlight 設定區塊加入 shape 選擇器（circle/rect），選 rect 時額外顯示 borderRadius 數字輸入框；中英文 i18n 新增翻譯鍵。 ✓ 完成於 branch: feature/spotlight-shape-rect-20260617

[x] `highlight-box` 效果虛線邊框樣式：目前 `highlight-box` 的邊框固定為實線；應新增 `highlightBorderStyle?: 'solid' | 'dashed' | 'dotted'`（預設 `'solid'`），允許使用者選擇虛線或點線邊框；後端同步更新 `AnimationEffect`/`EffectSchema`（`z.enum(['solid','dashed','dotted'])`）/序列化；前端 `types.ts` 新增欄位；`SlideRenderer.tsx` 在 `highlight-box` 的 `border` style 中以 `highlightBorderStyle ?? 'solid'` 取代硬編碼 `'solid'`；`AnimationEditorTab.tsx` 在 highlightBorderWidth 輸入框旁加入邊框樣式選擇器（select，選項 solid/dashed/dotted）；中英文 i18n 新增翻譯鍵。 ✓ 完成於 branch: feature/highlight-border-style-20260617

[x] `step-list` 效果邊框顏色：目前 `step-list` 方框沒有外框；應新增 `stepListBorderColor?: string`（CSS hex，選填）欄位，設定後在方框加上 `border: 2px solid {color}` 樣式；後端同步更新 `AnimationEffect`/`EffectSchema`（色碼 regex 驗證）/序列化；前端 `types.ts` 新增欄位；`SlideRenderer.tsx` 條件加入 `border` style；`AnimationEditorTab.tsx` 在 stepListBorderRadius 下方加入帶勾選框的顏色選擇器；中英文 i18n 新增翻譯鍵。 ✓ 完成於 branch: feature/effect-border-radius-color-20260617

[x] `formula` 效果邊框顏色：目前 `formula` 方框沒有外框；應新增 `formulaBorderColor?: string`（CSS hex，選填）欄位，設定後在方框加上 `border: 2px solid {color}` 樣式；後端同步更新 `AnimationEffect`/`EffectSchema`（色碼 regex 驗證）/序列化；前端 `types.ts` 新增欄位；`SlideRenderer.tsx` 條件加入 `border` style；`AnimationEditorTab.tsx` 在 formulaBorderRadius 下方加入帶勾選框的顏色選擇器；中英文 i18n 新增翻譯鍵。 ✓ 完成於 branch: feature/effect-border-radius-color-20260617
...
- 內容: 新增 `spotlight` 效果矩形形狀選項。後端 `pageAnimation.ts` 在 `AnimationEffect` 介面新增 `spotlightShape?: 'circle' | 'rect'` 及 `spotlightBorderRadius?: number` 欄位，`EffectSchema` 分別以 `z.enum(['circle','rect'])` 和 `z.number().int().min(0).max(32)` 驗證，序列化時一併輸出（borderRadius 夾緊至 0-32）。前端 `types.ts` 同步新增兩個欄位；`SlideRenderer.tsx` 依 `spotlightShape ?? 'circle'` 決定 `borderRadius`：`'circle'` 維持 `'50%'`，`'rect'` 改用 `${spotlightBorderRadius ?? 8}px`；`AnimationEditorTab.tsx` 在 spotlightSoftEdge 下方新增形狀選擇器（select，選項 circle/rect），選 rect 時條件顯示圓角半徑數字輸入框（min=0, max=32, step=4）；中英文 i18n 新增 `play.animation.spotlightShape`、`play.animation.spotlightShape.circle`、`play.animation.spotlightShape.rect`、`play.animation.spotlightBorderRadius` 四個翻譯鍵。前後端 TypeScript 均通過。

- 時間: 2026-06-17 12:30:00 +0800
- 分支: feature/highlight-border-style-20260617
- 內容: 新增 `highlight-box` 效果虛線邊框樣式控制。後端 `pageAnimation.ts` 新增 `highlightBorderStyle?: 'solid' | 'dashed' | 'dotted'` 欄位，`EffectSchema` 以 `z.enum(['solid','dashed','dotted'])` 驗證，序列化時直接輸出。前端 `types.ts` 同步新增欄位；`SlideRenderer.tsx` 將 `border: `${hBw}px solid ${hColor}`` 改為以 `hStyle = effect.highlightBorderStyle ?? 'solid'` 動態插入邊框樣式；`AnimationEditorTab.tsx` 在 highlightBorderWidth 輸入框之後新增 select 選擇器（solid/dashed/dotted 選項，使用 `t()` 翻譯）；中英文 i18n 各新增 `play.animation.highlightBorderStyle` 及三個子選項鍵。前後端 TypeScript 均通過。
