...
- [ ] **階段 1c（前端）**：`NotebookPanel` 單 cell 視圖（捲軸容器、command/edit 雙模式）＋`@jupyterlab/services` 連線 hook（lazy-load）＋`↑`/`↓` 切 cell、`Ctrl/Shift+Enter` 執行；在 `SlideRenderer` 的 `renderType` 分流新增 `notebook` 分支。
    ...
    - [ ] 1c-iii（後續）：`useJupyterKernel.ts`（`@jupyterlab/services` lazy-load、`requestExecute`、串流 iopub 接 `applyIopub`）＋`Ctrl/Shift+Enter` 執行＋command/edit 編輯模式；執行結果經 `savePageNotebook` 寫回（屬階段 1d）。
