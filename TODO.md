...
[ ] Skills UI 編輯功能：目前設定頁中的自訂技能只能新增/刪除/啟用停用，無法在建立後修改名稱或指令內容；應在每個自訂技能列表項目加入「編輯」按鈕，點擊後展開行內編輯表單（名稱 input + 指令 textarea + 套用範圍 select + 儲存/取消按鈕），儲存時呼叫 `PATCH /api/skills/:id`，並即時更新列表狀態，不需重新載入頁面。
