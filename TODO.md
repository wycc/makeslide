...
- 驗證：後端 `tsc` 全綠、新測試 11/11、既有 `export-import-notebook` 2/2、`page-operations-permission` 31/31；前端 `tsc`＋`vite build` 通過、前端全套 1114/1114（新增 15 項）。分支 `feat/page-elements`（三個 commit：設計文件／後端／前端），**未 merge、未做實機視覺驗證**。
