...
- 驗證：前端 `tsc --noEmit` 通過、`vite build` 通過、前端全套 1065/1065 通過（含新增的 8 個 i18n 鍵斷言）。分支 `feat/fullscreen-comment-badge`。
