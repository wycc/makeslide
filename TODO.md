...
- [ ] `pdfPageMarkers.ts` 補上單元測試：`backend/src/services/pdfPageMarkers.ts` 的 `formatPdfPageMarker`、`containsPdfPageMarkers`、`stripPdfPageMarkers`、`buildTextWithPdfPageMarkers` 皆為純字串處理函式（regex 替換/trim/join），完全無外部依賴，但目前沒有對應測試。應新增 `backend/test/pdf-page-markers.test.ts`，覆蓋空字串、無 marker、連續多個 marker、`stripPdfPageMarkers` 對多餘空行的 collapse 邏輯，以及頁碼從 1 起算的行為。

- [ ] `logSanitizer.ts` 補上單元測試：`backend/src/services/logSanitizer.ts` 的 `redactLogValue`/`sanitizeString`/`redactTextSummary` 等函式處理敏感欄位遮罩（API key、Bearer token、data URL、長 hex/base64、敏感欄位名單），全為同步純函式但 regex 與 `SAFE_METADATA_KEYS` 白名單邏輯交織複雜，目前無測試，一旦改錯容易造成「忘記遮罩敏感內容」這種安全性退化卻沒有測試能攔截。應新增測試覆蓋欄位名稱大小寫、白名單排除是否生效、巢狀物件/陣列、深度截斷與各 sensitive pattern 是否真的被取代。

- [ ] `imagePromptTemplates.ts` 的 `buildImagePrompt` 補上單元測試：純粹依各參數是否存在決定要不要加入對應描述行的字串組裝函式，無外部依賴，但分支多（多個可選欄位各自獨立判斷），容易在改動時不小心打亂行順序或漏掉某欄位條件導致 prompt 內容跑掉。應新增測試覆蓋「全部欄位皆有」「全部欄位皆無」「部分有/部分無」三種情境，鎖定輸出格式。