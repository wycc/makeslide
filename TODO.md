...
[ ] `text-callout` 效果邊框顏色：目前 `text-callout` 的方框沒有外框；應新增 `textCalloutBorderColor?: string`（CSS hex，選填）欄位，設定後在方框加上 `border: 2px solid {color}` 樣式；後端同步更新 `AnimationEffect`/`EffectSchema`（色碼 regex 驗證）/序列化；前端 `types.ts` 新增欄位、`SlideRenderer.tsx` 條件加入 `border` style；`AnimationEditorTab.tsx` 加入帶勾選框的顏色選擇器；中英文 i18n 新增翻譯鍵。
