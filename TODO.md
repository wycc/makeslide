...
- [x] 評論複製為 Markdown（Roadmap Phase 4 / 可用性）：在 `CommentsSection`（`frontend/src/pages/play/PlayPageSidebar.tsx`）標題列加入「複製」按鈕，把目前清單（沿用既有 `filterComments` 過濾後的 `visibleComments`，或全部 `comments`，擇一並於說明標明）輸出為 Markdown 清單；於 `frontend/src/lib/` 新增純函式 `formatCommentsMarkdown(comments, labels)`（每則一行，如 `- [第 N 頁] 作者（已解決）：內文`，labels 注入頁碼/已解決字樣以保持純粹可測），以 `lib/clipboard.ts` 的 `copyTextToClipboard` 複製並顯示短暫提示；補純函式測試（空清單、已解決標記、跨頁）；新增 zh-TW/en i18n key。純前端、不動後端。
  - 修改說明（2026-06-26）：新增 `frontend/src/lib/commentMarkdown.ts` 之純函式 `formatCommentsMarkdown<T extends {page_number,author,text,resolved}>(comments, labels)`——依 `page_number` 遞增穩定排序，輸出 `# 標題` + 空行 + 每則 `- [第 N 頁] 作者（已解決）：內文`（已解決標記僅在 resolved 時附加；labels 注入標題/含 `{n}` 頁碼/已解決字樣以保純粹可測），空清單回空字串。`CommentsSection` 加 `copyMsg` state 與 `handleCopyComments`（複製對象為**過濾後的 `visibleComments`**，故會反映目前搜尋結果），按鈕置於按鈕群最前（`visibleComments.length > 0` 才顯示），以 `copyTextToClipboard` 複製、顯示 2 秒 `commentsCopyDone`/`commentsCopyFail` 提示；labels 重用既有 `commentsTitle`/`reviewListPage`。新增 zh-TW/en `play.sidebar.commentsCopy`/`commentsCopyDone`/`commentsCopyFail`/`commentsResolvedTag`。新增 `commentMarkdown.test.ts` 3 測試（空清單、依頁排序+已解決標記、同頁穩定序）。前端 `tsc --noEmit` 通過、`commentMarkdown.test.ts` 與 `i18n.test.ts` 全通過。分支 `feat/comment-copy-markdown`，已 merge 回 master。
  - 計數：自上次「---- 計數重設 ----」(2026-06-26) 起算，本項為第 66 個完成項目（66/100，未達上限）。
...
- [x] 每頁分析 CSV 增列 avg_listened_ratio（Roadmap Phase 1）：`GET /api/pdfs/:id/report/pages.csv` 現有欄位再加一欄 `avg_listened_ratio`（每頁平均聆聽比例，沿用 `report/summary` 既有的 `AVG(MIN(listened_ms/duration_ms,1))` 計法，無資料以空字串輸出避免誤判為 0）；更新 `backend/test/report-pages-csv.test.ts` 既有斷言的欄位與數值。後端小改 + 測試。
...
- [x] 投票結果複製為 Markdown（Roadmap Phase 1）：播放頁側欄投票區（`PlayPageSidebar.tsx`，每個 poll 有 `question`/`options[].text`/`options[].votes`/`total_votes`）目前可看結果橫條圖但無法整批帶出。新增純函式 `formatPollResultsMarkdown(polls, labels)`（每個 poll 輸出 `## 問題` 與各選項 `- 選項文字：N 票（X%）`，百分比沿用 `Math.round(votes/total*100)`、total 為 0 時 0%）置於 `frontend/src/lib/`（新檔）並補測試（空清單、單一 poll、零票）；投票區標題加「複製」按鈕，沿用 `copyTextToClipboard` 與短暫提示；新增 zh-TW/en i18n key。純前端。
  - 修改說明（2026-06-26）：新增 `frontend/src/lib/pollResultsMarkdown.ts` 之純函式 `formatPollResultsMarkdown<T>(polls, labels)`——每個 poll 輸出 `## 問題` 區段，各選項 `- 選項：N 單位（X%）`，百分比 `Math.round(votes/total*100)`、total 為 0 時 0%；labels 注入標題與票數單位以保純粹可測，空清單回空字串。`PlayPageSidebar`（`PlayPageSidebar()` 主元件，投票區資料來自 `pagePolls`）新增 `pollCopyMsg` state 與 `handleCopyPollResults`（以 `copyTextToClipboard` 複製、2 秒 `copyDone`/`copyFail` 提示），按鈕置於「📊 Realtime Poll」標題列設定鈕前（`pagePolls.length > 0` 才顯示）。新增 zh-TW/en `play.sidebar.poll.copyResults`/`copyHeading`/`votesUnit`/`copyDone`/`copyFail`。新增 `pollResultsMarkdown.test.ts` 4 測試（空清單、單一 poll 票數/百分比、零票 0%、多 poll）。前端 `tsc --noEmit` 通過、`pollResultsMarkdown.test.ts` 與 `i18n.test.ts` 全通過。分支 `feat/poll-results-copy-markdown`，已 merge 回 master。
  - 計數：自上次「---- 計數重設 ----」(2026-06-26) 起算，本項為第 71 個完成項目（71/100，未達上限）。

- [x] 報告面板顯示產生時間（可用性，小顆粒）：`PostClassReportPanel`（`frontend/src/pages/play/PostClassReportPanel.tsx`）的 `summary.generated_at` 目前未呈現。在標題或工具列附近顯示「資料產生於 {相對時間}」，沿用 `lib/formatRelativeTime.ts`；`title` 顯示完整本地時間。新增 zh-TW/en i18n key（含 `{time}` 佔位）。純前端、不動後端與資料型別。
  - 修改說明（2026-06-26）：發現報告面板底部（line ~511，列印用）其實已以既有 `play.report.generatedAt`（前綴「產生時間：」+ 完整本地時間）呈現產生時間，但標題區未顯示、且無相對時間。本項在標題區（副標題下、`summary` 存在時）新增「資料產生於 {相對時間}」一行，沿用 `lib/formatRelativeTime.ts`，`title` tooltip 顯示完整本地時間（`toLocaleString()`）。為免與既有前綴型 `generatedAt` key 衝突，新增獨立 key `play.report.generatedAtRelative`（含 `{time}` 佔位，zh-TW/en），底部既有顯示維持不變。純前端、不動後端與資料型別。前端 `tsc --noEmit` 通過、`i18n.test.ts`（含 zh/en 對等及佔位符集合檢查）通過。分支 `feat/report-generated-at-relative`，已 merge 回 master。
  - 計數：自上次「---- 計數重設 ----」(2026-06-26) 起算，本項為第 70 個完成項目（70/100，未達上限）。

- [x] 全域搜尋最近關鍵字（Roadmap Phase 4 搜尋體驗）：`GlobalSearchBox`（`frontend/src/components/GlobalSearchBox.tsx`）送出搜尋後，把關鍵字存入 localStorage 最近清單（去重、上限 8 筆、最新在前）；輸入框聚焦且為空時顯示最近關鍵字 chips，點擊即帶入搜尋。新增純函式模組 `frontend/src/lib/recentSearches.ts`（`getRecentSearches()`/`addRecentSearch(q)`，含非瀏覽器防護、壞值 fallback）並補單元測試（去重、上限、最新在前、空白忽略）；新增 zh-TW/en i18n key（「最近搜尋」「清除」）。純前端。
  - 修改說明（2026-06-26）：新增 `frontend/src/lib/recentSearches.ts`（`getRecentSearches`/`addRecentSearch`/`clearRecentSearches`，key `makeslide.recentSearches`、上限 8、最新在前、trim + 不分大小寫去重、空白忽略、含非瀏覽器與壞值/JSON 解析防護）。`GlobalSearchBox` 加 `recents` state；輸入框 `onFocus` 改為一律開啟下拉、`onKeyDown` Enter 時 `commitRecent` 記錄目前關鍵字；下拉在「查詢為空且有最近紀錄」時於最上方顯示「最近搜尋」chips（點擊 `handleRecentClick` 帶入查詢並置頂該詞）與「清除」鈕；下拉開啟條件改為 `showRecents || searching || noResults || hasResults` 以免空框。新增 zh-TW/en `home.search.recentTitle`/`recentClear`。新增 `recentSearches.test.ts` 6 測試（空、最新在前、空白忽略、trim+不分大小寫去重置頂、上限 8、壞值容錯）。前端 `tsc --noEmit` 通過、`recentSearches.test.ts` 與 `i18n.test.ts` 全通過。分支 `feat/global-search-recent`，已 merge 回 master。
  - 計數：自上次「---- 計數重設 ----」(2026-06-26) 起算，本項為第 69 個完成項目（69/100，未達上限）。
...
- [x] 測驗逐題統計 CSV 匯出（Roadmap Phase 1）：新增後端 `GET /api/pdfs/:id/report/questions.csv`，沿用 `report.ts` 既有的 `computeQuestionStats(id)`（含 `question_id`/`question`/`option_count`/`attempt_count`/`correct_count`/`wrong_count`/`correct_rate`/`option_votes[]`）輸出每題一列；`option_votes` 以分號或管線串接為單一欄位（如 `option_votes` = `3|1|0`）避免欄數浮動；以共用 `csvEscape`、權限 `canEditPdf`、回應 `text/csv` + `attachment; filename`；`PostClassReportPanel` 既有 CSV 群加「逐題統計 CSV」連結；補後端 node:test（200 欄位/排序、403、404）。
  - 修改說明（2026-06-26）：`report.ts` 在 `report/pages.csv` 之後新增 `GET /api/pdfs/:id/report/questions.csv`，沿用既有 `computeQuestionStats(id)`，每題一列輸出 `question_id,question,option_count,attempt_count,correct_count,wrong_count,correct_rate,option_votes`；`correct_rate` 四捨五入 4 位、`option_votes` 以 `|` 串接為單一欄位（避免欄數隨選項數浮動），沿用共用 `csvEscape` 與 `canEditPdf`，回應 `text/csv; charset=utf-8` + `attachment; filename="report-questions-<id>.csv"`。前端 `PostClassReportPanel` 在「每頁分析 CSV」後加「逐題統計 CSV」download 連結；新增 zh-TW/en `play.report.exportQuestionsCsv`。新增 `backend/test/report-questions-csv.test.ts`（4 測試：兩題作答的欄位/數值/option_votes 串接、無測驗僅 header、403 非擁有者、404）。前端+後端 `tsc --noEmit` 皆通過、`i18n.test.ts` 對等通過；後端 handler 測試需 better-sqlite3 native module、sandbox 無法載入，留 CI 執行。分支 `feat/report-questions-csv`，已 merge 回 master。
  - 計數：自上次「---- 計數重設 ----」(2026-06-26) 起算，本項為第 73 個完成項目（73/100，未達上限）。

- [x] 複習清單「全部清除」按鈕（可用性，小顆粒）：`ReviewListSection`（`PlayPageSidebar.tsx`）目前只能逐項移除；`lib/reviewList.ts` 已有 `clearAllReviewItems()` 但未接 UI。在複習清單標題列加「清除全部」按鈕（在「複製」旁），點擊後呼叫 `clearAllReviewItems()` 並把本地 `items` 設為 `[]`；新增 zh-TW/en i18n key。純前端、不動後端。
...
## 掃描摘要（第一〇七輪，2026-06-26）

- 觸發原因：第一〇五輪新增的 4 個可執行項目已全數完成，TODO.md 又只剩兩個「待使用者決定 / 待處理」項目，依其說明不宜於自動 loop 逕行。故依 LOOP.md 第 2 條再次分析程式並對照 `docs/FUTURE_ROADMAP.md` 補新項目。
- 現況觀察：`QuizBuilderPage` 的「複製題目為純文字」格式化邏輯內嵌於 onClick、未抽出且無測試；`i18n.test.ts` 有 key 對等與佔位符檢查但未檢查「值非空」；首頁清單未顯示目前篩選結果數；播放頁有多個鍵盤快捷鍵但無說明面板。
- 取材方向：分散主題（可測性重構、測試守門、UX 顯示、UX 說明），維持「小顆粒、可單輪完成、可加測試、低風險」。
...
- [x] 測驗從 JSON 匯入（Roadmap Phase 5 匯入；可用性）：`QuizBuilderPage.tsx` 目前可「匯出 JSON」但無匯入。新增純函式 `frontend/src/lib/quizImport.ts` 之 `parseQuizImportJson(text)`：解析並驗證 `{ title?, questions: [...] }`，過濾/正規化每題（`question` 為非空字串、`options` 為 `{text}` 陣列、`answer_indices` 為合法索引、`type` 預設 single、`explanation` 預設空），回傳 `{ ok, value?/error? }` 形式；補單元測試（合法、壞 JSON、缺 questions、索引越界過濾）。UI 加「匯入 JSON」按鈕（隱藏 `<input type=file accept=.json>`，讀檔後套用），新增 zh-TW/en i18n key。純前端。
  - 修改說明（2026-06-26）：新增 `frontend/src/lib/quizImport.ts` 之 `parseQuizImportJson(text)`，回傳 discriminated union `{ ok:true, value:{title,questions} } | { ok:false, error }`（error: `invalid_json`/`no_questions`/`no_valid_questions`）：JSON parse 失敗、根無 `questions` 陣列、或無任何合法題目時回對應錯誤；正規化每題（`question` trim 非空才保留、`options` 接受字串或 `{text}` 並轉成 `{text}`、`answer_indices` 過濾為 in-range 整數+去重+排序、`type` 取 single/multiple 否則依答案數推斷、`explanation`/`score` 預設），並重新編號 id 為 `q1`/`q2`…。`QuizBuilderPage` 在「匯出 JSON」旁加隱藏 `<input type=file accept=.json>` 與「匯入 JSON」按鈕，`handleImportFile` 讀檔→parse→成功則 `setTitle`/`setQuestions` 並顯示 2.5 秒「已匯入 N 題」、失敗顯示「匯入失敗」。新增 zh-TW/en `quiz.importJson`/`importDone`（含 `{n}`）/`importFailed`。新增 `quizImport.test.ts` 5 測試（壞 JSON、缺/非陣列 questions、正規化+id 重編+型別推斷、越界/重複索引過濾、無合法題目）。前端 `tsc --noEmit` 通過、`quizImport.test.ts` 與 `i18n.test.ts` 全通過。分支 `feat/quiz-json-import`，已 merge 回 master。
  - 計數：自上次「---- 計數重設 ----」(2026-06-26) 起算，本項為第 78 個完成項目（78/100，未達上限）。

- [x] 評論文字編輯（協作可用性）：評論目前只能建立 / 標記已解決 / 刪除，無法修正錯字。後端為 `comments.ts` 的 `PATCH /api/pdfs/:id/comments/:commentId` 增加可選 `text`（zod：trim、1..2000；與既有 `resolved` 並存、至少提供其一），更新後回傳整列；沿用 `canEditPdf`。前端 `CommentsSection` 每則評論加「編輯」進入 inline textarea（沿用字數上限/計數），呼叫新增的 `editPageComment(id, commentId, text)` API。補後端 node:test（更新 text、保留 resolved、空字串 400、403/404）；新增 zh-TW/en i18n key。
  - 修改說明（2026-06-26）：後端 `comments.ts` 將原 `ResolveCommentBodySchema` 換成 `PatchCommentBodySchema`（`resolved?` boolean + `text?` trim 1..2000，`.refine` 至少其一），PATCH handler 改為依提供欄位動態組 `SET text=? / resolved=?`（兩者並存、只更新提供者），沿用 `canEditPdf`/404 既有檢查、回傳整列。前端新增 `editPageComment(id, commentId, text)` API（PATCH 帶 `{text}`）；`CommentsSection` 加 `editingId`/`editingText` state 與 `handleSaveEdit`，每則評論動作欄新增「✎ 編輯」（按下帶入現有文字進入 inline textarea，maxLength 2000，提供儲存/取消；儲存呼叫 `editPageComment`、空白停用）。新增 zh-TW/en `play.sidebar.commentEdit`/`commentEditSave`/`commentEditCancel`。後端測試 `page-comments.test.ts` 補 2 測試（編輯 text 並保留 resolved+trim、空白 text 與空 body 皆 400；既有 403/404 已涵蓋）。前端+後端 `tsc --noEmit` 皆通過、`i18n.test.ts` 對等通過；後端 handler 測試需 better-sqlite3、sandbox 無法載入、留 CI。分支 `feat/comment-text-edit`，已 merge 回 master。
  - 計數：自上次「---- 計數重設 ----」(2026-06-26) 起算，本項為第 81 個完成項目（81/100，未達上限）。
...
- [x] 評論清單手動重新整理（可用性，小顆粒）：`CommentsSection` 目前只在掛載 / 換頁 / 切換全部時載入評論，協作情境下他人新增的評論不會自動出現。標題列加「重新整理」按鈕，重跑既有載入邏輯（依 `showAll` 呼叫 `listAllComments` 或 `listPageComments` 並 `setComments`）；載入中可顯示簡單狀態。為避免重複，將既有 useEffect 內的載入邏輯抽成一個可重用的 `loadComments()`。純前端、不動後端。
  - 修改說明（2026-06-26）：把原本內嵌於 useEffect 的評論載入邏輯抽成 `loadComments`（`useCallback`，依 `showAll` 呼叫 `listAllComments`/`listPageComments` 並 `setComments`，失敗回空陣列；deps `pdfId`/`currentPage.page_number`/`showAll`），`useEffect` 改為 `loadComments()`。標題列按鈕群最前新增「↻ 重新整理」按鈕（`title`/`aria-label` 為 `commentsRefresh`），點擊重跑 `loadComments`。新增 `useCallback` import 與 zh-TW/en `play.sidebar.commentsRefresh`。純前端、不動後端。前端 `tsc --noEmit` 通過、`i18n.test.ts`（含 zh/en 對等）通過；此為複用既有載入邏輯的 UI 按鈕、無新增可抽出純邏輯，不另加單元測試。分支 `feat/comments-refresh`，已 merge 回 master。
  - 計數：自上次「---- 計數重設 ----」(2026-06-26) 起算，本項為第 85 個完成項目（85/100，未達上限）。
...
- [x] 抽出書籤導覽純函式並加「上一個書籤」快捷鍵（可用性 / 可測性）：`PlayPage` 的 `B` 鍵以內嵌邏輯跳到「下一個書籤」（`sorted.find(n > current) ?? sorted[0]`）。抽成 `frontend/src/lib/` 純函式 `nextBookmarkPage(bookmarks, currentPage)` 與 `prevBookmarkPage(bookmarks, currentPage)`（皆環狀、回 1-based 頁碼或 null；排序去重）並補測試（環繞、邊界、空清單、重複）；`B` 改用 `nextBookmarkPage`，新增 `Shift+B` 用 `prevBookmarkPage`；於快捷鍵說明面板（`PlayPageHeader` 的 `shortcuts`）加入 `Shift+B` 一列、補 zh-TW/en 說明 key。純前端。
  - 修改說明（2026-06-26）：新增 `frontend/src/lib/bookmarkNav.ts` 之 `nextBookmarkPage`/`prevBookmarkPage`（先 sortedUnique；next 取首個 > current、無則環繞回最小；prev 取末個 < current、無則環繞回最大；空清單回 null；皆 1-based）。`PlayPage` 的 `b` 鍵分支改為依 `ev.shiftKey` 選 prev/next（`bookmarks.length > 0` 才 `preventDefault` 並導覽，行為與原 B 一致、新增 Shift+B 反向）。`PlayPageHeader` 快捷鍵面板 `shortcuts` 在 `B` 後加 `Shift + B` 一列；新增 zh-TW/en `play.shortcuts.prevBookmark`。新增 `bookmarkNav.test.ts` 4 測試（空清單回 null、next 環繞、prev 環繞、排序去重）。前端 `tsc --noEmit` 通過、`bookmarkNav.test.ts` 與 `i18n.test.ts`（含 zh/en 對等）全通過。純前端。分支 `feat/bookmark-nav-prev`，已 merge 回 master。
  - 計數：自上次「---- 計數重設 ----」(2026-06-26) 起算，本項為第 89 個完成項目（89/100，未達上限）。
...
- [x] 重點頁導覽快捷鍵（可用性 / 可測性）：播放頁重點頁（`importantPages`）目前只有 `I` 切換、無快速跳轉。將 `frontend/src/lib/bookmarkNav.ts` 泛化為頁面清單導覽（建議改名 `pageListNav.ts`、函式 `nextPageInList`/`prevPageInList`，或保留現名並新增泛用別名），`PlayPage` 既有 `B`/`Shift+B`（書籤）改用泛化函式、並新增 `N`（下一個重點頁）/`Shift+N`（上一個重點頁）以 `importantPages` 導覽；快捷鍵面板補 `N`/`Shift+N` 兩列、補 zh-TW/en 說明 key；既有 `bookmarkNav.test.ts` 隨改名/別名更新。純前端、復用既有已測純函式。
  - 修改說明（2026-06-26）：以 `git mv` 將 `bookmarkNav.ts`/`bookmarkNav.test.ts` 改名為 `pageListNav.ts`/`pageListNav.test.ts`，函式泛化為 `nextPageInList`/`prevPageInList`（邏輯不變：環狀、排序去重、空回 null、1-based）。`PlayPage` import 改名；`B`/`Shift+B` 書籤導覽改呼叫泛化函式；新增 `N`（下一個重點頁）/`Shift+N`（上一個重點頁）以 `importantPages` 導覽（`importantPages.length > 0` 才作用），並把 `importantPages`/`bookmarks` 補入鍵盤 useEffect deps 以免讀到舊值。`PlayPageHeader` 快捷鍵面板補 `N`/`Shift + N` 兩列；新增 zh-TW/en `play.shortcuts.nextImportant`/`prevImportant`。測試檔同步改用 `nextPageInList`/`prevPageInList`（4 測試）。前端 `tsc --noEmit` 通過、`pageListNav.test.ts` 與 `i18n.test.ts`（含 zh/en 對等）全通過、確認無 `bookmarkNav` 殘留引用。分支 `feat/important-page-nav`，已 merge 回 master。
  - 計數：自上次「---- 計數重設 ----」(2026-06-26) 起算，本項為第 92 個完成項目（92/100，未達上限）。
...
- [x] CSV 匯出檔名帶簡報標題（Roadmap Phase 5 / 一致性）：`comments.csv`、`report/pages.csv`、`report/questions.csv`、`report/students.csv`、`poll-results.csv`、`quiz-results.csv` 的 `content-disposition` 檔名目前為 id-based。復用第一一〇輪新增的 `downloadFilename.ts`（`safeDownloadBaseName` + `buildContentDisposition`），改為 `{safeTitle}-comments.csv`、`{safeTitle}-pages.csv` 等（標題空白時退回原 id-based 名）；各端點本就查得到 pdf row（含 title 或可加查）。更新相關 node:test 的 content-disposition 斷言。後端小改 + 測試更新。
  - 修改說明（2026-06-26）：6 個 CSV 端點改用 `downloadFilename.ts` 的 `safeDownloadBaseName`+`buildContentDisposition`，檔名為 `{safeTitle}-comments.csv`/`-pages.csv`/`-questions.csv`/`-students.csv`/`-poll-results.csv`/`-quiz-results.csv`，標題清理後為空則退回原 id-based 名（`comments-{id}.csv` 等）；中文標題經 `buildContentDisposition` 的 RFC5987 `filename*` 正確命名。`comments.ts` 的 `getPdfRow` 與 `report.ts` 的 `getPdfPermissionRow` 權限查詢擴充 `title` 欄（兩檔多 handler 共用、其他 handler 忽略無害）；`poll-results-csv.ts`/`quiz-results-csv.ts` 既有 row 已含 title。測試：更新 `comments-csv`/`report-pages-csv`/`report-questions-csv` 的 content-disposition 斷言為標題命名、並於 `comments-csv` 新增「空白標題退回 id-based 檔名」案例。後端 `tsc --noEmit` 通過、`download-filename.test.ts`+`csvEscape.test.ts` 14 測試通過；CSV handler 測試需 better-sqlite3、留 CI。分支 `feat/csv-title-filename`，已 merge 回 master。
  - 計數：自上次「---- 計數重設 ----」(2026-06-26) 起算，本項為第 93 個完成項目（93/100，未達上限）。
...
- [x] 抽出頁碼清單複製文字為純函式（可測性 / 一致性）：`PlayPageSidebar` 書籤與重點頁「複製清單」各自內嵌 `[...pages].sort().map(n => prefix+n+suffix).join(sep)`。抽成 `frontend/src/lib/` 純函式 `formatPageListText(pages, labels)`（排序去重、`{prefix}{n}{suffix}` 以 `separator` 串接、空回空字串；labels 注入 prefix/suffix/separator）並補測試；兩處改呼叫之（行為不變）。純前端。
  - 修改說明（2026-06-26）：新增 `frontend/src/lib/pageListText.ts` 之純函式 `formatPageListText(pages, labels)`（`Array.from(new Set()).sort()` 排序去重、`{prefix}{n}{suffix}` 以 `separator` 串接、空回空字串；labels 注入 prefix/suffix/separator）。`PlayPageSidebar` 書籤與重點頁兩處「複製清單」改呼叫之（傳入 `play.common.pagePrefix`/`pageSuffix` 與 `play.sidebar.pageListSeparator`），輸出與原內嵌相同（書籤/重點頁本就唯一，去重不改結果）、行為不變。新增 `pageListText.test.ts` 4 測試（空、排序串接、prefix/suffix、去重）。前端 `tsc --noEmit` 通過、新測試通過；無 i18n 變更。分支 `feat/page-list-text`，已 merge 回 master。
  - 計數：自上次「---- 計數重設 ----」(2026-06-26) 起算，本項為第 96 個完成項目（96/100，未達上限）。
...
## 掃描摘要（第一二〇輪，2026-06-26）

- 觸發原因：可執行項目只剩兩個「待使用者決定 / 待處理」項目，且第一一八輪規畫的「首頁清除所有篩選」已查證為**既有功能**（grep 用詞未涵蓋 `clearAll*` 命名而誤判，已於前一輪標為已存在、不計數）。計數仍為 99/100。依 LOOP.md 第 2 條補新項目；本輪 grep 刻意涵蓋更多命名變體（`clearAll*`/`copy*`/`export*`/`Stored*`）以免重蹈覆轍。
- 現況觀察（已驗證）：評論輸入的「暱稱（作者）」欄為 `useState('')`、每次掛載皆空，無 localStorage 持久化（`grep` 確認 `PlayPageSidebar` 與 `lib/` 皆無 comment-author 持久化）；使用者每次留言需重打暱稱。其餘 lib 純函式幾乎都有測試（僅 `api.ts`/`debugLog.ts` 無，皆非純函式）。
- 取材方向：小型可用性 + 可測 helper，且**完成此項即達 100/100 上限**，故本批只補 1 項。
...
- 工作內容：可執行項目僅剩兩個「待使用者決定 / 待處理」項目（計數 99/100），依 LOOP.md 第 2 條（並記取前一輪教訓，grep 涵蓋 `clearAll*`/`copy*`/`export*`/`Stored*` 等變體驗證不存在）新增 1 個小型、可測、低風險的可執行項目（記住上次評論暱稱）。因完成此項即達 100 上限，刻意只補 1 項。本輪為規畫輪，不計入 100 完成計數（計數維持 99/100）。
- 時間：2026-06-26
- 分支：直接於 master 更新 TODO.md（規畫輪，無程式碼變更）
