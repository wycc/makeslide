...
- [ ] `detail.ts` 媒體與內容讀取端點補上讀取權限檢查（影響範圍最大，建議拆成獨立 PR）：以下 11 個 `GET` 端點目前都只檢查資源是否存在，完全沒有檢查請求者是否有權限看這份簡報——`GET /api/pdfs/:id/cover`、`GET /api/pdfs/:id/cover/thumbnail`、`GET /api/pdfs/:id/video`、`GET /api/pdfs/:id/outline`、`GET /api/pdfs/:id/source-audio`、`GET /api/pdfs/:id/pages/:n/image`、`GET /api/pdfs/:id/pages/:n/thumbnail`、`GET /api/pdfs/:id/pages/:n/text`、`GET /api/pdfs/:id/pages/:n/script`、`GET /api/pdfs/:id/pages/:n/audio`、`GET /api/pdfs/:id/pages/:n/polls`。任何人只要知道 PDF id（與頁碼）就能直接讀取別人私有簡報的投影片圖片、逐字稿全文、語音檔案、原始 YouTube 來源音訊等核心內容，完全繞過 `GET /api/pdfs/:id` 本身的權限檢查（因為這些端點各自獨立、不需要先呼叫過 `/api/pdfs/:id`）。應比照本檔案 `GET /api/pdfs/:id` 與其他路由檔案的既有慣例，逐一補上 `canReadPdf()`/`shareAccessForPdf()` 檢查（這 11 個端點修法完全相同，只是套用位置不同），並新增測試覆蓋非擁有者對 private 簡報應得 403、擁有者/`public`/分享 token 讀取應正常回應。建議拆成 2-3 個較小的 PR（例如先處理 `pages/:n/image`、`pages/:n/script`、`pages/:n/audio`、`pages/:n/text` 這四個最常被存取的核心內容端點，再處理封面/影片/大綱/來源音訊/投票清單）。
