...
- [ ] `detail.ts` 其餘媒體端點補上讀取權限檢查（第二批，剩餘 6 個）：`GET /api/pdfs/:id/cover`、`GET /api/pdfs/:id/cover/thumbnail`、`GET /api/pdfs/:id/video`、`GET /api/pdfs/:id/outline`、`GET /api/pdfs/:id/source-audio`、`GET /api/pdfs/:id/pages/:n/polls` 同樣完全沒有讀取權限檢查。修法與第一批完全相同（補上 `canReadPdf()`/`shareAccessForPdf()`），只是套用位置不同；嚴重性略低於第一批（封面/縮圖/投票多半是「列表預覽」用途，影片/大綱/來源音訊則是較少被存取的附屬內容），但仍應修正以維持一致性。
