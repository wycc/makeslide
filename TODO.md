...
- [ ] **階段 1：MCP 加 React 頁工具**（`get_page_react_slide`／`set_page_react_slide`／`get_react_slide_contract`）。目前 `mcp-server.ts` 有 40+ 個工具但 react-slide 一個都沒有，外部 agent 根本碰不到這種頁面。`get_react_slide_contract()` 回傳 MakeSlide 當下的規格（元件契約、CSP 允許/禁止清單、主題 token、畫布尺寸），讓 agent 不必猜、也不必把規格寫死在自己的提示詞裡——這是「用外部 agent 取代內建 coding agent」能成立的前提。純包裝既有端點，不觸碰執行路徑。
