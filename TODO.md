...
## 8 新功能
* 可以使用 javascript 產生動態的內容。我們可以在提示詞中手動要求，或是由 LLM 自動決定使用動態內容工具。每個動態內容放在 iframe 中獨立執行。
* 提供簡執模式，不撥放語音，頁面也不自動跳，由使用者手動控制。支援錄音模式，會把講者的語音錄下來，使用 ASR 轉錄成文字存下來。可以在產生逐字稿時選擇附在每頁提示詞中。
* 可以選用 gemini API。讓不同功能可以選用不同模型。在系統設定檔中提供。
* 選擇性附上其它文件做為素材參加
* 讓 LLM 使用工具上網查資料
* 將網路改用英文