...
- [ ] `quizzes.ts` 的測驗作答提交改為伺服器端重新計算分數，不信任前端送來的 `score`：`POST /api/pdfs/:id/quizzes/:quizId/attempts` 應該讀取該 quiz 的 `questions_json`（含每題 `answer_indices` 與可選的逐題配分），用送出的 `answers` 自行重新計算分數後存入 DB，忽略或僅做為參考用途記錄請求體中的 `score` 欄位；需保留前端目前的計分邏輯（`QuizBuilderPage.tsx` 的 `calcQuestionScore()`/`normalizeQuestionScores()`，單選/多選計分規則、未設定配分時的預設值）在後端重新實作或抽出共用，確保前後端算出的分數一致；新增後端測試覆蓋送出錯誤答案但聲稱滿分時，伺服器回傳/儲存的分數應為伺服器自行算出的正確值而非前端宣稱值。
- 實作前發現範圍更深的根因（已併入同一待辦處理）：`backend/src/routes/pdfs/quizzes.ts` 第 58-66 行的 `QuizQuestionSchema` 完全沒有定義 `score` 欄位，但前端 `QuizQuestion` 型別（`frontend/src/types.ts` 第 514 行）與 `QuizBuilderPage.tsx`（約 862-877 行「每題配分」輸入框）確實有讓老師設定逐題自訂配分的 UI。因為 Zod `z.object()` 預設會在 `.parse()`/`.safeParse()` 時去掉 schema 沒定義的欄位，`PUT`/`POST /api/pdfs/:id/quizzes` 儲存題目時與 `GET /quizzes` 讀回題目時都會經過這個 schema，導致老師設定的逐題自訂配分**從未真正被持久化**，重新整理頁面或下次讀取就會消失、回退成預設平均分配——這是先前任何一輪都沒發現的功能性缺陷（不是權限或安全問題，是資料遺失），必須先補上 `score` 欄位才能讓伺服器端重新計分時讀到老師真正設定的配分。
