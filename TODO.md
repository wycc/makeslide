...
- [ ] 在伺服器啟動時偵測並標記「依提示詞新增頁面」任務崩潰後遺留的孤兒頁面為 `failed`：應在 `rescanPendingOnStartup()` 同一個啟動流程中（或新增一個姊妹函式），找出 `pdfs.status = 'ready'` 但其 `pages` 中仍有頁面狀態不是 `audio_ready`/`failed` 的 PDF（依上述已驗證的不變量，這種組合只可能是新增頁面任務被中斷的孤兒資料），把這些頁面的 `status` 更新為 `failed` 並寫入清楚的 `error_message`（例如「新增頁面流程因伺服器重啟而中斷，請重新整理頁面或手動重新生成」），讓使用者至少能在介面上看到明確錯誤而不是無限卡住的空白頁；不需要嘗試自動續做或恢復原本的任務進度（範圍刻意縮小，避免引入更複雜的續傳邏輯風險）。新增測試覆蓋：啟動時應正確找出並標記這類孤兒頁面、不應誤標記正常情境（例如主管線本身仍在 `processing` 中、或頁面確實已 `audio_ready`/原本就 `failed` 的 PDF）。
