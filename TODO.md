...
- [x] **ZIP 匯入挾帶程式碼的漏洞**（分支 `fix/react-slide-import-recompile`；查證設計文件待決事項時發現，與互動頁無關、是當時就存在的問題，故先行修掉）：匯出把 `.slide.jsx` 與 `.slide.js` 都打包、匯入整個目錄複製，而讀取路徑**只在 compiled 缺失時**才重新編譯——所以一份手工構造的 ZIP 可以讓 `.slide.jsx` 看起來人畜無害、`.slide.js` 是完全不同的東西，而沙箱執行的正是後者；存檔時的 deny list 完全碰不到這條路徑，在 CSP 補上之前那段程式碼還能自由外連。改為匯入時一律從各頁自己的原始碼重新編譯（走與正常存檔相同的 `validateAndCompileReactSlide`，長度上限／deny list／契約檢查一次到位），原始碼編不過的頁面把過期的 `.slide.js` **刪除**而非留著——留著就是這個漏洞本身；該頁保留原始碼（可修）與 JPG（畫面仍有東西），讀取路徑之後會按需重編。另修掉 `react-slides.json` 沒被列入 `SIDECAR_FILES` 的疏漏（其餘 sidecar 都有，導致它被原樣複製進新簡報的儲存目錄）。新增 3 個測試涵蓋「竄改的 .slide.js 被原始碼取代」「原始碼過不了 deny list 時不留下編譯檔」「編不過的頁被拒且不影響其他頁」。驗證：後端 tsc、新測試 3/3、匯出匯入與 React 相關回歸 12/12 ＋ 純函式 44/44，0 失敗。
