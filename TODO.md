...
[ ] `highlight-box` 效果半透明填充色：目前 `highlight-box` 效果只有外框線，框內無任何填充；應新增 `highlightFillColor?: string`（CSS hex，選填，例如 `'#ef444430'` 表示 19% 透明紅色）欄位，讓使用者在動畫編輯器中設定框內的半透明填充顏色；後端同步更新 `AnimationEffect`/`EffectSchema`（同色碼 regex 驗證）/序列化；前端 `types.ts` 新增欄位、`SlideRenderer.tsx` 在 `highlight-box` div 的 `backgroundColor` style 中加入 `highlightFillColor`（預設 `'transparent'`）；`AnimationEditorTab.tsx` 在邊框顏色選擇器後新增一個帶勾選框的顏色選擇器（類似 shapeFillColor 的實作）；中英文 i18n 新增翻譯鍵。
