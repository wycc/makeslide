...
- [ ] **階段 1c（前端）**：`NotebookPanel` 單 cell 視圖（捲軸容器、command/edit 雙模式）＋`@jupyterlab/services` 連線 hook（lazy-load）＋`↑`/`↓` 切 cell、`Ctrl/Shift+Enter` 執行；在 `SlideRenderer` 的 `renderType` 分流新增 `notebook` 分支。
    ...
    - [ ] 1c-iii-b（後續）：`useJupyterKernel.ts` hook——`import('@jupyterlab/services')` lazy-load（需加該相依＋`@lumino/*` peer）、`ServerConnection.makeSettings`、per-file `KernelManager`/kernel registry、`requestExecute` 串流 iopub 經 `iopubMessageFrom`→`applyIopub` 即時更新輸出；`NotebookPanel` 接 `Ctrl/Shift+Enter` 執行＋command/edit 編輯模式。需真實 Jupyter server 端到端驗證。
    - [ ] 1c-iii-c（後續，屬階段 1d）：執行結果經 `savePageNotebook`（debounce）寫回 `.ipynb`。
- [ ] **階段 1d**：無音訊頁處理（TTS／播放計時／就緒判定把 `render_type==='notebook'` 視為無語音頁略過）；執行結果即時寫回 `.ipynb`。
    ...
    - [ ] 1d-ii（後續）：前端播放計時／就緒判定（`playbackReadiness.ts`）／總時長統計／自動換頁把 notebook 頁視為無語音頁（notebook 頁已無 `audio_url` 故播放自然略過，待接 kernel 後一併驗證互動頁的自動換頁行為）。
    - [ ] 1d-iii（後續）：執行結果即時經 `savePageNotebook` 寫回 `.ipynb`（同 1c-iii-c，需 kernel）。
