...
- [ ] **階段 2**：完整輸出（markdown/raw cell、image/html/latex、kernel 狀態列、重啟/清除）。
    ...
    - [ ] 2b-ii（後續）：HTML 輸出改走 sandbox iframe（現以逸出文字顯示）。
- [ ] **階段 3**：cell 內容編輯、語法 highlight（CodeMirror）。
    ...
    - [ ] 3b（後續）：以 CodeMirror 取代 textarea 提供語法 highlight（JupyterLab 同款，lazy-load）。
