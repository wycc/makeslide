...
[ ] 依 `docs/pdf-figure-extraction-design.md` §12 的 V2 設計，實作向量圖形萃取：偵測純向量繪圖區域（追蹤 `OPS.constructPath` 並聚類）、多子圖群組化與 caption 比對、改用 poppler 整頁 render + sharp 裁切輸出向量圖表、過濾被向量內容完全遮蓋的 raster 殘留影像，讓 `hRUVHXrNqW`（`2605.29548v2.pdf`）的 Figure 1-9 等純向量/混合圖表能被正確抽取並配對到 caption。
