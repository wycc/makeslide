...
[ ] Skills 整合至 regenerate 流程：目前技能 prompt 只在初次生成逐字稿（pipeline.ts）時注入，`regenerate.ts` 的 `runRegenerateScripts` 函式在重新生成逐字稿時並未讀取啟用技能；應在 `runRegenerateScripts` 中加入與 pipeline.ts 相同的技能合併邏輯（`getEnabledSkillPrompts` + 合併至 `userPrompt`），確保重新生成時也套用使用者的技能設定。
