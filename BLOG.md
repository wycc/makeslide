...
## 修正播放頁的頁碼深連結（`?page=`）失效，並把複習清單接上 AI 導師

### 背景

盤點「AI 導師自學模式入口正式化」這條待辦時，發現它想要的「測驗後個人化複習清單」「答錯題回看」其實早就
做好了——但也意外發現一個影響面比這條待辦本身更廣的既有 bug：播放頁（`PlayPage`）從來沒有讀取過網址上的
`?page=` 參數。不管是測驗答錯題的「回看這一頁」連結、品質檢查面板的跳頁連結、搜尋結果「開新分頁」，還是
側邊欄觀看紀錄的跳頁按鈕，點下去都只會靜默地停在第 1 頁，完全沒有真的跳到目的頁。

### 使用方式

不需任何操作：現在點擊上述任何一種「跳到第 N 頁」連結，播放頁都會正確打開在該頁，而不是回到第一頁。

另外，複習清單（測驗答錯且有頁碼的題目會自動加入）現在每個項目多了一顆「問 AI 導師」按鈕：點下去會跳到
那一頁、把題目文字預先填進 AI 導師的提問框，並直接切到側邊欄「AI 助手」分頁的「導師」子分頁——複習一題答
錯的題目、順手問 AI 導師，一個按鈕就完成，不用先跳頁再自己找 AI 分頁。

### 實作重點

- `PlayPage.tsx` 原本「首次載入時從 localStorage 回復播放進度」的 effect，現在會先檢查網址的 `?page=`
  （複用既有已測的 `parseGotoPage`），有就優先套用並跳過進度回復——顯式深連結是明確的目的地，理應蓋過
  「接續上次看到哪」。
- 新增 `OPEN_AI_TUTOR_EVENT`（比照既有讓 header 品質徽章跳轉到品質子分頁的 `OPEN_QUALITY_PANEL_EVENT`
  模式），複習清單的「問 AI 導師」鈕觸發後由 `PlayPageSidebar` 監聽並切分頁；題目文字預填則直接透過
  context 的 `setPageAskInput`。

分支：`fix/playpage-page-query-param`。前端 `tsc`、前端測試 818/818（含 i18n parity）、`vite build` 通過。

## 首頁搜尋結果可以批次加入書籤

### 背景

首頁全域搜尋（`GlobalSearchBox`）選取模式下原本已經有兩個批次動作：「建立新簡報」（把選中的頁面組成一份新
簡報）與「加入複習清單」。但盤點 roadmap 建議時發現，「把搜尋結果加進收藏」這件事其實還沒有任何入口——
現有的書籤功能只存在播放頁內，且是針對「目前這一份簡報」，橫跨多份簡報搜尋出來的結果完全碰不到它。

### 使用方式

首頁搜尋結果按「多選頁面」進入選取模式、勾選想要的頁面後，工具列會多一顆「加入書籤（N 頁）」按鈕。按下去
後，這些頁面會分別寫進各自所屬簡報的書籤清單；之後打開該簡報的播放頁，側邊欄與投影片面板本來就會顯示的
書籤標記就能看到剛剛加入的頁面。

### 實作重點

- 沒有新增後端 API 或資料表——書籤原本就是純前端、per-deck 存在 `localStorage`（
  `makeslide.bookmarks.<pdfId>`），播放頁的 `toggleBookmark` 一直都是讀寫這個 key。新動作只是把同一把
  key 從另一個入口（搜尋結果）寫進去。
- 搜尋結果可能橫跨多份簡報，批次動作依 `pdf_id` 分組後，每份簡報各自讀出既有書籤清單、把新頁碼併入
  （用 `Set` 去重＋排序）再寫回，而不是逐頁呼叫播放頁那種「切換」語意的 `toggleBookmark`——批次「加入」
  應該是冪等的，不會因為某頁本來就已加過書籤而被誤刪。
- 複用既有已測的 `readNumberArrayFromStorage`（安全讀取 localStorage 中的數字陣列）。

分支：`feat/search-add-to-bookmarks`。前端 `tsc`、前端測試 818/818（含 i18n parity）、`vite build` 通過。
...
notebook 頁一次只顯示一個 cell，而且是固定高度、內部捲動的。如果某一格印出成百上千行、或丟出很長的錯誤
traceback，會把整個檢視塞滿、很難操作。這一步讓過長的輸出自動折疊。

### 使用方式

不需任何設定：當一格的文字輸出或錯誤訊息超過 16 行時，只會先顯示前 16 行，下方出現「**顯示其餘 N 行**」；
按下即可展開，再按「**收合**」收回。短輸出、圖片、表格（HTML）、數學式不受影響。

### 實作重點

- 新增純函式 `collapseText`（取前 N 行並回報被藏起來的行數；內容本來就短、或參數無效時原樣返回），有單元測試。
- `NotebookPanel` 的輸出區以一個可折疊元件包住文字／錯誤輸出；錯誤 traceback 折疊後仍保有 ANSI 上色。

分支：`feat/notebook-collapse-output`。`collapseText` 測試 3/3、前端 `tsc`＋i18n 38/38＋`vite build` 通過。

## Jupyter Notebook 整合（階段 6d）：一鍵複製 cell 的原始碼或輸出

### 背景

在 notebook 頁看到一段程式碼或它的執行結果時，常會想把它複製出去貼到別處。先前只能手動選取，尤其執行輸出
夾雜著顏色碼時很難乾淨地複製。這一步加上一鍵複製。

### 使用方式

在 notebook 頁的底部，對著目前這一格可以按：

- **複製原始碼**：把這一格的內容複製到剪貼簿。
- **複製輸出**：把這一格的執行輸出（純文字）複製到剪貼簿——只有程式碼 cell 且有輸出時才出現。

唯讀觀看者也能使用。

### 實作重點

- 新增純函式 `outputsToPlainText`：把一格的各種輸出攤平成純文字——標準輸出串流、執行結果的 `text/plain`、
  以及把錯誤 traceback 的 ANSI 顏色碼去除後的文字（沒有 traceback 時退回「錯誤類型: 訊息」）；純圖片輸出不
  貢獻文字。有單元測試。
- 複製動作走瀏覽器的 `navigator.clipboard`，採 best-effort：在不支援或被拒絕時安靜略過。

分支：`feat/notebook-copy-cell`。`nbformatModel` 測試 24/24、前端 `tsc`＋i18n 38/38＋`vite build` 通過。
...
階段 5d 讓 notebook 頁可以新增與刪除 cell，但還不能調整它們的先後順序——整理教材時常需要把某一格往上或往下
搬。這一步補上 cell 的上下移動。

### 使用方式

在可編輯的 notebook 頁，工具列的 cell 操作區新增 **⬆**／**⬇** 兩顆按鈕：對著目前這一格按下就會把它往上或往下
移動一格，選取會跟著移動後的那一格；已經在最上／最下時對應的按鈕會變灰。所有變更即時寫回該頁的 `.ipynb`。

### 實作重點

- 新增純函式 `moveCell`（不可變更新、回傳移動後的新索引、在清單邊界或索引越界時不動作），有單元測試。
- 移動前會先提交進行中的編輯草稿；因為 cell 索引會位移，移動時也會清掉「執行中」的高亮，避免指到錯的一格。
- 由於元件內原本已有一個負責「切換目前檢視 cell」的 `moveCell`，這個新的重排純函式以別名匯入以免混淆。

分支：`feat/notebook-cell-reorder`。`nbformatModel` 測試 20/20、前端 `tsc`＋i18n 38/38＋`vite build` 通過。
...
一般簡報頁播完語音後會（在自動換頁開啟時）自動翻到下一頁。但 notebook 頁是互動的、沒有語音，理應
**停在該頁讓使用者操作**，不該被自動跳過。先前有個隱藏問題：播放器在換頁時，如果新頁沒有語音，會直接
略過而**沒有清掉前一頁的音訊**；於是當你從一個有語音的頁面自動翻到 notebook 頁時，前一頁殘留的音訊一旦
播完，就會再次觸發「自動換頁」，把你剛到的 notebook 頁又跳過去。

### 這一步做了什麼

修正播放器換頁時的音訊處理：當一頁**沒有語音**（例如 notebook 頁）時，主動把 `<audio>` 停下、清掉音訊
來源並重置狀態，而不是把前一頁的音訊留著。這樣 notebook 頁就不會有任何音訊在背景播放、不會觸發「播放
結束」事件、也就不會自動翻頁——播放到 notebook 頁會**乖乖停在那裡**，等你手動操作（切 cell、之後執行程式
碼），符合整合計畫「notebook 頁不參與播放計時／自動換頁」的設計。

另外，簡報總語音長度的統計本來就會略過沒有語音的頁面，所以 notebook 頁自然不會被算進總時長。

### 使用方式

使用者不需要做任何事：把某頁設為 notebook 後，播放（含自動換頁）走到該頁會停下來，直到你手動換頁。

分支：`fix/notebook-no-audio-autoadvance`。前端 `tsc`＋`vite build` 通過。
...
課堂／演講途中發起即時投票（poll）時，台下聽眾若還沒進到簡報，就無法參與投票。過去要口頭念網址或
另外開分享對話框秀 QR，流程零散。這次讓「投票進行中」自動在螢幕上顯示一張可掃描的 QR。

### 使用方式

擁有者（主講）在某頁按下「開始投票」後：

- **全螢幕**：畫面左下角自動出現一張 QR 卡（標題「📱 掃描加入投票」），聽眾用手機一掃即以分享連結
  開啟該簡報。
- **一般檢視**：側邊欄「即時票數」卡片下方同樣顯示這張 QR。
- 掃描開啟的分享連結會**自動開啟同步模式**（沿用既有「帶 share token 進入即自動同步」的行為），因此
  聽眾一進來就落在同步中的簡報、可直接投票。
- 投票結束時 QR 立即消失，不會殘留在畫面上。

### 實作重點

- 純函式 `buildJoinQrImageUrl(data, size?)`（`frontend/src/lib/joinQr.ts`）產生 QR 圖片 URL，並把原本
  內聯在分享設定裡的同段建構收斂為共用（去重）。
- `usePollJoinQrCode` hook 依「投票進行中（`pollStarted`）＋自己是可主控的擁有者」惰性建立唯讀分享連結
  並轉成 QR；狀態經 `PlayPageContext` 提供給全螢幕與側邊欄渲染。
- i18n 新增 `play.fullscreen.pollJoinQr{Title,Hint,Alt}`（zh-TW／en）。

分支：`feat/poll-join-qrcode`。

## 簡報旁白：錄音時播過的合成語音，重播時也會一起播

### 背景

講者錄旁白時常會戴耳機，一邊自己講、一邊讓系統念某幾頁的合成語音（TTS）。因為戴耳機，這些 TTS 不會被麥克風收進去。使用者希望**重播旁白時，這些當時播過的合成語音也能在對應時間一起播出來**，字幕也切換成該頁的語音字幕。

### 使用方式

錄旁白的過程中，你在哪一頁按了播放、讓系統念了那頁的語音，系統都會記下來。之後播放這段旁白時，到了當時的時間點就會**自動播放那一頁的合成語音**，投影片上的字幕也會在那段期間切換成該頁的逐字稿；沒有語音的時間則顯示你自己的旁白逐字稿。

### 技術

- **記錄**：錄音時監聽主播放器的播放狀態與當前頁，把每一次 TTS 播放記成一段「音訊區間」`{ startMs, endMs, page, fromSec }`（相對這段旁白的時間；`fromSec` 是該頁音檔的起播位置）。換頁或暫停就關閉當前區間。
- **重播**：旁白播放時，用一個獨立的隱藏 `<audio>`（與旁白音檔分開）在每個區間載入該頁的語音檔、seek 到對齊位置後播放；旁白暫停/結束/離開區間就停掉。`audioCueAtTime` 純函式負責找出當下所在的區間。
- **字幕**：在 TTS 區間內顯示該頁逐字稿（等同一般播放模式的字幕），其餘時間顯示旁白自己的逐字稿。
- 後端每段旁白新增 `audioCues`（帶上限保護的 schema），GET 回 `audio_cues`。

### 測試

新增 `audioCueAtTime` 純函式測試與後端 `audio_cues` 往返測試。前後端 `tsc` 通過、`narrationTracks` 8/8、後端 `narration` 5/5、i18n 24/24。分支 `feat/narration-record-tts`。

## 簡報旁白：錄音直接用原本的畫筆（顏色、橡皮擦都保留）

### 背景

先前旁白錄製用的是一支內建的紅筆，只有單一顏色、沒有橡皮擦。使用者希望**全螢幕錄旁白時就用原本的畫筆工具**——換顏色、橡皮擦、粗細通通保留。這一版把旁白的手繪改成沿用既有的畫筆系統。

### 使用方式

在全螢幕開啟畫筆後照常畫（可換色、可用橡皮擦、可調粗細），這些動作會在錄旁白時一起被記下來。播放旁白時，畫面會依當時的時間**一段段重現你畫的內容**，包含顏色與橡皮擦的擦除；滑鼠游標則以**十字**樣式同步重現。

### 技術

- **畫筆＝原生 DrawingCanvas**：不再用自製紅筆。原本的畫筆元件每次筆劃變化都會回報一份完整的畫面快照（`onLocalChange`，內含每筆的顏色/粗細/是否橡皮擦）。錄音時把這些快照打上時間戳（畫的過程節流取樣，但「完成一筆或橡皮擦刪除」一定記錄）存成 `drawSnapshots`。
- **重播＝唯讀 DrawingCanvas**：播放時取「當下時間對應的最後一份快照」，交給一個唯讀模式（`remoteData`）的 DrawingCanvas 還原，所以顏色、橡皮擦、粗細都與錄製時一致。
- **游標不搶畫筆**：游標的擷取改掛在 `SlideRenderer` 最外框（畫筆 canvas 與疊加層的共同祖先）的 `onPointerMove`，靠事件冒泡收到移動、同時讓畫筆照常收到指標事件，因此游標與畫筆可以並存。重播時游標畫成十字。
- 後端每段旁白新增 `drawSnapshots`（帶上限保護的 DrawingData schema）；舊的簡易筆跡欄位保留以相容既有資料。

### 測試

新增 `drawingSnapshotAtTime` 純函式測試與後端 `drawSnapshots` 往返測試。前後端 `tsc` 通過、`narrationTracks` 7/7、後端 `narration` 5/5、i18n 24/24。分支 `feat/narration-native-pen`。

## 簡報旁白：筆畫隨時間畫出、錄的當下就看得到

### 背景

前一版雖然會記錄手繪筆跡，但有兩個體驗問題：**重播時整條線瞬間出現**（不是隨聲音一筆一筆長出來），而且**錄的當下畫面上看不到自己畫的線**。這一版把兩者都修好。

### 使用方式

使用方式不變：錄一段時按住拖曳畫紅筆、移動記游標。差別是——錄的時候你會**即時看到紅色筆跡跟著游標長出來**；播放時筆畫會**依當時的節奏一段段畫出來**，和聲音同步。

### 技術

- **漸進重播**：原本每一「筆」只存一個起筆時間，裡面的點沒有各自時間，所以起筆時間一到就整筆回傳。現在擷取時**每個點都記自己的時間戳**；重播的 `strokesUntil` 改成依點的時間裁切、並在最後一段內插筆尖位置，筆畫因此隨時間平順長出。舊錄音（點沒有時間）仍整筆顯示，向後相容；後端 `StrokeSchema` 的點新增選填 `tMs`。
- **錄製即時顯示**：`NarrationSlideOverlay` 用它本來就攔到的指標事件，在本地維護「目前正在畫的筆跡與游標」並即時畫出來，所以講者邊畫邊看得到紅線，不必等到回放。

### 測試

新增漸進重播的純函式測試（起筆前不顯示、逐點長出、末端內插、超界顯示全部）。前後端 `tsc` 通過、`narrationTracks` 6/6、後端 `narration` 5/5、i18n 24/24。分支 `fix/narration-progressive-strokes`。

## 簡報旁白：全螢幕也能擷取與重播游標／畫筆

### 背景

前一版把游標與畫筆的擷取／重播疊加層只放在**一般投影片面板**上。但實際錄旁白時，講者幾乎都是在**全螢幕**下進行——結果全螢幕錄製時筆跡根本沒被記下（自然也無法重播），游標擷取也不一致。這一版把它修好：不論一般檢視或全螢幕，錄音與播放都能正常擷取與重現。

### 使用方式

使用方式不變：錄一段時在投影片上移動指標記游標、按住拖曳畫紅筆；播放時同步重現。差別是**現在在全螢幕下錄也一樣有效**。

### 技術

- 把原本內嵌在 `PlayPageSlidePanel` 的擷取層與重播疊加抽成共用元件 `NarrationSlideOverlay`（擷取 `<div>` 在 z-50、游標圓點與 SVG 筆跡在 z-40，皆讀 `PlayPageContext` 的 `narrationCapture`／`narrationOverlay`）。
- 這個元件同時掛進**一般面板**與**兩處全螢幕 `SlideRenderer`**。因為擷取與重播都相對各自 `SlideRenderer` 的 `inset-0` 疊加層取正規化座標，即使全螢幕有 letterbox（`object-contain`），擷取與重播仍共用同一座標空間、彼此一致。

### 測試

前端 `tsc` 通過、i18n parity 24/24。分支 `fix/narration-fullscreen-capture`。

## 簡報旁白：同步記錄游標與繪圖

### 背景

錄旁白時，講者常會用滑鼠指著重點、或在投影片上圈畫。這一版讓旁白**連同游標移動與手繪筆跡一起錄下來**，播放時和聲音、翻頁一起同步重現。

### 使用方式

- **錄製**：開始錄一段後，投影片上會有一層擷取層。你在投影片上**移動指標**會被記成游標軌跡；**按住拖曳**則會用內建的紅筆畫線（放開結束一筆）。這一切都不影響原本的繪圖工具，是錄音專用的簡易標註。
- **播放**：播放該段時，投影片上會同步重現當時的**游標小圓點**與**逐漸畫出的筆跡**，跟著聲音與翻頁走。

### 技術

- 擷取層在 `PlayPageSlidePanel` 疊在投影片上，透過 `PlayPageContext` 把指標事件送進 `useNarrationRecorder`：移動記游標（每 40ms 取樣一次以控制大小）、按住拖曳記成一筆筆畫，座標都正規化成 0–1。停止時連同音檔一起上傳。
- 後端 narration 的每段多存 `cursorTrack` 與 `drawTrack`；重錄該段時會一併更新（並清掉過期的逐字稿）。
- 重播由純函式 `cursorAtTime`（在相鄰取樣點間內插使游標移動平順）與 `strokesUntil`（依時間漸進顯示筆畫）驅動；投影片面板用 SVG 畫筆跡、用一個小圓點畫游標。

### 測試

前端 `narrationTracks.test.ts`（4 組：游標內插、取樣點、超界夾住/起點前回 null、筆畫漸進顯示）；後端 narration 測試加「游標/繪圖軌跡上傳→GET 往返」，共 9/9。前後端 `tsc` 通過。

## 簡報旁白：分段錄音

### 背景

初版旁白是「一整份簡報一段錄音」。使用者希望能**分段錄**——一次錄一小段、可以單獨重錄某一段、調整段落先後順序，並在列表上看到每一段各講了哪些頁。這一版把資料模型從「單段」升級為「多段」。

### 使用方式

在播放頁側邊欄「slides」分頁的「🎙 簡報旁白」面板：

- **錄一段**：按「🎙 錄一段」開始，講完按「停止並儲存」，就新增一段到列表最後。可以錄很多段。
- **段列表**：每一段顯示「第 N 段 · 頁 1, 2 · 12s」——一眼看出這段講了哪些頁、多長。
- **重錄某段 / 刪除 / 調整順序**：每段都有「重錄此段」「刪除此段」與「▲▼」上下移。
- **播放**：按某段的「▶ 播放」會播那段的音檔，並依錄製時的翻頁時間自動翻頁。（把所有段串起來連續播放會在下個步驟做。）

### 技術

後端把旁白改成 `narration/manifest.json`（有序的 segment 清單，每段記其時長與翻頁時間軸）加上逐段音檔 `<segId>.webm`，並提供新增/重錄/刪除/排序/串流各段的 API。前端 `NarrationPanel` 改成段列表 UI，`useNarrationRecorder` 支援「新增一段」或「重錄指定段」。

### 測試

後端 `narration.test.ts`（3 組）涵蓋：新增兩段→列出（含每段頁面）→排序→重錄→串流→刪除的完整往返、非擁有者上傳 403、非法時間軸 400。前後端 `tsc` 通過、i18n 24/24。

## 簡報旁白錄音（MVP）

### 背景

一直以來 NEW_FEATURE 裡就想做「錄音模式」：把講者的旁白錄下來、同時記住每一頁講了多久，之後打開簡報就能「邊聽旁白、投影片自動翻頁」。先前只完成了背後的資料層純函式（產生翻頁時間軸、依播放時間查當下頁碼、累積錄音 session），但沒有任何介面會用到它們。這一版把它接成**實際可用的功能**（先做錄音+同步播放，影片輸出留待後續）。

### 使用方式

在播放頁右側側邊欄的「slides（投影片）」分頁，會看到一個「🎙 簡報旁白」面板：

- **錄製（簡報擁有者或協作者）**：按「🎙 錄旁白」後開始錄音，接著就正常地一頁一頁講、一頁一頁翻——系統會同時錄下你的聲音、並記住你在每一頁停留的時間。講完按「停止並儲存」即上傳；不想要就按「取消」。已經有旁白時可「重錄」或「刪除旁白」。
- **播放（任何能看這份簡報的人）**：面板會出現一個音訊播放器，按下播放後，投影片會**依錄製時的翻頁時間自動翻到對應頁**，達成「旁白 + 簡報同步播放」。

### 運作方式（技術）

- **前端**：`useNarrationRecorder` hook 用瀏覽器的 `MediaRecorder` 錄音，同時用先前做好的 `recordingSession` 記錄翻頁事件；停止時以 `stopRecording` 產生 `{page, startMs, endMs}` 的時間軸，連同音檔（webm/opus）上傳。播放時 `NarrationPanel` 監聽 `<audio>` 的 `timeupdate`，用 `slideAtTime(時間軸, 目前毫秒)` 算出當下該顯示哪一頁，再自動切換。
- **後端**：新增四個端點 `POST/GET/DELETE /api/pdfs/:id/narration` 與 `GET …/narration/audio`。每份簡報存一段旁白（音檔 + 時間軸 JSON）於該簡報的儲存目錄下，錄製/刪除需編輯權限、播放需讀取權限，時間軸以 zod 驗證。

### 測試

後端新增 `narration.test.ts`（3 組：擁有者「上傳→讀取→串流→刪除」完整往返、非擁有者上傳被擋 403、非法時間軸回 400）。前後端 `tsc --noEmit` 通過；底層純函式 `recordingSession`（7）、`slideTimeline`（13）回歸通過；i18n 中英對齊 24/24。錄音與播放屬瀏覽器端行為（麥克風、`MediaRecorder`、`<audio>`），無法在單元測試環境重現，改由端到端的 API 往返測試與純函式測試涵蓋整條資料流。

### 尚未涵蓋

影片檔輸出（把畫面依時間軸配上音訊合成 mp4）、逐頁重錄、錄音中暫停/續錄等，列為後續。
...
分享的「存取權限」面板與系統設定的「群組」管理，都有一個「輸入 email 才能按加入」的判斷，各自定義了一模一樣的 email 檢查正規表示式 `EMAIL_RE`。同一條規則寫在兩個檔案裡，容易在其中一處調整時漏改另一處，而且沒有測試。

### 變更內容

- 新增 `frontend/src/lib/isValidEmail.ts` 的 `isValidEmail(email)`：沿用原本的寬鬆規則（本地部分 + `@` + 網域 + `.` + TLD），僅擋明顯無效的輸入，實際有效性仍由後端把關。
- `AccessControlPanel`（加入名單）與 `GroupsManager`（加入群組成員）移除各自的 `EMAIL_RE`，改用這個共用函式。

### 使用方式

純內部重構，兩處「輸入 email 是否可加入」的判斷行為不變。email 格式規則現在只有單一來源與測試，日後要放寬或收緊都只需改一處。

### 測試

新增 `isValidEmail.test.ts`（3 組：接受一般 email、拒絕缺少 @／網域／TLD 的字串、拒絕空白與含空格的輸入）。前端 `tsc --noEmit` 通過、3/3 通過。
...
簡報的標籤是以逗號分隔的字串儲存。首頁與簡報卡片在好幾個地方都要把它拆成標籤陣列——顯示標籤晶片、依標籤過濾、加標籤等——原本每一處都各自寫一份 `(tags ?? '').split(',').map(t => t.trim()).filter(Boolean)`，重複且沒有測試。

### 變更內容

- 新增 `frontend/src/lib/parseTags.ts` 的 `parseTags(raw)`：把逗號分隔字串拆成去空白、去空項的標籤陣列，`null`／`undefined`／空字串回空陣列。
- `HomePage`（標籤晶片、標籤過濾、加標籤）4 處與 `PdfCard` 1 處改用這個共用函式。其中「標籤過濾」原本的變體沒有濾掉空字串，但因為被比對的過濾標籤本身都非空、`includes` 的結果不受影響，改用會濾空的版本行為完全等價。

### 使用方式

純內部重構，標籤的顯示與過濾行為不變。抽成純函式後，標籤字串的解析有了單一來源與測試，日後若要改變分隔或正規化規則也只需改一處。

### 測試

新增 `parseTags.test.ts`（4 組：逗號分割並去空白、濾掉空項（開頭/結尾逗號、純空白）、`null`/`undefined`/空字串回空陣列、單一標籤）。前端 `tsc --noEmit` 通過、4/4 通過。
...
測驗允許為個別題目設定自訂分數，但整份測驗滿分固定是 100 分。後端在建立測驗時會做一道權威驗證：如果各題自訂分數加總超過 100，就拒絕儲存（否則全對可能拿到像 160/100 這種超出承諾滿分的分數）。這個加總由 `explicitScoreSum` 計算，先前是模組內部函式、沒有獨立的單元測試。

### 變更內容

- 把 `backend/src/routes/pdfs/quizzes.ts` 的 `explicitScoreSum` 改為匯出，並新增針對它的單元測試。
- 函式行為不變：只加總「有限且非負」的分數，其餘（未設定、`null`、負數、`NaN`、`Infinity`）一律當 0。

### 使用方式

純測試補強，建立測驗時的分數上限驗證行為不變。這道驗證是伺服器端權威計分的一環，補上單元測試後，日後調整分數規則時能更早抓到回歸。

### 測試

新增 `explicit-score-sum.test.ts`（5 組：加總有效分數、缺少/`null` 視為 0、負數/`NaN`/`Infinity` 視為 0、空清單回 0、偵測加總超過 100 分）。後端 `tsc --noEmit` 通過、5/5 通過。

## 測驗題目完整性檢查抽出純函式

### 背景

測驗編輯頁的「儲存」按鈕只有在整份測驗填寫完整時才會啟用。判斷「每一題是否完整」的規則其實不算平凡——每題要有題幹，而且申論題不需要選項、其他題型至少要有兩個非空選項——但原本這段規則直接內聯在 `canSave` 的判斷式裡，沒有測試。

### 變更內容

- 新增 `frontend/src/lib/quizValidation.ts` 的 `allQuestionsComplete(questions)`：檢查每一題都有非空題幹，且申論題（`essay`）免選項、其餘題型至少要有兩個非空選項。空陣列回 `true`（沿用 `every` 的語意），由呼叫端另外檢查「至少一題」。
- `QuizBuilderPage` 的 `canSave` 改用這個共用函式。

### 使用方式

純內部重構，「儲存」按鈕的啟用條件不變。抽成純函式後，這條「測驗可否儲存」的完整性規則有了獨立、明確的測試，日後調整題型規則時比較不會不小心改壞。

### 測試

新增 `quizValidation.test.ts`（6 組：選擇題有題幹且選項足夠→合格、空題幹→不合格、非空選項不足 2 個→不合格、申論題免選項→合格、有一題不完整就不合格、空陣列回 true）。前端 `tsc --noEmit` 通過、6/6 通過。

## 已作答題數計算抽出共用純函式

### 背景

測驗頁在好幾個地方需要知道「學生已經作答了幾題」——例如送出前的檢查、以及作答進度顯示。原本這個計算（把有選取答案的題目數出來）在 `QuizBuilderPage` 裡重複寫了三份，而且沒有測試。

### 變更內容

- 新增 `frontend/src/lib/countAnsweredQuestions.ts` 的 `countAnsweredQuestions(questions, answers)`：一題只要有至少一個選取答案就算已作答，回傳已作答的題數；泛型設計，只依賴題目的 `id`。
- `QuizBuilderPage` 三處內聯計算改用這個共用函式。

### 使用方式

純內部重構，作答進度與送出檢查的行為不變。抽成純函式後，「已作答題數」的判定有了單一來源與獨立測試。

### 測試

新增 `countAnsweredQuestions.test.ts`（5 組：計算已作答題數、缺少或空的答案視為未作答、沒有題目回 0、全部作答、忽略沒有對應題目的答案）。前端 `tsc --noEmit` 通過、5/5 通過。

## 書籤/重點頁切換抽出共用純函式

### 背景

播放頁可以把某一頁加入「書籤」或標記為「重點頁」，兩者都是「按一下就切換該頁是否在清單中」。原本 `toggleBookmark` 與 `toggleImportantPage` 各自寫了一份一模一樣的邏輯——若已在清單就移除、否則加入後重新排序——而且沒有測試。

### 變更內容

- 新增 `frontend/src/lib/toggleSortedNumber.ts` 的 `toggleSortedNumber(list, value)`：值已存在就移除、不存在就加入並保持升冪排序，回傳新陣列、不改動輸入。因為書籤/重點頁本來就不會有重複值，沿用原本的 filter/append 語意，行為與先前完全一致。
- `PlayPage` 的兩個 handler 改用這個共用函式。

### 使用方式

純內部重構，書籤與重點頁的切換行為不變。抽成純函式後，這個「切換一個數字於升冪清單」的操作有了獨立測試，也可在其他類似需求重用。

### 測試

新增 `toggleSortedNumber.test.ts`（5 組：加入缺少值並保持升冪、移除已存在值、空清單加入、移除唯一值變空、不改動輸入）。前端 `tsc --noEmit` 通過、5/5 通過。
...
播放頁頂部有一個「複製全部逐字稿」按鈕，會把整份簡報每一頁的逐字稿組成一段 Markdown 複製到剪貼簿。原本這段「依頁碼排序、每頁加一個 `## 第 N 頁` 標題、缺稿留空、頁與頁之間空一行」的組裝邏輯直接寫在按鈕的 onClick 裡，沒有測試。

### 變更內容

- 新增 `frontend/src/lib/allScriptsMarkdown.ts` 的 `buildAllScriptsMarkdown(pages, scripts, { pagePrefix, pageSuffix })`：依 `page_number` 排序（不改動輸入陣列），每頁輸出 `## <前綴>N<後綴>` 標題再接該頁逐字稿（沒有逐字稿的頁就留空），頁與頁之間以空行分隔。
- `PlayPageHeader` 的「複製全部逐字稿」按鈕改用這個共用函式。

### 使用方式

純內部重構，按鈕行為不變：點下去仍會把整份簡報的逐字稿依頁碼順序組成 Markdown 並複製。抽成純函式後，排序與格式化有了獨立測試、之後若要在別處（例如匯出）重用也有單一來源。

### 測試

新增 `allScriptsMarkdown.test.ts`（5 組：排序並套用標題格式、缺稿頁留空、無頁面回空字串、不改動輸入、支援自訂前綴/後綴）。前端 `tsc --noEmit` 通過、5/5 通過。
...
規畫中的「錄音模式」會在講者錄音的同時，記錄簡報翻頁的時間點，之後要能同步播放簡報與錄音、或把兩者合成影片。要做到這件事，需要一個共同的資料模型：把錄音過程中零散、可能亂序的「切頁事件」整理成一條乾淨、以錄音起點為 0 的時間軸。這個 section 先完成這個模型的核心純函式（尚未接錄音 UI 或儲存）。

### 變更內容

- 新增 `frontend/src/lib/slideTimeline.ts` 的 `buildSlideTimeline(recordingStartMs, events, recordingDurationMs)`，以及型別 `SlideSwitchEvent`（`{page, atMs}`，絕對時間戳）與 `SlideTimelineSegment`（`{page, startMs, endMs}`，相對錄音起點）。
- 函式行為：把每個切頁事件換算成「相對錄音起點」的偏移並夾到 `[0, 錄音時長]`（濾掉錄音開始前或結束後的雜訊）、依時間穩定排序、合併「連續切到同一頁」的重複事件、把第一段回溯到 0 讓時間軸完整覆蓋整段錄音、並濾除零長度區段（同一瞬間的多次切頁以最後一次為準）。真正「切回前一頁」則會保留成獨立區段。空事件或非正的錄音時長回傳空陣列。

### 使用方式

這是給後續功能使用的基礎模型：錄音時每次翻頁記一筆 `{page, atMs}`，錄完後呼叫 `buildSlideTimeline(起點, 事件, 時長)` 就能得到一串 `{page, startMs, endMs}` 區段，播放器可據此在對應時間顯示對應頁，影片合成也能用同一份區段。目前尚未接上錄音介面，屬功能的第一步。

### 測試

新增 `slideTimeline.test.ts`（9 組：連續區段生成、首段回溯到 0、亂序事件排序、連續同頁合併、真實回看前頁保留、錄音前/後事件夾界、同一瞬間零長度區段濾除與「較晚者勝出」、空事件／非正時長回空、忽略非整數頁碼與非有限時間）。前端 `tsc --noEmit` 通過、9/9 通過。

## 批次匯出的視覺進度條

### 背景

首頁的「匯出全部 ZIP」會把帳號下所有簡報打包成一個大 ZIP，這在教材多時可能要跑一段時間。後端其實早就以背景 job 逐份處理並回報 `progress`／`total`（前端也已在輪詢），但畫面上只有按鈕文字顯示「打包中… 3/12」這種純文字進度，沒有一眼可判斷的視覺進度條。

### 變更內容

- `HomePage` 在既有「ZIP 匯入」進度條旁，新增一條批次匯出進度條，只在批次匯出進行中（有 job 且已知總數）時顯示。
- 進度百分比沿用專案既有、已有測試覆蓋的純函式 `progressPercent(current, total)`（結果 clamp 在 0–100、對 `total ≤ 0` 或非有限值回 0，避免出現 `NaN%` 或溢位），用於進度條寬度與無障礙的 `aria-valuenow`。
- 樣式比照既有匯入進度條（`role="progressbar"` + 百分比文字），採用綠色系與匯入的藍色系區隔。新增 i18n 鍵 `home.batchExportProgressAriaLabel`（zh-TW／en）。

### 使用方式

在首頁點「匯出全部 ZIP」後，按鈕下方會出現一條綠色進度條與百分比，隨著每份簡報打包完成即時前進；打包完成後照舊自動下載整包 ZIP。

### 測試

純前端、無新計算邏輯（百分比走既有已測的 `progressPercent`）。前端 `tsc --noEmit` 通過、i18n parity 24/24。

## AI 導師的錯誤與空答處理

### 背景

「AI 導師」（`PageAskPanel`）有三個先前較弱的環節：(1) 當教材裡其實沒有相關資訊時，模型偶爾仍會杜撰答案；(2) 模型偶爾回傳空白／全是空白字元的答案，前端會顯示一個空的導師泡泡；(3) 提問失敗時，前端直接把後端原始錯誤訊息（有時是英文技術字串）丟給使用者。

### 變更內容

- **從源頭防杜撰**：`/ask` 的 system prompt 把原本「若查無資訊請誠實說明」強化為明確規則——只能依提供的頁面內容與原始來源作答，嚴禁杜撰或臆測；若找不到相關資訊，就直接回「找不到相關資訊」並簡短建議換個問法或查看相關頁面。
- **空答保底 + 換行正規化收斂**：新增純函式 `backend/src/routes/pdfs/tutorAnswer.ts` 的 `finalizeTutorAnswer(raw)`。它把先前內聯在 `/ask` 的「字面 `\n` 還原成真換行（同時保留 `\nabla`、`\rho`、`\right`、`\times` 等 LaTeX 指令不被誤傷）」正規化邏輯抽出並加上測試，並在整理後為空字串時回傳一段固定提示（`TUTOR_NO_ANSWER_FALLBACK`），確保前端不會出現空白的導師回答。
- **錯誤訊息人性化**：`usePageAsk` 的錯誤處理改走專案共用的 `mapApiErrorToHumanMessage`，回傳與上傳、匯入等流程一致、依錯誤碼在地化的訊息，取代原本直接顯示後端原始 message。

### 使用方式

對使用者透明、無需操作：問到教材沒有的內容時，AI 會直接說「找不到相關資訊」而不是硬掰；萬一模型回了空答，畫面會顯示一段清楚的提示而非空白；提問失敗時看到的是好懂的中文錯誤說明。

### 設計取捨

TODO 原本提到「偵測『查無資訊』語句時給固定提示」，這裡改以**空答保底**（模型回空 → 固定提示）落實，並把「不要杜撰」交給 prompt 從源頭處理；刻意不做脆弱的自然語言片語偵測，避免把正常回答誤判成「查無資訊」而傷害體驗。

### 測試

後端新增 `tutor-answer.test.ts`（7 組：字面 `\n\n`／`\n`／`\r\n` → 換行、保留 `\nabla`／`\rho`／`\right`／`\times`、trim、空／空白／`\n\n` → fallback、正常答案不變）；`page-ask.test.ts` 新增 2 組整合測試（prompt 含「禁止杜撰／找不到相關資訊」、模型回空白 → 回應為固定提示）。前後端 `tsc --noEmit` 通過；後端 ask 相關 13/13、前端錯誤映射 7/7。
...
播放頁的「AI 導師」（`PageAskPanel`）會綜合整份簡報回答問題，並依 prompt 規則在用到其他頁面的資訊時標示來源，例如「（第 3 頁）」「（第 5 頁逐字稿）」。原本這些引用是純文字，讀者若想查證得自己手動翻到那一頁，跨頁對照很不方便。

### 變更內容

- 新增純函式 `frontend/src/lib/extractCitedPages.ts` 的 `extractCitedPages(text)`：以寬鬆的「第 N 頁」樣式（容許「第3頁」「第 3 頁」「第  12  頁」等空白變化）掃描回答全文，回傳**升冪、去重、正整數**的頁碼清單。是否落在有效頁數範圍、是否排除目前頁，交由呼叫端決定，函式本身保持純粹、可測。
- `PageAskPanel` 在每則 AI 導師答案**下方**新增一列「引用頁碼」晶片按鈕（刻意不動 `MarkdownMath` 的答案渲染路徑，避免侵入式改動），只顯示實際存在於這份簡報、且不是目前所在頁的頁碼；點擊任一晶片即透過 `setCurrentIdx` 切換到該頁。
- 新增 i18n 鍵 `play.sidebar.pageAsk.citedPagesLabel`（「引用頁碼：」）與 `play.sidebar.pageAsk.jumpToPage`（「第 {page} 頁」）的 zh-TW／en 文案。

### 使用方式

向 AI 導師提問後，如果回答引用了其他頁面，答案下方會出現「引用頁碼：第 3 頁 · 第 5 頁 …」這樣的一排小按鈕，點一下就直接跳到對應投影片，方便一邊看回答一邊查證來源；若回答只用到目前頁或引用「原始來源」（無頁碼），則不顯示這排捷徑。

### 測試

新增 `frontend/src/lib/extractCitedPages.test.ts`（7 組：單頁、多頁升冪去重、空白變化、忽略「原始來源」與無頁碼、忽略第 0 頁、空／無引用回空、重複呼叫穩定不受 regex `lastIndex` 影響）。前端 `tsc --noEmit` 通過、新測試 7/7、i18n parity 24/24。

## AI 導師多輪對話脈絡的字數上限管理

### 背景

播放頁的「AI 導師」（`PageAskPanel`）支援多輪追問：每次提問時，前端會把先前的對話（history）連同整份簡報的逐頁文字／逐字稿（corpus）與原始來源全文一起送回後端 `/api/pdfs/:id/pages/:n/ask`，讓模型保有脈絡。

問題在於 history 的上限只限制「輪數」與「單則長度」——後端 schema 允許最多 20 輪、每則最多 8000 字，最壞情況可達約 160,000 字。相較之下，corpus 的字數預算是 14,000、來源全文是 12,000，history 一旦是長對話就會反過來把教材內容擠出 token 預算，導致回答品質下降甚至請求失敗。原本這段 history 是「全量帶入、完全不設上限」。

### 變更內容

- 新增純函式 `backend/src/routes/pdfs/askHistoryBudget.ts` 的 `budgetChatHistory(history, maxChars)`：把對話歷史（oldest-first）收斂到一個字元預算內，**保留最新的連續數輪**（超出預算時先丟最舊的，因為追問時最近的脈絡最重要）。若「最新一則」本身就超過預算，則保留它但截斷內容（在開頭加上「……（前略）……」省略標記、保留最近的尾段），確保只要有 history 且預算 > 0 就不會整段清空而失去眼前脈絡。函式不修改輸入、回傳新陣列。
- 新增常數 `ASK_HISTORY_MAX_CHARS = 8000`，並在 `/ask` 端點把 history 送入模型前先經 `budgetChatHistory` 收斂，再 `.map` 成訊息。輪數上限（20）仍由既有 schema 把關，本次補上的是「字數截斷」這一環。

### 使用方式

對使用者透明、無需任何操作。多輪追問時，若累積的對話很長，AI 導師會自動只保留最近、最相關的幾輪對話送給模型，把 token 預算留給教材內容，維持回答品質；短對話則完全不受影響、與先前行為一致。

### 測試

新增 `backend/test/ask-history-budget.test.ts`（7 組：完整放入不裁切、超預算丟最舊、保留 oldest-first 順序、最新一則過長改截斷不丟棄、預算 ≤ 0 回空、空歷史回空、不改動輸入）。後端 `tsc --noEmit` 通過；新測試 7/7，AI 導師整合測試 `page-ask` 亦回歸通過（以 Node 22 執行，共 10 綠）。

## 品質檢查面板的徽章顯示狀態抽出為可測純函式

### 背景

播放頁的「品質檢查」面板有三個分析區塊（基本品質檢查、逐字稿銜接分析、圖片內容比對），每個區塊的標題旁都會顯示一個小徽章：尚未執行時不顯示、執行完成且無問題顯示綠色 ✓、有問題則顯示問題數量。原本這段「未載入 → 隱藏；無問題 → ✓；有問題 → 數量」的判斷以巢狀三元運算式在三個區塊各寫了一份，容易在其中一處改動時與其他兩處不一致，且沒有獨立測試。

### 變更內容

- 在 `frontend/src/lib/qualityCheckSelection.ts` 新增純函式 `analysisBadgeState(hasRun, running, issueCount)`，回傳判別聯集 `{ kind: 'hidden' } | { kind: 'ok' } | { kind: 'issues', count }`：尚未執行或執行中回 `hidden`、完成且無問題回 `ok`、否則回帶問題數的 `issues`。
- `QualityCheckPanel` 三個區塊改用之（品質區塊以過濾後的 `issuePages.length` 當問題數、逐字稿/圖片區塊以各自結果長度），移除三份重複的巢狀三元運算式；顏色仍由各區塊 JSX 依語意自行選擇，顯示行為完全等價。

### 使用方式

純內部重構，品質檢查面板的徽章顯示行為不變。此純函式也為後續「§7.2 於播放頁側邊欄分頁顯示品質摘要徽章」提供可重用、可測的狀態判斷基礎。

### 測試

新增 4 組 `analysisBadgeState` 測試（未執行→隱藏、執行中→隱藏、完成無問題→ok、完成有問題→帶數量 issues）。前端 `tsc --noEmit` 通過、qualityCheckSelection 共 9 測試通過。
...
動畫編輯器允許使用者勾選多個動畫效果後「合併成一個」：合併後的效果**從最早的起始時間開始、延伸到最晚的結束時間**，其餘設定沿用最早的那個效果（包含其依逐字稿句子觸發的 `startTrigger`，使其維持錨定在該句；`start` 仍更新為解析出的最早秒數作為備援）。原本這段「取最小 start／最大 end／挑最早效果／組出合併效果」的計算內聯在事件處理函式裡，沒有獨立測試——而「合併後保留哪個效果的設定、duration 怎麼算」這種邏輯一旦回歸出錯，使用者不易立即察覺。

### 變更內容

- 在 `frontend/src/lib/animationSpec.ts` 新增 `mergeEffectRanges(ranges)` 純函式（及 `SelectedEffectRange` 介面）：輸入為「效果 + 已解析的 start/end 秒數」陣列，回傳合併後的效果（起點最早 start、長度 = 最晚 end − 最早 start、沿用最早效果的所有設定與 id），少於兩個範圍時回 `null`。
- `AnimationEditorTab` 的合併處理改用之（逐字稿觸發 → 秒數的解析仍留在元件，因其依賴目前的字幕時間軸），移除內聯計算。

### 使用方式

純內部重構，「合併選取效果」的操作行為不變。

### 測試

`animationSpec.test.ts` 新增 3 組測試（少於兩個回 null、跨最早到最晚並沿用最早效果的 id/設定、保留 `startTrigger` 並以解析出的最早秒數更新 start）。前端 `tsc --noEmit` 通過、animationSpec 61 測試全通過。
...
使用者回報：某份簡報第 42 頁的 AI 焦點動畫，紅框位置都對不上畫面內容，看起來「AI 好像沒看到正確的圖片」。

### 查證結果

經完整查證，**目前的程式碼是正常的——投影片圖片有正確送出，模型也確實看得到**：

1. 該頁 `image_path` 指向的圖檔內容正確（就是那張矩陣線性組合投影片），系統也能正常把它縮成 1024px
   寬的 data URL 送進視覺輸入。
2. 該帳號使用的 LLM 供應商（CGU Air）模型本身支援視覺——把同一張圖送過去問它看到什麼，它能精準描述出
   版面（左側 5 個項目卡片、右側矩陣公式與展開式、底部說明框）。
3. 用現行程式路徑對該頁重跑多次，每次都產生**貼合實際版面、位置多樣**的方框（左欄項目、底部展開式、
   無視覺元素的句子被正確略過）。

對照之下，存檔裡的「壞」規格全是固定的左欄座標、紅框由上往下機械式平均分佈、幾乎每句都顯示——這正是
**純文字模型（看不到圖片）只能把方框平均攤開**的典型特徵。結論是：那些壞規格是先前「新增多頁」失敗後
那批補產動畫留下的**舊殘留**，產生當下圖片未被模型套用，與現行程式碼無關。受影響頁面只要以現行設定
重新產生焦點動畫即可恢復正確。

### 變更內容

- **資料修復**：以現行設定，透過正式的動畫產生路徑重新產生受影響頁面的焦點動畫並寫回，方框位置即貼合
  版面。
- **程式碼**：修正 `generateAiFocusEffects` 說明文字中一條**已過時且會誤導排查方向**的註解。它原本宣稱
  投影片圖片「只有在 `LLM_PROVIDER=openai` 時才會真正被使用」（理由是 Gemini 會剝除非文字內容）。這個
  說法現在已不成立：Gemini 供應商會把圖片資料轉成 `inlineData` 送出，OpenAI 相容供應商（含 CGU Air、
  OpenRouter）則直接把圖片透傳給模型——也就是說，**圖片在目前所有供應商都會送達模型**。改寫後的註解
  明確指出：若結果看起來像純文字（方框機械地排成一欄、無視版面），代表是模型／閘道沒有套用視覺判讀，
  而**不是**程式碼把圖片丟掉了。

### 為何重要

這條註解正好對應使用者回報的症狀。若沿用舊註解，排查者很可能誤判成「我們用的不是 openai，所以圖片本來
就不會被用，這是預期行為」而停止追查；修正後能把注意力正確導向「圖片有送、是模型/閘道端的視覺處理」與
「重新產生即可修復舊殘留」。
...
測驗編輯頁的題目／作答分數，以及首頁、播放頁的預算成本警示，原本散落 8 處各自內嵌
`Math.round(x * 100) / 100`。這段邏輯主要是為了把浮點累加誤差收乾淨（例如三題各 33.33／33.33／33.34
相加在浮點下會變成 `100.00000000000001`），分散維護容易飄移，也沒有測試保護這個意圖。

### 變更內容

- 8 處分數／成本的兩位小數四捨五入改用同一個共用函式，計算結果完全一致。
- 以單元測試明確記錄「浮點累加誤差收斂」這個行為（如 `33.33+33.33+33.34 → 100`、`0.1+0.2 → 0.3`），
  避免日後有人「優化」掉這層四捨五入。

### 使用方式

此為內部重構，使用者操作不變：測驗編輯頁的分數欄位、首頁與播放頁的預算成本提示，數字皆由共用
函式四捨五入後顯示。

### 實作備註

新增 `frontend/src/lib/roundTo.ts`（`roundToTwoDecimals(value)` = `Math.round(value * 100) / 100`）。
`QuizBuilderPage`（6 處分數）、`HomePage`、`PlayPage`（各 1 處預算成本）共 8 處呼叫點改用此函式。新增
`roundTo.test.ts`（4 組：乾淨值不變、就近進位、浮點累加誤差收斂、較高精度成本截到兩位）。前端
`tsc --noEmit` 通過、新測試 4/4 通過；不動後端、不需新 i18n。
...
設定頁的「清除縮圖快取」與「清除產物快取」兩個按鈕，完成後都會顯示後端回報的釋放空間（KB）。
兩處原本各自內嵌相同的 `Math.round(data.bytes_freed / 1024)`，是重複邏輯；而且當後端回傳非預期值
（例如 `NaN`、負數）時，畫面會直接顯示出 `NaN KB` 之類的字串。

### 變更內容

- 兩處清除快取的 KB 換算改用同一個共用函式，行為一致。
- 後端回傳非有限值（`NaN`／`Infinity`）或負值時，一律當作 `0 KB`，不再出現 `NaN KB`。
- 顯示行為與原本一致（含不足約 512 bytes 會四捨五入成 0 KB）。

### 使用方式

此為內部重構，使用者操作不變：在「設定」頁點「清除縮圖快取」或「清除產物快取」，完成訊息中的
釋放空間數字即由共用函式計算。

### 實作備註

新增純函式 `bytesToRoundedKb`（位於 `frontend/src/lib/bytesFreed.ts`），輸入位元組數、回傳四捨五入後
的整數 KB，並對非有限值與負值做淨化。`SettingsPage` 的兩個清除快取 handler 改呼叫此函式。新增
`bytesFreed.test.ts`（精確 KB、四捨五入、sub-512-byte 進位成 0、`NaN`／`±Infinity`／負值淨化）。
前端 `tsc --noEmit` 通過、新測試 3/3 通過；不動後端、不需新 i18n。
...
## 卡片封面載入失敗退回佔位圖（2026-06-26）

### 功能目的
首頁每張簡報卡片（`PdfCard`）會顯示一張封面縮圖。封面的來源依序為：渲染中的即時頁面預覽、`cover_thumbnail_url`、`cover_url`。問題是當這個 URL 存在、但圖片實際載入失敗時（縮圖尚未產生、檔案被刪、404 或暫時性網路錯誤），原本的 `<img>` 沒有任何 `onError` 處理，瀏覽器就會顯示那個破掉的圖片小圖示，視覺上相當突兀。本次讓封面在載入失敗時優雅退回到原本「沒有封面」時顯示的 PDF 佔位圖示。

### 使用方式
無需任何操作。當某張卡片的封面縮圖無法載入時，會自動顯示 PDF 佔位圖（與完全沒有封面時相同的外觀），不再出現破圖。若稍後封面網址改變（例如簡報仍在渲染、即時預覽換到下一頁），會重新嘗試載入新的封面，不會因為先前某張失敗就永遠停在佔位圖。

### 技術細節
- **純函式**（新檔 `frontend/src/components/pdfCardCover.ts`）：`shouldShowCoverImage(coverSrc, failedSrc)` 在「有 `coverSrc` 且不等於剛失敗的那個 URL」時回傳 `true`（並以 TypeScript type guard 收窄為 `string`）。刻意以「失敗的 URL」而非單純的布林旗標當判斷依據——這樣當 `coverSrc` 變成另一個新網址時，由於不等於先前失敗的網址，會自動重試載入，而不會一次失敗就永久退回佔位圖。
- **元件**（`frontend/src/components/PdfCard.tsx`）：新增 `failedCoverSrc` state，封面區塊改用 `shouldShowCoverImage(coverSrc, failedCoverSrc)` 判斷要顯示 `<img>` 或佔位圖；`<img>` 加上 `onError={() => setFailedCoverSrc(coverSrc)}` 記錄失敗的網址。
- **測試**：`pdfCardCover.test.ts` 新增 4 個 node:test，涵蓋無 URL（null/undefined/空字串）、有未失敗 URL、剛失敗的 URL 退回佔位、以及失敗後換成新 URL 時重試。前端 371 個測試與 `tsc --noEmit` typecheck 全通過。

## 播放頁選單標籤 i18n（2026-06-26）

### 功能目的
延續先前多輪的前端 i18n 收尾工作。播放頁頂部工具列（`PlayPageHeader`）在窄螢幕會收合成一個「選單」按鈕，展開後是多個下拉群組（資訊、播放、生成、下載、逐字稿、分享）。這些群組標籤、行動版選單按鈕的文字與其無障礙 `aria-label` 先前都是直接寫死的中文字串，沒有經過翻譯函式。結果是當使用者把介面語言切換成英文時，這些選單標籤仍然顯示中文，與其他已完成 i18n 的介面不一致。本次把這些殘留的硬編中文補上翻譯。

### 使用方式
無需任何操作。介面語言為英文時，播放頁的行動版選單按鈕會顯示「Menu」、各下拉群組會顯示 Info／Playback／Generate／Download／Script／Share；中文介面維持原本的資訊／播放／生成／下載／逐字稿／分享。螢幕報讀軟體讀到的選單按鈕標籤也會跟隨介面語言。

### 技術細節
- **新增 i18n key**：`frontend/src/locales/zh-TW.ts` 與 `en.ts` 各新增 8 個 key——`play.header.menuToggle`（選單按鈕 aria-label）、`play.header.menu`（按鈕文字）、以及 `play.header.groupInfo`／`groupPlayback`／`groupGenerate`／`groupDownload`／`groupScript`／`groupShare`。
- **元件**（`frontend/src/pages/play/PlayPageHeader.tsx`）：行動選單按鈕的 `aria-label="開啟播放頁選單"` 與 `<span>選單</span>` 改用 `t('play.header.menuToggle')`／`t('play.header.menu')`；6 個 `HeaderDropdown` 的 `label="…"` 改用對應的 `t('play.header.group*')`。
- **驗證**：i18n 的 key 對齊由 `tsc --noEmit`（`TranslationKey = keyof typeof zhTW` 會讓缺漏的 key 變成編譯錯誤）與既有的 i18n 測試（檢查 zh-TW／en key 一致）一起把關。前端 371 個測試與 typecheck 全通過。

## figure 圖片路徑越界防護（2026-06-26）

### 功能目的
文件模式匯入的簡報會從 PDF 抽出圖表（figures），存成檔案並在 `figures.json` manifest 記錄每張圖的相對路徑 `imagePath`。前端透過 `/api/pdfs/:id/figures/:figureId/image` 路由取得圖片時，後端會以 `figureImageAbsPath(pdfId, figure)` 把 `imagePath` 解析成磁碟絕對路徑再串流給瀏覽器。原本的實作直接用 `path.join(pdfDir(pdfId), figure.imagePath)`，沒有做任何「結果是否仍在該 PDF 目錄內」的檢查。雖然目前 `figures.json` 一律由伺服器自己寫入（`imagePath` 形如 `figures/p1-xxx.png`），但這個函式讀取的是磁碟上的 JSON 檔——一旦 manifest 被竄改或損壞、`imagePath` 含有 `../`，就可能解析到 PDF 目錄之外的任意檔案並被串流出去（path traversal）。本次以縱深防禦補上越界檢查。

### 使用方式
無需任何操作，對正常的圖表圖片存取沒有任何影響。這是一道防禦性護欄：只有當 figure 的 `imagePath` 嘗試逃逸出該 PDF 自己的目錄時，才會被擋下並拋錯，避免任意檔案外洩。

### 技術細節
- **改用既有安全助手**（`backend/src/services/pdfFigures.ts`）：`figureImageAbsPath` 由 `path.join(pdfDir(pdfId), figure.imagePath)` 改為 `safeJoinPdfPath(pdfId, figure.imagePath)`。`safeJoinPdfPath` 會 `path.resolve` 後驗證結果仍以該 PDF 目錄（含尾端分隔符，可正確擋掉共用前綴的兄弟目錄）為前綴，否則拋出 `Path traversal detected`——與 `pdfDir` 對 `pdfId` 的越界守門一致。順帶移除此檔已不再使用的 `path` import。
- **測試**：新增 `backend/test/figureImageAbsPath.test.ts` 2 個 node:test：正常 `figures/p1-abc.png` 解析到 PDF 目錄下的正確位置、以及 `../pdf-2/...`、`../../etc/passwd` 等越界路徑會拋錯。此測試不依賴資料庫，可在 sandbox 執行並通過；後端 `tsc -p tsconfig.json` build 亦通過。

## 橡皮擦命中幾何抽離並補測試（2026-06-26）

### 功能目的
播放頁的繪圖標註功能提供橡皮擦：使用者拖曳橡皮擦時，系統要判斷哪些筆畫被擦到並移除。這個「命中判定」其實是一段非平凡的幾何計算——不只比對橡皮擦與筆畫上每個取樣點的距離，還要比對橡皮擦到「相鄰兩取樣點所連成線段」的最短距離（否則兩個間隔較遠的取樣點之間的細線就擦不掉）。這段邏輯原本內嵌在 `DrawingCanvas.tsx` 裡、沒有匯出、也沒有任何測試，是個容易在日後重構時不小心改壞、卻不會被測試發現的死角。本次把它抽成可測的純模組並補上單元測試，把正確行為鎖定下來。

### 使用方式
無需任何操作，純內部重構，橡皮擦行為完全不變。此變更的價值在於：橡皮擦的幾何命中判定現在有測試保護，日後若有人調整這段距離計算，跑測試就能立即發現回歸。

### 技術細節
- **純模組**（新檔 `frontend/src/components/drawingGeometry.ts`）：
  - `distSq(ax,ay,bx,by)`：平方歐氏距離（只比較大小時可省去開根號）。
  - `distPointToSegment(px,py,ax,ay,bx,by)`：點到線段的最短距離，含投影參數 `t` 夾擠在 [0,1]（超出端點時退化為到端點的距離），並處理零長線段（退化為點對點距離）。
  - `strokeHitsPoint(stroke, ex, ey, radius, cw, ch)`：把筆畫的正規化座標（[0,1]）以畫布寬高 `cw`/`ch` 還原成像素，逐一檢查每個頂點（平方距離 vs 半徑平方）與每段連線（點到線段距離 vs 半徑）是否落在橡皮擦半徑內。
- **元件**（`DrawingCanvas.tsx`）：移除這三個內聯函式，改 `import { strokeHitsPoint } from './drawingGeometry'`。`drawingGeometry` 以 `import type` 取得 `DrawingStroke` 型別（型別匯入在編譯後會被抹除），因此執行期沒有與 `DrawingCanvas` 的循環依賴，測試也不會載入整個畫布元件。
- **測試**：`drawingGeometry.test.ts` 新增 6 個 node:test，覆蓋平方距離、點到線段距離（垂足、端點夾擠、零長線段）、以及筆畫命中（貼近頂點命中、稀疏兩點之間的線段命中、遠離未命中）。前端 377 個測試與 `tsc --noEmit` typecheck 全通過。

## 側邊欄導覽縮圖隱藏破圖（2026-06-26）

### 功能目的
播放頁側邊欄有幾處會用小縮圖輔助導覽：「從選定頁面建立新簡報」的搜尋結果、書籤清單、以及重點頁清單。這些地方的縮圖 `<img>` 先前都沒有 `onError` 處理，一旦縮圖載入失敗（尚未產生、404、暫時性錯誤）就會顯示瀏覽器的破圖小圖示。這與側邊欄主要的大綱縮圖網格不一致——後者早已有完善的載入失敗降級處理。本次讓這三處導覽縮圖在載入失敗時一致地優雅降級。

### 使用方式
無需任何操作。當側邊欄搜尋結果、書籤或重點頁清單中的某張縮圖無法載入時，該縮圖會被隱藏，而不再顯示突兀的破圖示；清單項目的文字（頁碼等）仍正常顯示，導覽功能不受影響。

### 技術細節
- **元件**（`frontend/src/pages/play/PlayPageSidebar.tsx`）：三個導覽縮圖 `<img>`（「從頁面建立」搜尋結果、書籤、重點）加上 `onError={(e) => { e.currentTarget.style.display = 'none'; }}`，載入失敗時隱藏該圖。書籤與重點兩處的 `<img>` 字串完全相同，以一次 replace-all 同步處理，確保兩者一致。
- 此變更為單純的 DOM 事件處理、沒有可抽出的純邏輯，因此不另加單元測試；以前端既有 377 個測試與 `tsc --noEmit` typecheck 全數通過確認未造成回歸。

## add-pages outline 純函式抽離並補測試（2026-06-26）

### 功能目的
「從提示新增頁面」（add pages from prompt）功能會請 LLM 產生大綱，再把大綱解析成投影片、挑選插入點周圍的既有頁面文字當作生成情境。負責這些工作的 `parseOutlineText`（解析大綱成投影片）、`buildInsertionContext`（挑選插入點周圍頁面組成情境）、`renderNewSlideTexts`（把投影片渲染成頁面文字）三個函式本身都是純邏輯、沒有副作用，但它們原本住在 `addPagesFromPrompt.ts` 裡，而該檔在頂層 `import { db }`——於是這些純函式沒辦法在不啟動資料庫的情況下單元測試（其中 `buildInsertionContext` 還是被路由 `add-pages.ts` 匯入的公開 API，卻一直沒有直接測試）。本次把這三個函式抽到一個不依賴資料庫的純模組，讓它們可被測試並鎖定行為。

### 使用方式
無需任何操作，純內部重構，「從提示新增頁面」行為完全不變。價值在於這段大綱解析與情境挑選邏輯現在有測試保護，且可在不啟動 DB 的環境中快速驗證。

### 技術細節
- **純模組**（新檔 `backend/src/worker/addPagesOutline.ts`）：包含 `parseOutlineText`、`buildInsertionContext`、`renderNewSlideTexts` 與共用的 `OutlineSlide` 型別，完全不 import 資料庫或其他重量級 worker 相依。
- **相容性**：`addPagesFromPrompt.ts` 移除原本的函式定義，改 `import { buildInsertionContext, parseOutlineText, renderNewSlideTexts } from './addPagesOutline'` 供內部使用，並 `export { buildInsertionContext }` re-export——這樣原本從 `addPagesFromPrompt` 匯入 `buildInsertionContext` 的 `routes/pdfs/add-pages.ts` 完全不需更動。
- **測試**：`backend/test/addPagesOutline.test.ts` 新增 7 個 node:test，涵蓋大綱解析（至少兩個 bullet 才成立、`Slide N:` 前綴去除、出現在任何標題之前的孤兒 bullet 會被忽略）、插入情境（聚焦插入點前後的視窗、去重並依頁碼排序、略過空白頁、`maxChars` 截斷）、以及新頁渲染（連續的 `Slide N:` 標頭與 bullet 行）。此測試不依賴資料庫，可在 sandbox 執行並通過；後端 `tsc -p tsconfig.json` build 亦通過。

## 預設投票選項 i18n（2026-06-26）

### 功能目的
播放頁的即時投票功能讓老師在簡報過程中發起投票。建立投票時，「選項」欄位會帶一組預設值方便快速開始。先前這個預設值是寫死的中文 `同意 / 不同意`（出現在欄位初始值與「建立投票後重設欄位」兩處）。對使用英文介面的老師而言，一打開就是中文預設選項；若直接送出，學生端看到的投票選項也會是中文，與課程語言不符。本次把預設投票選項納入 i18n。

### 使用方式
無需任何操作。介面語言為英文時，新投票的預設選項會是 `Agree / Disagree`；中文介面維持 `同意 / 不同意`。老師仍可自由編輯這些選項。

### 技術細節
- **新增 i18n key**：`frontend/src/locales/zh-TW.ts` 與 `en.ts` 各新增 `play.sidebar.poll.defaultOptions`（值含換行，分別為 `同意\n不同意` 與 `Agree\nDisagree`，對應每行一個選項的輸入格式）。
- **hook**（`frontend/src/pages/play/usePagePolls.ts`）：選項欄位的 `useState` 初始值改為 lazy initializer `() => t('play.sidebar.poll.defaultOptions')`（只在掛載時讀取一次）；建立投票成功後重設欄位處改用 `t('play.sidebar.poll.defaultOptions')`（該 `handleCreatePoll` 的 `useCallback` 相依陣列本就包含 `t`）。
- 檢視此檔時另確認投票結果輪詢採用 `cancelled` 旗標 + 遞迴 `setTimeout` 的串接模式，本身不會有「過時回應覆蓋新結果」的競態。
- **驗證**：i18n key 對齊由 `tsc --noEmit` 與既有 i18n 測試把關；前端 377 個測試與 typecheck 全通過。

## 暫停播放／即時問答動畫預設文字 i18n（2026-06-26）

### 功能目的
動畫效果中有兩種會在播放到指定句子時暫停播放、並在投影片上疊加一段提示文字：「暫停播放」（pause-playback）與「即時問答」（realtime-poll）。先前這兩個 preset 在老師建立效果時，會把一段寫死的中文（`暫停：請按播放鍵繼續`、`📊 即時問答時間`）直接烤進效果的 `text` 欄位。於是在英文介面、英文課程的簡報中，觀看者仍會看到中文的暫停／問答疊加字。更值得注意的是，這與同類的「文字提示」（text-callout）preset 並不一致——text-callout 並不預設任何文字，而是留空、靠 placeholder 與顯示時的 fallback 處理。本次讓這兩個 preset 與 text-callout 一致，並把預設文字改成跟隨介面語言。

### 使用方式
無需任何操作。新增「暫停播放」或「即時問答」效果時，文字欄位會留空並顯示對應語言的提示（中文 `暫停：請按播放鍵繼續`／英文 `Paused: press play to continue`；中文 `📊 即時問答時間`／英文 `📊 Q&A time`）；播放時若該效果沒有自訂文字，會顯示對應語言的預設字。老師仍可自行輸入文字覆寫。

### 技術細節
- **新增 i18n key**：`zh-TW.ts`／`en.ts` 各新增 `play.animation.defaultPausePlaybackText` 與 `play.animation.defaultRealtimePollText`。
- **preset**（`AnimationEditorTab.tsx`）：`pause-playback`／`realtime-poll` 兩個 preset 的 `apply()` 移除 `text: DEFAULT_...`（不再烤入內容，與 text-callout 一致）；對應的輸入框 `placeholder` 改用 `t('play.animation.default*Text')`；移除已不需要的常數 import。
- **顯示 fallback**（`SlideRenderer.tsx`）：`EffectOverlay` 加入 `useI18n`，兩處 `effect.text?.trim() || <常數>` 的 fallback 改為 `|| t('play.animation.default*Text')`，使空白文字的效果在播放時顯示對應語言的預設字。
- **清理**：`animationSpec.ts` 移除已無人使用的 `DEFAULT_PAUSE_PLAYBACK_TEXT`／`DEFAULT_REALTIME_POLL_TEXT` 常數。
- **驗證**：preset 的既有測試只斷言 `exitDuration`／`pollId`，不受移除 `text` 影響；i18n key 對齊由 `tsc --noEmit` 與 i18n 測試把關。前端 377 個測試與 typecheck 全通過。

## custom-script 安全檢查擋下可呼叫的 Function 建構式（2026-06-26）

### 功能目的
「自訂腳本動畫」（custom-script）允許 AI 依老師描述產生一段 JavaScript，於投影片播放時在 sandboxed `<iframe>`（不含 allow-same-origin）中執行。雖然沙箱本身已在瀏覽器層級阻擋網路/儲存/跨框存取，後端仍以 `findUnsafeScriptPattern` 做一道縱深防禦，用一組正規表示式擋掉 `fetch`、`eval`、`new Function`、`localStorage`、`window.parent` 等危險 API。本次補上一個被遺漏的等價向量：可呼叫形式的 `Function` 建構式 `Function("…")()`——它與 `new Function("…")` 同樣能把字串當程式碼執行，但原本只擋了 `new Function`、沒擋這種不帶 `new` 的呼叫。

### 使用方式
無需任何操作。AI 產生的 custom-script 程式碼若包含 `Function("…")` 形式的呼叫，會在套用前被偵測並退回（與 `eval`／`new Function` 一致地擋下，必要時自動 fallback 成 highlight-box）。一般的小寫 `function(){}` 函式寫法完全不受影響。

### 技術細節
- **新增樣式**（`backend/src/services/animationCustomScript.ts`）：在 `UNSAFE_PATTERNS` 的 `new Function` 項之後加入 `{ pattern: /\bFunction\s*\(/, label: 'Function constructor' }`。刻意**不加 `i` 旗標**、只比對大寫開頭的 `Function`——因為 JavaScript 的 Function 建構式一律是大寫 F，而小寫 `function(` 是宣告/運算式關鍵字、在正常程式碼中極常見，若大小寫不敏感會造成大量誤判。`new Function(` 仍由前一條規則先比中、回報的 label（`new Function`）維持不變。
- **測試**（`backend/test/animation-custom-script.test.ts`）：在偵測案例中新增 `Function("return 1")()` 與 `Function ("alert(1)")` 兩個應被標為 `Function constructor` 的案例；另新增一個測試確認函式運算式、IIFE、以及定義 `window.renderAnimation = function (root, api) {...}` 的合法寫法都**不會**被誤判。相關測試 17 個全通過，後端 `tsc -p tsconfig.json` build 通過。

## API 錯誤提示 key 存在性 drift-guard（2026-06-26）

### 功能目的
當 API 呼叫失敗時，前端會用 `mapApiErrorToHumanMessage` 把後端的錯誤代碼（如 `INVALID_REQUEST`、`FILE_TOO_LARGE`、`PDF_NOT_FOUND`）對應成一組「標題／說明／下一步」的友善訊息，並依介面語言翻譯。為避免每個代碼都手寫三個 i18n key，程式以 `hintKeys(name)` 動態組出 `apiError.<name>.title`、`.message`、`.nextStep`，並以 `as TranslationKey` 轉型。問題是：這個轉型會繞過 TypeScript 對「這個翻譯 key 是否真的存在」的編譯期檢查——萬一某個 key 漏加、或名稱打錯，`t()` 會退而回傳 key 字串本身，使用者就會在錯誤對話框看到 `apiError.invalidRequest.title` 這種原始字串而非真正的訊息，而且編譯不會報錯。本次加入一道測試守門，把這類漂移在測試期就攔下來。

### 使用方式
無需任何操作，這是開發期的保護網。日後若有人新增錯誤代碼對應卻忘了補對應的 i18n 文字、或把 key 打錯，跑測試就會直接失敗並指出缺哪個 key，而不會等到使用者在 UI 上看到原始 key 才發現。

### 技術細節
- **export 對應表**（`frontend/src/lib/api/common.ts`）：將原本模組私有的 `ERROR_HINT_KEYS`（錯誤代碼 → `{title, message, nextStep}` 三個 TranslationKey）改為 `export`，供測試直接迭代真實的對應內容。
- **drift-guard 測試**（新檔 `frontend/src/lib/api/apiErrorHintKeys.test.ts`）：載入 `ERROR_HINT_KEYS` 與 `zhTW`／`en` 兩份字典，對每個錯誤代碼的 `title`／`message`／`nextStep` 三個 key 斷言它們都存在於兩個 locale。`mapApiErrorToHumanMessage` 內以字面字串寫死的 key（`apiError.requestFailed.*`、`apiError.unknownError.*`）本來就會被 tsc 檢查、不在此測試範圍。
- **現況**：實測目前所有 hint key 皆已存在於兩個 locale，沒有缺漏；此測試是為了鎖定未來不再漂移。前端 378 個測試與 `tsc --noEmit` typecheck 全通過。

## DrawingCanvas getNorm 防零面積除法（2026-06-26）

### 功能目的
播放頁的繪圖標註（`DrawingCanvas`）在使用者按下/移動指標時，會把指標的螢幕座標換算成相對畫布的正規化座標（0~1），再存進筆劃資料並送到伺服器。換算方式是把指標位移除以畫布 `getBoundingClientRect()` 的寬高。問題是這裡沒有對「寬或高為 0」做任何防護——若畫布在某個瞬間被收合或隱藏（寬高為 0），除法會得到 `NaN` 或 `Infinity`，這些無效座標會被寫進筆劃，序列化成 JSON 時變成 `null`，污染儲存的繪圖資料。本次補上守門，避免任何情況下產生無效座標。

### 使用方式
無需任何操作，正常繪圖行為不變。這是一道防禦：當畫布暫時沒有可量測面積時，指標座標會安全地回傳 `[0,0]`，而不會把 `NaN` 座標存進筆劃。

### 技術細節
- **純函式**（`frontend/src/components/drawingGeometry.ts`）：新增 `normalizeCanvasPoint(clientX, clientY, rect)`，僅在 `rect.width > 0 && rect.height > 0` 時才換算成 `[(x-left)/width, (y-top)/height]`，否則回傳 `[0,0]`（用 `!(width > 0)` 寫法同時擋掉 0、負值與 `NaN`）。
- **元件**（`DrawingCanvas.tsx`）：`getNorm` 由內聯的除法改為呼叫 `normalizeCanvasPoint(e.clientX, e.clientY, canvas.getBoundingClientRect())`，import 一併更新。
- **測試**：`drawingGeometry.test.ts` 新增 2 個 node:test——一般換算（中心 → `[0.5,0.5]`、左上 → `[0,0]`、右下 → `[1,1]`）以及零面積 rect 回 `[0,0]` 而非 `NaN`。前端 380 個測試與 `tsc --noEmit` typecheck 全通過。

## 移除 routes/pdfs.ts 重構前遺留死碼（2026-06-26）

### 功能目的
PDF 相關的 API 路由早期集中在單一檔案 `backend/src/routes/pdfs.ts`，後來已重構拆分到 `backend/src/routes/pdfs/` 目錄底下的多個模組（`detail.ts`、`figures.ts`、`report.ts`…）。但重構後，舊的 `pdfs.ts` 並未清乾淨：檔案裡仍留著約 450 行重構前的程式碼（各種 route helper，以及與 `pdfs/shared.ts` 一字不差的重複 `extractYoutubeVideoId`、`streamFile` 等共 38 個函式）。這些全是無法到達的死碼——整個檔案唯一被匯出、被使用的，只有檔案底部一個很短的 `pdfRoutes` shim，它把工作委派給 `./pdfs/index`。`server.ts` 的 `import('./routes/pdfs')` 因為「檔案解析優先於同名目錄」而正好命中這個 shim。本次把整個檔案縮減成只剩這個 shim，清掉死碼與重複實作。

### 使用方式
無需任何操作，純內部清理，API 行為完全不變。`import('./routes/pdfs')` 仍解析到此檔、仍委派到 `./pdfs/index`，路由註冊與先前一致。
...
- **AI 分頁子面板補做**：使用者再回報「AI 助手」分頁仍為深色——`PageAskPanel`（AI 導師問這一頁）與 `QualityCheckPanel`（品質報告）是獨立元件檔，首輪 `PlayPageSidebar` 掃描未涵蓋。以相同手法遷移：中性骨架 token 化、彩色徽章/按鈕加 `dark:` 變體；「提問」鈕收斂為 indigo 主色實心，品質狀態徽章與「腳本脈絡分析／圖片內容分析」觸發鈕改為淺色淡 chip。
- **中央播放面板的「觀看深／編輯淺」分工**：使用者再指中央這塊也要改。經確認方向後採混合——`PlayPageSlidePanel` 的投影片觀看區、字幕覆蓋層、播放控制列與播放設定**維持深色**（沉浸觀看），下方逐字稿/提示詞/來源/系統分頁編輯區（行 1065–1720）改為語意 token 淺色面板、彩色強調加 `dark:` 變體。分頁列底改 `bg-surface`、active 分頁用 `bg-surface-muted`＋深色文字凸顯（同時保住深色模式「active 較亮」的關係）。動畫/圖表兩子分頁為獨立元件（`AnimationEditorTab`/`FigureAssetsTab`），仍深色、列為後續。
...
## Notebook 跨 cell 文字搜尋（2026-07-11）

### 功能目的

notebook 頁一次只顯示一個 cell，簡報稍微長一點、cell 數一多，想找「哪個 cell 印出了那個錯誤訊息」或「哪裡寫過那行程式碼」就得一個一個往下翻。這次補上跨整份 notebook 的文字搜尋，同時涵蓋每個 cell 的原始碼跟它執行後留下的輸出文字。

### 使用方式

notebook 工具列新增一顆「🔍 搜尋」按鈕，點開後出現一列搜尋框：輸入文字就會即時跳到第一個命中的 cell，旁邊顯示「第 N / 共 M 項」；按 Enter 或點 ▶ 跳下一筆、Shift+Enter 或點 ◀ 跳上一筆（會循環），按 Esc 或點 ✕ 關閉搜尋。搜尋範圍同時包含 cell 的原始碼跟它的輸出（例如 print 出來的文字、執行結果、錯誤訊息），不分大小寫。

### 技術細節

- 新增純函式 `searchNotebookCells(notebook, query)`（`frontend/src/lib/nbformatModel.ts`），對每個 cell 分別比對兩塊文字：原始碼（既有的 `cellText`）與攤平後的輸出文字（既有的 `outputsToPlainText`，本來就是為了「複製輸出到剪貼簿」那個功能寫的，這次直接重用，涵蓋 stream 輸出、執行結果的 `text/plain`、以及去除 ANSI 色碼的錯誤 traceback）。回傳結果標明每個命中的 cell 是命中在原始碼、輸出、還是兩者都有。空白查詢一律回空陣列——沒有「全部醒目提示」這種有意義的結果，也避免使用者清空搜尋框時畫面被洗版。
- 跳轉邏輯抽成共用的 `jumpToCell(index)`：先呼叫既有的 `commitEdit()`（若目前正在編輯會先把草稿寫回，不是編輯中則是無操作）再切換 `cellIndex`，這樣搜尋跳轉跟既有的 ↑/↓ command-mode 換頁共用同一套「先提交再跳」的安全性保證，不會因為跳轉把正在編輯的內容弄丟。
- **驗證**：`nbformatModel` 新增 4 個測試（原始碼命中、輸出命中且原始碼沒有這段文字的情況、空白/純空白查詢皆不命中、完全無符合結果），前端 `tsc`、前端測試 783/783、`vite build` 都通過。

至此 notebook 顯示層強化（階段 7）只剩 7b「markdown cell 編輯時即時預覽切換」尚未動工。
