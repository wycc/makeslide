...
test('the settings page has the image provider select with all four choices and saves every image field', () => {
  ...
  for (const locale of [zhTW, en]) {
    for (const k of ['settings.imageProvider', 'settings.imageProviderAuto', 'settings.imageProviderHint', 'settings.openaiImageModelLabel', 'settings.geminiImageModelLabel', 'settings.qwenImageModelLabel', 'settings.qwenApiKeyHint'] as const) {
