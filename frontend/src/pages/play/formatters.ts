...
export function formatDurationMs(ms: number | null | undefined, noRecordLabel: string): string {
  // A negative duration is as meaningless as a missing one — treat it like the
  // null/NaN case instead of rendering e.g. "-5ms".
  if (ms == null || !Number.isFinite(ms) || ms < 0) return noRecordLabel;
