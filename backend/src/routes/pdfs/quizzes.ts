...
const SaveQuizBodySchema = z
  .object({
    ...
    // 防弊（離開作答超過寬限即計違規、超過上限自動交卷並鎖定）；關閉時只警告並記錄離開次數與時間。
    strict_proctor: z.boolean().default(true),
  })
...
function rowToQuiz(row: QuizSetRow) {
  ...
  return {
    ...
    strict_proctor: row.strict_proctor == null ? true : Boolean(row.strict_proctor),
    created_at: row.created_at,
    updated_at: row.updated_at,
  };
}
...
export async function registerQuizRoutes(app: FastifyInstance): Promise<void> {
  app.get('/api/pdfs/:id/quizzes', async (request, reply) => {
    ...
    const rows = db
      .prepare(`SELECT id, pdf_id, title, prompt, questions_json, time_limit_seconds, shuffle_questions, is_public, record_camera, strict_proctor, created_at, updated_at FROM quiz_sets WHERE pdf_id = ? ORDER BY updated_at DESC`)
...
const quizLanguage = assistantLanguage(getRuntimeAiSettings().contentLanguage);
  ...
  app.post('/api/pdfs/:id/quizzes', async (request, reply) => {
    ...
    const now = nowIso();
    const result = db.prepare(`INSERT INTO quiz_sets (pdf_id, title, prompt, questions_json, time_limit_seconds, shuffle_questions, is_public, record_camera, strict_proctor, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(parsed.data.id, body.data.title, body.data.prompt, JSON.stringify(normalizeSavedQuestions(body.data.questions)), body.data.time_limit_seconds, body.data.shuffle_questions ? 1 : 0, body.data.is_public ? 1 : 0, body.data.record_camera ? 1 : 0, body.data.strict_proctor ? 1 : 0, now, now);
    const row = db.prepare(`SELECT id, pdf_id, title, prompt, questions_json, time_limit_seconds, shuffle_questions, is_public, record_camera, strict_proctor, created_at, updated_at FROM quiz_sets WHERE id = ?`).get(result.lastInsertRowid) as QuizSetRow;
  ...
  app.put('/api/pdfs/:id/quizzes/:quizId', async (request, reply) => {
    ...
    const now = nowIso();
    const result = db.prepare(`UPDATE quiz_sets SET title = ?, prompt = ?, questions_json = ?, time_limit_seconds = ?, shuffle_questions = ?, is_public = ?, record_camera = ?, strict_proctor = ?, updated_at = ? WHERE id = ? AND pdf_id = ?`).run(body.data.title, body.data.prompt, JSON.stringify(normalizeSavedQuestions(body.data.questions)), body.data.time_limit_seconds, body.data.shuffle_questions ? 1 : 0, body.data.is_public ? 1 : 0, body.data.record_camera ? 1 : 0, body.data.strict_proctor ? 1 : 0, now, parsed.data.quizId, parsed.data.id);
    if (result.changes === 0) return reply.code(404).send(errorResponse('QUIZ_NOT_FOUND', `Quiz ${parsed.data.quizId} not found`));
    const row = db.prepare(`SELECT id, pdf_id, title, prompt, questions_json, time_limit_seconds, shuffle_questions, is_public, record_camera, strict_proctor, created_at, updated_at FROM quiz_sets WHERE id = ?`).get(parsed.data.quizId) as QuizSetRow;
  ...
  app.post('/api/pdfs/:id/quizzes/:quizId/copy-to/:targetId', async (request, reply) => {
    ...
    const quiz = db
      .prepare(`SELECT title, questions_json, prompt, time_limit_seconds, shuffle_questions, record_camera, strict_proctor FROM quiz_sets WHERE id = ? AND pdf_id = ?`)
      .get(quizId, id) as Pick<QuizSetRow, 'title' | 'questions_json' | 'prompt' | 'time_limit_seconds' | 'shuffle_questions' | 'record_camera' | 'strict_proctor'> | undefined;
    if (!quiz) return reply.code(404).send(errorResponse('QUIZ_NOT_FOUND', `Quiz ${quizId} not found`));

    const now = nowIso();
    const result = db
      .prepare(`INSERT INTO quiz_sets (pdf_id, title, prompt, questions_json, time_limit_seconds, shuffle_questions, record_camera, strict_proctor, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(targetId, quiz.title, quiz.prompt, quiz.questions_json, quiz.time_limit_seconds, quiz.shuffle_questions, quiz.record_camera, quiz.strict_proctor, now, now);
    const newRow = db
      .prepare(`SELECT id, pdf_id, title, prompt, questions_json, time_limit_seconds, shuffle_questions, record_camera, strict_proctor, created_at, updated_at FROM quiz_sets WHERE id = ?`)
