...
export async function registerDetailRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/api/pdfs/:id', async (request, reply) => {
    ...
    if (!shareAccess && !canReadPdf(sub, row, aclCtx(request, parsed.data.id))) {
    ...
    const pages = db
      .prepare(
        `SELECT pdf_id, page_number, image_path, text_path, script_path,
                ...
                notebook_path, page_notes, status, error_message, created_at, updated_at
    ...
    // to read-write ACL users and editable-token holders alike, and never show edit controls the
    // backend would reject.
    const accessLevel = maxAccessLevel(
      resolvePdfAccessLevel(row.id, sub, sessionEmail(request), row),
      resolveTokenAccessLevel(request, row.id),
    );
    if (shareMode) {
      return reply.send({
        ...detail,
  ...
  app.post('/api/pdfs/:id/share', async (request, reply) => {
    ...
    // A share token is a standalone capability: creating one no longer changes the
    // presentation's visibility. Identity-based access (visibility + ACL) and token
    // capability are two independent systems; the effective access for any action is
    // the higher of the two. The response echoes the current (unchanged) visibility.
  ...
  app.delete('/api/pdfs/:id/polls/:pollId', async (request, reply) => {
    ...
    const pdf = db.prepare(`SELECT id, owner_sub, visibility FROM pdfs WHERE id = ?`).get(id) as Pick<PdfRow, 'id' | 'owner_sub' | 'visibility'> | undefined;
    if (!pdf) return reply.code(404).send(errorResponse('PDF_NOT_FOUND', `PDF ${id} not found`));
    if (!canDestructivelyEditPdf(sessionSub(request), pdf, aclCtx(request, id))) return reply.code(403).send(errorResponse('FORBIDDEN', 'No edit permission'));
  ...
  app.patch('/api/pdfs/:id/polls/:pollId', async (request, reply) => {
    const parsed = PollParamSchema.safeParse(request.params);
    if (!parsed.success) return reply.code(400).send(errorResponse('INVALID_REQUEST', 'Invalid id or poll id'));
    const body = z.object({ is_active: z.boolean() }).safeParse(request.body ?? {});
    if (!body.success) return reply.code(400).send(errorResponse('INVALID_REQUEST', 'Body must include is_active: boolean'));
    const { id, pollId } = parsed.data;
    const pdf = db.prepare(`SELECT id, owner_sub, visibility FROM pdfs WHERE id = ?`).get(id) as Pick<PdfRow, 'id' | 'owner_sub' | 'visibility'> | undefined;
    if (!pdf) return reply.code(404).send(errorResponse('PDF_NOT_FOUND', `PDF ${id} not found`));
    if (!canDestructivelyEditPdf(sessionSub(request), pdf, aclCtx(request, id))) return reply.code(403).send(errorResponse('FORBIDDEN', 'No edit permission'));
    ...
    if (!row) return reply.code(404).send(errorResponse('POLL_NOT_FOUND', `Poll ${pollId} not found`));
    const now = nowIso();
    db.prepare(`UPDATE page_polls SET is_active = ?, updated_at = ? WHERE id = ?`).run(body.data.is_active ? 1 : 0, now, pollId);
    const updated = db
      .prepare(`SELECT id, pdf_id, page_number, question, options_json, is_active, show_results, created_at, updated_at FROM page_polls WHERE id = ?`)
      .get(pollId) as PagePollRow;
    return reply.send({ poll: rowToPoll(updated) });
  });
  ...
  app.get('/api/pdfs/:id/video', async (request, reply) => {
    ...
    if (!canReadPdf(sessionSub(request), row, aclCtx(request, id))) {
