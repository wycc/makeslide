...
export async function synthesizeGeminiSpeech(params: {
  ...
  /** Persona for solo narration (no "Speaker N:" labels in the text). */
  persona?: string | null;
  /** Personas for dual-host text, where one request voices both speakers. */
  speaker1Persona?: string | null;
  speaker2Persona?: string | null;
}): Promise<Buffer> {
  const apiKey = getGeminiApiKey();
  const voiceName = normalizeGeminiVoiceName(params.voiceName);
  // 只有腳本實際出現 Speaker 1:/Speaker 2: 對白時才用多人模式。判斷要看**原文**：前置的指示
  // 區塊裡有「Speaker 1 的角色設定：」這種字樣，拿它去判斷會誤判。
  const hasSpeakerDialog = /(^|\n)\s*Speaker\s*1\s*:/i.test(params.text)
    || /(^|\n)\s*Speaker\s*2\s*:/i.test(params.text);
  // 音色一律由 prebuilt voice 決定；這裡前置的是「怎麼唸」——語言與人設——而不是聲線，
  // 並採 Google 文件建議的「指示＋冒號＋內容」形式，模型會當成指示而非要唸出來的字。
  // 人設依模式給：獨白只有一位朗讀者，雙人則要指名哪一段設定屬於哪一位講者。
  const ttsPrompt = params.language
    ? withTtsPrompt(params.text, {
        language: params.language,
        persona: hasSpeakerDialog ? null : params.persona,
        speaker1Persona: hasSpeakerDialog ? params.speaker1Persona : null,
        speaker2Persona: hasSpeakerDialog ? params.speaker2Persona : null,
      })
    : params.text;
