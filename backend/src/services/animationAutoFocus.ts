 ...
 */
const AUTO_FOCUS_AI_EFFECT_TYPES = ['highlight-box', 'spotlight', 'text-callout', 'shape', 'step-list', 'custom-script'] as const;
...
function buildAutoFocusSystemPrompt(): string {
  return [
    ...
    '2. type：highlight-box（醒目方框，框出重點）、spotlight（聚光燈，方框外區域變暗）、text-callout（淡入一段精簡文字摘要）、shape（淡入一個簡單的 SVG 圖形，用於標示位置）、step-list（淡入一個條列步驟/要點清單方框）或 custom-script（淡入一段自訂的資料視覺化動畫）。沒有特別理由時優先選 highlight-box；若這句的重點適合用一句精簡摘要（例如關鍵數據、結論）強化，可選 text-callout；若這句在描述多個步驟、流程或要點，可選 step-list；若需要用箭頭、圓形等圖形標示位置，可選 shape；若這句描述的內容（例如數值隨時間變化、流程示意、座標關係）適合用一段簡短的動態視覺化呈現，且前述類型都無法表達，可選 custom-script——但這類效果產生成本較高，整頁最多只能有一個，且僅在確實必要時才選用，多數情況下請優先使用其他類型。',
    ...
    '7. scriptPrompt（僅當 type 為 custom-script 時提供，其他 type 請省略此欄位）：用一兩句話描述要產生的視覺化內容（例如「畫一個座標平面，顯示一個點沿曲線從左下移動到右上，代表數值隨時間成長」），會交給程式碼產生器轉換成可執行的動畫，請使用與逐字稿相同的語言，不超過 300 字。',
    '8. scriptDurationSeconds（選填，僅當 type 為 custom-script 時提供，其他 type 請省略此欄位）：此視覺化完整播放一輪所需的秒數，建議在 3 到 10 秒之間，讓畫面有足夠時間呈現變化。',
    '9. exitDuration（選填，秒）：效果淡入完成後要停留多久才自動淡出。如果這句只是短暫提示某個重點，可設定 1-3 秒；如果整句都在說明這個重點，可以設定一個比較長的時間。',
    '',
