...
function buildAutoFocusSystemPrompt(): string {
  return [
    ...
    '7. formulaLatex（僅當 type 為 formula 時提供，其他 type 請省略此欄位）：要顯示的 LaTeX 數學公式（不含 $ 或 $$ 分隔符），例如 E = mc^2 或 \\frac{1}{\\sigma\\sqrt{2\\pi}}e^{-\\frac{(x-\\mu)^2}{2\\sigma^2}}，不超過 200 字元；缺少或無效時會退回 highlight-box。',
