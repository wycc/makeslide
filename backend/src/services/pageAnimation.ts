...
export const ANIMATION_SHAPE_KINDS = ['circle', 'rect', 'ellipse', 'arrow', 'line', 'triangle', 'star', 'hexagon'] as const;
...
export interface AnimationEffect {
  ...
  /**
   * Border width (px integer) for `highlight-box` effects (ignored by other
   * effect types). Defaults to `DEFAULT_HIGHLIGHT_BORDER_WIDTH` (4).
...
/** Default border width (px) for `highlight-box` effects. */
...
const EffectSchema = z.object({
  ...
  highlightShadow: z.boolean().optional(),
  ...
  stepListHighlightIndex: z.number().int().min(0).optional(),
...
export function validateAnimationSpec(input: unknown): ValidateAnimationSpecResult {
  ...
  const effects: AnimationEffect[] = parsed.data.effects.map((effect) => {
    ...
    return {
      ...
      ...(effect.highlightShadow !== undefined ? { highlightShadow: effect.highlightShadow } : {}),
      ...
      ...(effect.stepListHighlightIndex !== undefined ? { stepListHighlightIndex: Math.max(0, Math.round(effect.stepListHighlightIndex)) } : {}),
