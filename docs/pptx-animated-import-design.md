...
3. 產生每一步的變體 pptx → LibreOffice 一次批次轉 PDF → 取出對應頁 → 1920×1080 JPG。
   變體的內容是「把還沒點到的形狀移除，**並把已經退場的形狀也移除**」。省成本的做法是讓
   「與原檔相同」的畫面共用原檔的那一次轉換，但判斷的依據必須是「這一格有沒有藏起任何形狀」
   （`hiddenShapeIdsForStep` 回空集合），**不能假設最後一格等於原檔**：有 exit 效果的投影片，
   全部點完的狀態比原檔**少**東西（原檔仍含已退場的形狀）。照「最後一格＝原檔」處理的話，
   退場的形狀會重新畫回來疊在最後一格上，而真正的最後一格根本不會被產生。
