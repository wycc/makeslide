...
- 文件版本：V1.11（2026-09-06：§9.11 效果上限與補回；V1.10：§9.10 預覽用未剪下的原圖；V1.9：§9.9 剪下歷史與草稿式編輯；V1.8：§9.8 編輯時顯示已剪下的區域；V1.7：§9.5 標題立即顯示、其餘提早一句；V1.6：§9.7 重生對話框的批次剪下；V1.5：§9.6 改為 XY-cut 遞迴切割；V1.4：§9.6 自動偵測剪下區域；V1.3：§9.5 出現時機與位置由 AI 決定；V1.2：新增 §9 剪下區域做動畫；V1.1：文字改為 Markdown＋數學、線條獨立成型別、合成改以 Chrome 優先）
- 狀態：依本文件實作中
...
剪下之後底圖大多被抹成背景，用它做縮圖列、封面等預覽幾乎是一片空白，認不出是哪一頁。有剪下歷史的頁面，縮圖與封面改由「剪下原圖」產生（`cutoutPreviewSourcePath()`），detail 多回 `has_cutouts`；播放時仍需要抹除後的底圖讓動畫有東西可揭露，因此一般面板對這種頁面改用完整圖片而不是縮圖（`slideImageUrlForPage`）。歷史被清掉（更換底圖、AI 重繪）時縮圖回到一般規則。

### 9.11 動畫效果上限：剪前預檢、事後補回

`UvfBOfejHb` 第 4 頁的十塊剪下都沒有放回去。查證：那一頁原本就有 14 個 AI 焦點效果，再加 10 個 `overlay-image` 超過當時的上限 20，規格驗證失敗、效果一個都沒寫，但底圖早已抹除、補丁也記了——結果就是圖上留了洞、什麼都不會回來。三個修正：

1. **上限提高到 40**（前後端同步）：有剪下的頁每塊各佔一個效果，20 在真實簡報上不夠。
2. **剪前預檢**（`applyCutoutChanges` 開頭）：既有效果數加上要剪的區域數超過上限就直接以 `ANIMATION_LIMIT`（409）拒絕，**任何檔案都不動**、也不呼叫模型；批次重生對這種頁面記錄後略過。
3. **事後補回**：清單以 `missingEffect` 標示「已剪下但沒有效果、也不是隱藏」的塊，面板顯示紅色提示與「補上動畫效果」（`POST …/cutouts/reattach`）；每次套用變更結束時也會自動嘗試補回。補回走與剪下相同的定時定位（§9.5），不動圖片。
