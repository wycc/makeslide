...
- 文件版本：V1.4（2026-09-05：§9.6 自動偵測剪下區域；V1.3：§9.5 出現時機與位置由 AI 決定；V1.2：新增 §9 剪下區域做動畫；V1.1：文字改為 Markdown＋數學、線條獨立成型別、合成改以 Chrome 優先）
- 狀態：依本文件實作中
...
| 問題 | 落到規格的哪裡 |
| --- | --- |
| 講到哪一句時該出現 | `startTrigger: { type: 'transcript-line', line, anchor: 'start' }`——那一句開始時淡入；數字 `start` 仍寫成依序排列的值，作為沒有旁白時間時的備援 |
...
1. **影像分析（決定性、不需模型）**：把底圖縮到 320px 寬，取邊框最常見的顏色當背景色，與背景色距離超過門檻的像素視為內容；做一次膨脹（closing）讓一段文字的字形連成一塊，找 4-連通元件的外框，再把相接或相距很近的框合併；去掉太小（< 0.15% 面積，雜點）與太大（> 85%，那是整頁）的框，最多 20 個，由上到下、由左到右排序。
