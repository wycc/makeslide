...
- 文件版本：V1.10（2026-09-06：§9.10 預覽用未剪下的原圖；V1.9：§9.9 剪下歷史與草稿式編輯；V1.8：§9.8 編輯時顯示已剪下的區域；V1.7：§9.5 標題立即顯示、其餘提早一句；V1.6：§9.7 重生對話框的批次剪下；V1.5：§9.6 改為 XY-cut 遞迴切割；V1.4：§9.6 自動偵測剪下區域；V1.3：§9.5 出現時機與位置由 AI 決定；V1.2：新增 §9 剪下區域做動畫；V1.1：文字改為 Markdown＋數學、線條獨立成型別、合成改以 Chrome 優先）
- 狀態：依本文件實作中
