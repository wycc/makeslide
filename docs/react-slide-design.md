...
沙箱內部的座標系統固定為 1920×1080（與 open-slide 相同、也與 MakeSlide 既有 16:9 空白頁一致）。外層量測 iframe 容器的實際寬高，以 `transform: scale(min(w/1920, h/1080))` 縮放內容並置中。生成的程式碼因此永遠只需要對著 1920×1080 排版，不必寫 responsive；而播放頁、全螢幕、編輯預覽三處縮放比例不同也不會跑版。
