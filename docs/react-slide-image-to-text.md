...
- [ ] **把量到的顏色自動放進 rich text**。現在整塊套一個顏色，所以原圖裡「句子中間某幾個字是藍的」這種強調會被抹平——而 rich text 已經表達得了它，只是沒有人去填。**零件都在**：`sampleTextColorRuns()` 逐欄量出一行的顏色分佈、`splitTextByColorRuns()` 用與字級同一套字寬把像素位置對回字串，兩者是先前把「一色一元素」拿掉時刻意留下的。要接的是：抽取時把切出來的段落組成 `<span style="color: …">`，當作 `insertText` 的初始標記（`textElementJsx` 走 `richTextToJsxChildren` 而不是 `textAsJsxChildren`）。
  - 這一步之後，**顏色邊界不準不再是問題**：它只是初始值，使用者在 rich text 裡拖過去重新上色就好。比起先前「一色一元素」的做法（邊界不準就等於元素位置錯了），這是同一份分析放在正確的地方用。
  - 需要一併決定的：整塊的 `color` 樣式與 span 的顏色重複時要不要收斂——例如把出現最多的顏色留在元素上、只有不同的段落包 `<span>`，否則每一塊都被包滿 span，程式碼會很吵。
