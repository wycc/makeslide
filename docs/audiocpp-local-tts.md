...
- **留空** → 不帶任何 voice 參數，由該模型家族用自己的預設聲音（Qwen3 Base 這種沒有預設音色的
  家族會直接失敗——它一定要 `--voice-ref`）。`qwen3_tts` 家族例外：CustomVoice 沒有預設講者，
  少了 `--speaker` 會以 `Qwen3 custom voice prefill requires speaker` 中斷該段，因此最後補上
  內建講者 `vivian` 並記一筆 warn（`AUDIOCPP_QWEN3_FALLBACK_SPEAKER`）。
...
**沒有 `Speaker N:` 前綴的段落**（單人旁白頁）先看簡報自己的聲音，簡報沒設或設的是別家音色名時，
接著往下用**講者 1** 的聲音（簡報的、再來是全域的）。原本這種段落只看簡報聲音就停住，於是一份
只設過講者 1／2 聲音的簡報，每一頁對話都合成得出來，唯獨那一頁單人旁白因為沒有任何聲音而失敗。
