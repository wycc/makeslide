...
「導師問答」（`/ask`）是**給學生的**面板：任何看得到這份簡報的人都進得去，包含透過分享連結進來的人。一個會主動說「我幫你把逐字稿改好了，要套用嗎」的工具不該出現在那裡——學生沒有理由改老師的教材，而一個他隨手按下去的按鈕就會改到。

所以編輯能力只在**本頁問答**（`/chat`），而且還要通過 `canEditPdf`。導師問答維持唯讀工具（查別頁的文字、逐字稿、畫面），那些只會讓它答得更好，不會動到任何東西。
...
**有標示區域就只改那一塊**。使用者若已在投影片上拉了選取框，這個工具改走 inpaint（以該區域為遮罩），否則整張重繪。這不是最佳化而是正確性：被要求「把這條線重畫」時，整張重繪會把畫面上其他每一個東西都換成略有不同的版本，而使用者沒有要求那些改變。區域跟著提問一起送到後端，遮罩在後端依比例產生（前端的遮罩留給「修改圖片」按鈕自己那條路）。

> 遮罩有一個會**無聲失敗**的細節：`dest-out` 是用**來源**的 alpha 去減目標，所以挖洞的矩形必須是**不透明**的——用透明矩形會什麼都減不掉、遮罩沒有洞，模型於是重畫整張圖，看起來就像區域被忽略。已用直接斷言像素 alpha 的測試把關（`image-edit-region.test.ts`）。
...
> **實作註記（2026-08-17）**：能不能呼叫工具，取決於模型願不願意在推理開啟時使用 function tools。gpt-5.6 在 `/v1/chat/completions` 上會直接拒絕（`Function tools with reasoning_effort are not supported…`），而程式碼原本的處理是**靜默退回無工具生成**——於是產品裡所有的工具呼叫（包含導師問答查別頁那些既有的唯讀工具）從來沒有真的執行過，只留下一行 warn。現在遇到這個特定錯誤會**帶 `reasoning_effort: 'none'` 重試一次**，再失敗才退回。
