...
- **v1 範圍**：純 SVG 2D 向量圖形 + 上述動畫手法；不含 LaTeX/MathTex 渲染（需 KaTeX/字型等外部資源，sandbox 禁止網路存取）、~~不含 `Axes`/`NumberPlane` 座標軸繪製輔助~~（已於 `window.Manim` 新增 `coordinateSystems.axes`/`coordinateSystems.numberPlane`，詳見 §12；分支 `feature/animation-manim-axes-numberplane-20260616`）、不含 3D、`transform` 僅做簡單屬性線性插值（非真正路徑變形）。後續版本見 §12。
