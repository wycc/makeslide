...
>
> 擴充註記（2026-06-16，圖片插入效果 overlay-image）：
> - 新增 `overlay-image` 效果類型：與 `highlight-box`/`spotlight`/`text-callout`/`shape`/`step-list` 同為 overlay 疊加層（`OVERLAY_EFFECT_TYPES`），新增 `effect.figureId?: string`（上限 `MAX_OVERLAY_IMAGE_FIGURE_ID_LENGTH` = 200 字元）指定要疊加顯示的圖片，圖片來源為該頁 PDF 擷取的圖表素材（`GET /api/pdfs/:id/pages/:n/figures`），渲染時透過 `figureImageUrl(pdfId, figureId)`（`GET /api/pdfs/:id/figures/:figureId/image`）取得圖片網址；位置/大小沿用 §5.1 的 `xPct`/`yPct`/`widthPct`/`heightPct`（預設 30/30/40/40），可選填 `exitDuration` 自動淡出（§5.3）。動畫編輯器新增「插入圖片」效果類型，於選定該類型時呼叫 figures API 列出本頁已擷取的圖表供下拉選擇，並即時顯示縮圖預覽，另新增「插入頁面圖片」範本。為 §12 V2「overlay image」之 v1 落地（僅手動從既有圖表中選取，AI 自動選圖/產生新圖留待後續）。詳見 §5.7、§6.6、§7、§12。
>
...
| overlay-image | 於指定區域淡入顯示一張圖片（取自該頁已擷取的圖表素材） | opacity: 0 → 1，圖片來源見 `effect.figureId`，詳見 §5.7 |
| formula | 於指定區域淡入顯示一個以 KaTeX 渲染的數學公式 | opacity: 0 → 1，公式內容見 `effect.formula`，詳見 §5.8 |
...
- **v1 範圍**：純 SVG 2D 向量圖形 + 上述動畫手法；~~不含 LaTeX/MathTex 渲染（需 KaTeX/字型等外部資源，sandbox 禁止網路存取）~~（已新增 `Manim.tex(latex, opts?)` 透過 postMessage 向 host 頁面請求 KaTeX MathML 渲染，sandbox 無需網路存取；詳見 §12；分支 `feature/animation-manim-tex-mathtex-20260616`）、~~不含 `Axes`/`NumberPlane` 座標軸繪製輔助~~（已於 `window.Manim` 新增 `coordinateSystems.axes`/`coordinateSystems.numberPlane`，詳見 §12；分支 `feature/animation-manim-axes-numberplane-20260616`）、不含 3D、`transform` 僅做簡單屬性線性插值（非真正路徑變形）。後續版本見 §12。
...
`shape` 是 §12 V2「SVG 圖元」的 v1 實作：與 `highlight-box`/`spotlight`/`text-callout` 共用同一套 overlay 疊加層機制（§6.6），額外新增 `effect.shape` 欄位決定渲染的 SVG 圖元種類。
...
// effect.shape：SVG 圖元種類，未提供時預設 'circle'
'circle' | 'rect' | 'ellipse' | 'arrow'
```

- 渲染為一個 `viewBox="0 0 100 100"` 的 inline `<svg>`，依 `effect.shape` 繪出對應圖元，固定使用玫瑰色（`#f43f5e`）描邊、`fill="none"`：
  - `circle`：置中圓形（`preserveAspectRatio="xMidYMid meet"`，維持正圓不隨外框比例變形）。
  - `ellipse`：填滿外框的橢圓（`preserveAspectRatio="none"`，隨 `widthPct`/`heightPct` 比例伸縮）。
  - `rect`：填滿外框的圓角方框（`preserveAspectRatio="none"`）。
  - `arrow`：由外框左下指向右上的箭頭線段（`preserveAspectRatio="none"`，含箭頭 `<marker>`）。
- `autoAlpha` 由 0 淡入至 1，淡入後預設維持顯示；可選填 `exitDuration` 自動淡出，詳見 §5.3（與 highlight-box/spotlight/text-callout 相同機制）。
...
### 5.7 圖片插入效果（overlay-image）

`overlay-image` 是 §12 V2「overlay image（小圖疊加內容）」的 v1 實作：與 `highlight-box`/`spotlight`/`text-callout`/`shape`/`step-list` 共用同一套 overlay 疊加層機制（§6.6），額外新增 `effect.figureId` 欄位指定要疊加顯示的圖片。
...
// effect.figureId：要疊加顯示的圖片，取自該頁已擷取的圖表素材（PageFigure.id，見 §8 figures 路由）
// 選填，長度 1~MAX_OVERLAY_IMAGE_FIGURE_ID_LENGTH（200）字元
string
```

- 圖片來源為該頁 PDF 擷取的圖表素材清單（`GET /api/pdfs/:id/pages/:n/figures`，見 `docs/pdf-figure-extraction-design.md`）。動畫編輯器於效果類型為 `overlay-image` 時呼叫此 API 取得清單，以下拉選單列出每張圖（顯示 caption 或 id），選定後即時顯示一張小縮圖預覽；尚未取得清單或本頁無圖表時分別顯示「載入中」/「本頁未偵測到可用圖片」。
- 渲染為一個 `<img>`，`src` 取自 `figureImageUrl(pdfId, effect.figureId)`（`GET /api/pdfs/:id/figures/:figureId/image`）、`object-fit: contain`。`SlideRenderer` 新增選填 prop `resolveFigureImageUrl(figureId): string`，由播放頁面組裝實際網址（含分享連結的 `share` token）並傳入；未提供 `resolveFigureImageUrl` 或 `effect.figureId` 時 `EffectOverlay` 不渲染任何內容。
- `autoAlpha` 由 0 淡入至 1，淡入後預設維持顯示；可選填 `exitDuration` 自動淡出，詳見 §5.3（與其他 `OVERLAY_EFFECT_TYPES` 相同機制）。
- 驗證規則：`figureId` 為選填字串，長度 1~`MAX_OVERLAY_IMAGE_FIGURE_ID_LENGTH`（200）字元，與 `GET /api/pdfs/:id/figures/:figureId/image` 路由參數驗證一致；後端不驗證該 id 是否確實存在於該頁的圖表清單中——若指向不存在的圖片，`<img>` 載入失敗僅顯示空白，不影響其他效果。
- **v1 範圍**：僅能從該頁既有圖表素材中手動選取單張圖片疊加顯示；依逐字稿內容由 AI 自動選擇/插入圖片、上傳全新圖片留待後續版本（見 §12）。

### 5.8 公式效果（formula）

`formula` 是 §12 V2「公式」的 v1 實作：與 `highlight-box`/`spotlight`/`text-callout`/`shape`/`step-list`/`overlay-image` 共用同一套 overlay 疊加層機制（§6.6），額外新增 `effect.formula` 欄位作為 LaTeX 來源，以 KaTeX 渲染為 HTML。
...
// effect.formula：要渲染的 LaTeX 來源，例如 "E = mc^2"
// 選填，長度 1~MAX_FORMULA_LENGTH（200）字元
string
```

- 渲染時呼叫 `katex.renderToString(effect.formula ?? '', { throwOnError: false, displayMode: true })` 將 LaTeX 轉換為 HTML（無效語法時 `throwOnError: false` 會改以 KaTeX 內建的錯誤訊息呈現，不會中斷整頁渲染），透過 `dangerouslySetInnerHTML` 顯示於一個深色圓角方框內（與 `text-callout`/`step-list` 視覺風格一致）。
- 前端新增 `katex`（執行期）與 `@types/katex`（型別）依賴；KaTeX 字型樣式表 `katex/dist/katex.min.css` 於 `frontend/src/main.tsx` 全域匯入一次，供主應用 bundle 使用。
- `autoAlpha` 由 0 淡入至 1，淡入後預設維持顯示；可選填 `exitDuration` 自動淡出，詳見 §5.3（與其他 `OVERLAY_EFFECT_TYPES` 相同機制）。
- 驗證規則：`formula` 為選填字串，長度 1~`MAX_FORMULA_LENGTH`（200）字元；後端不驗證 LaTeX 語法是否合法——無效語法由前端 KaTeX 以 `throwOnError: false` 呈現錯誤訊息，不影響其他效果。
- **v1 範圍**：僅提供純 LaTeX 字串輸入與即時預覽；依逐字稿內容由 AI 自動生成公式、可視化公式編輯器（符號鍵盤）留待後續版本（見 §12）。`custom-script`（§5.4）的 sandboxed iframe 因網路限制無法直接載入 KaTeX，故公式渲染僅支援 `formula` 效果類型本身，不適用於自訂腳本內。
...
`highlight-box`/`spotlight`/`text-callout`/`shape`/`step-list`/`overlay-image`/`formula`/`custom-script` 效果（`OVERLAY_EFFECT_TYPES`，定義於 `frontend/src/lib/animationSpec.ts`）以額外的疊加 `<div>`（`shape` 為 `<svg>`、`overlay-image` 為 `<img>`，`custom-script` 為 `<iframe>`）實作，渲染於 animated stage 內（`img` 與 `children` 之後）：

- `SlideRenderer.tsx` 對 `spec.effects` 中屬於 `OVERLAY_EFFECT_TYPES` 的每個效果，渲染一個帶 `data-effect-id={effect.id}` 的元素（`EffectOverlay`），初始 `opacity: 0`、`position: absolute`、`pointer-events: none`，位置/大小取自 `getFocusEffectParams(effect)`（`xPct`/`yPct`/`widthPct`/`heightPct`，含預設值）；`text-callout` 額外以 `effect.text` 作為文字內容，`shape` 渲染為 `viewBox="0 0 100 100"` 的 `<svg>`，依 `getShapeKind(effect)` 繪出對應圖元（詳見 §5.5），`step-list` 渲染為一個深色圓角矩形內含 `<ul>`/`<li>` 條列清單（依 `effect.items`，過濾空白項目後逐項顯示，詳見 §5.6），`overlay-image` 依 `effect.figureId` 透過 `resolveFigureImageUrl` prop 解析圖片網址後渲染為 `<img>`（未提供 prop 或 `figureId` 時不渲染，詳見 §5.7），`formula` 將 `effect.formula` 以 `katex.renderToString(..., { displayMode: true })` 轉換為 HTML 後，以 `dangerouslySetInnerHTML` 渲染於深色圓角方框內（詳見 §5.8），`custom-script` 則為 `<iframe sandbox="allow-scripts" srcDoc={buildCustomScriptSandboxDoc(effect.code)} />`（詳見 §5.4）。
...
- 後端（`backend/src/routes/pdfs/page-animation.ts`）讀取本頁 OCR 文字（`text_path`）與本頁渲染圖片（`image_path`，縮小至 `OPENAI_SCRIPT_IMAGE_MAX_WIDTH`、轉 JPEG base64，沿用 `generateScript` 的縮圖設定；讀取失敗則回退為純文字，不中斷）；連同請求中的逐字稿句子、`hints` 一起傳入 `generateAiFocusEffects`（`backend/src/services/animationAutoFocus.ts`）組成提示詞，透過 `callChatJSON`（沿用 `LLM_PROVIDER`/`openaiLlmModel`/`geminiLlmModel` 設定）請 LLM 針對每句（最多 `MAX_SLIDE_ANIMATION_EFFECTS` = 20 句）回傳：
  - `show`：是否顯示效果。
  - `type`：`highlight-box`、`spotlight`、`text-callout`（淡入一段 AI 生成的精簡文字摘要，適合用一句重點數據或結論強化畫面）、`shape`（淡入一個 SVG 圖元，用於標示位置）或 `step-list`（淡入一個條列步驟/要點清單方框）。
  - `xPct`/`yPct`/`widthPct`/`heightPct`：方框（或文字框、圖形、清單方框）位置與大小（百分比，0-100）；`text-callout`、`step-list` 建議放在畫面空白處，避免遮住重點內容。
  - `text`（僅當 `type` 為 `text-callout` 時提供）：要顯示的文案，限制在 `MAX_TEXT_CALLOUT_LENGTH`（80）字以內，並使用與逐字稿相同的語言。
  - `shape`（僅當 `type` 為 `shape` 時提供）：`circle`/`rect`/`ellipse`/`arrow` 之一；未提供時前端預設為 `circle`。
  - `items`（僅當 `type` 為 `step-list` 時提供）：條列項目文字陣列，最多 `MAX_STEP_LIST_ITEMS`（6）項、每項最長 `MAX_STEP_LIST_ITEM_LENGTH`（60）字，並使用與逐字稿相同的語言。
  - `exitDuration`（選填，秒，0-30）：淡入完成後停留多久自動淡出。
- 後端將 `show: true` 的項目轉換為 `AnimationEffect`：`type`/`params`/`exitDuration`/`text`（`text-callout` 時）/`shape`（`shape` 時）/`items`（`step-list` 時）取自 AI 回應（數值、文字長度與陣列長度會 clamp 到合理範圍），`start = 0`、`duration = 1.2`、`ease = 'power1.out'`、`startTrigger = { type: 'transcript-line', line }`；`show: false` 或重複/超出範圍的 `line` 會被忽略。若 AI 選擇 `text-callout` 卻未提供有效（非空白）`text`，或選擇 `step-list` 卻沒有任何有效（去除空白後非空）項目，該項目會退回為 `highlight-box`（避免產生空白文字框/清單）。回應 `{ effects: AnimationEffect[] }`，**不會**寫入已儲存的 spec。
...
- V2：overlay image（小圖疊加內容）、物件 target、公式、逐步條列；依 `AnimationSpec.hints` 與逐字稿內容由 LLM 生成動畫 JSON——焦點方框（`highlight-box`/`spotlight`）的時機與位置已於 §7.4 落地，~~`text-callout`（含 AI 生成文案）的 AI 生成~~（已將 `auto-focus-ai` 的 `AUTO_FOCUS_AI_EFFECT_TYPES` 擴充為 `highlight-box`/`spotlight`/`text-callout`，LLM 可為適合以精簡摘要強化重點的句子選擇淡入一段 AI 生成文案（`text`，上限 `MAX_TEXT_CALLOUT_LENGTH` = 80 字），缺少有效文字時退回 `highlight-box`；分支 `feature/animation-auto-focus-ai-text-callout-20260615`）；~~SVG 圖元~~（已新增 `shape` 效果類型，提供 `circle`/`rect`/`ellipse`/`arrow` 四種基本 SVG 圖元 overlay，位置/大小與淡入淡出機制沿用 `highlight-box`/`text-callout`，固定玫瑰色描邊、不提供樣式自訂；分支 `feature/animation-svg-shape-effect-20260616`）；~~逐步條列~~（已新增 `step-list` 效果類型，提供條列文字 overlay，容器立即可見、各項目以 `effect.duration` 為總長度逐一交錯淡入，位置/大小與整體淡出機制沿用 `highlight-box`/`text-callout`；分支 `feature/animation-step-list-effect-20260616`）；~~`auto-focus-ai` 的 `shape`/`step-list` AI 生成~~（已將 `AUTO_FOCUS_AI_EFFECT_TYPES` 擴充為 `highlight-box`/`spotlight`/`text-callout`/`shape`/`step-list`，LLM 可為適合用圖形標示位置或條列步驟/要點的句子選擇 `shape`（指定圖元種類）或 `step-list`（提供 `items`），缺少有效 `items` 時退回 `highlight-box`；分支 `feature/animation-auto-focus-ai-shape-step-list-20260616`）；~~overlay image~~（已新增 `overlay-image` 效果類型，可從該頁已擷取的圖表素材中手動選取單張圖片以 `effect.figureId` 疊加顯示，位置/大小與淡入淡出機制沿用 `highlight-box`/`text-callout`；AI 自動選圖/產生新圖留待後續；分支 `feature/animation-overlay-image-effect-20260616`）；~~公式~~（已新增 `formula` 效果類型，新增 `effect.formula?: string`（上限 `MAX_FORMULA_LENGTH` = 200 字元）作為 LaTeX 來源，以 KaTeX 渲染為 HTML 並疊加顯示，位置/大小與淡入淡出機制沿用 `highlight-box`/`text-callout`；依逐字稿內容由 AI 自動生成公式、可視化公式編輯器留待後續；分支 `feature/animation-formula-effect-20260616`）；~~`custom-script` 的 AI 生成~~（已將 `AUTO_FOCUS_AI_EFFECT_TYPES` 擴充為 `highlight-box`/`spotlight`/`text-callout`/`shape`/`step-list`/`custom-script`：LLM 可為適合以簡短資料視覺化呈現的句子選擇 `custom-script`，提供 `scriptPrompt`（視覺化描述）與 `scriptDurationSeconds`（一輪播放秒數，預設 6、限制 2-20），每頁最多 1 個（超出或缺少 `scriptPrompt` 時退回 `highlight-box`）；後端再呼叫既有的 `generateCustomScriptCodeStream`（與手動「自訂腳本」對話框相同的安全模式檢查 `findUnsafeScriptPattern` 與契約檢查 `findCustomScriptContractIssue`）產生程式碼並寫入 `effect.code`；v1 不含互動式「實作步驟」規劃與多輪迭代，產生失敗、不安全或不符契約時退回 `highlight-box`，使用者仍可事後透過既有對話框手動迭代；分支 `feature/animation-auto-focus-ai-custom-script-20260616`）；物件 target 仍待後續。
...
- V2.x：`window.Manim`（manim 風格輔助函式庫，§5.4）的擴充——~~`Axes`/`NumberPlane`（座標軸、格線、`coordsToPoint`）~~（已新增 `Manim.coordinateSystems.axes`/`Manim.coordinateSystems.numberPlane(svg, opts)`：`opts` 可含 `xRange`/`yRange`（`[min, max, step?]`，預設 `[-7,7,1]`/`[-4,4,1]`）、`xLength`/`yLength`（預設 14/8）、`color`/`gridColor`；回傳 `{ el, kind: 'axes'|'numberPlane', svg, coordsToPoint }`，`axes` 繪製 x/y 兩軸與依 `step` 產生的刻度，`numberPlane` 額外繪製整面格線；`coordsToPoint(x, y)` 將資料座標線性映射到場景座標（原點對應 `(0,0)`），可直接作為 `Manim.shapes.*` 的 `x`/`y`；回傳值可像其他 mobject 一樣傳入 `Manim.animate.*`（`create`/`write` 退化為 `fadeIn`）；分支 `feature/animation-manim-axes-numberplane-20260616`）、~~`MathTex`/`Tex`（KaTeX 公式渲染）~~（已新增 `Manim.tex(latex, opts?)` — 回傳 `Promise<HTMLElement>`，透過 `window.parent.postMessage({ type: 'renderLatex', id, latex }, '*')` 向 host 頁面請求 KaTeX MathML 渲染，host 頁面（`PlayPage.tsx` 的 `useEffect`）以 `katex.renderToString(latex, { output: 'mathml', throwOnError: false, displayMode: true })` 渲染後以 `{ type: 'latexResult', id, html }` 回覆，sandbox 的 `message` 事件接收後 resolve Promise，得到含 MathML 的 `<div>` 元素；此架構完全離線可用，不需 sandbox 自行存取 KaTeX 字型，渲染結果為瀏覽器原生 MathML；`opts` 可含 `color`/`fontSize`；使用時可 `const el = await Manim.tex("E=mc^2"); root.appendChild(el);`；分支 `feature/animation-manim-tex-mathtex-20260616`）、`transform` 的真正路徑變形（path morphing，而非目前僅線性插值共有屬性）、3D 場景；v1 僅提供 2D SVG mobject（`circle`/`square`/`rectangle`/`line`/`arrow`/`dot`/`polygon`/`text`）與 `Create`/`Write`/`FadeIn`/`FadeOut`/`Transform`/`Shift`/`Rotate`/`Scale`/`GrowFromCenter` 等基本動畫手法。
